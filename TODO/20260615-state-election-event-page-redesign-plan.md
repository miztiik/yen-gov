...
| R5   | NEW `StateEventCrossEventSankey.svelte` (diverging-bar always-on + Sankey collapsed) + opt-in caption                                                                                                                                                | [~] IN-FLIGHT (PR open) | #1073 (stacked on #1071 base) | M      |
...
| R7   | NEW build-step: per-event share-card PNG at `bun run build` (`frontend/scripts/build-share-cards.ts` + `@resvg/resvg-js` devDep + `<svelte:head>` og:* + twitter:card meta on `StateElection.svelte`); J-elevated-14                                  | [~] IN-FLIGHT (PR open) | #1077 (313/313 cards generated end-to-end; +24 vitest assertions)  | M      |
| SC   | Scrub `Lok Sabha` / `Vidhan Sabha` from citizen chrome + freeze with `english-only-citizen-chrome.test.ts` contract test                                                                                                                          | [~] IN-FLIGHT (PR open) | #1074                       | XS     |
