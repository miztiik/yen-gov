...
| PR-SYM-4a.i | SUPERSEDED by 4a.ii | _hand-authored silhouettes; rejected as inauthentic_ | none | n/a | n/a | #543 |
| PR-SYM-4a.ii | DONE (pilot 4/40, REAL Commons) | `frontend/public/party-symbols/{bjp-lotus,inc-hand,bsp-elephant,aap-broom}.svg` (overwritten), `notes/20260601-party-symbol-pilot-batch-v2.md`, `frontend/package.json` (+svgo devDep) | none | sanitizer empirical pass 4/4 (via svgo normaliser) | unblocks PR-SYM-4a-rest | #_pending_ |
| PR-SYM-4a-rest | PENDING (~36 left) | remaining Tier 0 SVGs via same svgo pipeline, `datasets/taxonomy/sources.parquet` `wikimedia_commons` row | none | sanitizer empirical pass + Tier-A validate | PR-SYM-4b | _pending_ |
