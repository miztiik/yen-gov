...
| 2026-06-12 | FU#1 audit | 110 pending tiles classified into 4 buckets: **A** (1 tile, slug "andaman-and-nicobar-islands" not in SLUG_TO_ECI map — A&N entity carries the "-islands" suffix vs the LGD canonical "andaman-and-nicobar"); **B** (84 tiles, electoral.csv carries `eci_no=0` for the PC entity — publisher gap from TCPD/ECI when the seat number wasn't published; spans 14 states with TN/13, UP/11, KE/12 the worst hit); **C** (20 tiles, mapped slug + eci_no>0 but the tile layout doesn't carry that key — drift on S01/S03/S29 plus U04↔U06 Lakshadweep re-numbering, U07-7 Puducherry, U08-6 J&K, U09-4 Ladakh); **D** (3 orphan placeholder tiles, the U06 Lakshadweep legacy code + 2 "Rest of J&K"/"Rest of Ladakh" eci_no=999 catch-alls). |
| 2026-06-12 | FU#1 ship | PR [#???] merged. ONLY Bucket A fixable here: added `EXTRA_SLUG_ALIASES = { "andaman-and-nicobar-islands": "U01" }` overlay in `frontend/src/lib/view-models/election-results.ts`; kept `ECI_TO_LGD_SLUG` untouched in `frontend/src/lib/boundaries/sources.ts` because that map's values feed `boundaries/in/{panchayats,wards}/state=<slug>/...` path constructors that must stay anchored on the LGD canonical slug. Closes 1 tile (110 -> 109). Buckets B + C + D (109 residual tiles) require data-tier work — split into FU#1.1 + FU#1.2 below. |


## Plan complete (2026-06-12)

**Closure.** All 7 rows A-G landed in PR [#958](https://github.com/miztiik/yen-gov/pull/958) (`315f14e15`). Pre-2024 LS events still show the placeholder card per scope-lock; per-state PC tile layout authoring is deferred. Follow-up #1 partial-closed (Bucket A only); see FU#1.1 + FU#1.2 for the data-tier residuals.

1. **National PC tile pending rate (partial closure).** PR #958 left 110/545 tiles pending (~20%). FU#1 audit at 2026-06-12 reclassified into 4 buckets (see ledger row): **A** (1 tile) closed in the follow-up PR via a SLUG_TO_ECI alias for the "andaman-and-nicobar-islands" electoral.csv variant. **B + C + D** (109 tiles) are NOT fixable at the loader and split into:
    - **FU#1.1 (publisher gap, ~84 tiles).** Backend ingest follow-up: when `eci_no` is missing from the upstream source (TCPD / ECI Statistical Report), the canonical writer currently emits `eci_no=0` for the PC entity in `datasets/data/entities/electoral.csv`. Two paths: (a) backfill `eci_no` from the canonical (state_slug, normalised_pc_name) -> (entity_id, eci_no) spine when the publisher did publish the seat number elsewhere (mirrors the G16 single-step lookup doctrine in `/memories/repo/yen-gov-architecture.md`); (b) carry NULL instead of 0 so the route can detect "unknown seat number" vs "seat zero" and skip the tile-join attempt cleanly. Hans + Max owned.
    - **FU#1.2 (tile-layout drift, ~25 tiles).** Data-tier follow-up: the `datasets/grapher/election_tile_layouts.json` national-PC layout was authored with delim_year=2008 + legacy ECI state codes (U06 Lakshadweep instead of the current U04; missing eci_no=4 for Ladakh; missing eci_no=6 for J&K; missing eci_no=7 for Puducherry; etc). Needs regeneration via `tools/elections/generate_tile_layouts.py` (or equivalent — verify the tool exists) using the current `datasets/data/entities/electoral.csv` PC inventory + ECI state codes. Hans + Max owned.
2. **Alliance backfill** — user-confirmed queued. Opens as a separate plan-doc.

This plan-doc is preserved in `TODO/` as the audit trail for PR #958 + the FU#1 partial-closure PR.
