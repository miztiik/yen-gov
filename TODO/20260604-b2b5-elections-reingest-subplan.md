...
| 3 | **PRI constituency super-file** (re-export as CSV, ALL states) | AC + PC (each WITH its ECI code) + AC->PC parent + AC->district edges (+ subdistrict + block) | `entities/electoral.csv` (eci_no folded) + `entities/electoral_district_membership.csv` (`is_primary`) | required |
| 4 | All Sub-districts of India | lgd_subdistrict_id, name, parent district | `entities/geo.csv` subdistrict rows | parked |

SUPERSEDED: the thin `Assembly_parliament_constituency_*.csv` (7 columns, LGD codes only, NO ECI code, NO district) is dropped in favour of #3. The standalone all-districts file (#2) is STILL required - it is the district REGISTER (district -> state); the PRI report supplies the constituency->district EDGE, not the district registry. On disk today the PRI export exists only for Andaman & Nicobar (HTML, 2026-06-01); the operator MUST re-export it for ALL states as CSV before PR-stage 0c.

**(2) `eci_no` bind = DIRECT JOIN for the current delimitation; name-match only for historical cycles.** The TCPD result files (`All_States_AE.csv` etc.) key each row on `(State_Name, DelimID, Constituency_No)` where `Constituency_No` is the ECI/delimitation serial. The PRI report binds `(State, Assembly Constituency ECI Code) -> lgd_ac_id` for the CURRENT (2008) delimitation. So for current-delim cycles the bind is a DIRECT JOIN `TCPD.Constituency_No = PRI.AC_ECI_Code` scoped by state - no name-clean. Pre-2008 delimitations are NOT in the LGD/PRI export, so those cycles need delim-scoped name-match or a historical crosswalk. This SHRINKS the feared ~4000-AC name-clean to historical cycles only; the current delimitation is a clean direct join.
...
### Round-8c open enrichment corpus (the operator keeps adding CSVs; process EVERY file present, route by manifest, never hardcode a fixed list)

The ephemeral folder holds more than the spine + result files, and MORE WILL BE ADDED over time (the user: presidents, vice-presidents, chief ministers, candidate affidavits "all for enrichment"). The executing agent MUST enumerate `datasets/ephemeral/*.csv` at run time and route each file via the manifest below, emitting a skip-receipt under `datasets/_ops/` for any unrecognized file (never silently ignore, never hardcode a closed list). Three tiers:

- **Tier S (spine):** the four LGD files above -> `entities/state_codes.csv` / `geo.csv` / `electoral.csv` / `electoral_district_membership.csv`.
- **Tier R (results, core electoral facts):** TCPD `All_States_AE.csv` (assembly), `All_States_GE.csv` (parliament/LS), the per-year LS detailed-result CSVs, TCPD party + CM-id registries -> `elections/assembly/...`, `elections/parliament/...`, `entities/parties.csv`.
- **Tier E (enrichment, OPEN + GROWING):** office-holders (presidents, vice-presidents, chief ministers), candidate affidavits, electoral rolls, gender counts, pincode directory, ... - each routes to its OWN long-format datapoint family (`datasets/data/datapoints/...`) or entity attribute, each carries `source_id`, each is its OWN ingest row. NON-EXHAUSTIVE: more arrive over time; the agent processes whatever is present.
...
| `parlimentConstituencyAndAssemblyConstituencyPRI*` (re-export CSV, all states) | S | `entities/electoral.csv` + `entities/electoral_district_membership.csv` |
| `Assembly_parliament_constituency_*.csv` (thin) | SUPERSEDED | dropped (PRI super-file replaces) |
...
Enrichment rows (Tier E) do NOT block the core elections delivery (Tier S + Tier R); each is processed as its own ingest row with a concept + `source_id`, independently shippable.
