...
> **0e scope note (2026-06-05, autonomous decision - user delegated "work autonomously, make good decisions").** The mandated sweep was run; the audit (greps + Explore subagent, recorded in `datasets/_ops/eci-census-decommission-sweep-2026-06-05.md`) found that `eci_st_code` is NOT a contained elections-spine column but a **live internal translation key** (legacy-parquet `IN-S22` -> LGD slug) in >=7 backend surfaces (energy / livestock / governments / state-tiers / election-events / pincode datapoint loaders + the `eci_ae_panel`/`eci_ls` inventory adapters + the `writer.py` Hive-partition CASE), 8 currently-green test files, the `office_holdings.csv` output (still `IN-S<NN>`-keyed, governments not yet slug-migrated), the `state=in_s22` boundary/manifest **partition grammar** (already mid-migration in the parallel boundary slug-partition chunk - the source of the 35 pre-existing `pytest (ingest pipeline, non-admin)` baseline failures), and the being-retired frontend `canonicalEntityToLegacy` reader. Per the sub-plan's own step 4 ("clean what you own; forward-point cross-surface hits to their owning chunk") these are forward-pointed, NOT rewritten inside the elections sub-plan: doing so would (a) span subsystems this plan does not own, (b) turn 8 green tests red until each owning chunk repoints, (c) require re-keying `office_holdings.csv` (B2b.3 territory), and (d) collide head-on with the in-flight boundary partition migration. **The election RESULTS (Tier R, B2b.5.2+) bind via `eci_no` (RETAINED), never `eci_st_code`, so they are fully unblocked.** The gate-closing repoint is sequenced into the coordinated ECI->slug transition already underway in B2b/B3/X1a; this receipt is the audit + forward-pointer set those chunks consume.
