...
- **R2 RESOLVED -> OWID-standard is the guidance (Hans + Max, data-shape authority).** The long-format `entities/state_aliases.csv` IS the canonical alias mechanism (OWID standardization-layer shape: one row per (entity, alias_kind) carrying validity + per-alias `source_id`). The pipe `aliases` column on `geo.csv`/`electoral.csv` survives ONLY as a build-time DERIVED denormalization for cheap transliteration/display-synonym lookup (and yen-ask grounding): no validity, no registry code, regenerated from the long-format table, never hand-authored. The final micro-call (drop the derived pipe entirely vs keep it) sits with Hans + Max per [docs/concepts/owid-alignment.md](../docs/concepts/owid-alignment.md); until they rule, the executing agent keeps the derived pipe. OWID-pure long-format is the direction.
...
### Round-8 LGD source files the operator provides (drop into `datasets/ephemeral/`; XLS preferred per 0d.5)
The operator supplies the LGD portal exports below; each is parsed ONCE into a committed CSV under `datasets/reference/lgd/` at PR-stage 0a. XLS preferred (`openpyxl`, cells read as strings - no leading-zero coercion, no HTML chrome); HTML accepted. v1-required unless marked parked.

| # | LGD export | Yields | Feeds | v1 |
| --- | --- | --- | --- | --- |
| 1 | All States/UTs of India | lgd_state_id, lgd_name, kind, census_2001/2011 codes | `state_codes.csv` spine + census `state_aliases` rows | required |
| 2 | All Districts of India | lgd_district_id, lgd_name, lgd_state_id, census codes | `geo.csv` district rows + district FK for membership | required |
| 3 | All Sub-districts of India | lgd_subdistrict_id, name, parent district | parked (parse-and-park, 0d.5 / section-0c item 7) | parked |
| 4 | Assembly Constituencies (LGD AC register) | lgd_ac_id, ac_name, lgd_state_id, AND (if present) the ECI/delimitation AC number | `electoral.csv` AC rows + the `eci_no` direct-join bind | required |
| 5 | Parliamentary Constituencies (LGD PC register) | lgd_pc_id, pc_name, lgd_state_id, AND (if present) the ECI PC number | `electoral.csv` PC rows + the PC `eci_no` bind | required |
| 6 | AC <-> PC mapping (combined assembly-parliament-constituency export) | AC -> parent PC of same delimitation | `electoral.csv` `parent` | required |
| 7 | Constituency Coverage Report (AC <-> District, 1:many) | (lgd_ac_id, lgd_district_id) coverage edges | `electoral_district_membership.csv` (`is_primary`) | required |

Load-bearing note: files #4/#5 are the prize for the `eci_no` bind. If the LGD AC/PC export carries the ECI/delimitation constituency number alongside the LGD id, the `eci_no` bind is a DIRECT join (no name-clean, and 0c does not need its own sub-plan). If it does NOT, fall back to name+alias match within (state, delim_year) per 0c, which may spawn the ~4000-AC name-clean sub-plan. Ask the operator to confirm which columns #4/#5 actually carry BEFORE starting PR-stage 0c.
...
2. `grep -rniE 'census_20(01|11)_code'` - find any indicator/datapoint/loader that EXACT-MATCH-JOINS on a census code (vs carrying it as a label). Each such join becomes a validity-scoped `state_aliases` (or district-alias) row, NEVER a key - census codes renumber across 2001/2011 + reorganizations and go null for post-census entities (Telangana/Ladakh/DNH-DD), so a `JOIN ON census_*_code` silently drops them (phantom Rosling Gap).
