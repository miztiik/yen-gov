...
| 0b | `entities/state_codes.csv` (LGD spine + ISO seed) + columns.json file_class + fk-validator | fk-validator + iso-seed-coverage | - | TODO (blocks on 0a) |
| 0c | regenerate `electoral.csv` (+eci_no,+aliases) + `geo.csv` (+census,+aliases) + `electoral_district_membership.csv` + eci_no DIRECT JOIN + diff-receipt + `parties.csv` rename | fk-validator + eci-bind-coverage + diff-receipt | - | TODO (blocks on 0b) |
| 0e | ECI + census decommission sweep + receipt under `datasets/_ops/` | eci-census-decommission-sweep | - | TODO (blocks on 0c) |
