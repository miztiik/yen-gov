...
**Status**: Proposed - PR-A plan-doc row to open and merge first.
...
| --- | :---: | --- | --- | --- | --- | --- |
| A | _pending_ | In progress | Gregor + Fowler + Hans + Max | Create this plan-doc. No runtime behavior. | File exists under `TODO/`; status ledger and PR DAG are clear; `git diff --check` clean. | Stop if unrelated dirty worktree changes would need staging. |
