...
**Phase 2 (per-family ingestion)** is the active phase. Phase 1 + all infrastructure (T.1 / T.2 / T.3 / T.0d / T.0e / G.1 / S.1) ✅ DONE. P.1 Energy partly done (P.1.A + P.1.B SHIP-LIFT-ONLY merged; C5+C6 reader-switch + P.1.C + P.1.D pending). P.2 Livestock-NDLM in progress on parallel branch.
...
| - | --- | :-: | --- |
| 1 | **P.1 Energy -- 7c-N residue triage** (10 shards remain in `datasets/indicators/in/energy/`) | ◻ NEXT | Per-shard classify into (a) composer / compute-on-read per D33.8, (b) P.1.C meadow input via `git mv`, or (c) dead -> `git rm`. Mandatory audit: `grep_search` for `load_(shard|meadow)\([^)]*"<basename>"` across `backend/**/*.py` before any `git rm`. Completion criterion ([ADR-0041](../docs/architecture/decisions/0041-meadow-tier.md)): `git ls-tree origin/main -- datasets/indicators/in/energy/` empty. |
| 2 | **P.1 Energy -- Tier-B fence file rename** (`datasets/_ops/legacy-folded-indicator-shards.txt` -> `datasets/_ops/meadow-shard-contract.txt`) | ◻ READY | DEFERRED from PR 7c-4 per ADR-0041 §Doc-impact. Rename + rewrite header ("countdown to retirement" -> "perimeter for canonical-input contract") + update `backend/yen_gov/validate.py` Tier-B symbol + scrub doc references. Safe to fold into PR 1. |
| 3 | **P.1 Energy -- sources.parquet vintage backfill + Tier-B vintage check** | ◻ READY | Backfill ICED + RBI 2024-25 rows where `vintage` was empty pre-7c; add Tier-B rule `tier_b_meadow_vintage_matches_source_id` that walks `_meadow/*/`, derives `(source, vintage)` from the path, and asserts every observation's `source_id` resolves to a row with matching `producer` + `vintage` per ADR-0041 non-negotiable #4. |
| 4 | **P.1 Energy -- PR 7d IA editorial pass** | ◻ READY | Prune 36-card wall on `/s/<state>/t/energy` per Jony; ACS-ARR copy rewrite per Citizen; scroll-narrative cascade. MANDATORY subagent invocation: Jony + Citizen. May cascade 2-3 follow-up PRs. |
| 5 | **Caveat-authoring next batch** (extend PR-E + PR-H pattern to ~30 stub indicators) | ⏳ partly | Audit `methodology.known_caveats: []` + canonical-allowlist `caveats?` gap; group by family; Hans authors bullets per family. Recommend per-family bundling (5-7 PRs total). |
