...
| 1.8a-bis | **PENDING (PR-O-prereq)**: SQL-identifier decoupling — prerequisite for every per-family rename in 1.8b–f. Add optional `table_name: string` field to `datasets/schemas/manifest.schema.json` (minor bump + `x-changelog` entry). Backend `canonical/writer.py` populates `table_name` per Parquet entry in `_regenerate_manifest`. Frontend `lib/duckdb.ts` derives the DuckDB view name from manifest `table_name` instead of from the filename stem; the hardcoded filename→view-name map is **deleted** (Fowler delete-first). Backend `admin/inventory.py` classifier moves from stem-substring matching onto a `kind` field also surfaced from the manifest (or a small lookup keyed on the new naming rule). Tests: backend `test_canonical_writer*` extended to assert `table_name` is emitted; frontend `duckdb.test.ts` extended to assert lookup-through-manifest works for at least one renamed and one not-renamed entry. No files renamed in THIS row — only the indirection added. Lands as its own PR-O-prereq. | Gregor + Fowler |
| 1.8b | **PENDING (PR-O — split per Option C; lands AFTER 1.8a-bis merges, per Fowler two-hat rule)**: stop the bleed for elections legacy shards. **Does NOT delete files yet.** Sub-steps: (1) retire backend writers that still emit `parties.json`, `result.summary.json`, `_inventory.json` (`pipeline/run.py:105`, `cli.py:564`, `pipeline/people_ingest.py:38,213`); (2) reroute `coverage.py:176-177` off the on-disk shards onto canonical Parquet / dim tables; (3) refactor `frontend/src/lib/charts/stacked-trend/adapter-elections.test.ts` to use in-memory `ResultSummaryDoc` fixtures instead of `readFileSync` of `result.summary.json`; (4) update `frontend/src/lib/paths.test.ts:23` example path; (5) rename `datasets/elections/observations.parquet` → `election_results.parquet` and populate manifest `table_name: "election_results"` (the indirection added in 1.8a-bis is the prerequisite); (6) **citizen-honest rename surface (Hans)**: append first entry to new `datasets/CHANGELOG.md` (append-only, dated, citizen-readable) AND add top-level `deprecations[]: [{old_path, new_path, deprecated_at, removed_at?}]` array to `datasets/manifest.json` (schema minor bump). Frontend loader logs one-line console warning if anything still resolves `observations.parquet` during the deprecation window; (7) new pytest `backend/tests/test_no_legacy_json_emit.py` against `tmp_path` corpus per Fowler — runs pipeline, asserts `rglob("*.json")` under `datasets/elections/` is empty (Holy Law #10: the test that would have caught the regression). **Sqlite emitter (`emit/sqlite.py`) left alone here** — couples to 1.8e. | Fowler + Hans |
| 1.8b-ii | **PENDING (PR-O-ii)**: actual deletion of `datasets/elections/<event>/<state>/parties.json`, `result.summary.json`, `_inventory.json` after one clean pipeline run under PR-O proves no writer regenerates them. Prune every empty parent directory (`find datasets/elections -type d -empty -delete` re-run until idempotent). Append per-path deletion rows to `docs/architecture/canonical-pivot-deletion-manifest.md` §6a. | Fowler |
...
1. **PR-O-prereq (row 1.8a-bis)** — SQL-identifier decoupling via manifest `table_name`. Pure structural Tidy-First commit; no files renamed; loader prefers `table_name`, falls back to filename stem. Backend writer test + frontend `duckdb.test.ts` extension. Merge, deploy, observe one clean Pages run.
2. **PR-O (row 1.8b)** — elections legacy-shard writer retirement + rename `elections/observations.parquet` → `elections/election_results.parquet` + `datasets/CHANGELOG.md` first entry + `manifest.deprecations[]` extension + new pytest `test_no_legacy_json_emit.py`. Citizen-facing: zero URL/copy change; console warning logs only during deprecation window. Does NOT delete files yet.
3. **PR-O-ii (row 1.8b-ii)** — actual `git rm` of `parties.json` / `result.summary.json` / `_inventory.json` after PR-O proves no regenerator. Empty-parent prune. Deletion-manifest §6a entries.
4. **PR-P (row 1.8c)** — delete ~7,254 per-AC `results/<ac>.json` shards. Playwright sample verification.
5. **PR-Q (row 1.8d)** — small dead sweep + move `taxonomy/operator_state.parquet` → `_ops/operator_state.parquet`.
...
7. **PR-R (row 1.8e, DEFERRED)** — sqlite deletion blocked on psephlab migration.
8. **PR-S (row 1.8f, DEFERRED to Phase 2+)** — people sidecars deletion blocked on `dim_candidates` richness extension.
