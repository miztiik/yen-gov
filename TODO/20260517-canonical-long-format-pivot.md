...
| 6 | **P.1 Energy -- P.1.C** (9 indicators: coal / oil / primary / final / pipeline / thermal-retired / renewable-grid / plant-load-factor / renewable-potential) | ⏳ IN PROGRESS PR-Q #307 at `_pending_` (1/9 coal shipped) | Sequenced AFTER PR 1 -- bucket-(b) shards become P.1.C meadow input. PR-Q (2026-05-25) ships `state-coal-consumption-mt` via NEW `energy_fuel_consumption` parquet stem (450 obs, FY 2006-07 to 2024-25, 26 sub-national entities + IN-prefix; ICED 2024-25 source `src-c222a8e2cd61`; 3 Hans caveats on the canonical-allowlist descriptor). Follow-up PR-R / PR-S / PR-T / PR-U cohort: `state_rooftop_solar_capacity_mw` (321, non-faceted), `india_thermal_capacity_retired_mw` (29, 2 facets), `national_primary_energy_supply_mtoe` (140, 7-fuel), `state_oil_product_consumption_kt` (2901, 7-product), `state_plant_load_factor_pct` (1652, 8-fuel), `state_power_purchase_share_pct` (2658, 12-source), `national_final_energy_consumption_by_sector_mtoe` (360, sector x fuel composite). Subagent: Max (indicator IDs), Hans (tier assignments), Gregor (paired-test atomicity per §15). |
