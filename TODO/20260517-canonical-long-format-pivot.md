...
**Last Updated**: 2026-05-22 (Phase-2 pre-flight: PR1 `8de71a4a` shipped Tier-B forbid-new-folded-indicator-shards check; PR2 `8fb3e935` shipped descope = T.1 status reconcile + features-KEEP doc + G.1 strangler-fig handover; **G.1.a entity-lift SHIPPED** as `ee441193` (PR #89) — 31 office_bearer rows registered in `entities.parquet` (185 → 216); `entity.schema.json` v1.1 → v1.2; Tier-A parity oracle pins `dim_offices.office_id == entities.entity_id[entity_type='office_bearer']`. **G.1.b reader-switch IN-FLIGHT** on `feat/phase-2-preflight-g1b-cm-terms-reader-switch` — `cm_terms_seed.py` now reads office identity from `entities.parquet WHERE entity_type='office_bearer'` instead of computing it from cm_terms.json state codes; Parquet outputs verified byte-identical to baseline (`dim_offices.parquet` 2,062 B, `governments_office_holdings.parquet` 25,473 B, `sources.parquet` 6,203 B); NEW Tier-A parity oracle `test_g1b_cm_terms_reader_switch_parity.py` (3 tests). G.1.c JSON+seed-delete still open per [`TODO/20260522-g1-cm-terms-retirement-handover.md`](20260522-g1-cm-terms-retirement-handover.md). Next after G.1.b merges: G.1.c.)
