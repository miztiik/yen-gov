...
| **T.3** | Indicator catalogue widens for topic tags + drops topic prefix. Bump `indicator.schema.json` minor: add `topic_tags[]` (FK → `taxonomy/topics.parquet`), add `id_aliases[]` (one-release back-compat), enforce new id shape per `canonical-store.md §2a`. Migrate 110 legacy ids; populate `id_aliases` with old `<topic>/<id>` form; frontend dereferences via alias. Add `taxonomy/indicator_topic_tags.parquet` M:N join + `topic_tags[]` denormalised projection on `taxonomy/indicators.parquet`. **Fused atomic commit.** | ⏳ STACKED-PR-OPENED #98 — branch `feat/t3-indicator-catalogue-v1.1-topic-tags-and-aliases @ f22f2552` opened as PR #98 with `--base feat/p1-energy-pivot` (Path E stacked-PR pattern; GitHub auto-retargets to main when PR #?? P.1.A merges). 3 commits: `4c87b85e` (schema bump + Pydantic + Tier-B + 30 tests + 23 vitest) → `f13708df` (docstring polish) → `f22f2552` (parquet regen + §10 anti-patterns + handover doc). Validation gates RE-RUN green on f22f2552 (2026-05-22): backend pytest 844 passed / 41 skipped (311s); Tier-B validate OK 0 issues; frontend vitest 15,691 passed / 6 skipped (57s); §13 browser smoke 3 routes OK. Pre-rebase audit (`git log --all --oneline -- <file>`) revealed `indicators_seed.py` + `test_indicators_seed.py` were INTRODUCED by P.1.A C3 `624852ff` and only ENHANCED by T.3 — carve-to-main rebase fails with DU conflicts (steals authorship from other agent). Stacked PR is the honest path. Schema bump v1.0→v1.1 + Pydantic + TS Zod + Tier-B 60-day window + 33-col parquet regen DONE; CLAUDE.md §10 anti-pattern bullets added 2026-05-22 (`id_aliases` pairing rule + topic-prefix-forbidden rule); 110 legacy id rewrites + frontend dereferencer DESCOPED (deferred to P.* — each family's PR rewrites its own ids and fills `id_aliases[]` at lift time, per the locked Q2 = Option A scope). Handover: [20260522-t3-ready-blocked-on-p1a-handover.md](20260522-t3-ready-blocked-on-p1a-handover.md). | structural + behavioural (fused) | T.2 + P.1.A merge |
...
| `datasets/boundaries/in/geojson/*.{sources,metadata,unkeyed}.json` (115 sidecars) + `S22-villages-index.json` | `datasets/boundaries/boundary_layers.parquet` + FK to `taxonomy/sources.parquet` | T.0d | ◻ READY (awaiting §1 sign-off) |
| Flat `datasets/boundaries/in/geojson/*.geojson` (73 files) | Hive-partitioned `datasets/boundaries/in/{country,states,districts,ac,pc,subdistricts,villages,postal}/state=<S>/...` per [T.0d spec §1](20260522-t0d-boundaries-consolidation-spec.md) | T.0d | ◻ READY (awaiting §1 sign-off) |
