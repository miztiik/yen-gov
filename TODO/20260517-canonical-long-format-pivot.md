...
| 1.8a | **DONE 2026-05-18 (PR-N revised)**: scope correction recorded. Reverted false "deletion complete" claims in `backend/yen_gov/AGENTS.md`, `frontend/src/AGENTS.md`, `CLAUDE.md` §10 anti-pattern. Added `datasets/<family>/` directory invariant to `docs/architecture/data/canonical-store.md` §2 (canonical Parquet + dim/taxonomy Parquet only; no per-event JSON shards or sqlite). Inserted this 1.8a–f breakdown. Extended `docs/architecture/canonical-pivot-deletion-manifest.md` with the per-family deletion plan + audit columns for end-of-pivot validation. | Fowler |
| 1.8b | **PENDING (PR-O)**: delete elections "dead shard" sweep — `datasets/elections/<event>/<state>/parties.json`, `result.summary.json`, `_inventory.json`. Replacement: `dim_parties.parquet` + `dim_party_alliances.parquet` + `observations.parquet` party-totals (PRs G/H/I). Gate: `git grep -n "parties\.json\|result\.summary\.json\|_inventory\.json" frontend/src admin/src` returns zero matches against citizen-readable paths. | Fowler |
| 1.8c | **PENDING (PR-P)**: delete elections per-AC shards — `datasets/elections/<event>/<state>/results/<ac>.json` (~7,254 files). Replacement: `loadConstituencyResult` against `observations.parquet` + `dim_candidates.parquet` + `dim_acs.parquet` (PR-E). Gate: extended-routes Playwright passes after deletion for one TN AC + one KL AC + one WB AC representative sample. | Fowler |
| 1.8d | **PENDING (PR-Q)**: small dead sweep — `datasets/events/in/eci/` (superseded by `taxonomy/entities.parquet` event rows), `datasets/taxonomy/delimitation_lineage.json`, `datasets/taxonomy/facet-axes.json` (last two are pre-pivot intermediate forms; the live equivalents are the `.parquet` siblings already in `datasets/taxonomy/`). Gate: same git-grep + frontend/backend test pass. | Fowler |
| 1.8e | **DEFERRED until psephlab migrated (PR-R)**: delete 41 `datasets/elections/<event>/<state>/results.sqlite` files. Blocker: `frontend/src/lib/sql.ts` + `getDb` still serve `/psephlab` Compare/Psephlab routes (intentionally retained at PR-L). 1.8e cannot land until those routes either migrate to DuckDB-WASM views or are retired. Owner: Fowler + Citizen (decide retire vs migrate). | Fowler |
...
7. **Cleanup (Phase-1.8 pattern, mandatory)**: once the family's citizen chart is green and the migration is proven, `git grep` the codebase for any reader of pre-pivot artifacts in that family's tree (`datasets/<family>/in/**`, sidecar JSON shards, legacy sqlite); if zero matches, delete the legacy files in a follow-up sub-PR; record the deletion in [`docs/architecture/canonical-pivot-deletion-manifest.md`](../docs/architecture/canonical-pivot-deletion-manifest.md) §6a with the legacy path + canonical replacement + the gate command that proved no live reader. The family's `datasets/<family>/` directory MUST satisfy the directory invariant from [canonical-store.md §2](../docs/architecture/data/canonical-store.md) — Parquet only, no per-event nested JSON or sqlite — before the family is declared DONE.
