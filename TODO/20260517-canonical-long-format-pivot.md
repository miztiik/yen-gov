...
| 1.8b | **PENDING (PR-O — split per Option C)**: stop the bleed for elections legacy shards. **Does NOT delete files yet.** Sub-steps: (1) retire backend writers that still emit `parties.json`, `result.summary.json`, `_inventory.json` (`pipeline/run.py:105`, `cli.py:564`, `pipeline/people_ingest.py:38,213`); (2) reroute `coverage.py:176-177` off the on-disk shards onto canonical Parquet / dim tables; (3) refactor `frontend/src/lib/charts/stacked-trend/adapter-elections.test.ts` to use in-memory `ResultSummaryDoc` fixtures instead of `readFileSync` of `result.summary.json`; (4) update `frontend/src/lib/paths.test.ts:23` example path; (5) rename `datasets/elections/observations.parquet` → `election_results.parquet` per 1.8a-bis (depends on 1.8a-bis landing first). After this PR, one full pipeline run MUST NOT regenerate any of the retired shards. **Sqlite emitter (`emit/sqlite.py`) left alone here** — couples to 1.8e. | Fowler |
