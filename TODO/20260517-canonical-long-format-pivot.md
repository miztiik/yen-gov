...
| 1.8b | **PARTIALLY DONE — sub-step (5) shipped in PR-O.1 (row 1.8b-i). REMAINING work = PR-O.2 (PENDING, lands AFTER PR-O.1 merges + one clean Pages run, per Fowler two-hat rule)**: stop the bleed for elections legacy shards. **Does NOT delete files yet.** Sub-steps still open: (1) retire backend writers that still emit `parties.json`, `result.summary.json`, `_inventory.json` (`pipeline/run.py:105`, `cli.py:564`, `pipeline/people_ingest.py:38,213`); (2) reroute `coverage.py:176-177` off the on-disk shards onto canonical Parquet / dim tables; (3) refactor `frontend/src/lib/charts/stacked-trend/adapter-elections.test.ts` to use in-memory `ResultSummaryDoc` fixtures instead of `readFileSync` of `result.summary.json`; (4) update `frontend/src/lib/paths.test.ts:23` example path; (6) **citizen-honest rename surface (Hans)**: append first entry to new `datasets/CHANGELOG.md` (append-only, dated, citizen-readable) AND add top-level `deprecations[]: [{old_path, new_path, deprecated_at, removed_at?}]` array to `datasets/manifest.json` (schema minor bump). Frontend loader logs one-line console warning if anything still resolves `observations.parquet` during the deprecation window; (7) new pytest `backend/tests/test_no_legacy_json_emit.py` against `tmp_path` corpus per Fowler — runs pipeline, asserts `rglob("*.json")` under `datasets/elections/` is empty (Holy Law #10: the test that would have caught the regression). **Sqlite emitter (`emit/sqlite.py`) left alone here** — couples to 1.8e. | Fowler + Hans |
