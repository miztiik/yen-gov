...
| 1.9 | **REMEASURED 2026-05-19 (post PR-Q, end of Phase-1 deletion sweep)**: `.git/` = 115 MB (Δ −5 MB vs 2026-05-18 baseline), `datasets/` = 196 MB (Δ −47 MB / −19%), JSON file count = 4,349 (Δ −9,300 / −68% vs implied pre-cleanup ~13,649). Parquet count = 7 (canonical store + small extras); sqlite count = 41 (all deferred to 1.8e). Cleanly under the 2 GB / 60 s clone-time thresholds. Forecast "expect `datasets/` to drop by ~9,300 JSON files" met. **Next checkpoint**: remeasure after 1.8e (sqlite sweep, blocked on Psephlab migration). If `.git/` growth crosses 500 MB before Phase 2, escalate to Fowler + Gregor on Git LFS. **2026-05-18 baseline retained for reference**: `.git/` = 120 MB, `datasets/` = 243 MB, total ≈ 363 MB. | Fowler |
