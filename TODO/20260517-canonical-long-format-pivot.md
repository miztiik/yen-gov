...
| 1.6b | **Explore.svelte → DuckDB-WASM**: migrate the SQL playground off `getDb` / `results.sqlite` onto canonical Parquet via `query()` + `registerTable()`. Last remaining `getDb` caller on the citizen path. Preserve the SQL allowlist guard + preset catalogue; rewrite presets in terms of `elections.{observations,dim_acs,dim_candidates,dim_parties}`. Required before 1.8 can delete `_old/`. | Fowler |
