...
| 1.3c | Multi-route migration (PR-G): swap `Party.svelte`, `ElectionSeatsTrend.svelte`, `Settings.svelte`, `IndiaMap.svelte` to view-model loaders. Delete `fetchResultSummary` + `fetchParties` from `lib/data.ts` in the same commit. `result.summary.json` + `parties.json` become Phase 1.8 delete candidates. | Fowler + Jony |
