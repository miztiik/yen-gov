...
**We kept SQL. We dropped SQLite.** The canonical store is queried in SQL — DuckDB-WASM in the browser, DuckDB CLI locally, DuckDB Python in the pipeline. DuckDB IS a SQL engine. What the canonical pivot retires is **SQLite as a shipped artifact** (`datasets/elections/<event>/<state>/results.sqlite` — 41 files) plus the `backend/yen_gov/emit/sqlite.py` emitter that produces them. That deletion is row 1.8e, deferred until `psephlab` no longer imports those files. Citizens / researchers can point any DuckDB client at `https://miztiik.github.io/yen-gov/data/elections/election_results.parquet` and run the same SQL the site runs — strictly better than the per-state shards.
...
| 1.8b-writers-b | **PENDING (PR-O.3b — lands AFTER PR-O.3a merges + one clean Pages run on main, per Fowler two-hat rule)**: behavioural writer retirement now that emitters no longer depend on disk JSONs. **Sub-steps**: (1) retire the 3 `write_artifact` calls in `backend/yen_gov/pipeline/run.py` (per-AC `results/<n>.json` at line 76, `result.summary.json` at line 105, `parties.json` at line 116). The local-XLSX path `backend/yen_gov/cli.py::eci_statreport_emit_local` keeps its `write_artifact` calls for hand-import provenance until its caller decides otherwise; the live-fetch `cli.py::run` no longer needs them since the canonical Parquet + sqlite + csv all flow from in-memory data. (2) refactor `backend/yen_gov/pipeline/people_ingest.compare_winner_votes` (line 106) to query `election_results.parquet` via DuckDB instead of reading per-AC JSON shards; the "file not found" graceful fallback gets deleted since canonical Parquet is now the source of truth. (3) refactor `backend/yen_gov/pipeline/canonical_eci_backfill.py` — either rewrite to consume in-memory envelopes from a single `pipeline.run` invocation OR deprecate the standalone tool entirely (since `pipeline/run.py` will now write canonical inline once writers are wired, the standalone backfill becomes redundant for new ingest; keep for one-shot historical-event imports if needed). (4) add `backend/tests/test_no_legacy_json_emit.py` against `tmp_path` corpus — runs pipeline end-to-end, asserts `rglob("*.json")` under `datasets/elections/` is empty (Holy Law #10: the test that would have caught the regression had it existed). (5) reroute or delete the known trap `test_election_events_default_uniqueness_and_data_status_alignment` in `backend/tests/test_datasets_integrity.py:234` (reads `result.summary.json` from real corpus; will break the moment writers retire). **Out of scope here**: file deletion (row 1.8b-ii owns that, lands after PR-O.3b proves no writer regenerates the legacy shards). | Fowler + Hans |
| 1.8b-writers | **SUPERSEDED 2026-05-19** by the explicit -a/-b split: 1.8b-writers-a (PR-O.3a, emitter structural refactor — DONE), 1.8b-writers-b (PR-O.3b, writer retirement + people_ingest reroute + backfill rewrite + no-emit test — PENDING). The original single-row scope mixed structural emitter changes (cheap to land) with behavioural writer retirement that couples to a third consumer (`people_ingest`) the row failed to identify; honest mid-PR re-scoping per Fowler two-hat rule. Keep this row as the historical pointer; do not add new sub-steps here. | (closed) |
| 1.8b | **SUPERSEDED 2026-05-19** by the explicit per-PR split: 1.8b-i (PR-O.1, structural rename — DONE), 1.8b-min (PR-O.2-minimal, deprecation surface + test refactor — DONE), 1.8b-writers-a (PR-O.3a, emitter structural refactor — DONE), 1.8b-writers-b (PR-O.3b, writer retirement + people_ingest reroute + backfill rewrite + no-emit test — PENDING), 1.8b-ii (PR-O.4, file deletion — PENDING). Keep this row as the historical pointer; do not add new sub-steps here. | (closed) |
| 1.8b-ii | **PENDING (PR-O.4 — lands AFTER PR-O.3b writer-retirement merges + one clean pipeline run proves no writer regenerates them)**: actual deletion of `datasets/elections/<event>/<state>/parties.json`, `result.summary.json`, `_inventory.json`. Prune every empty parent directory (`find datasets/elections -type d -empty -delete` re-run until idempotent). Append per-path deletion rows to `docs/architecture/canonical-pivot-deletion-manifest.md` §6a. | Fowler |
| 1.8c | **PENDING (PR-P)**: delete elections per-AC shards — `datasets/elections/<event>/<state>/results/<ac>.json` (~7,254 files). Replacement: `loadConstituencyResult` against `election_results.parquet` + `dim_candidates.parquet` + `dim_acs.parquet` (PR-E). Gate: extended-routes Playwright passes after deletion for one TN AC + one KL AC + one WB AC representative sample. Prune empty parent dirs. | Fowler |
| 1.8d | **PENDING (PR-Q)**: small dead sweep — `datasets/events/in/eci/` (superseded by `taxonomy/entities.parquet` event rows), `datasets/taxonomy/delimitation_lineage.json`, `datasets/taxonomy/facet-axes.json` (last two are pre-pivot intermediate forms; the live equivalents are the `.parquet` siblings already in `datasets/taxonomy/`). Plus move `taxonomy/operator_state.parquet` → `_ops/operator_state.parquet` per 1.8a-bis. Gate: same git-grep + frontend/backend test pass; empty parents pruned. | Fowler |
| 1.8e | **DEFERRED until psephlab migrated (PR-R)**: delete 41 `datasets/elections/<event>/<state>/results.sqlite` files. Blocker: `frontend/src/lib/sql.ts` + `getDb` still serve `/psephlab` Compare/Psephlab routes (intentionally retained at PR-L). 1.8e cannot land until those routes either migrate to DuckDB-WASM views or are retired. Also retires `backend/yen_gov/emit/sqlite.py`. Owner: Fowler + Citizen (decide retire vs migrate). | Fowler |
| 1.8f | **DEFERRED to Phase 2+ (PR-S)**: delete `datasets/people/<event>/<ac>/<cand>.json` (~3,983 files). Blocker: `fetchPerson` at `frontend/src/lib/data.ts:323` still drives the candidate-detail page with biographic fields (affidavit summary, social URLs, etc.) that the current `dim_candidates.parquet` does NOT carry. Out of Phase-1 scope: Phase 1 was an elections-results pivot, not a biographic-richness pivot. Either extend `dim_candidates` (or add a `dim_candidate_bio.parquet`) in Phase 2 prep, or retire the candidate-detail page; decide before scheduling. | Fowler + Hans |
| 1.9 | **BASELINE MEASURED 2026-05-18 (PR-N revised)**: pre-cleanup repo size — `.git/` = 120 MB, `datasets/` = 243 MB, total ≈ 363 MB. Comfortably under the 2 GB / 60 s clone-time thresholds. **Not closed**: remeasure after 1.8b–d land (expect `datasets/` to drop by ~9,300 JSON files); remeasure again after 1.8e + 1.8f. If `.git/` growth crosses 500 MB before Phase 2, escalate to Fowler + Gregor on Git LFS. | Fowler |
| 1.10 | **PENDING (PR-T) — Proto-ontology bootstrap (Hans + Max + Jony, 2026-05-18)**: minimum OWID-floor metadata on indicator/entity/party taxonomy so a future LLM (Phase 4) can ground answers, AND citizens get a visible win in the same PR (Jony ROI gate). **Indicator schema additions** (`datasets/schemas/indicator.schema.json` minor bump): required `description_short` (≤280 chars, citizen-readable), optional `description_long` (multi-para markdown methodology — numerator/denominator/scope/known breaks), optional `unit` + `short_unit` (e.g. `"percent"` / `"%"`, `"INR crore"` / `"₹cr"`), optional `derivation_note` (one sentence naming numerator+denominator when `derivation != "raw"`), optional `source_ref[]` (catalogue-level FK array into `taxonomy/sources.parquet`; per-observation `source_id` unchanged), optional `valid_period_grain` enum (`year`/`fiscal_year`/`election_date`), optional `valid_entity_grain` enum (`country`/`state`/`district`/`ac`/`pc`), optional `is_input_output_outcome` enum (input/output/outcome — disambiguates MGNREGA-disbursed vs rural-wage-real). **Entity schema additions** (`datasets/schemas/entity.schema.json` minor bump): required `entity_type` enum, optional `parent_entity_id` (district→state→country), optional `lifespan: {from, to}` (Telangana lifespan starts 2014-06-02; J&K status change 2019). **Party schema additions** (`datasets/schemas/taxonomy-parties.schema.json` minor bump): optional `successor_party_id` / `predecessor_party_id` for splits/mergers (Shiv Sena / NCP). **Backfill same PR**: populate `description_short` on every existing indicator row; populate `unit`/`short_unit` on every numeric indicator; populate `lifespan` for Telangana/J&K. **Citizen surface (Jony — ships in same PR)**: extend chart-wrapper component to read `indicator.short_unit` for Y-axis label and `indicator.description_short` for one-line legend caption (visible at page load, NOT behind tooltip/expander/modal). One generic-component change pays for all 30+ indicators. **Explicitly OUT of scope**: RDF/JSON-LD, SKOS, embedding vectors, `synonyms[]`, `related_indicators[]`, separate concepts file, methodology modals/drawers — defer to Phase 4/5 when the LLM ships. **Out-of-scope-but-allowed-later additions**: a `datasets/CHANGELOG.md` entry for the schema bumps. | Hans + Max + Jony |
...
### §13b. CURRENT NEXT STEPS (2026-05-18, post four-agent debate on SQL/ontology/1.8 scope)

Phase 1 elections migration is complete through 1.7. PR #41 (`feat/canonical-pivot-1.8-old-deletion-cleanup`) is OPEN on `main` carrying the §1 SQL-vs-SQLite framing, the 1.8a–f sub-row breakdown, the Parquet naming convention, the deletion manifest, and the new row 1.10 (proto-ontology bootstrap). **Merge PR #41 first**, then execute in this exact order:

1. **PR-O-prereq (row 1.8a-bis)** — *DONE 2026-05-18, see row 1.8a-bis above.* SQL-identifier decoupling via manifest `table_name` + `kind`. Pure structural Tidy-First commit; zero files renamed; loader prefers `table_name`, falls back to filename stem. Backend writer tests + frontend `duckdb.test.ts` extended. Awaiting parent-agent merge + deploy + one clean Pages run before PR-O begins.
2. **PR-O.1 (row 1.8b-i)** — *DONE 2026-05-18, see row 1.8b-i above.* Pure structural split: `git mv elections/observations.parquet elections/election_results.parquet` + writer per-family fact-table stem (`FAMILY_FACT_TABLE_STEM`) + manifest regen + frontend/admin/test view-name sweep. Zero behavioural change; tests pass before AND after. Awaiting merge + one clean Pages run before PR-O.2 begins.
3. **PR-O.2 (row 1.8b, remaining sub-steps)** — elections legacy-shard writer retirement (sub-steps 1–4) + `datasets/CHANGELOG.md` first entry + `manifest.deprecations[]` schema minor bump (sub-step 6) + new pytest `test_no_legacy_json_emit.py` (sub-step 7). Citizen-facing: zero URL/copy change; console warning logs only during deprecation window. Does NOT delete files yet.
4. **PR-O-ii (row 1.8b-ii)** — actual `git rm` of `parties.json` / `result.summary.json` / `_inventory.json` after PR-O.2 proves no regenerator. Empty-parent prune. Deletion-manifest §6a entries.
5. **PR-P (row 1.8c)** — delete ~7,254 per-AC `results/<ac>.json` shards. Playwright sample verification.
6. **PR-Q (row 1.8d)** — small dead sweep + move `taxonomy/operator_state.parquet` → `_ops/operator_state.parquet`.
7. **PR-T (row 1.10)** — proto-ontology bootstrap. Three schema minor-bumps (indicator, entity, party) + backfill of existing rows + ONE citizen surface (chart Y-axis short_unit + legend description_short caption). Hans + Max + Jony joint ownership. This row's metadata is the grounding layer Phase 4 will consume — NO separate ontology file.
8. **PR-R (row 1.8e, DEFERRED)** — sqlite deletion blocked on psephlab migration.
9. **PR-S (row 1.8f, DEFERRED to Phase 2+)** — people sidecars deletion blocked on `dim_candidates` richness extension.

**Three decisions from the 2026-05-18 debate that are now binding** (do not relitigate):

- **SQL stays. SQLite goes.** DuckDB IS SQL. Anything claiming the pivot "drops SQL" is wrong; correct it on sight (see §1 SQL-vs-SQLite paragraph).
- **`dim_*` carve-out preserved.** Kimball convention. `<family>_<role>.parquet` for fact tables only. Do NOT rename `dim_candidates.parquet` → `elections_dim_candidates.parquet` (would couple cross-family dims to one family).
- **1.8a-bis ships separately from 1.8b** (Fowler two-hat rule, rollback blast radius). Structural change merges, deploys, then behavioural rename follows.
