...
| 1.8b-writers-b-main | **PENDING (PR-O.3b-main — lands AFTER PR-O.3b-pre merges)**: behavioural writer retirement now that `build_slice_envelope` exists as the in-memory primary API. **Sub-steps** (unchanged from the original 1.8b-writers-b scope, minus the structural extraction that already shipped in -pre): (1) retire the 3 `write_artifact` calls in `backend/yen_gov/pipeline/run.py` (per-AC `results/<n>.json` at line 76, `result.summary.json` at line 105, `parties.json` at line 116). The local-XLSX path `backend/yen_gov/cli.py::eci_statreport_emit_local` keeps its `write_artifact` calls for hand-import provenance until its caller decides otherwise; the live-fetch `cli.py::run` no longer needs them since the canonical Parquet + sqlite + csv all flow from in-memory data. (2) refactor `backend/yen_gov/pipeline/people_ingest.compare_winner_votes` (line 106) to query `election_results.parquet` via DuckDB instead of reading per-AC JSON shards; the "file not found" graceful fallback gets deleted since canonical Parquet is now the source of truth. (3) switch the `backfill_elections` driver to call `build_slice_envelope` directly when invoked from `pipeline/run.py` (so the live pipeline writes canonical inline without round-tripping through `_process_slice`'s disk read); keep `_process_slice` for the standalone `cli.py::canonical-backfill-eci` tool that one-shot-imports historical events from existing on-disk JSON corpora. (4) add `backend/tests/test_no_legacy_json_emit.py` against `tmp_path` corpus — runs pipeline end-to-end, asserts `rglob("*.json")` under `datasets/elections/` is empty (Holy Law #10: the test that would have caught the regression had it existed). (5) reroute or delete the known trap `test_election_events_default_uniqueness_and_data_status_alignment` in `backend/tests/test_datasets_integrity.py:234` (reads `result.summary.json` from real corpus; will break the moment writers retire). **Out of scope here**: file deletion (row 1.8b-ii owns that, lands after PR-O.3b-main proves no writer regenerates the legacy shards). | Fowler + Hans |
