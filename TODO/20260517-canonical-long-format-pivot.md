...
**Last Updated**: 2026-05-20 (Phase 1.8f ✅ closed by PR-S.1 + PR-S.2; sole ⏳ remaining row is 1.8e closeout / Psephlab finalization)
**Status**: Phase 0 complete (14/15 ✅ DONE; 0.13 ⊘ DROPPED with documented replacement pattern). Phase 1 — elections-results pivot + dim tables + view-model loaders + deletion sweep 1.8a–1.8d ✅ DONE (on-disk audit 2026-05-19), 1.8f ✅ DONE 2026-05-20 (PR-S.1 lifted bio onto `dim_candidates.parquet` v1.2; PR-S.2 retired the 3,983 person JSONs + `people.entity.schema.json` + `fetchPersonEntity` + `slugifyCandidate` + one-shot backfill tool + refactored `people_ingest` to UPSERT onto canonical store). **1 honestly-named ⏳ NOT DONE row remains (1.8e Psephlab sqlite migration finalization)** — Phase 2 (Energy) does NOT start until it lands per user direction 2026-05-19 (Option A: finish Phase 1 honestly).
...
**Phase 0 verdict** (audited 2026-05-19): 14/15 ✅ DONE; 1/15 ⊘ DROPPED with documented replacement pattern. Phase 0 IS complete *as scoped*, but its `_old/` deletion intent is fulfilled by Phase 1 rows 1.8a–1.8f — so closure on that intent waits for rows 1.8e + 1.8f.
...
**Exit criteria** (audited 2026-05-19; ⏳ items below are the ONLY blockers between Phase 1 and Phase 2):
...
- ⏳ **1.8e — Psephlab + Compare migration to DuckDB-WASM canonical loaders; delete 41 `.sqlite` files + `lib/sql.ts` + `emit/sqlite.py`. Scheduled PR-R.1 → PR-R.2 → PR-R.3.**
- ⏳ **1.8f — `dim_candidates` schema extension with biographic columns; delete 3,983 person JSONs + retire `people_ingest.py`. Scheduled PR-S.1 → PR-S.2.**

**Phase 2 (Energy) does NOT start until both ⏳ items above are ✅ DONE.** Per §0d (no goalpost-moving via vocabulary drift). If either is re-scoped to ⊘ DROPPED, this exit-criteria block is amended in the same commit that flips the status.
