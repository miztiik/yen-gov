...
| 1.8 | **DONE 2026-05-18 (PR-N)**: `datasets/_old/` deletion confirmed and prose references swept. User decision overrode the original 5-day observation gate — git is the rollback path. The actual `git rm -rf datasets/_old/` happened in an earlier pivot commit (not on disk, not in HEAD at the time of this row's tick); PR-N closes the loop by purging stale present-tense `_old/` mentions from `backend/yen_gov/AGENTS.md`, `frontend/src/AGENTS.md`, `CLAUDE.md` §10 anti-pattern, and `frontend/src/lib/view-models/constituency.ts`. Defensive sentinel-skip in `inventory.py` and its `test_admin_inventory.py` fixture remain (correct as-is: a future commit re-introducing the directory would be silently ignored, not surfaced to operators). Historical docs (`canonical-pivot-deletion-manifest.md`, ADR-0030, migration-ledger) retain `_old/` references intentionally — they document the migration. `v1.0-canonical` tag deferred to a future operator decision. | Fowler |
| 1.9 | **DONE 2026-05-18 (PR-N)**: repo size measured — `.git/` = 120 MB, `datasets/` = 243 MB, total ≈ 363 MB. Comfortably under the 2 GB / 60 s clone-time thresholds; no Git LFS escalation needed for Phase 1. Revisit when Phase 2/3 land additional families. | Fowler |
