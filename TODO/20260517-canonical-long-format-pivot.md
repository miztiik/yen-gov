...
| 1.8b-writers | **PENDING (PR-O.3 — lands AFTER PR-O.2-minimal merges + one clean Pages run on main, per Fowler two-hat rule)**: retire the elections legacy JSON writers, coupled with the sqlite/csv migration the writers feed. Original 1.8b sub-steps (1) + (7) plus the sqlite/csv refactor that the user pre-emptively flagged. **Sub-steps**: (1) retire writers that still emit `parties.json`, `result.summary.json`, `_inventory.json` — `backend/yen_gov/pipeline/run.py:105` (3 write_artifact calls), `backend/yen_gov/cli.py:564 + 825`, `backend/yen_gov/pipeline/people_ingest.py:38 + 213`; (a) refactor `backend/yen_gov/emit/sqlite.py:72-76` to accept in-memory `RunResult` instead of reading per-AC JSON + parties.json from disk (~50 LOC); (b) refactor `backend/yen_gov/emit/csv_bundle.py:66` similarly to consume in-memory `RunResult` (~50 LOC); (c) drop `backend/yen_gov/pipeline/canonical_eci_backfill.py`'s legacy-JSON read path (it currently reads per-AC JSONs as the canonical writer's INPUT — the canonical envelope must come from the pipeline directly post-refactor); (2) new `backend/tests/test_no_legacy_json_emit.py` against `tmp_path` corpus per Fowler — runs pipeline end-to-end, asserts `rglob("*.json")` under `datasets/elections/` is empty (Holy Law #10: the test that would have caught the regression had it existed). **Known pre-existing trap**: `test_election_events_default_uniqueness_and_data_status_alignment` in `backend/tests/test_datasets_integrity.py:212` reads `result.summary.json` from the real corpus; passes today (files exist), will break the moment PR-O.3 stops regenerating them. Either reroute the test to the canonical Parquet OR delete (it duplicates Tier-A coverage already in `test_coverage.py`). **Out of scope here**: file deletion (row 1.8b-ii owns that, lands after PR-O.3 proves no writer regenerates the legacy shards). | Fowler + Hans |
| 1.8b | **SUPERSEDED 2026-05-19** by the explicit per-PR split: 1.8b-i (PR-O.1, structural rename — DONE), 1.8b-min (PR-O.2-minimal, deprecation surface + test refactor — DONE), 1.8b-writers (PR-O.3, writer retirement + sqlite/csv migration — PENDING), 1.8b-ii (PR-O.4, file deletion — PENDING). Keep this row as the historical pointer; do not add new sub-steps here. | (closed) |
| 1.8b-ii | **PENDING (PR-O.4 — lands AFTER PR-O.3 writer-retirement merges + one clean pipeline run proves no writer regenerates them)**: actual deletion of `datasets/elections/<event>/<state>/parties.json`, `result.summary.json`, `_inventory.json`. Prune every empty parent directory (`find datasets/elections -type d -empty -delete` re-run until idempotent). Append per-path deletion rows to `docs/architecture/canonical-pivot-deletion-manifest.md` §6a. | Fowler |
