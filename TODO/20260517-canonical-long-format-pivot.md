...
| 1.3d | Party.svelte migration (PR-H): extend `elections.dim_parties` with `alliance` column (`recognition` already present), build `loadPartySummary` view-model, swap `Party.svelte` off `fetchParties` + delete `fetchParties` from `lib/data.ts`. `parties.json` becomes a Phase 1.8 delete candidate. | Hans + Fowler |
