...
**Last Updated**: 2026-05-22 (Phase-2 pre-flight: PR1 `8de71a4a` shipped Tier-B forbid-new-folded-indicator-shards check; PR2 `8fb3e935` shipped descope = T.1 status reconcile + features-KEEP doc + G.1 strangler-fig handover; **G.1.a entity-lift SHIPPED** as `ee441193` (PR #89) — 31 office_bearer rows registered in `entities.parquet` (185 → 216); `entity.schema.json` v1.1 → v1.2; Tier-A parity oracle pins `dim_offices.office_id == entities.entity_id[entity_type='office_bearer']`. **G.1.b reader-switch SHIPPED** as `cc9ad5b7` (PR #90) — `cm_terms_seed.py` reads office identity from `entities.parquet WHERE entity_type='office_bearer'` instead of computing it from cm_terms.json state codes; Parquet outputs verified byte-identical to baseline; NEW Tier-A parity oracle `test_g1b_cm_terms_reader_switch_parity.py`. **G.1.c consolidate + retire SHIPPING 2026-05-22** on `feat/phase-2-preflight-g1c-office-holdings-consolidation` (Option B chosen after Hans + Fowler + Max 3-persona review): 31 per-state `cm_terms.json` → 1 `datasets/taxonomy/office_holdings.json` (359 holdings + 31 office_citations); NEW `office_holdings.schema.json` v1.0 + `office_holdings_seed.py` (~430 LOC) + 6 Tier-A tests; frontend `governments.ts` rewritten with in-loader field-name adapter back to legacy `GovernmentTerm` shape (Svelte components unchanged); `git rm` of 31 JSON + `cm_terms_seed.py` + `test_cm_terms_seed.py` + `state_government.schema.json` + `tools/author_cm_terms.py` + `tools/lift_cm_offices_to_entities.py`. Byte-identical Parquet verified 3rd time. 779 backend + 15983 vitest green; Tier-B 0 issues; §13 browser smoke clean on 5 routes.)
...
| **G.1** | **Office-bearers consolidation.** Create `governments/governments_office_holdings.parquet` fact (one row per `(person_id, office_id, tenure_start, tenure_end)`). Create `governments/dim_offices.parquet`. Add `entity_type='office_bearer'` rows to `taxonomy/entities.json` per §0e.6. Migrate `governments/in/states/<state>/cm_terms.json` → fact rows. Delete `governments/in/states/`. **DEFERRED 2026-05-22 to 3-PR strangler-fig (G.1.a entity-lift → G.1.b reader-switch → G.1.c JSON+seed-delete)** — audit during PR2 (`8de71a4a`+`3cb11036`) revealed `cm_terms.json` is the LIVE source-of-truth that `backend/yen_gov/canonical/cm_terms_seed.py:178` recompiles to Parquet every `emit-taxonomy` run; single-PR delete would brick the citizen "Your government" card. Full design + acceptance criteria + 4 rejected alternatives in [`TODO/20260522-g1-cm-terms-retirement-handover.md`](20260522-g1-cm-terms-retirement-handover.md). Original dependency (T.3 + S.1) still holds for G.1.a; G.1.b/c run after G.1.a regardless of T.3/S.1 status (entity-lift is the gating step). | structural + behavioural (fused per sub-PR) | yes if frontend consumes (today: `cm_terms` only — check usages) | T.3 + S.1 (for G.1.a) |
...
| `datasets/governments/in/states/` | `datasets/governments/governments_office_holdings.parquet` + `datasets/taxonomy/entities.parquet` (`entity_type='office_bearer'`) | G.1 (deferred 2026-05-22 to 3-PR strangler-fig G.1.a/b/c per [`TODO/20260522-g1-cm-terms-retirement-handover.md`](20260522-g1-cm-terms-retirement-handover.md)) |
