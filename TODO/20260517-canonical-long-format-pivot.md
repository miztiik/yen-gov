...
7. **PR-T (row 1.10)** — proto-ontology bootstrap. One indicator-schema minor bump (v4.3 → v4.4, additive: 8 optional fields including `description_short`, `description_long`, `short_unit`, `derivation_note`, etc.) + one party-schema BREAKING rename (v1.0 → v2.0: `successor_of` → `successor_party_id`, `predecessor_of` → `predecessor_party_id`) + ZERO entity-schema change (`entity_valid_from` / `entity_valid_to` already cover what row 1.10 originally called "lifespan"). Backfill: hand-author `description_short` on top-30 citizen-facing indicators + mechanical `short_unit` on numeric indicators + party FK rename migration + Tier-A TS union edits. ONE citizen surface (chart Y-axis `short_unit` + legend `description_short` caption, with graceful fallback to `unit` / `title`). Hans + Max + Jony joint ownership. This row's metadata is the grounding layer Phase 4 will consume — NO separate ontology file. Q1/Q2/Q3 amendments resolved 2026-05-19 (Hans + Max); see row 1.10 body for full rationale and rejected alternatives.
