...
> **Status (originally 2026-05-21):** AUTONOMOUS-UNSAFE BLOCKER. Surfaced 2026-05-21 during T.0c-ii consumer-port arc (PR #73 closeout). Branch `feat/canonical-pivot-T0c-ii-states-json-port` was opened, the parity audit ran, the blocker was identified, the branch was abandoned. **No code shipped from the port itself; this handover doc is the only artifact.**
>
> **UPDATE 2026-05-21 (next session, Path B per orchestrator):** Phase A landed via PR #75 (merge `5d82a276`). Audit-then-act: the 7 UT entries flagged "missing" turned out to be **ALREADY HAND-AUTHORED** in `taxonomy/entities.json` (U01 A&N, U02 Chandigarh, U03 DNH+DD merger, U04 Lakshadweep, U05 NCT Delhi, U07 Puducherry, U08 J&K UT, U09 Ladakh + U03-OLD and U06 as historical predecessors). `emit-taxonomy` ran clean (`entities.parquet` byte-stable at 185 rows). New parity oracle `backend/tests/test_states_parity.py` (4 tests, 192 LOC) asserts current-shape parity between `states.json` and `entities.json` with one whitelisted name deviation (`U08: "Jammu and Kashmir" vs "Jammu and Kashmir (UT)"` — temporal-aware disambiguation from historical `S09` row). §12 envelope per-row source_id deferred to Phase A.5 (schema v1.1→v1.2 + Pydantic + 8 MoHA-notification sources too much scope for the unblock PR).
>
> **CI-bug archaeology (incidental but unblocked the live citizen site for first time in ~6 merges):** the workflow `.github/workflows/deploy-site.yml` had stale `test -f _site/data/elections/election_results.parquet` paths from before PR #63's state-partition pivot — every PR since had merged in UNSTABLE state with the build-citizen-site job failing first, MASKING a separate Playwright regression in `Constituency.svelte` where `data-testid="candidate-biographics"` was wrapped in `{#if c.bio}` and never rendered when bio data was null. Fixed in PR #76 (workflow paths → `state=in_s22/election_results.parquet`, merge `166ffab4`) + PR #77 (drop `{#if c.bio}` guard, always render testid with "Not declared" fallback, merge `957735b7`). All three PRs (#75 → #76 → #77 reverse-chronological merge order: #77, #76, #75) shipped same day.
>
> **Phase B (backend consumer port) is NEXT and SAFE** — `coverage.py STATES_REL` + `tools/lgd/backfill_lgd_codes.py STATES_JSON` + `backend/yen_gov/sources/india_geodata/power_plants.py` + `backend/tests/test_datasets_integrity.py STATES_REGISTRY_PATH` all repoint to `datasets/taxonomy/entities.json` with `entity_type IN ('state','ut') AND entity_valid_to IS NULL` filter. No browser smoke needed. Phase C (frontend port + states.json delete + §13 browser smoke) is the autonomous-unsafe one that follows.
