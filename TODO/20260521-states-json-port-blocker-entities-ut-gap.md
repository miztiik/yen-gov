...
>
> **Phase C: ✅ COMPLETE** — branch `feat/states-json-port-phase-c-frontend-delete`, 2026-05-22.
>
> Frontend `fetchStates()` in `frontend/src/lib/data.ts` rewritten as a wrapper that fetches `/data/taxonomy/entities.json`, filters `entity_type IN ('state', 'ut') AND entity_valid_to === null`, and projects to the existing `StatesCollection` / `StateEntry` shape so call sites (`Home.svelte`, `ScopePicker.svelte`, `states.svelte.ts`) stay byte-identical. The `capital` field that lived on the legacy shim had zero downstream consumers per the Phase A audit and is intentionally not projected. Vitest fixture in `frontend/src/lib/data.test.ts` repointed to the entities.json envelope shape with filter assertions (country row filtered out, current state + UT kept, historical `entity_valid_to=2019` filtered out).
>
> Deletions in the same Tier-A commit: `datasets/reference/in/states.json` (the legacy shim), `datasets/schemas/state.schema.json` (its schema with zero remaining validatees), `backend/tests/test_states_parity.py` (the Phase A parity oracle — its single job was asserting lockstep between the two files, which is moot once one of them is gone). Backend `StatesCollection` + `StateEntry` Pydantic models removed from `backend/yen_gov/core/models.py` (would have failed at import time once `state.schema.json` was gone); paired round-trip test removed from `backend/tests/test_core_models.py`. Two validator-fixture spots in `backend/tests/test_validate.py` repointed from `state.schema.json` to `entity.schema.json` (the Tier-A tests use a representative schema; any well-formed schema with x-version + x-changelog works).
>
> Documentation updates (all comment-only or single-line reference updates, no behaviour change): `docs/architecture/frontend/routing.md` (state slug source-of-truth + collision-test code block), `docs/architecture/decisions/0028-url-scheme-place-first-flat-indicator-slug.md` (geography registries list), `docs/architecture/backend/core.md` (state.schema.json → StatesCollection example replaced by district.schema.json), `docs/architecture/backend/sources-eci.md` (state-code regex provenance), `docs/architecture/backend/sources-iced-state-wise.md` (state-name match anchor), `docs/architecture/backend/sources-wikipedia.md` (user-facing taxonomy pointer), `docs/reference/data-coverage-report.md` (reference-data inventory row), `docs/reference/lgd-opendata.md` (state-code bridge plan), `tools/boundaries/AGENTS.md` (state-name → ECI-code map ownership), `backend/yen_gov/sources/rbi_hbs/name_map.py` (header comment), `backend/yen_gov/sources/wikipedia/urls.py` (header comment), `TODO/ELECTION-ENFORCEMENT.md` (state-code match clause). TODO closeout markers: this doc (status block), `docs/archive/plans/20260521-phase-2-preflight-audit-gregor.md` (cross-reference), `docs/archive/plans/20260517-canonical-long-format-pivot.md` (T.0c-ii closeout row). Migration ledger entries appended for `reference/in/states.json` and `schemas/state.schema.json` per `datasets/migration-ledger.csv` pattern.
>
> §13 browser smoke on representative slice (recorded in commit body): `/`, one state hub, one indicator page. Verified (a) the state list renders, (b) no new `[error]` console events, (c) no new 404s for `states.json` or `entities.json` paths.
>
> Backend pytest: **775 passed / 41 skipped** (parity oracle removed from suite). Frontend vitest: **16,055 passed / 6 skipped** (data.test.ts assertion repointed; `datasets-conform.test.ts` count drops by 1 with `states.json` gone but otherwise stable). Tier-B validator: **0 issues** (schema/data file pair deleted atomically; no orphan).
>
> **T.0c-ii arc closed** — the original Phase-0-closeout plan `docs/archive/plans/20260517-canonical-long-format-pivot.md §0e.10.4` `reference/in/states.json | DELETE | subsumed by entity_type='state' rows in taxonomy/entities.parquet | T.0c` row is now satisfied.

---

> **Phase C (frontend port + states.json deletion + §13 browser smoke)** — superseded by the Phase C closeout block above.
...
- Plan: docs/archive/plans/20260517-canonical-long-format-pivot.md §0e row 312 (states.json: DELETE → subsumed by entity_type='state'/'union_territory' in taxonomy/entities.parquet).
