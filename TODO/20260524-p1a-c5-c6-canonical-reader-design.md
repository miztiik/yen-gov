...
**Last Updated**: 2026-05-24 (revised)
**Status**: 🛑 DESIGN-PAUSED — retire-list invalidated by pre-implementation audit. The 16-shard retire-list in §4 / §6 below was built by **conceptual-map verification** (legacy indicator-id ↔ canonical indicator-id name similarity). A pre-impl audit running each pair through the actual on-disk Parquets found **8 of 16 shards cannot safely retire as planned** (5 are CEA 2026-03 35-state snapshots with no canonical equivalent; 3 hold FY04-14 / 2025-04 / unique-fuel-mix data that canonical does not carry). See [20260524-p1a-c5-retire-list-audit-findings.md](20260524-p1a-c5-retire-list-audit-findings.md) for the per-shard verdict + 3 forward paths. Pick **Path A** (retire 8 safe shards now, defer 8) or **Path B** (lift CEA snapshot first via P.1.A C4.5, then retire all 16) before code starts. Path C (retire all 16 + accept data loss) rejected per CLAUDE.md §0a Hans-authority on data fidelity.
