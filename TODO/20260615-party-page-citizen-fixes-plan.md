...
The 11-PR party-page reimagination ([docs/archive/plans/20260614-party-page-reimagination-plan.md](../docs/archive/plans/20260614-party-page-reimagination-plan.md)) shipped. The user's 2026-06-15 deployed-site walkthrough surfaced 13 concrete defects. D1-D9 are carried from the previous draft of this plan; D10-D13 were named in the 2026-06-15 review and are new in this rewrite.
...
| L-3 | 2026-06-15 | The OWID-aligned source-citation standard at [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md) (`dedupeToPills` -> one pill per `(producer x series-family)` with vintage summary, rendered by `<SourceList>`) is already documented and already in use by StateOverview + Yenask. D9 verdict: party page adopts the existing shared `<SourceList>` per card; no view-model widening beyond `dedupeToPills(cardSourceRows)`; no extension of source.csv columns; no per-card-pill-array invention. The previous draft's reader-before-writer split (PR-9a VM widen, PR-9b render swap) is overridden by section 0.5 RIP AND REPLACE doctrine - the writer surface is one line per card so the contract surface is local; collapse to one PR. Default verdict to bake on the `(producer x series-family)` bucketing question: ONE pill PER CARD (per the StateOverview pattern), max 3 pills inline with `+N more` overflow per the existing `SourceList.svelte` `max_inline=3` default. | baked - cites [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md) inline ADR `citation-ledger-5col` + `dedupeToPills`. |
...
| --- | --- | --- | --- | --- |
| PR-1  | D1 founded_year backfill (ECI source + processing_level=major per L-1)                                                       | [ ] PENDING | _pending_ | M  |
| PR-2  | D3 empty-square icon fix: inline `<TopicIcon>` swap (4 sites, 2 files)                                                        | [ ] PENDING | _pending_ | S  |
| PR-3  | D4 wikipedia.svg asset add + `PartyAboutCard` "W" logo render                                                                 | [ ] PENDING | _pending_ | S  |
| PR-4  | D6 DualAxisBarLine composite-mode y-axis `0.0%` baseline overlap                                                              | [ ] PENDING | _pending_ | S  |
| PR-5  | D2 recognition strip: inline party-symbol image + text                                                                        | [ ] PENDING | _pending_ | S  |
| PR-6  | D7 `<PageContainer>` primitive + cross-route migration + bare-`max-w-*` contract test                                         | [ ] PENDING | _pending_ | L  |
| PR-7  | D5 strongholds list redesign: retire+DELETE dot-strip, adopt one-line tally                                                   | [ ] PENDING | _pending_ | M  |
| PR-8a | D8c remove the 320x360 `PartyStrongholdMap` thumbnail AND delete the component file                                           | [ ] PENDING | _pending_ | S  |
| PR-8b | D8a `link.pc()` builder + clickable stronghold rows w/ delim-existence gate                                                   | [ ] PENDING | _pending_ | M  |
| PR-9  | D9 render swap (single PR per RIP doctrine): drop `PartySourcesStrip` + `PartyCoverageBadge` + adopt shared `<SourceList>`    | [ ] PENDING | _pending_ | M  |
| PR-10 | D10 immediate: chart x-axis skip+rotate fix on `DualAxisBarLine` / vote-share trend                                           | [ ] PENDING | _pending_ | S  |
| PR-11 | D11 alliance time-cap + DELETE 2 surviving italic in-page disclaimers                                                         | [ ] PENDING | _pending_ | M  |
| PR-12 | D12 global sticky-breadcrumb: mount `<Breadcrumb>` on Party.svelte + audit shared adoption + unify any bespoke holdouts       | [ ] PENDING | _pending_ | M  |
| PR-13 | D13 mint `docs/concepts/party-page-coverage.md` + add "About this page ->" footer link                                        | [ ] PENDING | _pending_ | S  |
| PR-14 | D10 future-proof (Level-3): time-axis range-brush primitive for vote-share trend + future wide-corpus charts                  | [ ] PENDING | _pending_ | L  |
| PR-15 | Closure: archive plan-doc per [docs/how-to/distill-a-plan.md](../docs/how-to/distill-a-plan.md)                               | [ ] PENDING | _pending_ | XS |
...
**Scope-change ledger row**: L-1 (above). Doctrine cite: [docs/concepts/data-quality.md](../docs/concepts/data-quality.md) per-row processing-level vocabulary.
...
**Scope-change ledger row**: L-3 (above). Doctrine cite: [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md) inline ADR `citation-ledger-5col` + `dedupeToPills`.
...
- New: `docs/concepts/party-page-coverage.md` with H1, `Last Updated: 2026-06-15`, sections:
  ...
  - **How sources are cited** (link to [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md); brief publisher-pill explainer).
  - **How processing levels work** (link to [docs/concepts/data-quality.md](../docs/concepts/data-quality.md); brief minor/major explainer).
...
**Scope**: Archive this plan-doc per [docs/how-to/distill-a-plan.md](../docs/how-to/distill-a-plan.md). Lift durable findings:
- D7 PageContainer primitive -> "Page-width discipline" section in the design-system concept doc.
- D9 verdict (shared `<SourceList>` is the family pattern; per-card `AboutThisData` deferred) -> cross-reference from [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md).
- D11 + D13 verdict (italic meta-disclaimers retire to docs/concepts/<page>-coverage.md) -> add a "page-coverage doc minting" sub-section to [docs/concepts/documentation-discipline.md](../docs/concepts/documentation-discipline.md).
...
- [docs/concepts/data-provenance.md](../docs/concepts/data-provenance.md) - inline ADR `citation-ledger-5col` (5-col source.csv contract); `dedupeToPills` doctrine (L-3 + PR-9 cite).
- [docs/concepts/data-quality.md](../docs/concepts/data-quality.md) - per-row processing-level vocabulary (`minor` / `major` + `processing_note`); L-1 + PR-1 cite.
- [docs/concepts/owid-alignment.md](../docs/concepts/owid-alignment.md) - OWID `origin.*` triple identity; named divergence #6 (per-row processing scope); the One Rule.
- [docs/concepts/citizen-first.md](../docs/concepts/citizen-first.md) - persona pipeline.
- [docs/how-to/ship-a-pr.md](../docs/how-to/ship-a-pr.md) - PR lifecycle the EXECUTION BLOCK references.
- [docs/how-to/distill-a-plan.md](../docs/how-to/distill-a-plan.md) - closure ritual.
- [docs/how-to/handle-scope-change.md](../docs/how-to/handle-scope-change.md) - STOP-AND-SURFACE doctrine.
- [docs/archive/plans/20260614-party-page-reimagination-plan.md](../docs/archive/plans/20260614-party-page-reimagination-plan.md) - predecessor plan; the 13 defects surfaced on its deployed-site review.
