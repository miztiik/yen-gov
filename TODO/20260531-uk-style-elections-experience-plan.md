...
**HANS + MAX VERDICT (baked 2026-06-01; research-only, both dispatched before PR-A1 starts):**
   - **2024 source = ECI `#33` direct-PC `.xls`** (postal-inclusive, official, no aggregation; needs `xlrd`). The `#34-Details-Of-Assembly-Segment-Of-PC.csv` (61,736 rows; cols `State/UT Name, PC NO, PC NAME, TOTAL ELECTORS IN PC, AC NO, AC NAME, TOTAL ELECTORS IN AC, TOTAL VALID VOTES IN STATE, NOTA VOTES EVM IN AC, CANDIDATE NAME, PARTY, VOTES SECURED EVM`) is **EVM-only / postal-excluded, AC-segment grain** - same shape as `All_States_GA.csv`. **Use `#34` ONLY as the 2024 AC->PC crosswalk + segment fanout, NEVER for PC totals/winner.**
   - **2019 source = ECI `#33` direct `.xls`** (already in inventory). **1999-2014 = TCPD's direct PC-level GE release** (the non-segment sibling - ACQUIRE it; confirm URL+licence in handover s7). `All_States_GA.csv` is **fallback-only**, every PC total then tagged `segment_approximate = true`.
   ...
   - **GRAIN-PREFIX FORK = Level-5 escalation BEFORE minting any `pc-*` id.** The live corpus uses grain-prefixed ids (`ac-winner-candidate-id`) which already contradicts [ADR-0044](../docs/architecture/decisions/0044-grain-over-entity.md) (grain lives on `entity_kind`, not the id) and `tier_b_one_indicator_per_concept` is ENFORCED. `pc-winner` scores >70% concept-overlap with `winner` - `check-overlap` + `pre-flight-ingest` will block a silent mint. Two paths: **Option A** (ADR-0044-pure: one grain-neutral `winner-candidate-id` carrying both `ac` + `pc` rows; add `pc` to each concept's `entity_kinds[]`; de-prefix existing `ac-*` via `id_aliases[]` + `deprecated_in` - breaking migration) vs **Option B** (mint `pc-*` + `*-pc` concepts - additive, matches existing `ac-*`, but perpetuates grain-in-id debt that post-PR-B9 no-grain-prefix enforcement will force unwinding). **Hans+Max+Gregor sign-off required; cite ADR-0044 + ADR-0045 + both gate reports.** This converts PR-A2's indicator-catalogue step from "mirror AC" into a contract decision.
...
- EDIT `datasets/taxonomy/indicators.json` - **BLOCKED on the grain-prefix fork (PR-A1 verdict, Level-5 Hans+Max+Gregor sign-off).** If Option B: add `pc-winner-party-id`, `pc-winner-candidate-id`, `pc-margin-pct`, `pc-margin-votes`, `pc-turnout-pct`, `pc-nota-pct`, `pc-votes-polled`, `pc-total-electors`, `pc-candidates-total` (mirror the AC set; family `elections`; declare `update_period_days: 1825` per guardrail #18). If Option A: add `pc` to existing concepts' `entity_kinds[]` + de-prefix `ac-*` ids via `id_aliases[]` + `deprecated_in`. EITHER way run `check-overlap` + `pre-flight-ingest` first (>70% overlap on `winner` will block a silent mint). NOTA indicators null (not zero) pre-2013; segment-sourced years carry `segment_approximate`.
