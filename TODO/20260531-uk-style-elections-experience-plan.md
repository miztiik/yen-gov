...
**HANS + MAX VERDICT (baked 2026-06-01; research-only, both dispatched before PR-A1 starts):**
   ...
   - **GRAIN-PREFIX FORK - RESOLVED (Gregor, 2026-06-01, research-only): Model C - mint per-grain PC sibling concepts.** Mint `pc-*` indicator ids mirroring the live `ac-*` set. The grain prefix is sanctioned: the grain-prefix gate (`GRAIN_PREFIX_RE = ^(state|district|national)-` in `backend/yen_gov/preflight/predicates.py` + Tier-B `tier_b_indicator_id_no_grain_prefix`) matches ONLY `state-/district-/national-`; it NEVER matches `ac-`/`pc-`, which [ADR-0044](../docs/architecture/decisions/0044-grain-over-entity.md) explicitly preserves as fact-grain prefixes. **The earlier claim that `ac-*` "contradicts ADR-0044" is WITHDRAWN - it is factually wrong against the regex.**
     - **CORRECTED 2026-06-01 (Gregor, research-only): the earlier "Option B with concept-binding" paragraph is SUPERSEDED by Model C.** Option B (every `pc-*` shares ONE `concept_id` with its `ac-*` sibling, extending that concept's `entity_kinds` to `[ac, pc]`) is **rejected**: it breaks the cluster-8 Tier-A guardrail pins (`backend/tests/test_concept_resolve_c8_winning_party_grain.py` hard-asserts disjoint per-grain `entity_kinds` and that the orphan shared `winning-party` was deleted), it re-merges the deliberate cluster-8 split, and it is **partially impossible** - 4 of 13 PC measures (`pc-nota-pct`, `pc-others-pct`, `pc-candidates-total`, `pc-effective-candidates-laakso`) have NO `ac-*` concept to share.
     - **Model C is binding:** each `pc-*` indicator FKs to its OWN pc-grain sibling concept in `datasets/taxonomy/concepts.json` with `entity_kinds: ["pc"]` and a grain-suffixed noun ("Votes polled (PC)", "Turnout (PC)", "Winning party (PC)", ...), mirroring the `winning-party-ac` / `winning-party-state` precedent. This is a genuinely new grain (MP/PC is a different office on a different boundary than MLA/AC), NOT guardrail-#13 proliferation (= same grain, multiple publishers). `find_overlap` returns `mint_new` on the distinct noun; zero cross-grain twins; no existing concept or test is touched. The 13 minted concept_ids: `electors-pc`, `votes-polled-pc`, `turnout-pc`, `nota-votes-pc`, `nota-share-pc`, `winner-pc`, `winning-party-pc`, `margin-absolute-pc`, `margin-pc`, `others-votes-pc`, `others-share-pc`, `candidates-total-pc`, `effective-candidates-pc`.
     ...
     - **Run before authoring any `pc-*` row:** `python -m yen_gov check-overlap --concept "Votes polled (PC)" --unit "votes" --entity_kind "pc"` then `pre-flight-ingest` with grain-suffixed nouns. Expect `mint_new` x13 (distinct nouns, no cross-grain twin). The PR-A1 pre-flight report (which recorded `add_facit`/`add_facet`) MUST be re-run under Model C and superseded. Exit 2 = fix proposal (Holy Law #5, no override). Cite ADR-0044 + ADR-0045 + both gate reports in the handover.
...
- EDIT `datasets/taxonomy/indicators.json` - **Grain-prefix fork RESOLVED to Model C - mint per-grain PC sibling concepts (Gregor, 2026-06-01; supersedes Option B).** Added the 13 `pc-*` indicators (`pc-total-electors`, `pc-votes-polled`, `pc-turnout-pct`, `pc-nota-votes`, `pc-nota-pct`, `pc-winner-candidate-id`, `pc-winner-party-id`, `pc-margin-votes`, `pc-margin-pct`, `pc-others-votes`, `pc-others-pct`, `pc-candidates-total`, `pc-effective-candidates-laakso`; family `elections`; `entity_kinds: ["pc"]`; `default_entity_kind: "pc"`; `update_period_days: 1825`). **Each `pc-*` indicator FKs its OWN pc-grain sibling concept** (NOT shared with the `ac-*` sibling - Option B rejected because it breaks the cluster-8 disjoint-entity_kinds guardrail pins and is partially impossible). NOTA indicators null (not zero) pre-2013; segment-sourced years carry `segment_approximate`.
- EDIT `datasets/taxonomy/concepts.json` - mint the 13 pc-grain sibling concepts the pc-* indicators FK to, each declaring `(noun grain-suffixed "(PC)", unit_canonical, normalisation, entity_kinds:["pc"])` (Model C): `electors-pc`, `votes-polled-pc`, `turnout-pc`, `nota-votes-pc`, `nota-share-pc`, `winner-pc`, `winning-party-pc`, `margin-absolute-pc`, `margin-pc`, `others-votes-pc`, `others-share-pc`, `candidates-total-pc`, `effective-candidates-pc`. Mirrors the `winning-party-ac` / `winning-party-state` per-grain precedent; does NOT extend any AC concept's `entity_kinds`.
