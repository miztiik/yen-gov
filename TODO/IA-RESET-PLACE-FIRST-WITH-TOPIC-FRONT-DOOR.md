...
**Status**: P1 + P2 + P2.5 + P3.1 + P3.2 + P3.3a + P3.3b + P3.3c + P3.3d + P4 complete (2026-05-13). Topic Front Door is live (`/t` and `/t/:topic`) with `?peer=…` deep-link state and breadcrumbs (P3.3d); generic indicator Compare is live at `/compare?i=&states=&peer=` (P4) with friendly empty state, peer filter, multi-state pin chips, and `IndicatorRanked` extended to honour `pinned_states`; LeftRail rewritten with grouped IA + StatePill (P3.3c). Validator green; vitest 9478/9478; svelte-check 0 errors (4 a11y warnings ignored per CLAUDE.md §0).
