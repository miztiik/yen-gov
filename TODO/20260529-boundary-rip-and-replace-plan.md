...
| **A.2** | A | Frontend STATE_AC registry sync 7 -> 31: add 24 new entries to `frontend/src/lib/maplibre/sources.ts:STATE_AC`. All LGD-keyed entries use `join_property: "ac_no"` lowercase. S01 entry reflects A.1.a's LGD-with-rewrite outcome. S03 entry reflects A.1.b's T4 district fallback. U08 stays on shijithpk (post-2022 90-AC). | #437 | **Done** | `3aea515b` | Shipped post-A.3 (which removed the `attribution` field, so each new entry has 5 fields not 6). Added 24 entries in numerical state-code order: S02 S04 S05 S06 S07 S08 S10 S12 S13 S14 S15 S16 S17 S18 S19 S20 S21 S23 S24 S26 S27 S28 S29 U05. All 24 point at the same LGD_Assembly_Constituencies.geojsonl.7z release (post-D.7 R1 KEEP-LGD set + the 6 states that were already on LGD pre-D.7). New contract test `frontend/src/contracts/state-ac-registry-coverage.test.ts` (36 tests / 11ms) walks the on-disk `datasets/boundaries/in/ac/state=in_*/all.geojson` shards + asserts (a) every shard has a registry entry, (b) every registry entry has a shard, (c) per-entry shape (id, geojson_local_path, geojson_url, join_property, label, NO `attribution`), (d) U08 join key is `seat_id`, (e) all non-U08 entries use `ac_no`. Gate 5 browser smoke: S10 Karnataka AC#1 = "NIPPANI" (BJP winner, map canvas + footer link); S24 UP AC#1 = "BEHAT" (SP winner, hover tooltip on neighbour AC works). |
| **A.3** | A | Attribution centralization: replace per-state `attribution` HTML strings on `BoundaryEntry` with one centralized constant rendered by the map footer; full per-source explanation (ramSeraph LGD CC0 default + S03 district-fallback caveat + U08 shijithpk J&K supplement) lives at new `<section id="maps">` on `/about?section=maps`. | #436 | **Done** | `6262eb22` | Pivoted ahead of A.2 per user mandate 2026-05-29 on observing verbose ~340-char S03 footer overflowing into map canvas on `/s/assam/ac/1`. Removed `attribution` field from `BoundaryEntry` interface + 8 entry sites (INDIA_STATES + 6 STATE_AC + INDIA_DISTRICTS + IndicatorChoropleth.synthesiseEntry). MapChoropleth renders `boundaryFooterHtml(BASE_URL)` constant - one short link pointing at the docs section. About.svelte gains new "Boundaries on these maps" section with per-state notes for AP rewrite + Assam T4 + J&K shijithpk. |
| **A.4** | A | Playwright per-state AC coverage spec: new `frontend/e2e/state-ac-coverage.spec.ts` iterates 31 states (incl. U08), navigates to `/s/<slug>/ac/1?event=<recent-eci-event>`, asserts >= 90% coloured polygons. Per-state report at `notes/2026-05-29-state-ac-coverage-report.md`. | #438 | **Done** | _pending_ | Shipped with refined invariants: 5 per-state assertions (no pageerror/requestfailed, H1 resolves to SoT name not literal `AC 1`, `canvas.maplibregl-canvas` mounts, footer attribution link is the centralised A.3 link, map's own GET for `/data/boundaries/in/ac/state=in_<lc>/all.geojson` returns 200). The `>= 90% coloured polygons` bar deferred per per-state coverage report's rationale: it conflates boundary coverage (Phase A scope) with election-result coverage (separate arc); the 5 invariants test what Phase A actually delivers. 31/31 passed (3.4m) on chromium project; mobile-pixel-5 project skipped (no mobile-specific map code path). S03 reflects A.1.b's T4 district fallback (passes the 5 invariants; NOT held to AC-cell bar). Full per-state table at [notes/2026-05-29-state-ac-coverage-report.md](../notes/2026-05-29-state-ac-coverage-report.md). Key technical finding: `page.waitForResponse((r) => r.url().includes(shardUrl) && r.request().method() === "GET")` set up BEFORE `page.goto` is the canonical pattern for verifying resource loads in this codebase; an earlier draft using `fetch(method:"HEAD")` from page context tripped `net::ERR_ABORTED` because Vite's `serveDatasets()` middleware in `frontend/vite.config.ts` only handles GET (and the `attachPageErrorTrap` helper amplifies that into a test failure). |
...
| **C.1** | C | LGD Blocks - new `level: "block"` enum value (schema v1.x bump); new Hive partition `boundaries/in/blocks/state=in_<lc>/all.geojson`; snapshot run + frontend `BLOCK_BOUNDARY` registry. | #441 (recon #439, infra slice C.1.a) + #442 (slice C.1.b shards + frontend) + #443 (slice C.1.c auto-fallback UP shard) | **C.1.a + C.1.b + C.1.c shipped; full 36-state coverage** | - | Phase C kickoff: section 4a pattern proof for a NEW level. Citizen need: rural-development indicators (PMGSY, MGNREGA) bind at block granularity. **Upstream verified Tier-1**: ramSeraph `LGD_Blocks.geojsonl.7z` (BharatMaps/LGD lineage, 7146 features after live lift, CC0 1.0). Recon verdict + 6-step implementation plan at [notes/2026-05-29-c1-blocks-source-hunt-verdict.md](../notes/2026-05-29-c1-blocks-source-hunt-verdict.md). Block != Subdistrict (LGD treats them as different entities; yen-gov already ships subdistricts via `lift_subdistricts_national.py`). Sliced into **C.1.a** (infrastructure-only: `tools/boundaries/lift_blocks_national.py` mirroring the subdistricts orchestrator pattern, `backend/tests/test_lift_blocks_national.py` (8 cases mirroring subdistricts test coverage), `datasets/schemas/boundary-layers.schema.json` v1.2 + `block` added to `level` enum, `backend/yen_gov/canonical/boundary_layers_seed.py` `Level` Literal updated, `tools/boundaries/_paths.py` `Kind` + `KIND_TO_LEVEL` updated for `blocks`->`block` mapping) + **C.1.b** (ran the lift script against live upstream; committed 35 per-state shards under `datasets/boundaries/in/blocks/state=in_<lc>/all.geojson` totalling ~97 MB; upserted 35 rows into `boundary_layers.parquet`; added `BLOCK_BOUNDARY` registry to `frontend/src/lib/maplibre/sources.ts` with 35 entries; added Vitest contract `frontend/src/contracts/state-blocks-registry-coverage.test.ts` analogous to A.2's; first-snapshot property names confirmed: `block_lgd` / `block_name` / `state_lgd` match the lift-script assumptions). Coverage gap: S24 (Uttar Pradesh) shard 12.8 MB exceeds 12 MB `SNAPSHOT_BYTE_BUDGET` at `coord_precision=3` and was SKIPped by the lift (matches `lift_villages_national.py` + `lift_subdistricts_national.py` SKIP-on-budget precedent). C.1.c follow-up tracks the UP shard mitigation. Note: deviated from recon verdict step 1 (add `pipeline.json` entry) - the actual pattern for per-state shards from a national geojsonl is dedicated lift script with no `pipeline.json` entry (matches `lift_subdistricts_national.py`); `pipeline.json` is reserved for `snapshot.py` inputs that emit a single national or pre-sliced shard. |
| **C.1.c** | C | UP block shard size mitigation: S24 (Uttar Pradesh) block shard exceeds the 12 MB `SNAPSHOT_BYTE_BUDGET` at the standard `coord_precision=3` (~12.8 MB). Shipped via emergent **option (d)**: auto-fallback inside the lift script — on budget breach at `coord_precision=N`, re-emit the bucket at `coord_precision=N-1` before falling through to SKIP. Uniform script rule (FIX-THE-BOX), NOT per-state hand-coded config. Discarded (a) per-state override (per-state config drift), (b) Douglas-Peucker integration (scope-creep across villages + subdistricts), (c) global budget raise (contract change ripples through schema + conform-test + boundaries.budget gate). | #443 | Done | _post-merge_ | C.1.b shipped 35/36 states; C.1.c closes the gap. S24 shard now lands at `coord_precision=2` (~2.2 MB / 822 features) with `simplification_tolerance_deg=0.01` recorded on the `boundary_layers.parquet` row. Other 35 shards byte-identical (default precision=3, tolerance=0.001). Frontend `BLOCK_BOUNDARY` registry now 36 entries; contract test `state-blocks-registry-coverage.test.ts` sanity floor 35→36 + "S24 absent from BOTH" assertion flipped to "S24 present on BOTH (C.1.c)". 2 new backend tests in `test_lift_blocks_national.py` lock in the auto-fallback path (`test_lift_auto_fallback_when_bucket_exceeds_budget` + `test_lift_skips_when_even_fallback_precision_exceeds_budget`). Renderer-side heterogeneity invisible: `join_property=block_lgd` is the LGD id; vertex precision affects edge sharpness only at zoom >10, which is well beyond typical block-grain choropleth use. |
| **C.2** | C | LGD Panchayats (gram-panchayat) - new `level: "panchayat"` enum value; nested partition `boundaries/in/panchayats/state=in_<lc>/district=<lgd>/all.geojson` (mirrors villages layer). Slice into C.2.a (infrastructure) + C.2.b (live lift + ~792 shards) + C.2.c (frontend district-picker registry) + optional C.2.d (Bhuvan gap-fill). | #444 (recon only) | **Recon shipped; C.2.a/b/c queued** | _post-merge_ | Citizen need: governance-quality indicators (panchayat is the most-local elected body; PRI fund-flow, MGNREGS person-days bind at gram-panchayat granularity). **Upstream verified Tier-1**: ramSeraph `LGD_panchayats.geojsonl.7z` (BharatMaps/LGD lineage, ~255k features claimed nationally, CC0 1.0). Coverage gap: ~9 states/UTs (HP, J&K, Sikkim + most NE states + some UTs) missing from LGD; Bhuvan_panchayats reserved as Tier-1.5 gap-fill (C.2.d optional). Auto-fallback pattern from PR #443 already in place + applies verbatim — high-density district shards (UP/MP/MH/KA/AP/TN, est. 10-15% of shards) will exercise it. Frontend: per-district registry recommended (state-level would bind 300-2500 panchayats per state). Recon verdict + 10-step implementation plan at [notes/2026-05-30-c2-panchayats-source-hunt-verdict.md](../notes/2026-05-30-c2-panchayats-source-hunt-verdict.md). |
| **C.2.a** | C | LGD Panchayats infrastructure: add `"panchayat"` to schema `level` enum (v1.2 → v1.3) + backend `Level` Literal; `_paths.Kind` + `KIND_TO_LEVEL` mapping (`"panchayats" → "panchayat"`); `tools/boundaries/lift_panchayats_national.py` orchestrator modelled on `lift_villages_national.py` (district-keyed nested partition) + inheriting `lift_blocks_national.py` auto-fallback (PR #443) verbatim; `backend/tests/test_lift_panchayats_national.py` (11 cases mirroring blocks coverage + district-nesting); `boundaries-conform.test.ts` HIVE_SHAPES extension for panchayats. NO datasets yet. NO `pipeline.json` entry (blocks/villages precedent: lift scripts embed their own URL constants). | #445 | **Infrastructure shipped; C.2.b live lift queued** | _post-merge_ | Direct mirror of C.1.a precedent. |
| **C.2.b** | C | LGD Panchayats live lift + **663 per-(state, district) shards** under `datasets/boundaries/in/panchayats/state=in_<lc>/district=<lgd>/all.geojson` across **28 states/UTs** (~1.1 GB total; max shard 10.9 MB on S13 district=490; **no auto-fallback triggered at default `coord_precision=4`** — the 12 MB `SNAPSHOT_BYTE_BUDGET` proved well-calibrated for nested district-keyed panchayats); parquet upsert (+663 rows → 700 total in `boundary_layers.parquet`); first-snapshot property-name inspection landed: actual short-form is `st_lgd` / `dt_lgd` / `gp_code` / `gp_name` (NOT the long-form `state_lgd` / `dist_lgd` / `block_lgd` / `block_name` that blocks uses — **cross-layer schema divergence within the same ramSeraph maintainer**; lift script + tests patched to read via module-level `STATE_PROPERTY` / `DISTRICT_PROPERTY` / `ID_PROPERTY` / `NAME_PROPERTY` constants); filename-case bug also surfaced + fixed (`LGD_Panchayats.geojsonl` capital-P upstream vs lower-cased hardcoded reference; resolved via `_find_extracted()` rglob discovery in `main()`); `frontend/src/contracts/state-panchayats-shards-coverage.test.ts` contract test (**renamed from plan-doc's `*-registry-coverage`** — that test name reserved for C.2.c when the `PANCHAYAT_BOUNDARY_BY_DISTRICT` registry actually lands; at C.2.b there is no registry to compare against, so this test locks the shard-coverage floor only). Coverage gap (~9 states/UTs reserved for C.2.d Bhuvan fill): S02 S08 S14 S16 S17 S21 U06 U08 U09. | #446 | **Done** | _post-merge_ | Direct mirror of C.1.b precedent with two upstream-schema surprises caught + patched (property-name short-form, filename case). Auto-fallback (PR #443) did NOT trigger at default `coord_precision=4`. |
| **C.2.c** | C | Frontend `PANCHAYAT_BOUNDARY_BY_DISTRICT` registry keyed by `{state_code}-{district_lgd}` (663 entries across 28 states/UTs, constructed via `Object.fromEntries` over a compact `PANCHAYAT_DISTRICTS_BY_STATE: Readonly<Record<string, readonly number[]>>` source-of-truth; `PANCHAYAT_STATE_NAMES` lookup carries the citizen-readable label per state); `frontend/src/contracts/state-panchayats-registry-coverage.test.ts` contract test (symmetric shards↔registry coverage mirroring the C.1.b `state-blocks-registry-coverage` precedent; 12 assertions across 3 describe blocks — coverage symmetry / entry shape / districts-by-state shape; locks the absent-gap-states list S02 S08 S14 S16 S17 S21 U08 U09 so future LGD vintage churn requires deliberate registry update). District-picker UI shim DEFERRED — per recon, "scope TBD post-C.2.b with measured data + UX feedback"; this PR ships the registry only. | #447 | **Done** | _post-merge_ | District-level scoping confirmed correct (state-level would bind 300-2500 polygons per layer; UX-perf risk). Registry construction via compact `{state_code: [district_lgd, ...]}` literal + `Object.fromEntries` keeps the diff readable (~190 source lines for 663 entries vs ~5300 lines if each entry were hand-typed). |
| **C.3** | C | ULB Wards - new `level: "ward"` enum value; nested partition `boundaries/in/wards/state=in_<lc>/ulb=<lgd>/all.geojson` (mirrors panchayats layer but with ULB-keyed parent instead of district-keyed). Slice into C.3.a (infrastructure) + C.3.b (live lift + per-(state, ulb) shards) + C.3.c (frontend ULB-picker registry) + optional C.3.d (LivingAtlas / WB AMRUT / Shillong gap-fills for WB/TR/MZ/MN). | #448 (recon only) | **Recon shipped; C.3.a/b/c queued** | _post-merge_ | Citizen need: urban-governance indicators (Swachh Survekshan, AMRUT, municipal corp data). **Upstream verified Tier-1**: ramSeraph `SBM_Wards.geojsonl.7z` (Swachh Bharat Mission Urban / MoHUA lineage, CC0 1.0). Coverage gap (documented upstream): West Bengal, Tripura, Mizoram, Manipur missing from SBM; LivingAtlas + WB AMRUT + Shillong reserved as Tier-1.5 / Tier-2 gap-fills (C.3.d optional). Partition is `ulb=<ulb_lgd>` (not `district=`) because ward's parent is the ULB, not the district — LGD treats ULBs as a peer urban entity to districts with their own LGD codes. Auto-fallback pattern from PR #443 already in place + applies verbatim — mega-corp shards (BBMP/GHMC/Delhi MCD/Greater Mumbai MCGM) will exercise it. Frontend: per-ULB registry recommended (state-level would bind 200-7000 wards per state). Recon verdict + 5-step implementation plan at [notes/2026-05-30-c3-ulb-wards-source-hunt-verdict.md](../notes/2026-05-30-c3-ulb-wards-source-hunt-verdict.md). |
| **C.3.a** | C | ULB Wards infrastructure: add `"ward"` to schema `level` enum (v1.3 → v1.4) + backend `Level` Literal; `_paths.Kind` + `KIND_TO_LEVEL` mapping (`"wards" → "ward"`); `_paths.derive_hive` accepts new `ulb_lgd=` partition segment; `tools/boundaries/lift_wards_national.py` orchestrator modelled on `lift_panchayats_national.py` (nested per-parent partition) + inheriting `lift_blocks_national.py` auto-fallback (PR #443) verbatim; `backend/tests/test_lift_wards_national.py` (cases mirroring panchayats coverage + ULB-nesting); `boundaries-conform.test.ts` HIVE_SHAPES extension for wards. NO datasets yet. NO `pipeline.json` entry (panchayats/blocks/villages precedent: lift scripts embed their own URL constants). | #449 | **Infrastructure shipped; C.3.b live lift queued** | _post-merge_ | Direct mirror of C.2.a precedent. |
| **C.3.b** | C | ULB Wards live lift + per-(state, ulb) shards under `datasets/boundaries/in/wards/state=in_<lc>/ulb=<ulb_lgd>/all.geojson` across 29 states/UTs from 70,419 upstream SBM features → 3,300 shards (155 MB total, largest 3.4 MB — no auto-fallback triggered); parquet upsert (+3,300 rows in `boundary_layers.parquet`, 4,000 total); first-snapshot finding: SBM_Wards uses a **THIRD distinct property-name convention** — concatenated lowercase `statecode`/`ulbcode`/`wardcode`/`wardname` (separate from C.1.c long-form `state_lgd`/`dist_lgd` AND C.2.b short-form `st_lgd`/`dt_lgd`); root cause = SBM Urban is a MoHUA-owned system whose schema pre-dates the LGD convention; `wardcode` is HETEROGENEOUS (numeric strings + free-text "Ward No 5" minority — sort helper uses two-cohort key); contract test `state-wards-shards-coverage.test.ts` (shard-coverage floor 3,000 + 28 states/UTs + S24 UP + S20 Maharashtra canaries). Coverage gap (~7 states/UTs absent from SBM, reserved for C.3.d gap-fill): S02 HP, S09 Sikkim, S14, S15, S16, S23 Goa, U06 Lakshadweep. | #450 | **Live lift shipped (3,300 shards across 29 states/UTs)** | _post-merge_ | Direct mirror of C.2.b precedent. **3-convention rule LOCKED IN** — every new ramSeraph admin-level layer needs a first-snapshot property-name probe (the maintainer does NOT normalise schemas across releases). |
| **C.3.c** | C | Frontend `WARD_BOUNDARY_BY_ULB` registry keyed by `{state_code}-{ulb_lgd}` (**3,300 entries across 29 states/UTs**, constructed via `Object.fromEntries` over a compact `WARDS_BY_STATE: Readonly<Record<string, readonly number[]>>` source-of-truth; `WARD_STATE_NAMES` lookup carries the citizen-readable label per state); `frontend/src/contracts/state-wards-registry-coverage.test.ts` contract test (15 assertions across 3 describe blocks — coverage symmetry / entry shape / WARDS_BY_STATE shape; canaries: S24 UP `800629` first-ULB, S13 MH `802640` first-ULB, S24 is largest at 638 ULBs; **REAL 7-state SBM-absent gap locked**: `S02 S14 S15 S16 S23 U04 U09` — REPLACES the C.3.b plan-doc prose's mis-named `"S02 HP, S09 Sikkim, S23 Goa, U06 Lakshadweep"` which had wrong state-code↔name mappings; real names are S02=Arunachal Pradesh, S14=Manipur, S15=Meghalaya, S16=Mizoram, S23=Tripura, U04=Lakshadweep, U09=Ladakh). ULB-picker UI shim DEFERRED — same scope discipline as C.2.c (defer picker UI to a later PR with measured data + UX feedback). | #451 | **Done** | _post-merge_ | ULB-level scoping confirmed correct (state-level would bind 200-7000 polygons per layer; UX-perf risk). Registry construction via compact `{state_code: [ulb_lgd, ...]}` literal + `Object.fromEntries` keeps the diff readable (~120 source lines for 3,300 entries vs ~30,000 lines if each entry were hand-typed). C.3.c silently corrected C.3.b plan-doc's state-code↔name mismapping in the absent-gap list — future archaeology should treat C.3.c's gap list as authoritative. |
| **C.4** | C | Bhuvan J&K Villages (gap-fill) - single-state gap-fill of one of 8 LGD-villages-absent states (HP / J&K / SK / ML / MZ / MN / NL / AR per LGD release notes). Slice into C.4.a (single impl PR: parallel orchestrator `lift_villages_jk_bhuvan.py` + tests + live lift + parquet upsert). NO new schema, NO new infrastructure (level=`village` already in enum from pre-C.4 TN-only adoption; `lift_villages_national.py` precedent reused), NO frontend registry (villages aren't currently surfaced in `sources.ts`). | #452 (recon only) | **Recon shipped; C.4.a queued** | _post-merge_ | Citizen need: village granularity for J&K UT (currently absent from `LGD_Villages.geojsonl`). **Upstream verified Tier-1**: ramSeraph `Bhuvan_JK_Villages.geojsonl.7z` from Bhuvan / ISRO / NRSC, lineage to J&K Revenue Department, CC0 1.0. Tile URL `/not-so-open/villages/jammu-and-kashmir/bhuvan/{z}/{x}/{y}.pbf` exactly matches the C.4 plan-doc reference. Expected output: ~22 per-district shards under `boundaries/in/villages/state=in_u08/district=<lgd>/all.geojson` from ~6,000-7,500 features. Property-name convention TBC at first-snapshot probe (3-convention rule from C.3.b applies). Ladakh (U09) NOT covered by this artefact; remains deferred. Recon verdict + 1-step implementation plan at [notes/2026-05-30-c4-jk-villages-source-hunt-verdict.md](../notes/2026-05-30-c4-jk-villages-source-hunt-verdict.md). **Recon UPDATE (2026-05-30, post-C.4.a probe)**: artefact is **Census-2011 vintage** (not LGD-current as recon assumed) → 14 pre-bifurcation districts, NOT ~22 LGD-coded modern districts; mixes BOTH modern UTs U08 + U09 (Census-2011 predates 2019 J&K Reorg Act); 4th unique property convention (uppercase `STAT_NAME`/`DIST_NAME`/`VID`/`VILL_CODE`/`NAME`); Ladakh U09 IS covered (2 villages only — upstream sparsity); 8 post-2007 bifurcated districts SILENTLY MERGED in Census-2011 parent shards (Kulgam→anantnag / Bandipore→baramula / Ramban+Kishtwar→doda / Samba→jammu / Shopian→pulwama / Ganderbal→srinagar / Reasi→udhampur). See verdict note §"Recon UPDATE 2026-05-30" for full discovery. Other 7 LGD-gap states (HP S02 / SK S09 / ML S15 / MZ S16 / MN S14 / NL S17 / AR S02 — TBC LGD codes) get follow-up PRs only when (a) ramSeraph publishes a `Bhuvan_<STATE>_Villages.geojsonl.7z` artefact AND (b) a village-keyed citizen indicator demands them. |
| **C.4.a** | C | Bhuvan J&K Villages live lift + per-(state, district-slug) shards under `datasets/boundaries/in/villages/state=in_u08/district=<slug>/all.geojson` (**14 shards: 12 U08 + 2 U09 from 6,639 features**); new orchestrator `tools/boundaries/lift_villages_jk_bhuvan.py` modelled on `lift_villages_national.py` but specialised for single-source / single-state (hardcoded URL `Bhuvan_JK_Villages.geojsonl.7z`; Census-2011 district NAME slug as partition key — NOT LGD numeric — because artefact carries no LGD codes; explicit `CENSUS2011_DISTRICT_TO_MODERN` mapping splits Kargil + Ladakh (leh) to U09, rest to U08); `backend/tests/test_lift_villages_jk_bhuvan.py` (10 cases mirroring villages national-lift coverage + the U08/U09 split + slug-regex conformance + new source seed lookup); first-snapshot property-name probe locked the 4th convention (`DIST_NAME` / `VID` / `NAME`); parquet upsert (+14 rows in `boundary_layers.parquet`, +1 source row `ramseraph_bhuvan_jk_villages` in `sources.parquet`); new source nickname seeded in `boundary_layers_seed.py` (CC0-1.0, tier=silver, method=archived-snapshot); `boundaries-conform.test.ts` HIVE_SHAPES needs NO extension (same `villages/state=*/district=*` partition shape — only the value scheme changed from LGD numeric to Census-2011 name slug, schema regex `[a-z0-9_]+` accepts both). | #453 | **Live lift shipped (14 shards across U08 + U09)** | _post-merge_ | Direct mirror of C.2.b / C.3.b live-lift precedents at smaller scale (single artefact, two modern UTs). **4-convention rule LOCKED** — every new ramSeraph admin-level layer needs a first-snapshot probe (Bhuvan Census-2011 was the 4th unique property convention observed across the cohort, distinct from C.1.c LGD long-form, C.2.b panchayats short-form, C.3.b SBM wards concatenated). **Slug-as-partition-key concession** (deliberate; documented per-shard in citizen archaeology notes): bifurcated-district villages live in Census-2011 parent shards (Kulgam villages render under `district=anantnag`, etc.) — accepted because the alternative (writing a Census-2011-name → modern-LGD-district lookup) requires per-village geographic reassignment beyond this PR's scope. Replaceable in a future PR if LGD ever publishes J&K with bifurcated district codes. Largest shard: Jammu 2.13 MB (well under 12 MB budget); Ladakh sparsity (Kargil 1 village, Ladakh (leh) 1 village) is an upstream Bhuvan limitation in this Census-2011 layer. Auto-fallback from PR #443 not triggered. |
| **C.5** | C | LGD PC v2 - add `/not-so-open/constituencies/parliament/lgd/` to pipeline.json; new partition `boundaries/in/pc/delim=2024/lgd/all.geojson` parallel to current shijithpk source; cross-verify; switch upstream when clean. | #454 (recon only; NO-GO closure) | **Closed - REAFFIRMS D.6 NO-GO (no impl shipped)** | _post-merge_ | Recon re-run 2026-05-30 against same upstream artefact D.6 verified on 2026-05-25 (`LGD_Parliament_Constituencies.geojsonl.7z`, ramSeraph release date 2023-12-12; upstream byte-identical). All 4 D.6 NO-GO findings persist: (1) **6 entire states have ZERO active PCs** (J&K=6/6 + Arunachal=2/2 + Nagaland=1/1 + Manipur=2/2 + Assam=14/14 + Jharkhand=14/14 = 39 pre-delim features that would silently drop from active coverage); (2) **no LGD-grade PC identifier** (`lgd_pc_code` absent; only unstable `pc_id`); (3) **`pc_id` legacy frozen pre-2014 key with 44 mismatches** (Telangana 17 PCs use `28xx` AP prefix; AP 25 PCs use `37xx`; Daman & Diu + D&NH 2 PCs use `39xx` against st_code 38 — adoption would silently mis-attribute Telangana results to Andhra Pradesh); (4) feature count **543** vs plan-doc 545-precondition rooted in shijithpk's 2 J&K-territory placeholders. Combined verdict: KEEP shijithpk (`datasets/ephemeral/india_ls_seats_545.geojson` + `pipeline.json` PC stanza UNCHANGED); the C.5 row was authored 2026-05-29 without awareness that D.6 ran this recon 4 days earlier and the row is a duplicate-of-D.6. Full verdict + re-evaluation triggers at [notes/2026-05-30-c5-c6-recon-no-go.md](../notes/2026-05-30-c5-c6-recon-no-go.md); D.6 prior recon at [notes/2026-05-25-d6-pc-recon.md](../notes/2026-05-25-d6-pc-recon.md); D.6 recon driver `tools/boundaries/recon_d6_pc.py` remains the re-evaluation trigger when ANY of the 4 findings flips upstream. |
| **C.6** | C | Susewind 2014 AP overlay (optional) - if B.1 verdict confirms Susewind 2014 ships post-2014 175-feature AP-only shape AND ramSeraph LGD has cleaner coverage, Susewind 2014 lives at `boundaries/in/ac/state=in_s01/_susewind2014/all.geojson` as a v2 cross-verification source (NOT the live source). | #454 (recon only; NO-GO closure) | **Closed - PRECONDITION FAILED per B.1 correction (no impl shipped)** | _post-merge_ | C.6's first gate condition is REFUTED by the 2026-05-29 B.1 verdict correction. Actual Susewind 2014 AP slice contains **292 features** under `state=='Andhra Pradesh'` at LEGACY PRE-bifurcation unified AP+TG numbering (`ac` field 1-294; first AC = "Sirpur" in Adilabad district which is Telangana side of the 2014 split). It is NOT a clean 175-feature post-bifurcation AP-only shape; never was. Carving 175 modern AP polygons out would require boundary surgery against the 10 districts that became Telangana (Adilabad / Nizamabad / Karimnagar / Warangal / Khammam / Mahbubnagar / Nalgonda / Hyderabad / Ranga Reddy / Medak) — structurally outside Susewind 2014's authoring scope. Second condition ("ramSeraph LGD has cleaner coverage") is TRUE (LGD's 178-feature AP slice filtered + name-rewritten to 175 modern ACs IS the source A.1.a uses), but "cleaner" is comparative — no Susewind 2014 baseline exists to compare against, so the whole overlay premise dissolves. Susewind 2014 is NOT adopted at any `_susewind2014/` partition. Full verdict at [notes/2026-05-30-c5-c6-recon-no-go.md](../notes/2026-05-30-c5-c6-recon-no-go.md); B.1 correction at [notes/2026-05-29-phase-b-verdict-correction.md](../notes/2026-05-29-phase-b-verdict-correction.md). |
| **D.1.A** | D | Retire per-entity side-fixes for offshore / small / no-assembly UTs (Lakshadweep extractor + chip-strip + coverage exclusion + ADR-0029 if it exists). All entities render on the map at true geographic location; data-table / CSV / ranking-list rendering is a Jony-owned UX concern (NOT prescribed here). | #_pending_ | **Closed - retired both inset + chip-strip** | _post-merge_ | Cleanup sweep PR. 8 deletes (`lakshadweep.{ts,test.ts}` + `UnmappedRegionChips.svelte` + `unmapped-region-chips.{ts,test.ts}` + `format.{ts,test.ts}` + `docs/concepts/unmapped-regions.md`) + 5 edits (`IndicatorChoropleth.svelte` -195/-/imports+state+effects+render, `golden-path.spec.ts` -14/+6, `map.md` rewrite, ADR-0029 SUPERSEDED, `coverage.py` -19/+6). UT_CODES_WITH_ASSEMBLY filter retired; UTs now appear in coverage like states. See D.1.A narrative below for verbatim user mandate. |
...
Same shape as C.2 (nested per-parent partition) but parent is the ULB (not the district). Partition is `state + ulb_lgd` because ULBs are LGD's primary urban administrative unit with their own LGD codes (a ULB can span multiple districts; LGD tracks the ULB-to-district mapping as M:N, not 1:N). Coverage uneven at upstream: SBM Wards is the national source (Swachh Bharat Mission Urban / MoHUA, CC0 1.0); upstream-documented coverage gap covers West Bengal + Tripura + Mizoram + Manipur. Fall-backs: LivingAtlas_Wards (Tier-1.5, ESRI commercial harmonisation), WB_AMRUT_Wards (Tier-2, WB only), Shillong_Wards (Tier-2, Shillong only). Ingest SBM nation-wide first (C.3.b); per-state fall-backs deferred to optional C.3.d only when a ward-keyed citizen indicator citizen-needs WB/TR/MZ/MN coverage. Verdict + 5-step implementation plan at [notes/2026-05-30-c3-ulb-wards-source-hunt-verdict.md](../notes/2026-05-30-c3-ulb-wards-source-hunt-verdict.md).
...
**CLOSED 2026-05-30 - REAFFIRMS D.6 NO-GO**: re-probed upstream `LGD_Parliament_Constituencies.geojsonl.7z` is byte-identical to D.6's 2026-05-25 verification (release 2023-12-12; no commits since). 6 zero-active-PC states (J&K + Arunachal + Nagaland + Manipur + Assam + Jharkhand = 39 pre-delim features), no LGD-grade PC identifier, 44 `pc_id` mismatches (Telangana mis-keyed as AP), 543 feature count vs 545 precondition. KEEP shijithpk. Full verdict at [notes/2026-05-30-c5-c6-recon-no-go.md](../notes/2026-05-30-c5-c6-recon-no-go.md); prior D.6 recon at [notes/2026-05-25-d6-pc-recon.md](../notes/2026-05-25-d6-pc-recon.md).
...
**CLOSED 2026-05-30 - PRECONDITION FAILED**: per the 2026-05-29 B.1 verdict correction, Susewind 2014 ships 292 features under `state=='Andhra Pradesh'` at LEGACY PRE-bifurcation unified AP+TG numbering (NOT 175 post-bifurcation). The first gate condition is unsatisfiable on the existing Susewind 2014 file; no overlay shipped. Full verdict at [notes/2026-05-30-c5-c6-recon-no-go.md](../notes/2026-05-30-c5-c6-recon-no-go.md); B.1 correction at [notes/2026-05-29-phase-b-verdict-correction.md](../notes/2026-05-29-phase-b-verdict-correction.md).
...
#### D.1.A - CLOSED 2026-05-30 (PR #_pending_)

Shipped per the file list above, plus three discoveries beyond the original plan-doc scope:

1. **The chip-strip subsystem had more files than the plan-doc enumerated.** Original list: `lakshadweep.{ts,test.ts}`, `unmapped-regions.md`, `map.md`, `golden-path.spec.ts`, `coverage.py`. Discovered via grep: `UnmappedRegionChips.svelte` (4191 bytes), `unmapped-region-chips.ts` (4879 bytes, contains `UNMAPPED_REGIONS`, `fetchUnmappedRegions()`, `buildPopulationMap()`, etc.), `unmapped-region-chips.test.ts` (3854 bytes). All three deleted. **Generalisable rule:** for any retirement PR sourced from a plan-doc, grep for ALL related identifiers (here `UnmappedRegionChips|unmapped-region-chips|VITE_UNMAPPED_REGION_CHIPS|formatPopulationShort`), not just the obvious entry point.
2. **`format.ts` + `format.test.ts` were chip-strip-only.** `formatPopulationShort` exported only from `format.ts`, imported only by `unmapped-region-chips.ts`. With the chip strip gone the function had zero callers. Both files deleted in the same PR.
3. **ADR-0029 was preserved as `superseded`, not deleted.** Per CLAUDE.md ADR convention: ADRs are durable audit trail. Status flipped to `superseded` with a new "Retirement (D.1.A, 2026-05-30)" section explaining why the user mandate retires BOTH the inset (the surface ADR-0029 was replacing) AND the chip strip (the surface ADR-0029 was proposing).

User mandate (verbatim, 2026-05-29):
> *"REMOVE ANY SIDE FIXES FOR LAKSHADWEEP AS DATA TABLE, IF THE MAPS INCLUDE IT, EVEN IF THE CHOROPLETH IS UNVISIBLE LETS JUST KEEP IT IN THE MAP."*

The generalised rule the user encoded: **if the polygon is on the map (even sub-pixel / invisible at default zoom), do not add side-fix surfaces; the map polygon is the only authoritative surface; if a citizen needs to read a sub-pixel UT they zoom in.** No more inset, no more chip, no UT exclusion in coverage reports, no feature flag to bring any of those back.
