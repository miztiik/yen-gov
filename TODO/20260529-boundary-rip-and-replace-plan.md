...
| **A.1.b** | A | Backend + frontend: **S03 Assam only**. Tier 1 exhausted (LGD release is pre-2023 with status='Pre delimitation'; name overlap to post-2023 SoT is 63.5%, geometry redrawn not just renumbered). Ladder per user mandate 2026-05-29: **T3 PDF digitization first** (sub-PR if Aug 2023 Delimitation Order PDF probes feasible via B.5 dispatch), **else T4 district fallback** (frontend-only interim), **else T2 Voronoi** (last resort via B.4 dispatch). | _planned_ | **Blocked on B.5** | - | B.5 dispatch (Explore subagent) probes Aug 2023 Delimitation Order PDF availability + page-list + vector-extractability + license. Decision tree applied to B.5 verdict. |
...
| A.1.b (S03 Tier 3 - PDF, FIRST CHOICE per user mandate) | `/s/assam/ac/1` | Map renders 126 post-2023 AC polygons digitized from Aug 2023 Delimitation Order; tooltip declares `derivation_method: pdf-vectorisation`; LGD pre-2023 `ac_no` + `AC_ID` preserved per LGD-golden where polygon overlap exists. |
| A.1.b (S03 Tier 4 - district fallback, SECOND CHOICE if T3 blocked) | `/s/assam/ac/1` | Map renders 14 post-2023 Assam DISTRICT polygons (not AC cells); footer notes "AC-level boundaries pending; showing district outlines"; election results aggregate up to district level for the choropleth fill. |
| A.1.b (S03 Tier 2 - Voronoi, LAST RESORT if T3 + T4 both blocked) | `/s/assam/ac/1` | Map renders 126 Voronoi cells derived from ECI polling stations; tooltip declares `derivation_method: polling-station-voronoi`. |
