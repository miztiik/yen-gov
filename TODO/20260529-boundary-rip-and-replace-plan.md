...
| **C.3.b** | C | ULB Wards live lift + per-(state, ulb) shards under `datasets/boundaries/in/wards/state=in_<lc>/ulb=<ulb_lgd>/all.geojson` across ~30 states/UTs (SBM coverage minus WB/TR/MZ/MN); parquet upsert (+N rows in `boundary_layers.parquet`); first-snapshot property-name inspection (expect `ward_code` / `ulb_code` short-form per C.2.b panchayat short-form precedent); contract test `state-wards-shards-coverage.test.ts` (shard-coverage floor only, mirroring `state-panchayats-shards-coverage.test.ts` from C.2.b). Coverage gap (~4 states reserved for C.3.d): S14 Manipur S15 Mizoram S16 Tripura S25 West Bengal. | _pending_ | Not started | - | Direct mirror of C.2.b precedent. Expect upstream-schema surprise per C.2.b lesson (cross-layer property-name divergence within the same ramSeraph maintainer); read property names via module-level constants. |
