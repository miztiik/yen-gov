...
| **C.3.c** | C | Frontend `WARD_BOUNDARY_BY_ULB` registry keyed by `{state_code}-{ulb_lgd}` (N entries across covered states/UTs, constructed via `Object.fromEntries` over a compact `WARDS_BY_STATE: Readonly<Record<string, readonly number[]>>` source-of-truth; `WARD_STATE_NAMES` lookup carries the citizen-readable label per state); `frontend/src/contracts/state-wards-registry-coverage.test.ts` contract test (symmetric shards↔registry coverage mirroring the C.2.c precedent; absent-gap-states list S14 S15 S16 S25 locked so future SBM vintage churn requires deliberate registry update). ULB-picker UI shim DEFERRED — same scope discipline as C.2.c (defer picker UI to a later PR with measured data + UX feedback). | _pending_ | Not started | - | ULB-level scoping confirmed correct (state-level would bind 200-7000 polygons per layer; UX-perf risk). Registry construction via compact `{state_code: [ulb_lgd, ...]}` literal + `Object.fromEntries` keeps the diff readable. |
