...
| **C.1** | C | LGD Blocks - new `level: "block"` enum value (schema v1.x bump); new Hive partition `boundaries/in/blocks/state=in_<lc>/all.geojson`; snapshot run + frontend `BLOCK_BOUNDARY` registry. | _pending_ (recon #439, infra-only slice C.1.a in PR-pending) | Infrastructure landing (C.1.a); snapshot + shards + frontend integration deferred to C.1.b | - | Phase C kickoff: section 4a pattern proof for a NEW level. Citizen need: rural-development indicators (PMGSY, MGNREGA) bind at block granularity. **Upstream verified Tier-1**: ramSeraph `LGD_Blocks.geojsonl.7z` (BharatMaps/LGD lineage, 7323 features, CC0 1.0). Recon verdict + 6-step implementation plan at [notes/2026-05-29-c1-blocks-source-hunt-verdict.md](../notes/2026-05-29-c1-blocks-source-hunt-verdict.md). Block != Subdistrict (LGD treats them as different entities; yen-gov already ships subdistricts via `lift_subdistricts_national.py`). Sliced into **C.1.a** (infrastructure-only: `tools/boundaries/lift_blocks_national.py` mirroring the subdistricts orchestrator pattern, `backend/tests/test_lift_blocks_national.py` (8 cases mirroring subdistricts test coverage), `datasets/schemas/boundary-layers.schema.json` v1.2 + `block` added to `level` enum, `backend/yen_gov/canonical/boundary_layers_seed.py` `Level` Literal updated, `tools/boundaries/_paths.py` `Kind` + `KIND_TO_LEVEL` updated for `blocks`->`block` mapping) + **C.1.b** (next PR: run the lift script against live upstream, commit ~33 per-state shards, add frontend `BLOCK_BOUNDARY` registry to `frontend/src/lib/maplibre/sources.ts`, add Vitest contract test analogous to A.2's `state-ac-registry-coverage.test.ts`, first-snapshot inspection of upstream property names with adjustment if `block_lgd`/`block_name` differ from assumption). Note: deviated from recon verdict step 1 (add `pipeline.json` entry) - the actual pattern for per-state shards from a national geojsonl is dedicated lift script with no `pipeline.json` entry (matches `lift_subdistricts_national.py`); `pipeline.json` is reserved for `snapshot.py` inputs that emit a single national or pre-sliced shard. |
