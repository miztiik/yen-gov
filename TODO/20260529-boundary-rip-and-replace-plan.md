...
|---|---|---|---|---|---|---|
| **A.1** | A | Backend snapshot + pipeline.json: re-flip S01 AP + S03 Assam to LATEST machine-readable source (gated on B.1 + B.2 subagent verdicts); re-emit `boundary_layers.parquet` | _planned_ | Not started | - | Pre-req: B.1 + B.2 source-hunt subagent verdicts. If LATEST source found, ingest it. If not found, retire HTL and ship the state as "boundaries pending" (no ribbon-on-stale). |
| **A.2** | A | Frontend STATE_AC registry sync 6 -> 31: add 25 new entries to `frontend/src/lib/maplibre/sources.ts:STATE_AC`. All LGD-keyed entries use `join_property: "ac_no"` lowercase. S01/S03 entries reflect A.1's outcome (LATEST source or "pending"). U08 stays on shijithpk (post-2022 90-AC). | _planned_ | Not started | - | Gated on A.1. New contract test `state-ac-registry-coverage.test.ts` asserts the 31-state set + per-entry shape conformance. |
...
| **A.4** | A | Playwright per-state AC coverage spec: new `frontend/e2e/state-ac-coverage.spec.ts` iterates 31 states (incl. U08), navigates to `/s/<slug>/ac/1?event=<recent-eci-event>`, asserts >= 90% coloured polygons. Per-state report at `notes/2026-05-29-state-ac-coverage-report.md`. | _planned_ | Not started | - | Gated on A.2 merge. |
| **B.1** | B | **Subagent dispatch** (Explore, thorough): hunt LATEST machine-readable AP post-2014 AC boundaries. Confirm Susewind 2014 175-feature shape; also probe Bhuvan, AP State GIS portal, ECI 2014 polling-station overlay rollup, OSM 2024 tags. Deliverable: verdict file at `notes/2026-05-29-s01-ap-source-hunt-verdict.md` with one of: (a) recommended source URL + feature count + sample names + join key, OR (b) exhaustion report with all probed URLs. | _planned_ | Not started | - | Drives A.1 + A.2 for S01. |
| **B.2** | B | **Subagent dispatch** (Explore, thorough): hunt LATEST machine-readable Assam post-2023 AC boundaries. Probe: ramSeraph display-server post-2023 routes, Bhuvan, Assam State GIS portal (ASDMA, Assam Forest Dept, NRC Assam), ECI Delimitation Commission GIS exports, OSM 2024 tags for `boundary=political + admin_level=*`, DataMeet community, gazette PDF availability. Deliverable: verdict file at `notes/2026-05-29-s03-assam-source-hunt-verdict.md`. | _planned_ | Not started | - | Drives A.1 + A.2 for S03. NO RIBBON. If exhausted, S03 ships as "pending" CTA. |
...
|---|---|---|
| A.1 (Tier 1) | `/s/andhra-pradesh/ac/1`, `/s/assam/ac/1` | Map renders; rendered polygon count matches expected feature count from snapshot; no console errors. |
| A.1 (Tier 2 - Voronoi) | Same routes | Map renders Voronoi cells; tooltip declares `derivation_method: polling-station-voronoi`. |
| A.1 (Tier 4 - district fallback) | Same routes | Map renders DISTRICT polygons for that state; footer notes "AC-level boundaries pending; showing district outlines". |
...
**Inputs**: B.1 + B.2 subagent verdicts.

**Fallback ladder (S01 + S03 evaluated independently)**: A.1 escalates DOWN the tiers only if the higher tier is genuinely exhausted. The chip/pending-CTA path is RETIRED per user mandate 2026-05-29 ("chip is unacceptable, we need to find other fallbacks if other sources fail").
