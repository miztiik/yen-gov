...
| **C.2.a** | C | LGD Panchayats infrastructure: add `"panchayat"` to schema `level` enum + backend `Level` Literal; `_paths.Kind` + `KIND_TO_LEVEL` mapping; `pipeline.json` entry; `tools/boundaries/lift_panchayats_national.py` orchestrator stub modelled on `lift_villages_national.py` (district-keyed nested partition) + inheriting `lift_blocks_national.py` auto-fallback; `backend/tests/test_lift_panchayats_national.py` (11 cases mirroring blocks coverage). NO datasets yet. | _planned_ | Not started | - | Direct mirror of C.1.a precedent. |
| **C.2.b** | C | LGD Panchayats live lift + ~792 per-(state, district) shards under `datasets/boundaries/in/panchayats/state=in_<lc>/district=<lgd>/all.geojson`; parquet upsert; first-snapshot property-name inspection (TBC: `panchayat_lgd` / `panchayat_name`); `boundaries-conform.test.ts` HIVE_SHAPES extension; `frontend/src/contracts/state-panchayats-registry-coverage.test.ts` contract test. | _planned_ | Not started | - | Direct mirror of C.1.b precedent. Estimated 10-15% of shards exercise auto-fallback. |
