...
| **C.2.a** | C | LGD Panchayats infrastructure: add `"panchayat"` to schema `level` enum (v1.2 → v1.3) + backend `Level` Literal; `_paths.Kind` + `KIND_TO_LEVEL` mapping (`"panchayats" → "panchayat"`); `tools/boundaries/lift_panchayats_national.py` orchestrator modelled on `lift_villages_national.py` (district-keyed nested partition) + inheriting `lift_blocks_national.py` auto-fallback (PR #443) verbatim; `backend/tests/test_lift_panchayats_national.py` (11 cases mirroring blocks coverage + district-nesting); `boundaries-conform.test.ts` HIVE_SHAPES extension for panchayats. NO datasets yet. NO `pipeline.json` entry (blocks/villages precedent: lift scripts embed their own URL constants). | #_pending_ | **Infrastructure shipped; C.2.b live lift queued** | _post-merge_ | Direct mirror of C.1.a precedent. |
| **C.2.b** | C | LGD Panchayats live lift + ~792 per-(state, district) shards under `datasets/boundaries/in/panchayats/state=in_<lc>/district=<lgd>/all.geojson`; parquet upsert; first-snapshot property-name inspection (assumed `panchayat_lgd` / `pname`; TBC on first snapshot per C.1.b precedent); `frontend/src/contracts/state-panchayats-registry-coverage.test.ts` contract test. | _planned_ | Not started | - | Direct mirror of C.1.b precedent. Estimated 10-15% of shards exercise auto-fallback. |
