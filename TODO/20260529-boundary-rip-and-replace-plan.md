...
|---|---|---|---|---|---|---|
| **A.1.a** | A | Backend snapshot + pipeline.json: re-flip **S01 AP only** to LATEST machine-readable source. Uses LGD release with `(State_LGD=28 AND st_name='ANDHRA PRADESH')` filter + name-based `ac_no` rewrite to align legacy LGD 119-294 numbering with post-bifurcation SoT 1-175. Re-emit `boundary_layers.parquet`. | _planned_ | Not started | - | Pre-req: B.1 corrected verdict (see `notes/2026-05-29-phase-b-verdict-correction.md`). Phase B correction shows 100% SoT name parity achievable after filter+rewrite; ACTIONABLE NOW. |
| **A.1.b** | A | Backend + frontend: **S03 Assam only**. Tier 1 exhausted (LGD release is pre-2023 with status='Pre delimitation'; name overlap to post-2023 SoT is 63.5%, geometry redrawn not just renumbered). Decision required: (T2) Voronoi via B.4 dispatch / (T3) PDF digitization via B.5 dispatch / (T4) district fallback interim. | _planned_ | **Blocked** | - | Awaiting user decision on tier escalation. Recommendation: ship T4 district fallback as interim (frontend-only, ~1 day, citizen sees coloured districts on AC routes); concurrently dispatch B.4 + B.5 and pick the better for a follow-up A.1.b.v2. |
...
|---|---|---|
| A.1.a (S01 Tier 1 LGD with filter+rewrite) | `/s/andhra-pradesh/ac/1` | Map renders 175 AP polygons (post-bifurcation modern AP); rendered polygon count matches snapshot; no console errors; verify_ac_parity --state S01 reports 175/175 name parity. |
| A.1.b (S03 Tier 4 - district fallback INTERIM) | `/s/assam/ac/1` | Map renders 14 post-2023 Assam DISTRICT polygons (not AC cells); footer notes "AC-level boundaries pending; showing district outlines"; election results aggregate up to district level for the choropleth fill. |
| A.1.b (S03 Tier 2 - Voronoi, if escalated) | `/s/assam/ac/1` | Map renders 126 Voronoi cells derived from ECI polling stations; tooltip declares `derivation_method: polling-station-voronoi`. |
| A.1.b (S03 Tier 3 - PDF, if escalated) | `/s/assam/ac/1` | Map renders 126 post-2023 AC polygons digitized from Aug 2023 Delimitation Order; tooltip declares `derivation_method: pdf-vectorisation`. |
...
- **A.1.b (S03 Assam)** is Tier-1 exhausted; awaiting user decision among Tier 2 (Voronoi via B.4 dispatch) / Tier 3 (PDF via B.5 dispatch) / Tier 4 (district fallback - recommended interim).
...
**Fallback ladder (escalate DOWN tiers only if the higher tier is genuinely exhausted)**: The chip/pending-CTA path is RETIRED per user mandate 2026-05-29 ("chip is unacceptable, we need to find other fallbacks if other sources fail").

| Tier | Source class | A.1 action | Subagent gate |
|---|---|---|---|
| **1 - Preferred** | LATEST machine-readable upstream (ramSeraph LGD post-2014/2023 slice, Susewind 2014, Bhuvan, State GIS portal, OSM 2024) | Switch `pipeline.json` block to LATEST source; re-snapshot; lowercase-normalise `join_key_property` to `ac_no`. | B.1 (S01) / B.2 (S03) |
| **2 - Acceptable** | Polling-station Voronoi rollup: ECI publishes polling-station lat/lon with `AC_NO` tags post-redelim; tessellate within state boundary to derive AC polygon proxies. Lives at `boundaries/in/ac/state=in_<lc>/_voronoi/all.geojson` with `derivation_method: polling-station-voronoi` declared on the ledger row. | Build `tools/boundaries/voronoi_from_pollingstations.py` (one-time); ingest the derived layer as a parallel source. | B.4 (NEW: ECI polling-station availability + AC-tagging probe; dispatched only if Tier 1 exhausted for that state) |
| **3 - Final-resort** | One-time vector digitization from gazette PDF (Delimitation Commission August 2023 Order for S03; ECI 2014 notification for S01). Output ships clean GeoJSON at `boundaries/in/ac/state=in_<lc>/_pdf2024/all.geojson` with `derivation_method: pdf-vectorisation` on the ledger row. | Tracked as a separate parallel PR `feat/boundary-digitize-<state>-from-pdf` since this is a manual QGIS effort, NOT a subagent task. | B.5 (NEW: PDF availability + page-list probe; dispatched only if Tier 1 + Tier 2 both exhausted for that state) |
| **4 - Graceful degradation (interim)** | Render the state at DISTRICT granularity for the AC route: re-use the existing `boundaries/in/districts/state=in_<lc>/all.geojson`; citizen sees coloured districts instead of coloured ACs. Tooltip / map footnote: "AC-level boundaries pending; showing district outlines as interim." | Frontend STATE_AC registry entry points at the district layer for that one state; map join key switches to district LGD. Pure routing decision in `frontend/src/lib/maplibre/sources.ts`; no pipeline change. | No subagent; ships as the default interim render while Tier 3 work proceeds in parallel. |
...
**Escalation discipline**: A.1 only commits Tier-2-onwards code if B.1/B.2 verdicts confirm Tier 1 is exhausted FOR THAT STATE. If Tier 1+2+3 all exhaust on first pass, Tier 4 (district fallback) ships as the citizen-visible interim and Tier 3 (PDF digitization) is queued as a separate PR to back-fill Tier 4 with the LATEST shape when complete. The state map ALWAYS renders something coloured for the citizen; never a blank pending placeholder.
