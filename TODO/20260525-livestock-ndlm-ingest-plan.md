...
**Status**: ◻ QUEUED — Phase 0 (taxonomy + topic + family seed) is the next PR; Phase 1 (snapshot + meadow lift) follows. **LGD-district FK risk FORECLOSED** by recon (2026-05-25; see [§7](#7-lgd-district-recon-result-gregors-1-risk-foreclosed)).
...
| Phase 1.D (NADCP snapshot + meadow) | 🔒 BLOCKED | Open question #2 - disease enumeration unknown; live probe returned 0 rows for `diseaseCd in {1, 200..225}`. Need a `/getDiseases` endpoint or NDLM Angular bundle constants. Andre + Max to recon. |
| Phase 1.E (Breeding snapshot + meadow) | 🔒 BLOCKED | Open question #3 - NDLM exposes ABIP / RGM / NAIP IV as separate dashboards. Indicators 7-8 may need 2-3 sub-source endpoints, each carrying its own `source_id`. Max + Gregor to recon. |
| Phase 2.A (Owner Reg canonical adapter + Parquet) | ◐ MERGED | PR #_pending_. Bundled per Q#4 verdict E: `backend/yen_gov/canonical/lgd.py` (memoized LGD->entity_id resolver, 3 unit tests) + `landholding` FacetAxis (6 brackets: landless_marginal / small / semi_medium / medium / large / not_specified; closed enum aligned with Agriculture Census 2015-16) + `backend/yen_gov/canonical/adapters/livestock/owner_reg.py` (two-pass: first SUMs over gender to collapse the publisher's composite facet, second SUMs district children to state-grain per ADR-0043; derivation=`sum` on both grains) + 14 catalogue rows (1 district parent compute-on-read + 6 district facet children + 1 state parent compute-on-read + 6 state facet children) + 12 Tier-A contract tests. Source `src-d98dc531ef7e`; target table `livestock_owner_registration`. 3055 district rows + 168 state rows = 3223 total. Gender axis lift deferred (would need a separate sex-stratified registration indicator family; Phase 1.A meadow preserves gender on every cell for that future PR). |
...
| Phase 2.C (NAIP IV canonical adapter + Parquet) | ◻ QUEUED | Depends on Phase 2.A (for the shared `_shared.py` SOURCE_IDS + LGD helper pattern). Splits the Phase 1.C composite facet onto the 8 catalogue indicators. |
| Phase 2.D (NADCP canonical adapter + Parquet) | 🔒 BLOCKED | Depends on Phase 1.D unblock. |
| Phase 2.E (Breeding canonical adapter + Parquet) | 🔒 BLOCKED | Depends on Phase 1.E unblock. |
...
| Phase 3.B (`/t/agriculture` extend district-grain to 9 other species) | ◻ QUEUED | Mount buffalo / goat / sheep / pig / mithun / yak / horse / donkey / mule each at district-grain on `/t/agriculture`. Independent of Phase 2.A. |
| Phase 3.C (`/t/agriculture` mount Owner Reg / NAIP IV / NADCP / Breeding) | ◻ QUEUED | Each indicator family gated on its Phase 2.x landing. Partial-coverage `/t/agriculture` is fine - each indicator family can ship independently as its canonical Parquet lands. |
