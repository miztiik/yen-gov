...
| 4 | Whether to promote the LGD->entity_id helper to a cross-family module at `backend/yen_gov/canonical/lgd.py` in the same PR as livestock Phase 1, or defer to a later refactor. Gregor verdict E recommends in-the-same-PR. | Fowler | Phase 1 PR |
...
| Phase 2.A (Owner Reg canonical adapter + Parquet) | ◻ NEXT | Open question #4 - should the `backend/yen_gov/canonical/lgd.py` cross-family helper land in this PR (Gregor verdict E) or be deferred to a later refactor? Awaiting decision. |
