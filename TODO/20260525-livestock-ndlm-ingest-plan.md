...
| Phase 1.D (NADCP snapshot + meadow) | !! ESCALATED | Open question #2 RESOLVED on enum (Max recon, 2026-05-25): NADCP scope = exactly FMD + Brucellosis per DAHD authoritative scheme page `https://dahd.gov.in/schemes/programmes/nadcp`. Probe map: diseaseCd 1 = FMD, diseaseCd 2 = Brucellosis. **NEW BLOCKER**: live POST-probe matrix of 60 cells (5 years 2019-2023 x CY+FY x diseaseCd {1,2} x states {TN/33, KA/29, UP/9}) ALL return `flg:true, data:{totalVaccinations:0, totalOutput:{}}` from `getNADCPVaccinationDistrictWise`. Including invalid `diseaseCd=99` which also returns `0 rows, 200 OK` (silent server-side validation). `isRoundWise=True` returns HTTP 400 with empty body. Existing `tools/ndlm_download.py` snapshots in `.runtime/raw/ndlm/2024/` + `2024-25/` confirm same upstream empty-response pattern. NADCP endpoint exists but publisher serves empty `totalOutput: {}` for ALL probed combinations - either auth-gated (no cookies/JWT in our probe), data not yet published via API, or migrated to different endpoint. **ESCALATION OPTIONS**: (a) human DevTools capture - user opens `https://bharatpashudhan.gov.in/` NADCP dashboard in Chrome -> Network panel -> paste exact POST body the SPA sends; (b) DAHD outreach for endpoint authentication contract; (c) pivot to alternative upstream (DAHD scheme page itself + parliamentary-answers-via-PRS for NADCP coverage numbers). |
| Phase 1.E (Breeding snapshot + meadow) | !! ESCALATED | Open question #3 PRE-RESOLVED via `tools/ndlm_download.py` design notes (line 43-46): "Breeding (ABIP/RGM) endpoints are dashboard-level not per-state-endpoint - out of scope for this tool; covered by a separate discovery PR." Confirmed by 14-endpoint probe matrix (2026-05-25): `getABIPHeaderCount`, `getABIPStateWise`, `getABIPDistrictWise`, `getRGMHeaderCount`, `getRGMStateWise`, `getRGMDistrictWise`, `getRashtriyaGokulMissionStateWise`, `getBreedingStateWise`, `getCattleBreedingStateWise` all return HTTP 404 on `https://bharatpashudhan-api.ndlm.co.in/epashu/v1/homepage/`. NDLM publisher exposes NO per-state Breeding API endpoint. **ESCALATION OPTIONS**: (a) DAHD/NDDB direct-publication outreach (ABIP and RGM both produce official annual reports - PDF tabular data can be extracted); (b) human DevTools capture of Breeding-dashboard SPA XHR if such a dashboard exists; (c) defer indefinitely (indicators #7 + #8 stay UNFILLED on `/t/agriculture`; not a critical-path blocker for the rest of the plan). |
