...
**Status**: ACTIVE - Row A plan-doc PR #501 open; implementation rows not started.
...
| --- | :---: | --- | --- | --- | --- | --- | --- |
| A | #501 | In review | `feat/schema-version-magic-plan` | Fowler | Add this TODO plan only. No runtime behavior. | File exists under `TODO/`; plan names subagent use, branch discipline, classification table, exclusions, and row gates; `git diff --check` clean. | Stop if another active plan already supersedes this exact scope on `main`. |
