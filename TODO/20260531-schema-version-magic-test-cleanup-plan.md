...
**Status**: ACTIVE - Row A #501, Row B #503, Row C0 #506, Row C1 #507, Row C2 #509, Row C2.5 #511, Row C3 #512, Row D livestock slice #513, Row G #514, Row D ICED backend slice #515, and Row E backend non-indicator test cleanup #516 merged; Row F indicator schema/catalogue test cleanup is in review as PR #517.
...
| D | #513, #515 | Merged - livestock + ICED slices; RBI remains open | `feat/iced-schema-version-emitter` | Fowler + Gregor | Active emitter cleanup in `tools/` and backend producer modules. #513 fixed livestock meadow tools; #515 fixes ICED backend meadow payload stamps. RBI/backend producer constants stay pending for later Row D PRs. | Tools do not import backend runtime. Tests prove emitted `$schema_version` equals source schema x-version. Targeted pytest/tool tests pass. `python -m yen_gov validate --root .` if datasets/config/schemas are touched. | Stop if a tool emits artifacts not clean under the current schema shape; fix shape before stamping current. |
...
- Leaves [tools/rbi_hbs_ingest_state_gdp.py](../tools/rbi_hbs_ingest_state_gdp.py) and backend producer constants for later Row D slices after shape classification.
