...
**Status**: ACTIVE - Row A #501, Row B #503, Row C0 #506, Row C1 #507, Row C2 #509, Row C2.5 #511, Row C3 #512, Row D livestock slice #513, Row G #514, Row D ICED backend slice #515, Row E backend non-indicator test cleanup #516, and Row F indicator schema/catalogue test cleanup #517 merged; Row D RBI state SDP backend-adapter slice is in review as PR #518.
...
| D | #513, #515, #518 | In review - RBI state SDP backend slice; Row D remains open | `feat/rbi-state-sdp-adapter` | Fowler + Gregor | Active emitter cleanup in `tools/` and backend producer modules. #513 fixed livestock meadow tools; #515 fixes ICED backend meadow payload stamps. The RBI state SDP slice migrates the stale tools emitter into a backend adapter and retires the tool. Other backend producer constants stay pending for later Row D PRs. | Tools do not import backend runtime. Tests prove emitted `$schema_version` equals source schema x-version. Targeted pytest/tool tests pass. `python -m yen_gov validate --root .` if datasets/config/schemas are touched. | Stop if a tool emits artifacts not clean under the current schema shape; fix shape before stamping current. |
...
2026-05-31 livestock slice in progress on `feat/tools-schema-version-livestock`:
...
- Leaves the RBI HBS-IE state SDP producer and backend producer constants for later Row D slices after shape classification.
...
2026-05-31 RBI state SDP backend slice in review as PR #518 on `feat/rbi-state-sdp-adapter`:
...
- Documents the adapter in [docs/architecture/backend/sources-rbi-hbs-ie-state-sdp.md](../docs/architecture/backend/sources-rbi-hbs-ie-state-sdp.md).
...
2026-05-31 Row F implementation in review as PR #517 on `feat/indicator-schema-test-cleanup`:
