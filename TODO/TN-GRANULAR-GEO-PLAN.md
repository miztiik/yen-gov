...
- [ ] **`frontend/src/lib/boundaries.ts`** — `loadBoundary(level, parentLgdCode?, stateLgdCode?)` returning a typed `BoundaryFeatureCollection`. Path composition (v4, matches existing on-disk filenames):
...
- [ ] **Vitest unit test** — `boundaries.path.test.ts`: pure path resolver — `("village", "603", "33") → "S22-villages-603.geojson"`, `("subdistrict", undefined, "33") → "S22-subdistricts.geojson"`, `("district") → "india-districts.geojson"`, `("state") → "india-states.geojson"`. Plus join-key resolver: `joinKeyFor("village") === "village_lgd"`.
- [ ] **Vitest contract test** — `boundaries.contract.test.ts`: every shipped boundary file has a sibling `<file>.geojson.sources.json` validating against `boundary.sources.schema.json`; where present, `<file>.geojson.metadata.json` validates against `feature_collection.metadata.schema.json` v1.1; where present, `<file>.geojson.unkeyed.json` validates against `boundary.unkeyed.schema.json` v1.0. Every feature on each LGD-keyed file carries the level's join-key property.
- [ ] **Vitest integration test** — `boundaries.integration.test.ts`: loader composes correct path for `("subdistrict", undefined, "33")`; `fetch` mock returns fixture; loader returns typed collection. 404 → `null`, not throw.
- [ ] **Bundle-budget test** (Tier-Contract, Fowler edit) — `boundaries.budget.test.ts`: assert max-file-size per district ≤ configured budget AND **exact** max-chunk-count from `import.meta.glob` ≤ **80** (existing india-states + india-districts + india-soi + S22-subdistricts + ~38 S22-villages-* + ~38 reserve for next state + 1 postal = 80; pin the number, bump deliberately on schema/file additions — Fowler v3 nit, "~" drifts).
- [ ] **Catalogue drift detector** (existing `catalogue-coverage.test.ts`) extended to also detect orphan boundary files (any `*.geojson` under `datasets/boundaries/in/geojson/` not reachable from `loadBoundary`'s path table).
