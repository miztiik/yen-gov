...
- [x] Mobile gesture tuning (pinch-to-drill enabled — was reserved in Phase 3). — done 2026-05-15: opt-in `pinch_to_drill` prop on MapChoropleth dispatches `onSelect` on touchend when (≥2 fingers at touchstart) AND (zoom delta ≥ 0.6). IndicatorChoropleth opts in via `pinch_to_drill={drill_enabled}`. Same `onSelect({key, properties, at})` shape as click — pending-spinner pins over the gesture point. Manual smoke (CLAUDE.md §13 touch DevTools) is the verification tier; no unit test for touch-on-maplibre (vitest can't mount it).
- [x] **Pincode polygons + `datasets/schemas/postal.schema.json` v1.0** — schema + Chennai pincode file land here under `datasets/boundaries/in/postal/IN-pincodes-chennai.geojson` with `<file>.geojson.sources.json` sidecar (Fowler YAGNI — deferred from Phase 0 since the consumer is the search affordance built in Phase 3; if Phase 3 search ships before Phase 4, promote this bullet to Phase 3). Loader path table extends with `("postal", "<pincode>", "33") → "../postal/IN-pincodes-chennai.geojson"`; bundle-budget chunk count remains 80. — done 2026-05-15 as **structural-only landing**: `postal.schema.json` v1.0 created (per-state pincode registry, modelled on `subdistrict.schema.json` but `id`=6-digit pincode and `id_source`=`indiapost`); `boundaries.ts` `GeoLevel` extended with `"postal"`, `JOIN_KEYS.postal = "pincode"`, `boundaryBasename("postal", _, "33")` returns `"../postal/IN-pincodes-chennai.geojson"`; `drilldown.ts` carries postal as a `-1` rank sentinel (postal is search-only, NOT a drill rung — Jony edit §d) so `nextLevel("postal") === null`. Path/contract/budget tests pass; `datasets-conform` validates the new schema against draft 2020-12 meta-schema. **Deferred to follow-up commits**: (a) actual `datasets/boundaries/in/postal/IN-pincodes-chennai.geojson` ingest + sidecar, (b) per-state `datasets/reference/in/states/S22/postal.json` registry data, (c) the Phase 3 search-affordance UI consumer that types pincode → zooms to polygon. Structural surface lands now so the schema-validator and `GeoLevel` union both know about `postal` before any data file references them.
