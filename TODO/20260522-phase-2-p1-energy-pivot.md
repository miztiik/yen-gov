...
**Status**: ◧ IMPLEMENTATION ACTIVE on `feat/p1-energy-pivot` (from `main@b4d4be45` post-PR #94 merge). Hans + Max design pass closed 2026-05-22; Gregor + Hans verdict pass closed 2026-05-22 (PRs #92/#93/#94 merged). Pre-flight Q1-Q4 resolved 2026-05-22: Q1 reader-switch = Option B loader-seam flip (Gregor + Jony, HIGH conf — see §8); Q2 no CI invokes energy adapters; Q3 multi-commit single PR (git bisect friendly); Q4 DuckDB-WASM plumbing already shipped (`frontend/src/lib/duckdb.ts` + `canonical/duckdb.ts`). Pre-flight on-disk discovery (per 2026-05-22 lesson #1): canonical schemas + writer + facet_axes `fuel_type` enum ALREADY authored at Phase 0.3 (2026-05-18); P.1.A is the first family pivot USING the contracts, not authoring them — see Q-schema below for the correction. Implementation sub-commits C0-C8 in progress; this commit is C0.
