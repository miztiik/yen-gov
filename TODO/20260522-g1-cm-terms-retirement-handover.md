...
> **Status**: G.1.a SHIPPED (`ee441193` merge of `99952afd` on `feat/phase-2-preflight-g1a-office-bearer-entities`, PR #89). G.1.b SHIPPED (`cc9ad5b7` merge of `09631239` on `feat/phase-2-preflight-g1b-cm-terms-reader-switch`, PR #90). **G.1.c SHIPPING 2026-05-22** on `feat/phase-2-preflight-g1c-office-holdings-consolidation` (Option B consolidation: 31 per-state `cm_terms.json` → 1 `datasets/taxonomy/office_holdings.json` after Hans + Fowler + Max 3-persona review). PR2 (`feat/phase-2-preflight-t1-status-features-audit-g1-deferred`, merged as `8fb3e935` / PR #88) descoped G.1 retirement after discovering the original Gregor audit estimate ("Pure structural, sub-day, data already in Parquet") underestimated the work; this 3-PR strangler-fig executes the honest retirement.
...
- **G.1.b — reader-switch** (SHIPPED 2026-05-22, `cc9ad5b7` / PR #90): rewrote `cm_terms_seed.py::compile_to_parquet` to resolve office identity from `entities.parquet WHERE entity_type='office_bearer' AND entity_code='CM'` instead of computing it from `cm_terms.json` state codes inline. Tenure rows still came from `cm_terms.json`. Verified byte-identical Parquet outputs (`dim_offices.parquet`, `governments_office_holdings.parquet`, `sources.parquet`) pre- vs post-rewrite. Old JSON stayed on disk as fallback / cross-check for one PR cycle.
- **G.1.c — consolidate + retire** (THIS PR, SHIPPING 2026-05-22): consolidated the 31 per-state `cm_terms.json` into ONE long-form `datasets/taxonomy/office_holdings.json` (359 holdings + 31 office_citations); new `office_holdings_seed.py` replaces `cm_terms_seed.py`; frontend `governments.ts` rewritten to read the consolidated file with an in-loader field-name adapter back to the legacy `GovernmentTerm` shape (Svelte components unchanged); `git rm` of the 31 JSON files + cm_terms_seed.py + test_cm_terms_seed.py + state_government.schema.json + tools/author_cm_terms.py + tools/lift_cm_offices_to_entities.py. Byte-identical Parquet outputs verified for the 3rd time.
