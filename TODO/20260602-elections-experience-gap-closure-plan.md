...
| --- | --- | --- | --- | --- | --- | --- |
| EGC-0 | 0 | Process guardrail: STOP-AND-SURFACE tripwire (no schema enums) | none `||` | [ ] PENDING | _pending_ | Gregor |
...
| EGC-B1 | B | ECI xls/xlsx constituency-wise historical LS source recon (1999-2019) | none `||` | [ ] PENDING | _pending_ | Max |
| EGC-B2 | B | Ingest historical PC series from chosen source + honesty flags | EGC-B1 | [ ] PENDING | _pending_ | Hans + Max |
...
### Lane B - historical Lok Sabha series from an ECI source (user: prefer ECI xls/xlsx; TCPD is ECI-derived; do not fight PDF)

**EGC-B1.** Recon-only. Find the best ECI constituency-wise source for the 1999-2019 Lok Sabha general elections that carries **electors + postal** (so turnout is recoverable), in `.xls/.xlsx` or `.csv`. Candidates to probe: ECI Statistical Reports / "Constituency Wise Detailed Results" archives per GE; the same ECI #33-style detailed-result workbooks that 2024 used, for prior years; TCPD's direct PC-level GE release (ECI-derived) as the documented fallback. Per the CSV-only ingest contract: if the chosen source is `.xls/.xlsx`, convert to CSV in a one-time documented prep step OUTSIDE the pipeline; the pipeline ingests CSV only (stdlib csv + DuckDB; no xlrd/pandas in `backend`). Deliverable: a recon note `notes/2026-06-02-eci-historical-ls-source-recon.md` naming the chosen source, its column set (does it carry electors/postal?), licence, per-year coverage, and which of the 13 `pc-*` indicators it yields. If electors/postal are unavailable from every ECI source AND only the segment file remains, that is a scope change -> STOP-AND-SURFACE (Section 4), do not silently ship a 9-of-13 fallback. Gates: G-docs; no code/data.
