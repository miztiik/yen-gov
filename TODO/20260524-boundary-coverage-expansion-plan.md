...
| **0.2** District backfill (639 rows → `entities.json`) | — | **Needed (active; independent parallel track)** | — | User direction 2026-05-25: districts are required for financial-inclusion, rainfall/water, topography density choropleths. Hand-curation is acceptable per ADR-0033; cross-enrichment from LGD master + Census-2011 columns already on the LGD CSV. Coverage rationale: [`docs/reference/boundary-data-sources.md` §"Coverage status"](../docs/reference/boundary-data-sources.md#coverage-status--what-we-have-what-we-dont-have). Independent of all D.* work; can ship in parallel. Tooling: `tools/lgd/backfill_entities_districts.py` (NEW). |
