...
**Last updated**: 2026-05-25 (after Phase D.2 — AC consolidation promote for 10 eligible states landed via PR #273 at `fcd481cf`: ramSeraph LGD_Assembly_Constituencies replaces HTL geojson for S04 / S07 / S08 / S17 / S18 / S19 / S23 / S26 / S28 / U05 = 1015 ACs at 97.3–100% name parity vs SoT; bundles D.3 keep-current Assam + D.4 keep-current J&K per D.1 §4 Outcome-3 verdicts; new permanent tool `tools/boundaries/verify_ac_parity.py` and additive `apply_exclude_filter` directive in `snapshot.py` shipped as general-purpose infrastructure; empirical inspection during this PR overturned D.1 §4's Pre-delim hypothesis — `exclude_filter` directive is NOT used on these 10 entries because 9 of 10 states have 0 Pre-delim rows and S17 Nagaland has 100% Pre-delim rows due to constitutional exemption from 2008 Delimitation).
...
>
> 1. **Phase D.5** — AC consolidation wrap-up (docs + ledger + ADR amend). Closes D.0–D.4. Can delete `tools/boundaries/recon_d1_ac.py` (one-shot D.1 tool); keep `tools/boundaries/verify_ac_parity.py` (permanent, re-usable by any future D.2-style promotion).
> 2. **Phase D.6** — PC polygon swap (shijithpk → ramSeraph). Independent of D.0–D.5; can ship before or after.
>
