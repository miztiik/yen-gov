...
| **A.1.a** Pincode directory — structural surface (no real data) | _pending_ | In progress | — | Split from A.1 because the CSV ingest blocks on operator captcha-fetch. Ships pure parser (`pincode_directory.py`), `ResourceMeta` entry (UUID `5c2f62fe-5afa-4119-a499-fec9d604d5bd`), Tier-A test against inline fixtures. No real data, no schema yet, no sources.parquet row yet. |
| **A.1.b** Pincode directory — ingest + emit | — | Blocked: captcha-fetch | — | **OPERATOR ACTION**: visit https://www.data.gov.in/resource/all-india-pincode-directory-till-last-month, solve captcha, drop CSV at `.runtime/raw/datagovin/pincode_directory.csv`. Then this phase emits `datasets/reference/in/pincodes/pincode-directory.csv` + dated companion + new `sources.parquet` row (Department of Posts, OGL-IN-1.0 if our enum permits, else `unknown-public`). |
