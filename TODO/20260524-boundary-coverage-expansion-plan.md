...
| **D.1** AC consolidation snapshot recon (one-shot, gating) | #270 | Merged | `33d97800` | Recon-only PR: new `tools/boundaries/recon_d1_ac.py` (one-shot, deletable after D.5) + `docs/archive/notes/2026-05-25-d1-ac-consolidation-recon.md` audit. Property schema resolved (`AC_ID` for `id_property`, `ac_name` for `name_property`, `State_LGD` for state route); 4,177 LGD features vs SoT 4,113 (+64); **10 states/UTs eligible for D.2** (Bihar S04, Haryana S07, HP S08, Nagaland S17, Odisha S18, Punjab S19, Tripura S23, Chhattisgarh S26, Uttarakhand S28, NCT of Delhi U05); 19 states keep-current (count and/or name mismatch, mostly off-by-1 to off-by-15 patterns suggesting pre-bifurcation residue or LGD vintage drift); Assam S03 (D.3) and J&K U08 (D.4) BOTH fall under "Outcome 3 mixed/unclear" — recommend keep current source for both. `status='Pre delimitation'` feature tag identified as the key vintage signal for D.2 promote PRs (use as `exclude_filter`). No data-store mutation; no §13 (recon-only). |
| **D.2** AC consolidation promote (10 eligible states) | #273 | Merged | `fcd481cf` | Promotes the 10 D.1-eligible states from HTL to ramSeraph LGD: Bihar S04 (243), Haryana S07 (90), HP S08 (68), Nagaland S17 (60), Odisha S18 (147), Punjab S19 (117), Tripura S23 (60), Chhattisgarh S26 (90), Uttarakhand S28 (70), NCT of Delhi U05 (70) — 1015 ACs total. Adds `apply_exclude_filter` to `tools/boundaries/snapshot.py` (additive, ~30 LOC) as a general-purpose capability for future per-state slices that need to drop a tagged subset (6 unit tests in `backend/tests/test_boundary_snapshot_exclude_filter.py`); the directive is NOT used on these 10 entries — see "Empirical correction to D.1 §4 Pre-delim claim" below. New permanent tool `tools/boundaries/verify_ac_parity.py` re-asserts post-snapshot the same 3 invariants D.1 recon measured (100% AC count match + 100% `ac_no` set-equality + ≥95% name parity, threshold from D.1 §5); all 10 states pass (97.3%–100% name parity). 10 per-state `state_filter={State_LGD: <int>}` entries replace the legacy HTL geojson entries; each shard at coord_precision=2; license CC0-1.0 (matches D.0 source row `src-a1dd899f902d`, no new sources row needed). Bundles D.3 + D.4 keep-current decisions per D.1 §4 verdict (Outcome 3 mixed). **Empirical correction to D.1 §4 Pre-delim claim**: the recon hypothesised `status="Pre delimitation"` would be a rare contaminant concentrated in Assam + S04 + S22 + U07; empirical inspection of the raw geojsonl during this PR found 9 of the 10 D.2-target states have `status=" "` (whitespace = current) for every feature, while S17 Nagaland has `status="Pre delimitation"` for ALL 60 of its features (Nagaland was constitutionally exempted from the 2008 Delimitation; the 1976-vintage 60-AC layout is what ECI uses today and what citizens see on `/s/nagaland/t/elections`). Filtering Pre-delim out would erase Nagaland entirely. Decision: drop the `exclude_filter` directive from all 10 pipeline entries; keep `apply_exclude_filter` + tests as general-purpose infrastructure for future use; annotate `docs/archive/notes/2026-05-25-d1-ac-consolidation-recon.md` §4 with the empirical correction. |
| **D.3** AC consolidation — Assam carve-out (keep current per D.1) | #273 | Merged | `fcd481cf` | Bundled into D.2 PR as keep-current decision per D.1 §4 Assam verdict: LGD count 134 vs SoT 126 (+8) with **1% name-match parity** — strongly suggests LGD carries the pre-2023 layout PLUS the 8 newly-created post-delim ACs without retiring the pre-delim originals. NOT eligible for clean swap. S03 stays on HTL `assam_AC.json` (the legacy 2008-delim shape) for now. Open question for Hans + Max: whether to ingest the LGD raw for an Assam-specific delim audit in a follow-up PR; not on this sprint. |
| **D.4** AC consolidation — J&K carve-out (keep current per D.1) | #273 | Merged | `fcd481cf` | Bundled into D.2 PR as keep-current decision per D.1 §4 J&K verdict: LGD count 101 vs SoT 90 (+11) with **6% name-match parity** — LGD carries the pre-2019-statehood 87 + extras rather than the post-2022 90-AC layout. NOT eligible. U08 stays on the existing shijithpk source (the post-2022 90-AC layout that backs the citizen-facing /s/jammu-kashmir/t/elections route today). Re-evaluate when ramSeraph upstream refreshes with the post-2022 J&K layout. |
| **D.5** AC consolidation wrap-up (docs + ledger + ADR amend) | #288 | Merged | `b696a8fc` | Closes D.0/D.1/D.2/D.3/D.4. Three doc surfaces refreshed: (1) ADR-0031 amendment 2026-05-25 records per-state authority table + Article 371A empirical correction for S17 Nagaland; (2) `docs/reference/boundary-data-sources.md` three off-by-one fixes (was "30 of 31 / 19 HTL", now "31 of 31 / 20 HTL" - the post-D.2/D.3/D.4 ledger reality is 10 ramSeraph + 20 HTL + 1 shijithpk = 31 elective state/UT AC layers); (3) handover trail extended. Ledger verify-only: `boundaries.in.states` carries `src-a1dd899f902d` (D.0 done); the 10 promoted AC rows carry `src-a1dd899f902d` (D.2 done); S03 carries `src-4ad56b409556` HTL (D.3 keep-current done); U08 carries `src-68ad69e02476` shijithpk (D.4 keep-current done) - no ledger rewrite needed. One-shot recon tool `tools/boundaries/recon_d1_ac.py` retired per CLAUDE.md section 10 "don't create helpers for one-time operations"; permanent `tools/boundaries/verify_ac_parity.py` kept. No data, schema, or code changes. |
| **D.6** PC polygon swap (shijithpk → ramSeraph `LGD_Parliament_Constituencies`) | _TBD_ | Closed (KEEP-CURRENT) | _no-data_ | Recon-only PR. `tools/boundaries/recon_d6_pc.py` (NEW, one-shot recon driver) snapshotted `LGD_Parliament_Constituencies.geojsonl.7z` (sha256 prefix `8ee32756f0fed264`, 18 MB archive / 83 MB extracted, 543 features). Four NO-GO findings (full audit in `docs/archive/notes/2026-05-25-d6-pc-recon.md`): (1) feature count 543, plan-doc gate requires 545 ±1 (shijithpk preserves 2 abolished Anglo-Indian seats); (2) 39 features marked `status="Pre delimitation"` covering SIX entire states with ZERO active coverage (J&K=6/6, Arunachal=2/2, Nagaland=1/1, Manipur=2/2, Assam=14/14, Jharkhand=14/14) — adopting would silently strip ~78 active PCs from the map; (3) no `lgd_pc_code` property (PC-id candidates `pc_id` and `pc_no` are not LGD codes); (4) `pc_id` unstable — Telangana 17 PCs use Andhra Pradesh's pre-2014 `28xx` prefix instead of TS `36xx` (44 mismatches total). Shipping the swap would CAUSE regressions vs current shijithpk. Keep `datasets/ephemeral/india_ls_seats_545.geojson` + `pipeline.json` PC entry unchanged. ADR-0031 amended 2026-05-25 with D.6 vintage-gate decision. Recon driver kept in-tree as re-evaluation trigger; re-run when ramSeraph publishes a release with active features for the six structural-gap states AND a stable LGD-grade PC code. |
...
| **D** | Survey-grade swap to ramSeraph BharatMaps lineage: state polygons (D.0), AC consolidation (D.1–D.5), PC swap (D.6) | **Complete** (D.0–D.5 merged; D.6 closed KEEP-CURRENT 2026-05-25) | — | D.1 snapshot recon must happen before any AC promotes; D.0 + D.6 are independent of AC work |
...
> **2026-05-25 update (D.5 landed)**: PR #288 closed the loop. ADR-0031 amendment 2026-05-25 records the per-state authority table (10 ramSeraph promote + S03 HTL keep-current + U08 shijithpk keep-current + 19 other HTL pre-Phase-D) and the S17 Nagaland Article 371A empirical correction. `boundary-data-sources.md` inventory fixed three off-by-one counts (was "30 of 31 / 19 HTL", now "31 of 31 / 20 HTL"). Ledger refresh was verify-only: D.2 already pointed the 10 promoted rows at `src-a1dd899f902d`, D.0 already pointed `boundaries.in.states` at the same row, and D.3 / D.4 keep-current rows already carry their respective HTL / shijithpk source_ids. One-shot recon tool `tools/boundaries/recon_d1_ac.py` retired per CLAUDE.md section 10 ("don't create helpers for one-time operations"); permanent `tools/boundaries/verify_ac_parity.py` kept as a re-usable D.2-style promote gate.

### D.6 — PC polygon swap (shijithpk → ramSeraph `LGD_Parliament_Constituencies`)

> **2026-05-25 update (D.6 closed KEEP-CURRENT)**: Recon-only PR landed. `tools/boundaries/recon_d6_pc.py` snapshotted the upstream archive and produced four structural NO-GO findings (full audit in `docs/archive/notes/2026-05-25-d6-pc-recon.md`):
>
> 1. **Feature count 543** (plan-doc gate required 545 ±1). 543 IS the current constitutional Lok Sabha elected-seat count post-2019 Anglo-Indian abolition; shijithpk's 545-feature file preserves the historical 2 nominated seats. The count alone is borderline.
> 2. **39 features marked `status="Pre delimitation"`** covering SIX entire states with ZERO active coverage: J&K (6/6 pre-delim), Arunachal (2/2), Nagaland (1/1), Manipur (2/2), Assam (14/14), Jharkhand (14/14). Adopting would silently strip ~78 current-delim PCs from the map.
> 3. **No `lgd_pc_code` property** present. The two PC-id candidates (`pc_id`, `pc_no`) are not LGD codes per CLAUDE.md §3 identifier discipline.
> 4. **`pc_id` is an unstable legacy frozen key**. Telangana (17 PCs, st_code=36) uses Andhra Pradesh's pre-2014 `28xx` prefix instead of TS `36xx` (44 of 543 features fail the obvious `st_code*100 + pc_no` derivation). Joining on `pc_id` would silently mis-attribute Telangana data to Andhra Pradesh.
>
> Shipping the swap would CAUSE regressions vs the current shijithpk baseline. Decision: **keep shijithpk** (`datasets/ephemeral/india_ls_seats_545.geojson` + `pipeline.json` PC entry unchanged; ADR-0031 amendment 2026-05-25 records the rationale). The recon driver is kept in-tree as the re-evaluation trigger; re-run when upstream publishes active features for the six structural-gap states AND a stable LGD-grade PC identifier. Note: the swap was scoped pre-emptively before any frontend PC consumer existed — at the time D.6 re-opens, a citizen-facing risk audit must weigh the current shijithpk lineage against whatever ramSeraph drift remains.
...
**Last updated**: 2026-05-25 (after Phase D.5 - AC consolidation wrap-up landed via PR #288 at `_pending_`: ADR-0031 amendment 2026-05-25 records per-state authority table + Article 371A empirical correction for S17 Nagaland; `docs/reference/boundary-data-sources.md` inventory three off-by-one fixes (30/19 -> 31/20); one-shot `tools/boundaries/recon_d1_ac.py` retired per CLAUDE.md section 10; permanent `tools/boundaries/verify_ac_parity.py` kept; ledger verify-only - no parquet rewrite needed because D.0/D.2/D.3/D.4 already populated `source_id` correctly).
...
> | 0.2 | District entity backfill (639 LGD-only districts → `entities.json`; 145 → 784) | #267 | `fa6137a3` |
...
> | A.1.b | Pincode CSV emit + sources row | #250 | `9545336d` |
...
> | D.1 | AC consolidation snapshot recon (recon-only; 10 eligible-D.2 + S03/U08 Outcome 3 mixed + 19 keep-current + U09 no-assembly) | #270 | `33d97800` |
> | D.2 | AC consolidation promote for 10 eligible states (HTL → ramSeraph LGD for S04/S07/S08/S17/S18/S19/S23/S26/S28/U05 = 1015 ACs; bundles D.3 + D.4 keep-current per D.1 §4 Outcome 3 verdicts; `apply_exclude_filter` shipped as general capability but NOT used on these 10 entries per empirical correction to D.1 §4 Pre-delim claim; permanent `verify_ac_parity.py`) | #273 | `fcd481cf` |
> | D.5 | AC consolidation wrap-up (docs + ledger + ADR amend); ADR-0031 amendment 2026-05-25 records per-state authority table + S17 Article 371A empirical correction; `boundary-data-sources.md` three off-by-one fixes; one-shot `recon_d1_ac.py` retired; ledger verify-only | #288 | `_pending_` |
>
...
>
> 1. **Phase D.6** - PC polygon swap (shijithpk -> ramSeraph `LGD_Parliament_Constituencies`). Independent of D.0/D.5; can ship standalone. Recon-gated: snapshot once, confirm 2024 General Election delim (post-J&K UT + post-Telangana, 545 PCs) before swap. Per-shard size ceiling 500 KB gzipped after Douglas-Peucker simplification (HARD GATE; see file-size policy). If still > 500 KB after D-P, shard by state in the same PR.
>
...
>
> Phase 0.2 shipped via PR #267 (`fa6137a3`). Track 2 has no remaining queued items; agents picking up the sprint can devote all attention to Track 1.
>
