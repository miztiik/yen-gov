...
| X1b parquet delete (partial, safe-only) | X1a, X1a-followup elections residuals, YA-apply | - | kill-50MB+oracle-non-skip | #814 | IN-FLIGHT (14 SAFE-TO-DELETE parquets: dim_parties + dim_pcs + dim_persons + dim_acs + elections_candidacies + taxonomy/ac_crosswalk + taxonomy/persons + taxonomy/sources + 6 tiny taxonomy orphans election_events / facet-axes / indicator_topic_tags / methodology_breaks / state_tiers / topics; ~40.1 MB physical delete; the dim_acs + elections_candidacies pair added mid-PR after YA-apply #813 landed at SHA d7b9323a 2026-06-06T17:23Z and flipped semantic-catalogue.ts to CSV via inline read_csv('data/entities/electoral.csv') + fetchElectionEvents() JSON helper; manifest.json entries pruned 22 -> 13 tables + 11 deprecation rows; `backend/tests/test_dual_read_parity.py` deleted whole per parent plan 22.6; F1.1 parity oracle verified CSV-only; X1b-gated MIGRATING markers in CLAUDE.md + backend/yen_gov/AGENTS.md flipped to plain assertions enumerating 14 retired parquets; STOP-AND-SURFACE in PR body: (a) S1 RESOLVED in-PR by YA-apply merge; (b) all writer modules (`canonical/ac_crosswalk.py` + dedicated `*_seed.py` + shared `writer.py` paths + `pipeline/people_ingest.py` + source seeders + CLI entry-points) DEFERRED to B3 per parent-plan ledger row separation 22.5 - X1b = parquet data delete; B3 = writer + dead-schemas + parquet-data delete in the broader sense; the data-delete-without-writer-delete is intentional because the writers are tightly coupled to shared modules that the next pipeline run would re-emit; B3 will do the surgical writer-extraction; (c) `kill-50MB` plan-doc-gate semantic per 22.6 = "home route under 50MB" in-browser transfer measurement (NOT >=50MB physical delete - prompt's heuristic misread); verified via §13 in-browser smoke on /india/tamil-nadu + /india/karnataka + /india/maharashtra + /lab/yenask + /explore: ZERO 404 requests for any of the 14 deleted parquets across 5 routes; (d) `oracle-non-skip` gate verified: `test_oracle_non_skip_gate` requires >=35 slices to actively execute, passing on the post-F1.1 CSV corpus) |
