...
| X1b parquet delete (partial, safe-only) | X1a, X1a-followup elections residuals | - | kill-50MB+oracle-non-skip | #814 | IN-FLIGHT (12 SAFE-TO-DELETE parquets: dim_parties + dim_pcs + dim_persons + taxonomy/ac_crosswalk + taxonomy/persons + taxonomy/sources + 6 tiny taxonomy orphans election_events / facet-axes / indicator_topic_tags / methodology_breaks / state_tiers / topics; ~25 MB physical delete; manifest.json entries pruned; `backend/tests/test_dual_read_parity.py` deleted whole per parent plan 22.6; F1.1 parity oracle verified CSV-only; X1b-gated MIGRATING markers in CLAUDE.md flipped to plain assertions; STOP-AND-SURFACE in PR body: (a) `elections.dim_acs` + `elections.elections_candidacies` parquets DEFERRED until YA-apply lands (yenask semantic-catalogue.ts:77+79 still uses `registerTable` for both - orchestrator label "X1a + YA-apply flipped readers" assumed YA-apply landed first; `feat/ya-yenask-repoint-apply` is still on origin/main `0b8731b8` un-started), defer-deletion is mechanically a follow-up X1b-pt2 row after YA merges; (b) all writer modules (`canonical/ac_crosswalk.py` + dedicated `*_seed.py` + shared `writer.py` paths + `pipeline/people_ingest.py` + source seeders + CLI entry-points) DEFERRED to B3 per parent-plan ledger row separation 22.5 - X1b = parquet data delete; B3 = writer + dead-schemas + parquet-data delete in the broader sense; the data-delete-without-writer-delete is intentional because the writers are tightly coupled to shared modules that the next pipeline run would re-emit; B3 will do the surgical writer-extraction; (c) `kill-50MB` plan-doc-gate semantic per 22.6 = "home route under 50MB" in-browser transfer measurement (NOT >=50MB physical delete - prompt's heuristic misread); verified via §13 in-browser smoke on /india/tamil-nadu + /india/karnataka + /india/maharashtra + /lab/yenask + /explore: ZERO 404 requests for any of the 12 deleted parquets across 5 routes; (d) `oracle-non-skip` gate verified: `test_oracle_non_skip_gate` requires >=35 slices to actively execute, passing on the post-F1.1 CSV corpus) |
