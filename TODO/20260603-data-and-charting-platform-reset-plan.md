...
| E5 ParliamentArc seats invariant fix + countSeats seam (25.6a/6b-seam) | X1b | E1 | seats-invariant-test | #820 | IN-FLIGHT (25.6a + 25.6b-seam): discovery uncovered the actual bug differs from the plan-doc 2x-double-count hypothesis - X1a PR #809 introduced a `dim_parties` CSV view with UNQUOTED reserved-word column aliases (`full AS full_name`, `short AS short_name`) in `frontend/src/lib/duckdb.ts:417-438`. DuckDB-WASM refuses the view (`Parser Error: syntax error at or near "AS" LINE 6: full AS full_name`). Every consumer of `dim_parties` is currently broken on main (StateOverview Loading election data forever; Psephlab Failed to load actuals; Compare same), so ParliamentArc has 0 dots not 2x dots. The existing X1a Playwright smoke (`/s/tamil-nadu` expects DMK/AIADMK visible) passes because those substrings appear in the government-term description text, not the seat chart. Per orchestrator's STOP-AND-SURFACE rule the divergence is surfaced (the 2x bug is not present; the actual bug is a parser error). Per CLAUDE.md user-preference BUNDLE-RELATED-WORK-IN-SAME-PR the parser fix ships in the same PR as the seam + assertion: (1) quote `"full"` + `"short"` in the dim_parties view body; (2) NEW `frontend/src/lib/charts/count-seats.ts` with `countSeats(method, candidacies, rules)` seam (FPTP only; ranked-choice / approval / proportional throw; per orchestrator section 25.6b-seam) + `assertSeatTallyInvariant` contract gate; (3) call the assertion in state-overview.ts post-aggregation + in psephlab/rules/fptp.ts post-allocation; (4) NEW vitest pure-function tests + NEW Playwright spec pinning TN AcGenApr2021 to 234 dots / 234 legend total. |
