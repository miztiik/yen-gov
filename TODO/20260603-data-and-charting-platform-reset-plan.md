...
| B4 delete fetch+http+legacy (PARTIAL pt1; broader cascade DEFERRED to B4-pt2 sprint) | X1b | B3 | suite-green | #817 | MERGED at SHA 93dbf134 2026-06-06T18:35:04Z (5 LIVE NETWORK test files retired per plan section 21.5 DELETE LIST "any fixture exercising the fetch pipeline"; total 586 lines deleted: test_sources_eci_live.py + test_sources_wikipedia_live.py + test_pipeline_run_live.py + test_sources_iced_common_client.py + test_sources_iced_power_fetch_pipeline.py; ZERO downstream callers + ZERO regression on backend pytest 36-fail baseline IDENTICAL to origin/main + vitest 5-file-fail baseline IDENTICAL to origin/main; STOP-AND-SURFACE: broader B4 cascade per plan 22.2 line 985 (delete core/http.py + Fetcher blocks in cli.py + sources/eci fetch + legacy/ + composers/ + fetch/parquet fixtures + range-mime-probe + deploy MIME/Range step) DEFERRED to a B4-pt2 sprint (proposed 3-5 PRs): pt2.1 surgical strip IcedClient + Fetcher paths from 10 MIXED ingest modules (iced_air_quality / iced_discom / iced_fuel / iced_ghg / iced_macro / iced_metatable / iced_power / iced_socio / rbi_xlsx / datagovin_ogd{,_pincode,_pincode_polygons}) preserving B1 CSV-emit path tested by `*_csv_repoint.py`; pt2.2 delete ~10 Fetcher-using cli.py commands (ingest-energy-power-plants / ingest-fiscal-rbi{,-appendix,-appt1,-datagovin} / ingest-energy-cea / ingest-iced-state-wise / ingest-people-panel / eci-statreport{,-emit}); pt2.3 delete pipeline/run.py + pipeline/reference.py after callers gone; pt2.4 delete sources/eci/static_catalog.py + statistical_report.py + urls.py + sources/wikipedia/urls.py + sources/india_geodata/power_plants.py + sources/iced_common/client.py + sources/iced_power/fetch_pipeline.py + sources/datagovin_ogd/urls.py + sources/rbi_xlsx/urls.py; pt2.5 delete core/http.py + legacy/folded_indicator_writer.py (after B3 removes core/io.write_artifact) + composers/ + datasets/_ops/range-mime-probe.parquet + deploy MIME/Range step + remove tenacity from backend/pyproject.toml; orchestrator's prompt deliverables A-F all resolved to N/A under their "if zero-caller" / "if empty" guards: A=N/A (backend/yen_gov/fetch/ does not exist), B=N/A (core/http.py has 7 prod callers), D=N/A (legacy/ has folded_indicator_writer.py imported by core/io.py write_artifact which is B3 territory), E=N/A (energy + livestock meadow dirs still actively read by adapters; only meadow dirs that exist), F=N/A (httpx has callers in core/http.py + iced_common/client.py + 4 ingest modules + 10+ tools/ scripts); MIGRATING markers gated on B4 (CLAUDE.md line 97 meadow-tier retire / line 17 + line 19 fetch-delete; backend/yen_gov/AGENTS.md line 9; datasets/livestock/AGENTS.md line 9) STAY MIGRATING - this PR did not retire the meadow tier or delete the network-fetch infrastructure; only delivered the safe live-test cleanup) |
