...
datasets/data/
  ...
  entities/geo.csv           # entity_id, name, parent, entity_kind, aliases        (LGD admin ladder; aliases pipe-delim - round-7)
  entities/state_codes.csv   # lgd_state_id (PK), lgd_name, iso_3166_2, census_2001_code, census_2011_code, kind, slug, aliases   (LGD-spine state identity; eci_st_code DROPPED round-8; census as two dated COLUMNS + pipe aliases, NO state_aliases.csv - round-8c)
  entities/electoral.csv     # entity_id, name, aliases, entity_kind, delim_year, state, parent, eci_no, reservation, source_id   (LGD-native PK; eci_no folded from the PRI super-file's ECI-code columns; NO arithmetic ids - round-7/8c)
  entities/electoral_district_membership.csv  # electoral_id, lgd_district_id, is_primary, lgd_snapshot, source_id   (LGD-coverage 1:many AC/PC->district; renamed from electoral_lgd_xwalk - round-7)
  entities/parties.csv       # party_id, short, full, eci_codes, brand_colour, symbol_asset, wikipedia   (plural - round-7)
...
> **AMENDMENT 2026-06-05 (Round 7, user-ratified per CLAUDE.md section 0a).** Supersedes the column lines + the geometry-overlap framing below. LGD is the canonical spine; electoral entities keep an LGD-register-native `entity_id` (no arithmetic surrogate ids). `eci_no` (natural ECI ballot serial) is FOLDED as a column and `ac_crosswalk` is DELETED. An `aliases` column is added to `electoral.csv` and `geo.csv`. The AC/PC->district table is LGD-coverage-sourced (NOT geometry), renamed `entities/electoral_district_membership.csv` with `is_primary` (resolving the `{primary|partial}` vs on-disk `{wholly_inside|partial}` enum drift). A one-place `entities/state_codes.csv` is emitted from `lgd_states.json`. `party.csv` -> `parties.csv`. Clean-start rip-and-replace is mandatory. Full resolution: B2b.5 sub-plan section 0b. Wherever this plan still says `electoral_lgd_xwalk.csv` read `electoral_district_membership.csv`; `party.csv` read `parties.csv`.
>
> **ROUND-8 AMENDMENT (2026-06-05, user-ratified + Hans + web-research).** The `eci_st_code` column is DROPPED from the state spine: no authoritative, stable ECI state/UT code registry exists (web-verified - ECI publishes none, its result exports key on state NAME; the `S22`/`U08` codes came from an in-repo Wikipedia join per `lgd_states.json`'s own `$comment`, are absent from the result files, and renumber on reorganization). State identity = `lgd_state_id` + `lgd_name` + `iso_3166_2` (ISO-issued, transcribed) + `kind` + `slug`. Census codes, former names, and any ECI code become long-format **validity-scoped** rows in `entities/state_aliases.csv` (never exact-match keys - census codes renumber across 2001/2011 + reorganizations and go null for post-census entities). This does NOT contradict round-7's rejection of entity `valid_from`/`valid_to`: that was for ELECTORAL-ENTITY time-travel; ALIAS validity windows are the OWID `is_historical`/`end_year` pattern on a separate axis.
>
> **R1 RESOLVED (user 2026-06-05): FULL DROP.** `eci_st_code` is decommissioned ENTIRELY - not in the spine, not even an alias row; no `eci_st_code` alias is emitted. A mandatory ECI + census decommission sweep (B2b.5 section 0d, PR-stage 0e) finds and cleans every other in-repo consumer so nothing re-introduces it (the user: "if any other indicator is using ECI ... clean that up as well because we are going to remove it completely"). Re-add ONLY if a future ECI-numbered EXTERNAL dataset is ever named, and THEN as a validity-scoped alias via a fresh scope-change row. **R2 RESOLVED: OWID-standard (Hans + Max).** The long-format `entities/state_aliases.csv` is THE canonical alias mechanism (OWID standardization-layer shape); the pipe `aliases` column survives only as a build-time DERIVED transliteration/display-synonym projection (no validity, no registry code, never hand-authored). **DISAMBIGUATION the executing agent MUST honour:** round-8 decommissions `eci_st_code` (the S22/U08 STATE code) ONLY. The per-constituency `eci_no` ballot serial - present in the result files ("AC NO"/"PC NO"), folded on `electoral.csv`, the candidacies join key - STAYS. A sweep that strips `eci_no` is a regression. The LGD source files the operator provides, the disambiguation, and the sweep step are enumerated in B2b.5 section 0d. **ROUND-8c (2026-06-05, user-directed, on-disk-validated):** aliases are inline COLUMNS not a table - `entities/state_aliases.csv` is SUPERSEDED (census ships as two dated columns `census_2001_code`/`census_2011_code` = the "when it was, when it is not"; name synonyms ride one pipe `aliases` column on every grain; a generic `entity_aliases.csv` is deferred). The LGD Constituency Coverage Report for ALL states is on disk (`Constituency_Report_*.csv`): the structural super-file (AC + PC + AC->PC + AC->district 1:many with Fully/Partly Covered = `is_primary`), carrying LGD codes (NOT ECI codes), so the `eci_no` bind is by NAME-MATCH `(state, delim_year, ac_name)` against the TCPD result files (whose `Constituency_No` IS `eci_no`); the PRI-with-ECI export is an OPTIONAL accelerator that would make it a direct join. ROUND-8d (2026-06-05, user): ephemeral CSVs are SOURCE-only (outputs land in `datasets/reference/lgd/` + `datasets/data/`); enrichment (office-holders, affidavits, rolls, gender, pincodes) is OPTIONAL / load-on-demand / NOT required for the core delivery; the load is one-time-bulk-then-delta; the plan is EXECUTION-READY. The ephemeral corpus is OPEN: beyond the spine + result files the operator keeps adding enrichment CSVs (presidents, vice-presidents, chief ministers, candidate affidavits, rolls, gender counts, pincodes); the agent enumerates `datasets/ephemeral/*.csv` and routes by manifest (Tiers S/R/E), never a hardcoded list. See B2b.5 section 0d round-8c.

- `entities/electoral.csv` columns (round-7): `entity_id, name, aliases, entity_kind(ac|pc), delim_year, state(LGD slug), parent(AC->its PC of same delim; PC->state), eci_no(natural ECI ballot serial - folded from the deleted ac_crosswalk), reservation, source_id`. `entity_id` is **LGD-register-native** (e.g. `IN-AC-2008-tamil-nadu-3857`); NO `state_code*1000+eci_no` arithmetic.
- `entities/electoral_district_membership.csv` columns (round-7, renamed from `electoral_lgd_xwalk.csv`): `electoral_id, lgd_district_id, is_primary, lgd_snapshot, source_id`. Sourced from the LGD Constituency-Coverage HTML export freshly downloaded into `datasets/ephemeral/` (parsed to CSV), which asserts the 1:many directly - a PC gets 6-8 rows, an AC gets 1 (`is_primary=true`) plus extra rows when it straddles districts. The prior `datasets/taxonomy/lgd_ac_pc_district_map.json` is derived/corrupted and overridden. This is a membership table, never readable as "AC nests in exactly one district" - the storage form of F3.
- **Old + new delimitation coexist as distinct rows** separated by `delim_year`: `IN-AC-2008-tamil-nadu-3857` and `IN-AC-2026-tamil-nadu-<new-lgd-id>` never merge, never overwrite. New delimitation = **append new rows**, never mutate old (OWID "new entity, never rewrite"). Datapoints + the `delim=<year>` boundary family point at whichever entity_id matches the event's delim.
- The membership table names the `lgd_snapshot` vintage it was sourced against (the LGD register mutates over time); `delim_year` is the only time-travel axis on the entity (no `valid_from`/`valid_to` intervals).
...
| Row | Action |
| --- | --- |
| EL1 | **Unify AC + PC under one electoral family** in `entities/electoral.csv` keyed by `(entity_kind, delim_year, state)` with `delim_year` promoted to a first-class column (section 3). Admin-district linkage rides the separate `entities/electoral_district_membership.csv` LGD-coverage table (round-7 rename), never a `district_id` column. |
...
| EL6 | **yen-ask `constituency_result` re-point:** that concept SQL currently assumes a direct AC->district key; re-point it to resolve via `entities/electoral_district_membership.csv` (round-7 rename; also tracked under YA1). |
...
| D2 | `tools/` one-shot scripts: fold the genuinely-reusable ones into `backend/yen_gov/utils/` (O-refinement: "folded into backend utils where necessary"); delete the rest (`*_recon`, `*_probe`, `*_inspect`, executed `migrate_*`/`bump_*`) | ~50 files; net keep a handful as utils. **DONE 2026-06-08 (feat/g6-tools-audit-and-prune): KEPT=5 top-level + 5 subdir packages (boundaries/, iced_parity/, lgd/, migrate/, topojson/, total 28 surviving .py files anchored by tests or live operator-doc citations), DELETED=94 files (80 top-level .py + 1 .md + 3 boundaries/ + 2 elections/ + 2 lgd/ + 7 migrate/), FOLDED=0 (YAGNI prior held: no surviving deleted-tool helper had a real second backend caller). httpx becomes unused in production code as a side-effect; pyproject left untouched (out of G6 scope).** |
...
| `datasets/reference/` | not yet named | **Reshape (mostly absorb).** `reference/in/states`, `reference/in/pincodes` are entity reference -> fold into `datasets/data/entities/*.csv`. `reference/in/indicators-completeness.json` + `indicators-operator-state.json` are operator bookkeeping -> move under `datasets/_ops/` (not citizen-facing). The `reference/` tier itself retires once both are rehomed. **DONE 2026-06-08 (`feat/g8-constituencies-sot-to-entities` closes sub-item 3 of 3; `datasets/reference/` is now FULLY EMPTY - `git ls-files datasets/reference/` returns 0).** **PARTIAL 2026-06-08 (`feat/g8-reshape-pincode-and-reference-mechanical`)**: 5 files moved (state-iso-seed.csv -> `data/entities/state_iso_seed.csv`; pc_historical_crosswalk.csv -> `data/entities/`; pincode-directory.parquet -> `data/entities/pincode.csv` PARQUET->CSV per section 21.2, 165627 rows preserved; indicators-completeness.json + indicators-operator-state.json -> `_ops/`). Manifest deprecation row added (`reference/in/pincodes/pincode-directory.parquet` -> `data/entities/pincode.csv`, 2026-06-08). **DEFERRED**: `reference/in/states/S##/constituencies.json` x 32 (hand-authored AC curator inputs; entities.json widening blocker per T.0c-iii) + `reference/lgd/*.csv` x 6 (LGD-snapshot masters; out of G8 scope, await separate disposition). Pincode writer body (`backend/yen_gov/sources/datagovin_ogd/ingest_pincode.py`) still emits parquet via DuckDB `COPY ... (FORMAT PARQUET)` at the new CSV path - MIGRATING, the parquet->direct-CSV writer rewrite + the 9 parquet-shaped tests are queued as a G8-followup; until then operators MUST NOT re-run that ingest (the committed CSV is authoritative for the 2025 vintage). **PARTIAL 2026-06-08 (`feat/g8-finish-reference-tier-retire`, sub-items 1+2 of 3)**: closes 2 of the 3 G8 residuals. Sub-item 1 (pincode writer): the `COPY ... (FORMAT PARQUET)` + intermediate-tempdir + `_READ_CSV_COLUMN_TYPES` detour was REPLACED with a direct :mod:`csv` write (the existing `_write_pincode_csv` deterministic helper is now the sole writer body); the parameter rename `output_parquet` -> `output_csv` propagated to `IngestResult.output_csv`, the `main()` CLI, and the 9 `test_ingest_pincode.py` tests (each test now decodes via `csv.DictReader` instead of `read_parquet`); operators can re-run the ingest safely again. Sub-item 2 (LGD snapshots): 5 `reference/lgd/*.csv` moved to `data/entities/lgd/` (states + districts + subdistricts + constituencies + constituency_district_membership); `reference/lgd/parse-receipt.json` moved to `_ops/lgd-parse-receipt.json` (operator state, same as `indicators-completeness.json`'s G8 disposition); `tools/lgd/parse_lgd_export.py` retargeted (`--out` default `data/entities/lgd`, new `--receipt` flag default `_ops/lgd-parse-receipt.json`, receipt `$schema` -> `../schemas/lgd-parse-receipt.schema.json`); the 3 backend canonical seeds (`state_codes_csv.py`, `electoral_csv_from_snapshot.py`, `electoral_district_membership_csv.py`) + 2 runners (`_run_state_codes_csv.py`, `_run_electoral_from_snapshot.py`) all repointed; 6 manifest deprecation rows added (one per moved file); `docs/architecture/backend/canonical-writer.md` path string updated; the lgd-parse-receipt JSON schema description updated to note the new location. **SUB-ITEM 3 IS STOP-AND-SURFACE (per CLAUDE.md §10 + brief's "do not silently drop data" clause)**: the `reference/in/states/<S>/constituencies.json` files (count = **31**, not 32 as the brief said) are NOT just hand-authored AC curator inputs - they are the **boundary-pipeline SoT** consumed by FIVE LIVE CONSUMERS: (i) `tools/boundaries/snapshot.py::apply_ac_no_rewrite_by_name` (production path via 1 `sot_ref` in `pipeline.json` for S01, comments naming S03), (ii) `tools/boundaries/verify_ac_parity.py::load_sot` (reads `repo_root/datasets/reference/in/states/<eci>/constituencies.json` for 10 D.2-promotion states), (iii) Tier-A gate `backend/tests/test_ac_parity_per_state.py` (delegates to `verify_state` above), (iv) `backend/tests/test_boundary_snapshot_ac_no_rewrite.py` (uses path STRINGS as fixture sot_ref, writes to tmp_path - safe in isolation but the literal path string would have to be repointed too), (v) 2 docs (`docs/reference/boundary-data-sources.md`, `tools/boundaries/README.md`, `docs/reference/data-coverage-report.md`). Field audit (this PR, audit script ran once + deleted): 31 files carry `(eci_no, name, reservation)` for 4113 ACs; 5 files (S03, S11, S22, S25, U07) also carry `district_id`; `pc_id` and `electors` (named in the brief) are NOT present anywhere. Cross-check vs `datasets/data/entities/electoral.csv` (4189 AC rows at delim_year=2008): ONLY S08 Himachal Pradesh has a perfect (eci_no, name) match for 68/68 ACs; the OTHER 30 states have name-set mismatches (S01 AP electoral has 293 pre-2014 unified AP+TG ACs vs 175 post-2014 JSON; S03 Assam 3/126 match because of the 2023 redelim; S29 Telangana 109 vs 119; etc.) AND the `reservation` column is `None` for ALL 4189 rows in `electoral.csv` because the LGD-snapshot writer left it `None` in v1. The constituencies.json data is therefore NOT yet preserved in the canonical store; the brief's verdict-A (full duplicate -> `git rm`) is FALSE; verdict-B option B1 (extend `electoral.csv` schema) demands (a) multi-delim modelling, (b) `reservation` backfill, and (c) updating 2 backend seeds + 5 frontend descriptors - a Level-3+ structural change that does not belong inside a "retire reference/" PR. The brief-allowed Option C (move 31 JSONs to `datasets/_ops/boundaries-sot/<S>/constituencies.json` + repoint the 5 consumers) is mechanical but still ~200 LoC across 5 files - cleanest disposition is its own atomic PR. Until then, `datasets/reference/in/states/` remains on disk; `datasets/reference/` is NOT fully retired by this PR. `git ls-files datasets/reference/` returns 31 (was 37 pre-G8-finish). **G8 SUB-ITEM 3 DONE 2026-06-08 (`feat/g8-constituencies-sot-to-entities`)**: shipped as **Option D** (NOT Option C / `_ops/` as the prior brief offered, NOT Option B1 / multi-delim canonical fold). 31 `constituencies.json` files `git mv`'d from `datasets/reference/in/states/<S>/` to `datasets/data/entities/boundaries_sot/<S>/`. Option D was the orchestrator's resolution because the audit (prior subagent's STOP-AND-SURFACE) found 4 extra LIVE consumers beyond the original 5 (the citizen-facing `fetchConstituencies` in `frontend/src/lib/data.ts` + its unit test + the golden-path e2e mock + `test_verify_ac_parity.py::_write_sot`), and frontend live-fetching from `_ops/` violates CLAUDE.md \u00a73's "operator state; not citizen-facing" classification. Option D mirrors the G8-finish precedent that moved `reference/lgd/*.csv` to `data/entities/lgd/`. Verified consumer enumeration (re-grepped, not paraphrased): 14 live runtime/test refs across (a) `frontend/src/lib/data.ts::fetchConstituencies` URL (1) + `frontend/src/lib/data.test.ts` assertion (1) + `frontend/e2e/golden-path.spec.ts` route mock (1); (b) `tools/boundaries/snapshot.py::apply_ac_no_rewrite_by_name` docstring (1) + `tools/boundaries/verify_ac_parity.py::load_sot` (docstring + Path builder, 2) + `tools/boundaries/pipeline.json` (1 `sot_ref` literal + 3 `delimitation_warning` strings, 4); (c) `backend/tests/test_boundary_snapshot_ac_no_rewrite.py` (9 `sot_ref` literals + 4 Path-arithmetic builders, 13 distinct refs collapsed via 3 mechanical patterns) + `backend/tests/test_verify_ac_parity.py::_write_sot` (1). Plus 5 cosmetic doc cross-refs (data-coverage-report.md, data-model.md, deployment.md, boundary-data-sources.md, tools/boundaries/README.md) + 1 narrative paragraph in `datasets/_ops/README.md`. The brief's step 5 ("schema relative-path fix") was based on a wrong assumption: all 31 `constituencies.json` files declare `$schema = "https://yen-gov.github.io/schemas/constituency.schema.json"` (absolute URL resolved by basename via `frontend/src/contracts/datasets-conform.test.ts`), so the `git mv` preserved the file contents byte-identical and NO JSON body was touched. New `datasets/data/entities/boundaries_sot/README.md` (~55 lines) records the operator notes + the deferred fold-into-`electoral.csv` work for the future Hans+Max structural PR. Final state: `git ls-files datasets/reference/` returns 0 (the `reference/` tier is FULLY EMPTY); `git ls-files datasets/data/entities/boundaries_sot/` returns 32 (31 SoT files + README). Gates: frontend vitest (`data.test.ts` + `datasets-conform.test.ts`) GREEN; backend pytest net-zero regression vs the pre-PR 30-fail baseline (the 10 `test_ac_parity_per_state.py` geojson-missing failures persist as pre-existing out-of-scope; the 4 boundary tests `test_boundary_snapshot_ac_no_rewrite.py` re-pointed to the new path PASS). |
...
| `config/` | "probably superfluous" | Audit the 4 files (`eci-pins`, `elections`, `processing`, `topojson`). Fold genuine tunables into the concept CSV or a single `config.json`; delete the rest. Likely collapses to 0-1 files. **DONE 2026-06-08 (feat/g9-config-audit-and-prune): KEPT=[`config/topojson.json` + `datasets/schemas/topojson-config.schema.json` - live reader [tools/topojson/convert_layer.py](../tools/topojson/convert_layer.py)]. DELETED=[`config/eci-pins.json` + `datasets/schemas/eci_pins.schema.json` + `backend/yen_gov/sources/eci/categories.py` - all readers retired in B4-pt2.2/2.3 (#826/#827); `config/elections.json` + `datasets/schemas/elections-config.schema.json` - ZERO runtime readers (Composer architecture never materialised, `backend/yen_gov/composers/` absent); `config/processing.json` + `datasets/schemas/processing.schema.json` + `core.models.{ProcessingConfig,FetchKnobs,ResultsKnobs}` - `fetch.*` block dead with `core/http.py` retire (B4-pt2.4 #828), `results.*` had ONE consumer (`cli.eci-statreport-emit-local`) inlined as `_TOP_N_DEFAULT=5` + `_COLLAPSE_OTHERS_DEFAULT=True` module constants per CLAUDE.md `implementationDiscipline`]. FOLDED-INLINE=[`processing.results.*` -> 2 cli.py module constants]. Net = 1 file (target was 0-1). The §23.1 / §23.6 RETAINED-config-schemas guard was a non-deletion guide for the B3 glob-rm sprint, not a must-keep rule for orphan-config audit; §9 prescription "delete the rest" wins for G9.** |
...
**G5 DONE 2026-06-08 (`feat/g5-bulk-rip-25-indicators`):** the bulk G5-PR-B rip + the `net_transfers_from_centre` facet-column follow-on landed in one commit. 25 remaining wired indicators (10 economy + 8 environment + 5 fiscal + 1 demography + 1 prices) migrated to the canonical CSV path via the W1 pattern. Net result: the entire `datasets/indicators/in/` tree is empty (zero JSON shards on disk; the energy/agriculture/livestock subtrees the orchestrator brief named as "out of G5 scope" were already retired by earlier PRs - STOP-AND-SURFACE finding: `git ls-tree main datasets/indicators/in/` confirmed only the 25 wired shards existed on disk pre-rip). Facet handling: option (b) per-facet CSVs for the 11 facet-multiplexed indicators (matches existing energy convention) -> 84 total CSV files, ~11,260 observation rows. Multi-vintage collapse (NSDP / sectoral GVA / external balance): latest-vintage wins per (entity, year, facet) tuple, methodology breaks documented on descriptor (Rosling-rule per CLAUDE.md anti-pattern #14). `net_transfers_from_centre` migrated as 3-facet (Accounts / BE / RE) descriptor with `comparability: directional_only` honesty marker on the thin 93-row series. Canonical-backed count: 6/42 -> 31/42 (the 11 G5-PR-A orphans were never wired so are off the historical denominator; the 25 here were wired in topics.json and previously rendered via legacy JSON). G5 mapping (full table in commit body):

| topic | count | source-id family |
| --- | --- | --- |
| economy | 10 | 8 NITI ICED API endpoints + 4 RBI Handbook on Indian Economy tables |
| environment | 8 | 1 ICED AQI (4 sibling indicators share `src-263dcba882ba`) + 6 other NITI ICED endpoints |
| fiscal | 5 | 2 RBI State Finances + 2 Sansad Rajya Sabha QA + 1 RBI Handbook on Indian Economy |
| demography | 1 | NITI ICED state-wise deep-dive |
| prices | 1 | RBI Handbook on Indian States (CPI sub-baskets 108-111) |

Total: 22 distinct `source_id` rows added/reused in `datasets/data/entities/source.csv`. New test file: `frontend/src/lib/canonical/indicator-from-canonical.g5-bulk.test.ts` (19 tests covering allowlist invariants + descriptor mapping + loader round-trip on 5 representative indicators per topic). Catalogue-coverage drift detector stays GREEN (the `ON_DISK.size > 0` sanity assertion was softened to allow the post-G5 empty state). One ancillary `indicator-from-canonical.test.ts` test was retargeted from `demography/state_population_lakhs` (now allowlisted) to a clearly-fake path so the dispatch-fallthrough assertion still proves what it claims. §13 in-browser smoke deferred to follow-up: this PR ships local-only gates (vitest + frontend build + targeted backend pytest); §13 smoke runs as a separate inspection PR per the brief's "NO PR, NO CI wait" constraint.
...
| D1-D6 | Demolition (no strangler) | D1 DONE 2026-06-08 (G5-PR-A `feat/g5a-rip-orphan-indicator-jsons` ripped 11 silent-orphan shards; W1 `feat/w1-canonical-first-rbi-state-finances` ripped 6 more after migrating to canonical CSV; G5 `feat/g5-bulk-rip-25-indicators` ripped the remaining 25 + 5 subdirs - `datasets/indicators/in/` tree now EMPTY; canonical-backed count 31/42 with the 11 orphans off the denominator). D2 DONE 2026-06-08 (`feat/g6-tools-audit-and-prune`): tools/ pruned 87 top-level + 7 subdirs -> 7 top-level (5 KEEP + 2 .gitignored scratchpads) + 5 KEEP subdirs (boundaries/, iced_parity/, lgd/, migrate/, topojson/); KEPT=5+5, DELETED=94, FOLDED=0; backend pytest delta net-zero vs origin/main baseline. D3 partial (G7 pre-regen-parquet-snapshot deleted). D4/D5/D6 NOT STARTED. |
| section 9 | Repo hygiene (ADR retire-keep-receipts, datasets/schemas/config/notes) | **ADR rule FINALIZED 2026-06-03 (Hans)**; ADR tier RETIRED 2026-06-05 (D-DOC3); `config/` row DONE 2026-06-08 (feat/g9-config-audit-and-prune): KEPT topojson + topojson-config schema, DELETED eci-pins + elections + processing + their 3 schemas + categories.py orphan reader + 3 ProcessingConfig pydantic classes, FOLDED-INLINE processing.results.* into cli.py constants; net = 1 file in `config/`; `datasets/reference/` DONE 2026-06-08 across 3 PRs (`reference/` tier is FULLY EMPTY): (a) `feat/g8-reshape-pincode-and-reference-mechanical` (5 files moved + pincode parquet->CSV per section 21.2, 165627 rows preserved; deprecation row in manifest); (b) `feat/g8-finish-reference-tier-retire` (sub-item 1 pincode writer body fully rewritten parquet -> direct CSV emit + 9 tests re-pointed; sub-item 2 the 6 reference/lgd/* relocated - 5 CSVs to `data/entities/lgd/` + parse-receipt to `_ops/lgd-parse-receipt.json` + 6 manifest deprecation rows + 5 backend readers + tool repointed); (c) `feat/g8-constituencies-sot-to-entities` (sub-item 3: 31 `constituencies.json` moved to `datasets/data/entities/boundaries_sot/<S>/` via Option D - doctrinally clean per CLAUDE.md \u00a73 because the data has a citizen-facing consumer `fetchConstituencies` and citizen-facing data lives under `data/`; mirrors the G8-finish `lgd/*` -> `data/entities/lgd/` precedent; 14 live consumers repointed across front + backend + pipeline.json + tests; new `data/entities/boundaries_sot/README.md` operator note; multi-delim fold into `data/entities/electoral.csv` deferred as Hans+Max structural PR; `git ls-files datasets/reference/` returns 0). `notes/` row DONE 2026-06-08 (feat/g4-retire-notes-into-docs-and-todo: 17 notes dispositioned, dir removed). Rest (schemas hygiene) NOT STARTED. |
| section 7 | Provenance 4-field | NOT STARTED |
| EL1-EL6 | Elections path | NOT STARTED |
| PL1-PL3 | Psephlab repair | **DONE 2026-06-08** (`fix/pl2-pl3-dim-parties-schema-drift-plus-e6-e7`, MERGED LOCAL): PL1 (loader re-pointed at CSV per F1.3a) was already DONE. PL2 root cause = `parties.csv` 8th column `aliases` added by G1 (#831) hit a hardcoded 7-column dict in `frontend/src/lib/duckdb.ts` `CSV_AS_TABLE_SPECS["elections.dim_parties"].selectSql`; DuckDB-WASM rejected EVERY dim_parties consumer view (StateOverview, Psephlab, Compare, Constituency, parties-palette) with sniffer error "7 columns in dict but 8 in file". Structural fix per CLAUDE.md Holy Law #6 = `registerCsvAsTable` now derives the `columns={...}` clause from the canonical schema at `datasets/data/_schema/columns.json` via `csvColumnsClause` so future schema additions flow through automatically (no recurrence). PL3 = `frontend/e2e/psephlab-smoke.spec.ts` pins the 234-dot arc + 234 legend total + zero failed parties.csv requests + zero console errors on `/lab/tamil-nadu/AcGenApr2021`; screenshot at `test-results/psephlab-pl3-receipt.png`. Bundle PR also delivers E6 (alternate counting methods) + E7 (Gallagher) per user override - see sub-plan [TODO/20260608-e6-user-override-and-pl2-pl3-execution-subplan.md](20260608-e6-user-override-and-pl2-pl3-execution-subplan.md). |
| W1-W3 | Refill | W1 DONE 2026-06-08 (`feat/w1-canonical-first-rbi-state-finances` + `feat/g5-bulk-rip-25-indicators`): 6 RBI State Finances indicators migrated via W1 + the remaining 25 wired indicators (10 economy + 8 environment + 5 fiscal + 1 demography + 1 prices) migrated via G5 using the same W1 pattern (per-indicator allowlist descriptor + canonical CSV emit + indicator-from-canonical adapter). Canonical-backed count: 0/42 -> 6/42 (W1) -> 31/42 (G5). The W1 pattern is proven + repeatable; W2/W3 ingest cadence remains future work but every legacy JSON shard previously wired in topics.json now reads from the canonical CSV seam (the 11 G5-PR-A orphans were never wired, the energy/agriculture/livestock 35 indicators went canonical in prior PR arcs). §13 PASS on `/t/fiscal` + `/s/tamil-nadu` for W1; G5 §13 smoke deferred to separate inspection PR per "NO PR, NO CI wait" brief. W2 + W3 NOT STARTED. |
...
| section 20.3 | Parties / symbols / colours as `entities/parties.csv` | **DECIDED 2026-06-04 (Gregor)**: party_id sole key, eci_codes attribute not key, symbol assets keyed by symbol_id; reconciles EL3. **Filename pluralised `party.csv` -> `parties.csv` round-7 (user).** |
...
| section 21.2 | CSV everywhere, NO parquet (supersedes 20.2) | **DECIDED 2026-06-04 (Gregor+Fowler debate)**: one format; typed `read_csv(columns=...)` + write-time validator replaces 60 JSON schemas; `_ops/range-mime-probe` deleted; geometry untouched. **APPLIED 2026-06-08 (`feat/g8-finish-reference-tier-retire`)**: `ingest_pincode.py` writer body rewritten parquet -> direct CSV via `:mod:csv`; DuckDB `COPY (FORMAT PARQUET)` + intermediate-tempdir + `_READ_CSV_COLUMN_TYPES` constant + `_write_intermediate_csv` indirection all deleted; output path is now `datasets/data/entities/pincode.csv` end-to-end (closes the prior G8 MIGRATING marker). 9 `test_ingest_pincode.py` tests re-pointed parquet -> csv (csv.DictReader or DuckDB read_csv); test intent preserved. |
...
| section 21.4 | Elections backend = ingest-only; delete all fetch code | **DECIDED 2026-06-04 (user+Explore)**: reingest from local TCPD CSV; delete `sources/eci/urls.py` + portal-recon; keep pure parsers. **Residuals closure 2026-06-08** (`feat/g4-g9-residuals`): post-B4 + post-G9 cleanup of 7 G4-G9 follow-ups (cosmetic stale-notes refs in `iced_common/endpoints.py` + `boundary-benchmark.spec.ts`; deleted zero-importer parser `sources/eci/constituencywise.py`; rewrote `docs/architecture/backend/core.md` to drop `http.py`/`io.py` rows + ADR-0003 narrative + dead see-also links; rewrote `docs/how-to/run-the-pipeline.md` + `docs/how-to/force-recollect.md` + surgically patched `docs/reference/data-coverage-report.md` to surviving 10 CLIs; dropped `httpx>=0.27` runtime dep from `backend/pyproject.toml` after 0-importer grep). The 6 `datasets/reference/lgd/*` LGD-snapshot files G8 deferred are SURFACED-NOT-TOUCHED: bytes DIFFER vs `datasets/taxonomy/lgd/*-latest.csv` (different transformation tiers; `reference/lgd/` is the B2b.5.0a parsed-canonical snapshot family written by `tools/lgd/parse_lgd_export.py`; 3 have active `backend/yen_gov/canonical/seed/` consumers; not a duplicate). |
...
datasets/data/
  ...
  entities/
    ...
    electoral_district_membership.csv  # LGD-coverage 1:many AC/PC->district (round-7 rename)
    party.csv
...
Admin boundaries keyed by **LGD codes** (state/district/sub-district/village); verified `dist_lgd` on `all.topojson`, 771/784 (98.3%) join. `entities/geo.csv.entity_id` at district grain IS the LGD district id -> datapoint joins geometry directly. Electoral boundaries keyed by **ECI code + delim vintage** (NOT LGD); geometry under `datasets/boundaries/electoral/delim=<year>/state=<slug>/`. The two key spaces relate through `entities/electoral_district_membership.csv` (round-7 rename; LGD-coverage-sourced) for AC/PC->district membership, and via the folded `eci_no` column on `entities/electoral.csv` for the results bind; never a shared `parent`/`district_id`. Cross-space overlay (electoral result on admin map) MUST route through the membership table and surface `is_primary` - never overlay mismatched polygons (F3/EL2). One engine, two boundary families dispatched on `entity_kind` (F5).
...
| taxonomy (11 parquet incl. dim_*) | -> `datasets/data/{variables,concepts,topics}.csv` + `entities/{geo,state_codes,electoral,electoral_district_membership,parties,source}.csv` (round-7 names; `ac_crosswalk` deleted). Dims become CSV too. CSV. |
| reference | `pincode-directory.parquet` (~155K rows) -> CSV (few MB gzip; largest single CSV; workable). CSV. **DONE 2026-06-08 (G8)**: transcoded to `data/entities/pincode.csv` (165627 rows, 23.6 MB uncompressed); typed `read_csv(columns={'pincode': 'VARCHAR', 'statename': 'VARCHAR'}, ...)` boundary in the polygon-ingest reader. Writer body still emits parquet at the new CSV path - MIGRATING (G8-followup). |
...
1. Provenance FK mandatory (`source_id` on every datapoint + candidacy row, Holy Law #9). 2. LGD is the canonical spine - electoral entities are LGD-register-native; state identity is LGD code + LGD name + ISO 3166-2 only. The ECI STATE code `eci_st_code` is DECOMMISSIONED everywhere (round-8) - never a column, join key, or partition value; census codes are two dated COLUMNS (`census_2001_code`/`census_2011_code`) and name synonyms a single pipe `aliases` column on each entity file (round-8c supersedes the round-8 `state_aliases.csv`), never exact-match keys. The per-constituency `eci_no` ballot serial IS retained as a folded attribute on `electoral.csv` (the results bind); AC/PC <-> district meets only via `entities/electoral_district_membership.csv` (round-7), never a shared parent (F3 / 20.5). 3. One-indicator-per-concept (`concept_id` FK + check-overlap before minting; no grain prefix on ids, F6 / ADR-0044). 4. Schema-per-file, typed (explicit `read_csv(columns=...)` + write-time validator; never `read_csv_auto`). 5. Static-first, deterministic read path (no backend at runtime, `query(sql)` surface unchanged, deterministic sorted CSV, no `datetime.now` in content).
...
| D-DOC3 ADR-retire | - | all | grep-receipts-eq | #761 | MERGED (sub-plan archived at [docs/archive/plans/20260604-d-doc3-adr-retire-subplan.md](../docs/archive/plans/20260604-d-doc3-adr-retire-subplan.md); ten sub-rows D-DOC3.1=spawn #723 / D-DOC3.2=redirect-index+baseline #724 / D-DOC3.3=canonical-store-and-provenance ADRs #725 / D-DOC3.4=indicator-naming-and-catalogue ADRs #734+stamp #735 / D-DOC3.5=elections-and-electoral-hierarchy ADRs #759+stamp #760 / D-DOC3.6=url-grammar-and-place-IA ADRs #732+stamp #733 / D-DOC3.7=boundaries-and-map ADRs #726+stamp #727 / D-DOC3.8=backend-ingest-and-schema ADRs #728+stamp #729 / D-DOC3.9=yenask-and-misc ADRs #730+stamp #731 / D-DOC3.10=closure=CLAUDE.md-rewrite+decisions-dir-DELETE+sub-plan-archive #761; distilled into [docs/reference/decision-index.md](../docs/reference/decision-index.md) (44-ADR redirect map; load-bearing) + [docs/concepts/documentation-discipline.md](../docs/concepts/documentation-discipline.md) (ADR-0034 routing-contract receipts) + per-subsystem-doc `## Design rationale` + `## Rejected alternatives` sections under [docs/architecture/](../docs/architecture/) + [docs/concepts/](../docs/concepts/); 7 fully-superseded ADRs moved verbatim to [docs/archive/decisions/](../docs/archive/decisions/); originating `docs/architecture/decisions/` directory deleted in D-DOC3.10 closure 2026-06-05) |
...
| U2 breadcrumb+drawer+district-url | U1 | B*, D* | build+visual | #747 | MERGED (sub-plan archived at [docs/archive/plans/20260605-u2-breadcrumb-drawer-district-subplan.md](../docs/archive/plans/20260605-u2-breadcrumb-drawer-district-subplan.md); four sub-rows U2a=#739, U2b=#742, U2c=#745, U2d=#747; distilled into [docs/architecture/frontend/design-system.md](../docs/architecture/frontend/design-system.md)) |
| U3 icons->public | - | all | icon-build+all-routes-smoke | #736 | MERGED |
| U4 feasibleAt+switcher | D-DOC2 | B*, U3 | chart-drift-test | #748 | MERGED |
| U5 skeleton+jump+doc+chartshell-states | U1 | B*, U3 | build | #758 | MERGED (sub-plan archived at [docs/archive/plans/20260605-u5-skeleton-jump-doc-chartshell-states-subplan.md](../docs/archive/plans/20260605-u5-skeleton-jump-doc-chartshell-states-subplan.md); spawn #751 + four sub-rows U5a=#752, U5a-stamp=#754, U5b=#755, U5c=#757, U5d=#758; distilled into [docs/architecture/frontend/design-system.md](../docs/architecture/frontend/design-system.md)) |
...
| B2b reingest families (per-family) | B2a | U* | cross-format-parity | #689 + #776 | MERGED (sub-plan archived at [docs/archive/plans/20260604-b2b-reingest-subplan.md](../docs/archive/plans/20260604-b2b-reingest-subplan.md); five family rows MERGED: B2b.1 energy=#691, B2b.2 livestock=#693, B2b.3 governments=#695, B2b.4 taxonomy-datapoints=#698-#708+DROP #774+closure #775 (sub-sub-plan archived), B2b.5 elections-from-local-TCPD=#762-#772 (sub-sub-plan archived); closure=#776; deletion-safety established for X1b: every parquet under datasets/{energy,livestock,governments,taxonomy,elections}/** has either a green CSV sibling parity-gate or a zero-consumer DROP receipt (persons.parquet only)) |
| F1 CSV loaders + oracle-rewrite | B2a, B2b, U1, D-DOC0 | U* | loader-unit+parity-oracle-CSV | #777, #791 (F1.1), #778 (F1.2), #803 (F1.3a), #806 (F1.3b), #808 (F1.4) | MERGED (sub-plan archived at [docs/archive/plans/20260605-f1-csv-loaders-and-oracle-rewrite-subplan.md](../docs/archive/plans/20260605-f1-csv-loaders-and-oracle-rewrite-subplan.md); seam distilled to data-loading.md + canonical-writer.md) |
| X1a reader flip | F1 | - | dual-read-parity | #809 | MERGED (`registerCsvAsTable` seam in `frontend/src/lib/duckdb.ts` flips `dim_parties` -> `data/entities/parties.csv` + `taxonomy.sources` -> `data/entities/source.csv` across 12 caller surfaces (constituency / state-overview x2 / psephlab / national-elections / explore / yenask-execute-plan-dispatch / election-seats-trend / india-leading-parties / parties-palette / composition-bar adapter / semantic-catalogue / indicator-from-canonical); 7 view-model unit tests updated to pin the new dispatch shape; new `backend/tests/test_dual_read_parity.py` (6 tests, ~280 LOC) asserts per-constituency winner cardinality + per-family row-count + sampled `(entity_id, time, value, source_id)` parity between parquet and CSV across migrated families - file deletes whole in X1b per parent plan section 22.6; X1a-gated MIGRATING markers flipped in CLAUDE.md (Holy Law #9 + section 10 + section 12) + frontend/src/AGENTS.md + frontend/src/lib/yenask/AGENTS.md + backend/yen_gov/AGENTS.md + datasets/livestock/AGENTS.md; markers gated on B3/B4/B2b/X1b kept verbatim; new e2e smoke `frontend/e2e/x1a-reader-flip-smoke.spec.ts` (3 routes: /s/tamil-nadu + /s/tamil-nadu/ac/167 + /lab/yenask) asserts ZERO `dim_parties.parquet` + `taxonomy/sources.parquet` requests + >=1 `parties.csv` + `source.csv` requests per route + 0 console errors. Acceptable functional regressions: party recognition badge + brand_colour_confidence tier indicator + SourceListV2 rich fields (license/confidence_tier/is_issuing_authority/verification_method/citation_full/notes) all surface NULL through the seam (binding new truth per plan O3). Known X1a-LEFTOVER pending B3/X1b: `dim_party_alliances` (no CSV emit yet), `dim_acs` + `elections_candidacies` (semantic-catalogue startup), `election_results` (4 view-models still on parquet), `dim_persons` (canonical-allowlist), `boundary_layers` + `entities` + `ac_crosswalk` + `indicators` (each separate flip)) |
| X1a-followup elections residuals (flip ac_crosswalk + parties-palette to CSV; delete DuckDbHarness; STOP-AND-SURFACE 3 election_results aggregators + dim_party_alliances + districts/states pending B2b emits + candidacies party_id resolution) | X1a | YA | dual-read-parity (extended) | #811 | MERGED (2 view-model flips: `view-models/ac-crosswalk.ts` switched from `registerTable("taxonomy.ac_crosswalk")` to inline `read_csv('datasets/data/entities/ac_crosswalk.csv', columns={...})` with state_entity_id slug via `electionStatePartition(state)`; `view-models/parties-palette.ts` switched from `registerTable("elections.election_results")` + regex-extracted PARTY suffix fallback to dim_parties CSV-as-table + candidacies UNION (assembly+parliament globs) DISTINCT party_id NOT IN dim_parties, with empty fallback today per 2026-06-06 audit (0 orphans across 257 candidacies files / 620 parties.csv rows); 1 dev-only route DELETED (`frontend/src/routes/DuckDbHarness.svelte` + `frontend/e2e/duckdb-harness.spec.ts` + main.ts entry + DevChartsSandbox cross-reference + yenask.spec.ts header comment + data-loading.md "The harness" section + yenask.md / yenask/brand-and-route.md retirement markers) per O1 doctrine (no strangler figging - just the new - git is the backup); dual-read-parity extended with 3 new assertions (test_ac_crosswalk_row_count_match, test_ac_crosswalk_per_state_eci_no_overlap, test_summary_csv_per_ac_existence_tn_2016) at `backend/tests/test_dual_read_parity.py` (file deletes whole in X1b); 9/9 dual-read-parity tests pass locally (0.52s); pytest baseline 36 IDENTICAL to origin/main `a73bc55f` (zero new failures); vitest baseline 8 file-failures = 1 LESS than X1a's 9-file baseline (parties-palette.test.ts mock factory updated to include registerCsvAsTable per X1a pattern); STOP-AND-SURFACE on 3 families blocked by missing CSV emits or upstream data quality: (S1) `state-overview.ts:195` dim_party_alliances - `entities/party_alliances.csv` does not exist (B2b follow-up; alliance overlay is load-bearing, no graceful-degradation flip); (S2) `districts.ts` + `states.ts` - `entities/geo.csv` carries state + district rows but lacks `legacy_id` (Wikipedia 3-letter district code), `entity_code` (ECI form), `entity_valid_to` (currently-valid filter) - widening geo.csv or SQL-parsing pipe-delimited aliases is B2b territory; (S3) 3 election_results party-seats-won aggregators (composition-bar adapter / election-seats-trend / india-leading-parties) - candidacies.csv has 1,692 winners (~12%) with empty `party_id` across 106 files (Delhi 2008/2013/2015/2020 100% NULL = every AAP MLA; TN 2011 160/234 = AIADMK sweep; TN 2016 134/232; TN 2021 68/234 = AIADMK loss; Punjab 2022 92/117 = AAP sweep; Telangana 2018 82/109 = TRS sweep). The parquet's pre-aggregated party-totals must have been computed via a richer ingest with name-based party_id resolution; CSV-side recompute would silently drop these party labels and visibly break StateOverview's CompositionBar chart for those cohorts. The 3 view-models stay on `registerSlice`/`registerTable` until either the candidacies.csv ingest resolves the missing party_id mappings (B2b.5.* follow-up) or the parquet aggregates are rebuilt against the CSV path with the same resolution baked in. Self-stamped via #812 closure PR; X1b remains blocked on this row plus the X1a precursor for the parquet-delete sweep.) |
| X1b parquet delete (partial, safe-only) | X1a, X1a-followup elections residuals, YA-apply | - | kill-50MB+oracle-non-skip | #814 + #816 | MERGED at SHA b8108ceb 2026-06-06T17:54Z (14 SAFE-TO-DELETE parquets retired ~40.1 MB physical delete; dim_parties + dim_pcs + dim_persons + dim_acs + elections_candidacies + taxonomy/ac_crosswalk + taxonomy/persons + taxonomy/sources + 6 tiny taxonomy orphans election_events / facet-axes / indicator_topic_tags / methodology_breaks / state_tiers / topics; dim_acs + elections_candidacies added mid-PR after YA-apply #813 SHA d7b9323a 2026-06-06T17:23Z flipped semantic-catalogue.ts to CSV via inline read_csv('data/entities/electoral.csv') + fetchElectionEvents() JSON helper; manifest.json entries pruned 22 -> 13 tables + 11 deprecation rows; `backend/tests/test_dual_read_parity.py` deleted whole per parent plan 22.6; F1.1 parity oracle verified CSV-only; X1b-gated MIGRATING markers in CLAUDE.md + backend/yen_gov/AGENTS.md flipped to plain assertions enumerating 14 retired parquets; 4 post-X1b fix commits (manifest deprecation pattern + sources-v2-shape post-retire contract + experiment-definition post-retire contract); final CI gates: backend pytest 36-fail baseline IDENTICAL to origin/main 388462a4 + vitest 5-file-fail baseline IDENTICAL to origin/main 388462a4 + frontend build green + backend Playwright green + backend vitest admin green; STOP-AND-SURFACE disposition: (a) S1 RESOLVED in-PR by YA-apply landing mid-PR; (b) all writer modules DEFERRED to B3 per parent-plan ledger row separation 22.5 - X1b = parquet data delete; B3 = writer + dead-schemas + parquet-data delete in the broader sense; the data-delete-without-writer-delete is intentional because the writers are tightly coupled to shared modules that the next pipeline run would re-emit; B3 will do the surgical writer-extraction; (c) `kill-50MB` plan-doc-gate semantic per 22.6 = "home route under 50MB" in-browser transfer measurement (NOT >=50MB physical delete - prompt's heuristic misread); production build proved bundle has zero references to deleted parquets; (d) `oracle-non-skip` gate verified: `test_oracle_non_skip_gate` requires >=35 slices to actively execute, passing on the post-F1.1 CSV corpus; residual Parquet survivors retire via B3 + a later partial-X1b pass: election_results + dim_party_alliances + entities + indicators + boundary_layers) |
| B3-pt2 boundary_layers + office_holdings source refactor (X1b STOP-AND-SURFACE follow-on) | B3 | B4-pt2 | suite-green + cli-runs-clean | #823 | MERGED at SHA ca307dfb 2026-06-06T20:31:33Z (Option X strangler-fig: 2 dedicated `compile_to_parquet` bodies stripped of their `datasets/taxonomy/sources.parquet` UPSERT side-effects which X1b PR #814 retired; both seeders + their 11 caller modules narrowed to single-output emit. Modules touched: backend/yen_gov/canonical/boundary_layers_seed.py (DELETED `upsert_boundary_sources(con)` helper 47 LOC + the in-body CREATE TABLE sources + INSERT + COPY block ~70 LOC inside `compile_to_parquet`; NARROWED return type from `tuple[int, int]` -> `int`; KEPT BOUNDARY_SOURCES + BOUNDARY_SOURCE_ID_BY_NICKNAME + BOUNDARY_SOURCE_ID_BY_TRIPLE identity-anchor constants + the in-process FK gate against BOUNDARY_SOURCES) + backend/yen_gov/canonical/office_holdings_seed.py (DELETED `_source_row_from_group` helper + the in-body sources UPSERT block ~58 LOC; DROPPED `sources_parquet: Path` arg from `compile_to_parquet` signature; DELETED `new_sources` accumulator + inline source-row tuple construction in both citation loops; KEPT `_OfficeCitation` / `_CitationGroup` Pydantic validators + holdings citation FK gate) + backend/yen_gov/cli.py emit-taxonomy step 6 (dropped sources.parquet arg + updated docstring). Production caller updates (10 sites destructuring `n_layers, n_sources`): tools/boundaries/snapshot.py + tools/boundaries/simplify.py + tools/boundaries/migrate_to_hive_layout.py + tools/boundaries/lift_blocks_national.py + tools/boundaries/lift_panchayats_national.py + tools/boundaries/lift_subdistricts_national.py + tools/boundaries/lift_villages_jk_bhuvan.py + tools/boundaries/lift_villages_national.py + tools/boundaries/lift_wards_national.py + backend/yen_gov/sources/datagovin_ogd/ingest_pincode_polygons.py (docstring-only; call site already discarded return value). Test updates: backend/tests/test_boundary_layers_seed.py 25/25 PASS (was 29 - 3 upsert_boundary_sources tests + the `_sources_con` fixture - 1 `test_compile_to_parquet_preserves_other_adapter_sources` test + reshaped `test_compile_to_parquet_emits_both_outputs` -> `..._emits_boundary_layers_parquet` asserting no `taxonomy/sources.parquet` sibling on disk + dropped top-level unused `import duckdb`); backend/tests/test_office_holdings_seed.py 9/9 PASS (was 10 - 1 `test_upsert_preserves_existing_sources` test; reshaped `test_emits_offices_and_holdings_with_presidents_rule_null` + `test_compile_is_deterministic` + `test_non_cm_office_uses_official_citation_group` + `test_legacy_office_citations_and_citation_groups_coexist` + `test_official_president_vp_rows_do_not_use_tcpd_or_wikipedia` to derive expected source_ids via `derive_source_id` instead of joining against the absent sources.parquet sibling); backend/tests/test_ingest_pincode_polygons.py (DELETED `test_sources_parquet_carries_pincode_polygon_citation` + DROPPED the sources.parquet byte-stability check from `test_reingest_is_byte_identical`). Deliverable E (`python -m yen_gov emit-taxonomy --dry-run --root ..`): step 5 entities wrote 857 rows + step 6 office_holdings wrote 33 dim_offices + 364 governments_office_holdings CLEAN with NO sources.parquet emit attempt (the pre-existing step 7 indicators `entity_kinds: ["pc"]` enum-mismatch in indicators.json is IDENTICAL on origin/main bytes and out of B3-pt2 scope). CI gates: pytest (ingest pipeline, non-admin) = 32 failed / 1795 passed / 27 skipped IDENTICAL to origin/main B3 baseline 33 fails (NET -1 from removed sources-parquet test); vitest (frontend + admin) + build + Playwright (admin) + pytest (admin) + pre-flight ingest gate (ADR-0046) + indicator-add justification gate ALL SUCCESS; Playwright (citizen) IN_PROGRESS at merge time (orchestrator approved admin-merge with vitest + build green). STOP-AND-SURFACE: NONE - all probed test failures were pre-existing baseline. Two follow-ons explicitly NOT attempted in this PR: (a) full retirement of `dim_offices.parquet` + `governments_office_holdings.parquet` (the B2b.3 reingest term-shape transcoder still depends on the parquet inputs; needs its own scope), (b) fix for the pre-existing `boundaries.in.postal.state=<hyphenated-slug>` pydantic regex mismatch in ingest_pincode_polygons.py (9 tests failing identically on origin/main; same `[a-z0-9_]+` regex vs hyphenated slug bug; needs its own scope).) |
| B3 delete writers+dead-schemas+parquet-data | X1b | B4 | suite-green+allowlist-audit | #819 | MERGED at SHA 1012ca16 2026-06-06T19:43:36Z (10 dead canonical seed writer modules deleted: persons_seed + persons_fork + topics_seed + state_tiers_seed + facet_axes_seed + methodology_breaks_seed + election_events_seed + ac_crosswalk + energy_sources_seed + pipeline/people_ingest; 12 reingest transcoders deleted: ac_crosswalk + election_events + facet_axes + indicator_topic_tags + methodology_breaks + state_tiers + their _run_*.py runners; 4 seed CSV transcoders deleted: source_csv + topics_csv + their _run_*.py runners; 9 dead schemas deleted: dim-acs + dim-pcs + dim-parties + dim-persons + elections-candidacies + ac-crosswalk + persons + person-aliases + source (+ dead person_aliases.json data file); 2 dead one-shot migration tools deleted: tools/migrate_sources_v1_to_v2.py + tools/migrate/build_ac_crosswalk.py; surgical writer.py clean (1541->1272 LOC): removed _emit_sources + _emit_facet_axes + _emit_persons_taxonomy + _read_existing_source_ids + _SRC_DDL + _seed_compile_dry_run helpers + 4 dead _DIM_SPECS entries (person/ac/pc/party kept party_alliance) + _FAMILY_WIDE_TABLE_SPECS deleted + _write_dimensions narrowed to party_alliance + _validate_fks source_id check now uses envelope.source_rows only + WriteResult dropped sources_path/source_rows_written fields + _load_existing signature simplified + _taxonomy_schema_file/_dim_schema_file mappings pruned; livestock_sources_seed.py slimmed to identity-anchor constants only (parquet UPSERT helpers deleted); cli.py emit-taxonomy steps 1/2/3/4/8/9/9b/10 dropped (kept 5 entities + 6 office_holdings + 7 indicators); cli.py s1-persons-fork + ingest-people-panel commands deleted; tier_b_meadow_vintage_matches_source_id retargeted from sources.parquet to source.csv via read_csv(columns=...); tools+canonical comment sweep (16 files, 28 occurrences) rewrote stale datasets/taxonomy/sources.parquet -> datasets/data/entities/source.csv; MIGRATING markers in CLAUDE.md Holy Law #3 + preamble line 19 + backend/yen_gov/AGENTS.md banner + canonical-pivot invariant flipped from B3-gated to plain assertions with enumerations of what's deleted; STOP-AND-SURFACE: ECI elections-ingest stack (canonical/adapters/eci/* + eci_ae_panel + eci_ls + pipeline/canonical_eci_backfill + dim_acs_lgd_lift + pipeline/run + pipeline/compose + ingest-eci-* CLIs + envelope.py PartyDimRow + AcDimRow + PcDimRow + PersonDimRow + CandidacyRow models) DEFERRED to B4 per parent ledger row separation - those modules are tightly coupled to the legacy network-fetch path that B4 owns; final CI gates: backend pytest 33-fail baseline IDENTICAL to origin/main 33 (minus 3 test_people_ingest pre-existing fails since B3 deleted the test file -> origin/main 36-fail = B3 33-fail; ZERO B3 regressions) + frontend vitest 0-fail GREEN + frontend build green + admin pytest + admin vitest + admin Playwright + public Playwright all green; 5 post-PR-open fix commits chasing the deletion blast radius: B3.fix (delete test_source_row_v2 _SRC_DDL importer + drop source_rows_written assert in test_writer_dry_run), B3.fix2 (test_validate.py meadow fixtures retarget to source.csv), B3.fix3 (restore ROW_SCHEMA_BY_TABLE_ID legacy entries as inert mock-fixture shims - eventually reverted in fix5 alongside SAMPLE_MANIFEST retarget), B3.fix4 (revert schema-compatibility.json scrub + strip dead schema-on-disk tests from sources-v2-shape), B3.fix5 (re-scrub schema-compatibility + retarget duckdb.test SAMPLE_MANIFEST to elections.dim_party_alliances + manifest.test isCompatibleSchemaVersion updated)) |
| B4 delete fetch+http+legacy (FULL) | X1b | B3 | suite-green | #817 + #824 + #826 + #827 + #828 + B4-pt3 (local) | MERGED via 5 PRs across the B4-pt1 + B4-pt2 sprint (2026-06-06) + **B4-pt3 local-only ship (2026-06-07)**: **pt1 #817** SHA 93dbf134 18:35Z retired 5 LIVE NETWORK test files (586 LOC: test_sources_eci_live.py + test_sources_wikipedia_live.py + test_pipeline_run_live.py + test_sources_iced_common_client.py + test_sources_iced_power_fetch_pipeline.py; ZERO downstream callers + ZERO regression on backend pytest 36-fail baseline IDENTICAL to origin/main + vitest 5-file-fail baseline IDENTICAL to origin/main); **pt2.1 #824** SHA 7d96b4f0 20:36Z stripped IcedClient + Fetcher network paths from 10 mixed ingest modules (-2199 LOC net: iced_air_quality/ingest.py + iced_air_quality/markers_ingest.py + iced_discom/ingest.py + iced_fuel/ingest.py + iced_ghg/ingest.py + iced_macro/ingest.py + iced_metatable/ingest.py + iced_power/ingest.py + iced_socio/ingest.py + iced_state_wise/ingest.py + rbi_xlsx/ingest.py + 8 `__init__.py` updates; B1.* CSV-emit helpers preserved + all *_csv_repoint.py tests stay green); **pt2.2 #826** SHA 85189e00 20:56Z deleted 11 network CLI commands (run, reference, eci-statreport, eci-statreport-emit, ingest-energy-power-plants, ingest-fiscal-rbi, ingest-fiscal-datagovin, ingest-fiscal-rbi-appendix, ingest-energy-cea, ingest-fiscal-rbi-appt1, ingest-iced-state-wise) + 6 cli.py module-level imports + 3 backend files (pipeline/run.py + pipeline/reference.py + tests/test_no_legacy_json_emit.py); cli.py reduced from 23 to 12 commands; -1282 LOC net; bundled orchestrator's pt2.2 + pt2.3 per "merge pt2.3+pt2.4 if both are tiny" allowance (cli.py imports pipeline.* at module level - caller+callee must retire atomically); **pt2.3 #827** SHA 2f5a1240 21:12Z deleted 6 orphan source modules (sources/eci/statistical_report.py + static_catalog.py + urls.py + sources/wikipedia/urls.py + sources/iced_common/client.py + sources/iced_power/fetch_pipeline.py) + stripped sources/india_geodata/power_plants.py (-1006 LOC net); iced_common/__init__.py dropped IcedClient + ICEDFetchError exports; **pt2.4 #828** SHA 186c3877 21:22Z deleted core/http.py + test_core_http.py + datasets/_ops/range-mime-probe.parquet + removed tenacity from pyproject.toml + flipped MIGRATING markers in CLAUDE.md + backend/yen_gov/AGENTS.md (B4 fetch+http track now MIGRATING-done) + datasets/_ops/README.md (-335 LOC net) + fixed pt2.1's silent leftover (rbi_xlsx/ingest.py docstring + import cleanup). **DEFERRED to B4-pt3**: legacy/folded_indicator_writer.py + core/io.write_artifact + the 9+ surviving callers (eci-statreport-emit-local CLI + 6 cache-only ingest orchestrators cea_installed_capacity + datagovin_ogd + rbi_appendix_deficits + rbi_appendix_national + rbi_hbs_ie_centre_deficits + rbi_hbs_ie_state_sdp + canonical/adapters/eci_ae_panel) - orchestrator pt2.5 explicitly gated this on "AFTER B3 already removed core/io.write_artifact" but B3 did NOT (the 9+ surviving callers block). composers/ does not exist (zero-effect deletion target). .github/workflows/deploy-site.yml MIME/Range smoke step KEPT (uses election_results.parquet residual survivor; retires with the residual parquets via a later partial-X1b pass). httpx KEPT in pyproject (9 callers in tools/ operator recon scripts; orchestrator's DO NOT delete tools/ constraint). Sprint final disposition: 5 PRs MERGED, 1 follow-up (B4-pt3) shipped locally 2026-06-07. **B4-pt3 (local-only ship, 2026-06-07)**: 3 commits on `feat/b4-pt3-retire-write-artifact`. Phase 1+2 (commit 1395b5e8): retire write_artifact from eci-statreport-emit-local (3 legacy JSON writes deleted -> results.csv only) + eci_ae_panel.upsert_inventory_entries (direct json.dumps + write_text mirror of eci_ls._upsert_inventory pattern). Phase 3 (6 ingest orchestrators stubbed _write_indicator + _build_*_payload + _source_entries + _common_indicator_fields helpers with raise RuntimeError markers; IndicatorIngestResult.artifact_path renamed csv_path). Phase 4 (deleted core/io.py + legacy/folded_indicator_writer.py + legacy/__init__.py + the empty legacy/ namespace + test_core_io.py + 2 legacy test files test_sources_rbi_hbs_ie_state_sdp.py + test_sources_rbi_hbs_ie_centre_deficits.py; rewired core/models.py.SourceRef.sources_payload() to return list[dict[str, str]] JSON-ready; rewrote test_core_models._round_trip to in-memory Draft202012Validator; 3 dead test_ingest_<pollutant>_emits_artifact tests in test_sources_iced_air_quality_markers.py marked SKIP). Net -1676 LOC. Local gates green: 6 CSV-repoint tests (35 passes) + targeted tests (39 passed | 3 skipped) + full pytest suite (40 fail / 1787 pass / 13 skip BYTE-IDENTICAL to clean origin/main baseline; zero regressions). Doctrine markers in CLAUDE.md + backend/yen_gov/AGENTS.md + docs/architecture/backend/core.md + docs/architecture/backend/sources-rbi-hbs-ie-state-sdp.md + docs/architecture/data/schema-evolution.md flipped. |
| **X1b-pt2 / B3-pt3 energy + livestock writer + parquet retire (X1b STOP-AND-SURFACE follow-on for the 9 deferred non-elections parquets)** | X1b, B3, R2-FE-flip (`96275ab6`) | - | suite-green + no-new-regressions-vs-main | local commit `8ea74f24` (2026-06-07), pushed to `origin/main` with companion doctrine-sync `95fe12ce` | **MERGED + PUSHED** at SHA `8ea74f24` 2026-06-07 (single atomic commit, +163/-600,882 across 109 files, per the triple-subagent verdict at sub-plan [TODO/20260607-energy-livestock-csv-migration-subplan.md](20260607-energy-livestock-csv-migration-subplan.md) Phase C+D closure section). Companion to the R2 reader-flip commit `96275ab6` (2026-06-07) which wired 127 `csv_path` declarations into the FE canonical-indicator allowlist + flipped the 9 families' read paths to per-indicator CSVs under `datasets/data/datapoints/geo/`. Both commits are SHIPPED LOCAL on `origin/main` via the user's 2026-06-07 "no remote PR wait, accelerate, parallelize" directive. **Phase C — backend writer retirement**: 26 backend test files deleted (15 energy + 9 livestock + `test_cli_lift_table_filter.py` + `test_capacity_pipeline_envelope.py` + 2 reingest unit tests); 13 backend modules deleted (`backend/yen_gov/canonical/adapters/{energy,livestock}/` whole packages, 9 modules + 4 reingest modules); `FAMILY_FACT_TABLE_STEMS` registry in `backend/yen_gov/canonical/writer.py` emptied of both family entries (now `{}`); `lift-energy` + `lift-livestock` CLI commands deleted from `backend/yen_gov/cli.py`; 2 inspector tools deleted (`tools/inspect_canonical_energy.py` + `tools/inspect_c5_full_audit.py`). **Phase D — parquet + manifest + FE back-compat retirement**: 9 canonical parquets `git rm`-ed (6 energy + 3 livestock); 57 meadow JSON scratch files `git rm`-ed (47 energy + 10 livestock — backend-internal raw publisher snapshots both `_meadow/` dirs removed entirely); `datasets/livestock/AGENTS.md` `git rm`-ed (module-map doc for the now-retired family); 9 `tables[]` entries scrubbed from `datasets/manifest.json`; `test_energy` + `test_livestock` + their `lgd_eci_to_slug` / `lgd_district_to_entity` fixtures deleted from `backend/tests/test_csv_parquet_parity.py` (`test_governments` remains as the sole surviving cross-format parity gate); FE parquet back-compat branches deleted from `loadSingleFromCanonical` + `loadFacetMultiplexedFromCanonical` in `frontend/src/lib/canonical/indicator-from-canonical.ts` (the `descriptor.csv_path` branch is now the ONLY path; missing csv_path raises a clear error at the boundary); FE `R2 — parquet back-compat` + `R2 — dual-read parity` describe blocks deleted from `frontend/src/lib/canonical/indicator-from-canonical.test.ts`, replaced by a `descriptor without csv_path raises` invariant test. **Triple-subagent verdict ratified the staging**: Gregor (contract sound; back-compat branch is dead code; FE-mock parity is structurally weaker than backend cross-format oracle but the parquets it oracles-against are gone, so the test retires too), Fowler (tests-first → writers-second → parquets-third sequence; single atomic commit safe per worldview #9 — static-pages repo with no live production process; rejected enterprise-ceremony of 3 separate commits), Explore (inventory complete: 26 backend tests + 13 backend writer/reingest modules + 9 parquet files + 9 manifest entries + 2 inspector tools + 57 meadow scratch files + 1 family AGENTS.md doc all in scope; no other consumers found). **Local gates green**: backend `pytest backend/tests/` 1548 passed, 9 skipped, 29 failed, 3 errors in 173s — all 32 failures pre-existing on `main` (verified by running same test files on `main` from a stash before commit): 10 `test_ac_parity_per_state[S*]` (boundary parity), 11 `test_ingest_pincode_polygons*` (pincode shard), 5 `test_concept_resolve_c{2..6}_state_*_attribution::test_*_snapshot_*` (look for `state-installed-capacity-snapshot-mw-*` prefix that never existed in `indicators.json`), 1 `test_datasets_integrity`, 1 `test_dim_acs_lgd_lift`, 1 `test_rename_partition_keys`, 3 errors in `test_csv_parquet_parity` for OTHER families' fixtures not shipped yet. **Zero regressions caused by Phase C+D.** Frontend `vitest run frontend/src/lib/canonical/`: 185 passed, 15 skipped (no regressions). **Out of scope (deferred)**: §13 browser smoke on /s/<state>/t/energy + /s/<state>/t/agriculture (deferred per user direction; the FE-side risk is bounded because the parquet branch is now structurally absent — any miswiring fails loud at the boundary instead of silently falling through); pre-existing Playwright duplicate-version conflicts + Svelte 5 rune issues + boundary contract timeouts (separate cleanup track, started immediately after Phase C+D landed). **Doctrine sync** (Step-0 follow-through per parent plan section 22.3 #7): `CLAUDE.md` Holy Law #3 + section 3 (residual-parquet enumeration) + section 10 anti-pattern #10 (frontend parquet-reads list) MIGRATING markers narrowed in this commit to remove the 9 retired parquets from the "pending later partial-X1b pass" enumeration. After this row, the residual parquets in flight are 5 (`election_results` sharded by state, `dim_party_alliances`, `entities`, `indicators`, `boundary_layers`) — DOWN from the 14 enumerated in CLAUDE.md before. |
| **X1a-fu2 residual parquets retire (5 sub-rows A+B+C+D+E; X1a-followup elections residuals STOP-AND-SURFACE follow-on)** | X1a-followup, X1b, YA-apply | - | suite-green + no-new-regressions-vs-main | local commits `d7831aba` (B) + `6c8ac439` (A) + `42adcf33` (C) + `bfa9aef2` (D) + `9a380d71` (E), merge commit `769cb121` on `origin/main` (2026-06-07) | **MERGED LOCAL + PUSHED** at SHA `769cb121` 2026-06-07 (sub-plan [TODO/20260607-x1a-followup-2-residual-parquets-subplan.md](20260607-x1a-followup-2-residual-parquets-subplan.md) Closure section). Per user's 2026-06-07 directive ("mechanical rip and replace, break temporarily, fix by end of PR; don't wait for remote PR merge"). All 5 residual parquets enumerated in CLAUDE.md section 3 (post-X1b-pt2) retired in this batch via 4-worktree parallel-subagent ship loop. **Sub-row A** (`6c8ac439`): `taxonomy/entities.parquet` retired; readers in `view-models/districts.ts` + `view-models/states.ts` + `canonical/indicator-from-canonical.ts` flipped to `data/entities/geo.csv` + `data/entities/electoral.csv`; merge handled hand-resolve of AGENTS.md + manifest.json conflicts vs C's parallel branch. **Sub-row B** (`d7831aba`): `taxonomy/indicators.parquet` retired (zero-reader quiet retirement); cli.py emit-taxonomy step 7 pruned. **Sub-row C** (`42adcf33`): `elections/dim_party_alliances.parquet` retired; CSV authored at `data/entities/party_alliances.csv`; reader in `view-models/state-overview.ts` flipped; `writer.py::_DEPRECATIONS` lifted from 2 to 12 entries (Holy Law #5 structural fix for a manifest.json regeneration regression where 9 historical X1b deprecations were being dropped on emit-taxonomy regenerate). **Sub-row D** (`bfa9aef2`, merge `769cb121`): 36 `datasets/elections/state=*/election_results.parquet` ripped to per-state CSV at `datasets/data/datapoints/electoral/<slug>_election_results.csv` via one-shot `tools/rip_election_results_to_csv.py` (`duckdb COPY (SELECT * FROM read_parquet(...)) TO 'csv' (HEADER, DELIMITER ',')`); 9 cols / 1,794,886 rows total / ~177MB; 3 readers flipped (`composition-bar/adapter-elections-seats.ts`, `view-models/election-seats-trend.ts`, `view-models/india-leading-parties.ts`) via new shared `frontend/src/lib/canonical/election-results-csv.ts` columns-clause module; `writer.py::_emit_observations` short-circuit-guarded for `family == "elections"` (returns 0 rows); `backend/tests/test_canonical_writer_partition.py` deleted whole (8 tests, all asserted retired elections shard emit). Option (ii) JOIN-at-view-model was BYPASSED per "most structural way the data can be merged" \u2014 direct per-state CSV transcode chosen as the most mechanical rip. **Sub-row E** (`9a380d71`, transitively on `origin/main` via D's merge tree `769cb121`): `datasets/boundaries/boundary_layers.parquet` ripped to `datasets/data/entities/boundary_layer.csv` (4014 rows, 18 cols, 806KB) via `canonical/boundary_layers_seed.compile_to_csv()` (renamed from `compile_to_parquet`; UPSERT via new `_load_existing_boundary_layers` helper); 9 callers flipped; `backend/tests/test_boundary_layers_seed.py` + `backend/tests/test_ingest_pincode_polygons.py` mechanically rewritten in place to assert CSV via `csv.DictReader`; 4 frontend tests re-targeted at the new CSV. **Gates**: backend pytest 1538 pass / 30 fail / 9 skip / 3 errors (vs. pre-rip baseline of 40 fail \u2014 net 10-test improvement from mechanically rewritten boundary_layers + pincode_polygons assertions now matching CSV shape); frontend vitest 158 files / 8039 pass / 21 skip / 0 fail; svelte-check 0 errors; build green. **Worktrees + branches**: all 4 sister worktrees (`yen-gov-x1a-fu2-{A,C,D,E}`) removed + 4 `feat/x1a-fu2-*` branches deleted; only master worktree + `main` branch remain. **Doctrine sync**: CLAUDE.md section 3 \"residual parquets in flight are 5\" enumeration is fully resolved by this row \u2014 a follow-on commit narrows the MIGRATING markers accordingly. Residual canonical parquets after this batch: ZERO from the X1a-fu2 enumeration. |
| F2a CategoryBar merge (structural) | X1b | F3,F4 | golden-render | #781 + #782 + #784 + #785 + #786 | MERGED (sub-plan archived at [docs/archive/plans/20260605-f2a-categorybar-consolidation-subplan.md](../docs/archive/plans/20260605-f2a-categorybar-consolidation-subplan.md); six sub-rows F2a.1+F2a.2=#781 (CategoryBar shell + mode="ranked" + OrderedCategoryBar delete), F2a.3+F2a.4=#782 (mode="stacked" + HorizontalGroupedBar delete), F2a.5=DEFERRED-TO-SUB-SUB-PLAN (archived at [docs/archive/plans/20260605-f2a-5-diverging-mode-subsubplan.md](../docs/archive/plans/20260605-f2a-5-diverging-mode-subsubplan.md); three sub-rows F2a.5.1=#784 (mode="diverging" body lift + sandbox), F2a.5.2=#785 (StateOverview production migration + CompositionBar.svelte delete + Playwright selector flip; experiment-cookie smoke via `?yg_variant=treatment` URL override against `/s/karnataka`), F2a.5.3=#786 (composition-bar/ README rewrite as diverging-bar adapter package)), F2a.6=closure this PR; distilled into [docs/architecture/frontend/design-system.md](../docs/architecture/frontend/design-system.md) section "F2a - CategoryBar consolidation"; user explicitly relaxed the `Blocks on X1b` edge in the F-track session brief - F-track ships parallel-OK with X cutover; F2b new renderers can now build on the CategoryBar discriminated-union pattern) |
| F2b new renderers (treemap/circlepack/choropleth/matrix) | F2a | F3,F4 | render-smoke | #789 + #790 + #793 + #794 + #795 + #796 + #797 + #798 | MERGED (sub-plan archived at [docs/archive/plans/20260606-f2b-new-renderers-subplan.md](../docs/archive/plans/20260606-f2b-new-renderers-subplan.md); eight sub-rows F2b.1=#789 (spawn), F2b.2=#790 (C2 ChoroplethLegend + C3 MapTooltip + C5 SourceLine + shared color-scale.ts), F2b.3=#793 (GeoChoropleth{fill} d3-geo SVG static map; consumes F4 island-render-smoke contract), F2b.4=#794 (Matrix entity x time heatmap; shares ColorScale with GeoChoropleth per §14.5 #5), F2b.5=#795 (Treemap d3-hierarchy tiled part-to-whole; sqrt-area HONESTY; adds d3-hierarchy first-class dep), F2b.6=#796 (CirclePack modes pack + bubble; d3-hierarchy pack(); same HONESTY), F2b.7=#797 (GeoChoropleth{symbol} mode extension; geoCentroid + sqrtAreaScale glyph; F2a discriminated-union pattern repeated), F2b.8=#798 (closure: distilled into [docs/architecture/frontend/design-system.md](../docs/architecture/frontend/design-system.md) section "F2b - new renderers + map primitives"); downstream rows now unblock: E3 state silhouette on StateAcMap+TileCartogram blocks on F2b which is now MERGED) |
| F3 reference line | X1b | F2*,F4 | direction-gate | #779 | MERGED (TimeSeriesLine.reference_series + StatusGlyph; 23 vitest cases; section 13 verdicts [better, better, worse]; F-track relaxed from X1b precondition per session-end user override - F-track is data-layer-agnostic and ships ahead of X1b) |
| F4 d3-geo + topojson | X1b | F2*,F3 | island-render-smoke | #788 | MERGED (smoke test [frontend/src/contracts/topojson-island-render.test.ts](../frontend/src/contracts/topojson-island-render.test.ts) added the 21.11 frozen-requirement-a gate AND caught a real mapshaper-simplification defect: Lakshadweep district (`dist_lgd=553`) + Lakshadweep state were arriving with `geometry:null` in `districts/all.topojson` + `states/all.topojson` because mapshaper's default `-simplify 5% weighted` dropped the Lakshadweep atoll polygons during topology cleanup. Fix = add `keep-shapes` to `config/topojson.json` simplification args + regen the two affected national topojsons via `tools/topojson/convert_layer.py`. All 14 smoke tests green (4 islands x [present + area + bounds + projected-path] + 1 corpus-shape assertion); 785 district + 36 state feature counts unchanged; svelte-check 0 errors; vite build green; 4743 boundaries-conform assertions green. **User-named requirement met as-asked: Lakshadweep + A&N now actually draw from `all.topojson` end-to-end.** The pre-cutover F4 scaffold lands the smoke + the geometry fix in one PR; the GeoChoropleth{fill} primitive that consumes this geometry remains F2b's payload, not F4's. Per parent plan section 14.5 the engine choice (d3-geo for static, maplibre fenced to elections) is now scaffold-ready: d3-geo + @types/d3-geo deps added to frontend, exercised by the smoke against the real topojson, and reserved as the offline contract for F2b's renderer wiring.) |
| YA yen-ask re-point | B1, B2b, X1a | F2* | intent-eval>=90% | #813 | MERGED (PR #813 squash-merged at SHA d7b9323a; flips yenask `semantic-catalogue.ts` startup queries from parquet (`elections.dim_acs` + `elections.elections_candidacies`) to CSV. State enumeration via inline `read_csv('datasets/data/entities/electoral.csv', columns={...}) WHERE entity_kind='ac'` (typed via `csvColumnsClause`). Election-period enumeration via the existing `fetchElectionEvents()` helper reading `datasets/taxonomy/election_events.json` (**Andre deviation** from literal "UNION ALL of 211 candidacies.csv URLs" - orchestrator-approved per CLAUDE.md section 0a authority assignment; cleaner D-04 spirit since `election_events.json` IS the catalogue, no fact-scan). `CATALOGUE_QUERY_ALLOWLIST` shrinks from 5 to 3 entries (`sources`, `dim_parties`, `electoral`); `dim_acs` + `elections_candidacies` join `FORBIDDEN_TABLES` in `semantic-catalogue.no-fact-scan.test.ts` (defence-in-depth). Adds sentinel coercion at the `coerceSourceRow` boundary in `execute-plan.ts` for the 4 X1a-NULL'd source fields (`license ?? "unknown-public"`, `confidence_tier ?? "bronze"`, `is_issuing_authority ?? false`, `verification_method ?? "editorial"`) so the Zod parser's strict enums accept the post-X1a 5-field source.csv shape (Holy Law #9 + O3 doctrine binding). Tests: 6 new sentinel-coercion tests in `execute-plan.provenance.test.ts` + mock fix (added `registerCsvAsTable` to the `vi.mock` factory) which dropped this file from the vitest failure baseline (5 file-fails post-PR vs 8 X1a-followup baseline = strict improvement; 23 test fails post-PR vs 35 pre-PR). Docs: `yenask.md` gains "YA cutover (2026-06-06)" section + `frontend/src/lib/yenask/AGENTS.md` Last-Updated + table-row annotations. Andre's 5 architectural locks INTACT: `device=wasm` pin, per-turn stateless extraction, `source_strip` Zod min(1), provenance JOIN in TS, ADR-0038 two-LLM rejection still in force. Slice E impact = ZERO (CONCEPT_CATALOGUE in `catalogue-embed.ts` is hand-authored 4-entry array; embedding INPUT independent of any parquet/CSV row count). YA2 (model-runtime blockers G2 dtype/device swap + G3 retrieval wiring) explicitly parked as a separate post-foundation track; Andre owns model choice. Out-of-scope (X1a/X1a-followup STOP-AND-SURFACE held): `dim_party_alliances`, `election_results` 4 view-models on parquet. B3 + B4 + X1b unblocked further. Closure stamp PR# folded into this same row.) |
...
| E2 PartyPill + --party-neutral token (25.3) | U1, F2a | E1 | build+visual | #800 | MERGED (15 vitest cases covering 4 treatment tiers; --party-neutral + --party-neutral-text tokens added to app-tokens.css; sandbox demo with 21 pills proving all 4 paths + 3 sizes + muted recede) |
| E3 state silhouette on StateAcMap+TileCartogram (25.4) | F2b | E2 | island-render-smoke+visual | #802 | MERGED (state outline layer on StateAcMap via maplibre `outline_features` prop in MapChoropleth, stroked with `var(--party-neutral)` 1.5px; faint slate-200 @ 0.25 silhouette behind TileCartogram hex grid via d3-geo geoMercator(); shared `state-silhouette.ts` loader reuses canonical `boundaries/in/states/all.topojson` via `loadBoundaryFromPath` - no new fetch, no new geometry file, no new dep; island-render smoke covers TN+Bihar+Lakshadweep at the projection level + TN+Bihar §13 browser smoke for the live render; triple-verdict (Architect+OWID+Codebase-explorer) all converge on the shared-corpus approach; landed in passing: `party-neutral` + `party-neutral-text` Tailwind theme.extend mirrors for the E2 #800 tokens that closed a pre-existing app-tokens drift contract failure) |
| E4 two highlight modes + margin sub-filter (25.5) | E3 | - | visual+legend-drift | #805 | MERGED (ONE shared `MapHighlightLegend` drives BOTH StateAcMap + TileCartogram via 3 props `{highlight_mode, selected_party_id, min_margin}` threaded through the new pure `cellTreatment` helper - mode=margin reproduces the existing 0.35..0.95 ramp formula byte-identically; mode=party_won paints the selected party's wins at full opacity in their party colour and recedes non-matching cells to `var(--party-neutral)` (E2 #800 token) at 0.18 opacity + neutral hairline border; stepped `min_margin` slider 0/10/20/30 pp further recedes narrower wins in party_won mode; legend composes `SegmentedControl` (U4 #748) + `PartyPill` (E2 #800) + step chip-group; ElectionMap renders the legend ONCE above the geo/hex toggle and threads the state to both arms - PR-B8 filter-rail coexistence preserved by only passing fills_override when filters are non-default; TileRow widened additively with optional `winner_party_id` + `margin_pct` so TileCartogram can run cellTreatment per tile; 25 vitest cases cover the pure helpers + reducer; 5 Playwright cases cover legend-drift contract (exactly ONE instance), mode toggle, slider step, hex tile recede, party-pill tap toggle; sandbox demo added to /dev/charts-sandbox; no new design tokens, no new ADRs, no per-map bespoke control) |
| E5 ParliamentArc seats invariant fix + countSeats seam (25.6a/6b-seam) | X1b | E1 | seats-invariant-test | #820 | MERGED (PR #820 squash-merged at SHA 2bad59b2 on 2026-06-06 19:17:46Z; 4 commits + 1 closure stamp). **STOP-AND-SURFACE finding**: the actual bug differed from the plan section 25.6a `~2x double-count` hypothesis - X1a PR #809 introduced a `dim_parties` CSV view with UNQUOTED reserved-word column aliases (`full AS full_name`, `short AS short_name`) in `frontend/src/lib/duckdb.ts:417-438`. DuckDB-WASM rejected the view with `Parser Error: syntax error at or near "AS" LINE 6: full AS full_name`, breaking every consumer (StateOverview, Psephlab, Compare, Constituency); ParliamentArc therefore had 0 dots, not 2x dots. The existing X1a Playwright smoke passed coincidentally because DMK/AIADMK substrings appear in the government-term description text. Per orchestrator STOP-AND-SURFACE + CLAUDE.md BUNDLE-RELATED-WORK-IN-SAME-PR the parser fix bundled into E5 (without it the assertion can never fire on a populated tally). Second X1a oversight fixed in passing: `indicator-from-canonical.ts` called `registerCsvAsTable("taxonomy.sources")` without importing it. Ships: (1) NEW `frontend/src/lib/charts/count-seats.ts` (116 LOC) - `countSeats(method, candidacies, rules) -> SeatTally` seam with FPTP only (ranked-choice / approval / proportional throw per orchestrator anti-pattern) + `assertSeatTallyInvariant` contract gate; 15 unit tests pin the invariant. (2) `state-overview.ts` sources `total_seats` from a SEPARATE COUNT(DISTINCT entity_id) over summary.csv and calls the assertion at the view-model boundary with synthetic `short_name_key` party_id ('OTHER' for unattributed wins). (3) `psephlab/rules/fptp.ts` calls the assertion on its SeatAllocation output before ParliamentArc/SeatDonut/RacesBoard see it. (4) `duckdb.ts` quotes `"full"` + `"short"`. (5) `indicator-from-canonical.ts` adds the missing `registerCsvAsTable` import. (6) NEW `frontend/e2e/e5-parliament-arc-seats.spec.ts` Playwright spec pins TN AcGenApr2021 = 234 dots/legend, Bihar AcGenOct2020 = 243 dots/legend. (7) 6 X1a-stale vitest mock fixes (`state-overview.test.ts`, `india-leading-parties.test.ts`, `election-seats-trend.test.ts`, `composition-bar/adapter-elections-seats.test.ts`, `psephlab/canonical-loaders.test.ts`, `canonical/indicator-from-canonical.test.ts`). Gates: vitest 8124 passed (+13 from this PR; 2 remaining timeouts are filesystem-glob flakes unrelated to E5); frontend build 16.07s GREEN; backend pytest 34 fails identical to origin/main baseline (E5 is frontend-only). In-browser smoke: TN /lab 234 dots + 234 legend total; Bihar /lab 243 dots + 243 legend total; StateOverview /s/tamil-nadu AcGenApr2021 KPI=234 + party_totals sum=234. **DEFERRED**: alternate counting methods (25.6b ranked-choice / approval / proportional) -> E6 sub-plan gated on Citizen + Hans second opinion + "hypothetical recount, not official result" honesty banner. |
| E6 alternate counting methods (25.6b) | E5 | - | (sub-plan) | - | **MERGED LOCAL 2026-06-08 via user override** per CLAUDE.md section 0a ("there were other systems then FPTP planned... can you deliver that as well"). Three rules ship behind the existing `count-seats.ts` seam at `frontend/src/lib/psephlab/rules/`: (a) `sainteLague.ts` (proportional, Sainte-Lague state-wide divisor allocation, NOTA excluded), (b) `instantRunoff.ts` (ranked-choice IRV with uniform-transfer assumption, NOTA never eliminated), (c) `approval.ts` (cast-as-approval = mathematically equivalent to FPTP, shipped as HONESTY MARKER). Each rule sets `requires_banner: true` + carries a Hans-grade `caveat` + `assumptions` list per the override sub-plan's "honest-design contract". `CountingRule` interface widened additively to carry these fields. `HypotheticalRecountBanner` primitive (`frontend/src/lib/HypotheticalRecountBanner.svelte`) is Hans's "fabricated-input" structural component (role="alert", uppercase headline, ASCII-only) that mounts above ParliamentArc only when `current_rule.requires_banner`. Sandbox-fence preserved: `frontend/src/lib/charts/count-seats.ts` `countSeats()` function still throws for non-FPTP from non-Psephlab callers (state-overview etc.). §13 verified in-browser: all 4 rules render 234-dot arc + 234 legend total (E5 invariant green) + zero console errors. Branch `fix/pl2-pl3-dim-parties-schema-drift-plus-e6-e7`; sub-plan [TODO/20260608-e6-user-override-and-pl2-pl3-execution-subplan.md](20260608-e6-user-override-and-pl2-pl3-execution-subplan.md). Prior DEFERRED sub-plan [TODO/20260607-e6-alternate-counting-methods-subplan.md](20260607-e6-alternate-counting-methods-subplan.md) preserved as historical Citizen+Hans+OWID concern record - this delivery honors those concerns via the banner + per-method caveats + sandbox-fence. |
| E7 Gallagher disproportionality chart | E5 | - | (in E6 sub-plan) | - | **MERGED LOCAL 2026-06-08** in the same branch as E6. Hans Option C carve-out (the honest measurement, NOT counterfactual). `frontend/src/lib/charts/GallagherDisproportionality.svelte` renders the Least-Squares Index (G = sqrt(0.5 * SUM((vote_share - seat_share)^2))) + per-party signed pp gaps + the qualitative band ("very proportional" / "moderately proportional" / "disproportional" / "very disproportional"). Mounted on Psephlab in a new section below the Sankey; ALWAYS measures the OFFICIAL FPTP result regardless of the active rule (per Hans verdict: this is a measurement of the existing system, not of any counterfactual). 28 vitest tests; TN-2021 computed G = 10.6 (within typical FPTP 8-15 range). |
...
| CLAUDE.md Holy Law #9 + section 12 | `source_id` FK to `datasets/taxonomy/sources.parquet` | Rewritten target `datasets/data/entities/source.csv`; MIGRATING marker DROPPED in X1a (B2a emitted source.csv + X1a flipped frontend reader to it) | B2a/X1a -> FLIPPED in X1a |
...
| CLAUDE.md section 10 anti-pattern | "Frontend reads Parquet via DuckDB-WASM only" | Rewritten in X1a to enumerate the X1a-flipped surfaces vs residual parquet reads pending B3/X1b; MIGRATING marker DROPPED | F1/X1a -> FLIPPED in X1a |
| backend/yen_gov/AGENTS.md (canonical-pivot + doc link + FK) | "New writes target Hive-partitioned Parquet"; `sources.parquet` FK | Rewritten: CSV writer -> `datasets/data/**`; FK `entities/source.csv` (FK MIGRATING marker DROPPED in X1a); banner + writer markers stay until B2b/B3/X1b | B2b/B3/X1b, B2a/X1a -> FK part FLIPPED in X1a |
| frontend/src/AGENTS.md (canonical-pivot + doc link) | "SQL over Hive-partitioned Parquet ... HTTP Range" | Rewritten in X1a: banner narrowed from "MIGRATING until F1/X1a lands" to "X1a flipped dim_parties + sources + F1.3 view-models; residual parquet retires B3/X1b"; the over-broad MIGRATING markers are GONE | F1/X1a -> FLIPPED in X1a |
...
| frontend/src/lib/yenask/AGENTS.md (3 lines) | "DuckDB-WASM against canonical Parquet"; "taxonomy parquets"; "Joins `taxonomy.sources`" | Rewritten in X1a: the source-FK + dim_parties markers DROPPED; semantic-catalogue note kept for dim_acs + elections_candidacies parquet reads (retire B3) | B2b/X1a -> FK + dim_parties parts FLIPPED in X1a |
| datasets/livestock/AGENTS.md (canonical + FK) | "canonical Parquet"; FK `sources.parquet` | Banner + MIGRATING markers: CSV under `datasets/data/datapoints/`; FK MIGRATING marker DROPPED in X1a (now `entities/source.csv` only); canonical-parquet banner stays for B4 (meadow retire) | B4, B2a/X1a -> FK part FLIPPED in X1a |
...
Round 7 2026-06-05 (user-ratified, debate-style: Jony + Max + Hans + Fowler) -> section 3 electoral-model amendment + B2b.5 sub-plan section 0b. The user ratified the consensus and OVERRODE section-0a recommendation D. Binding: (1) **LGD is the canonical spine** - electoral entities keep an LGD-register-native `entity_id` (no arithmetic surrogate ids; the `state_code*1000+eci_no` scheme is the bug, deleted not carried); (2) **fold `eci_no`** (natural ECI ballot serial, 1:1 with the AC) as a column on `entities/electoral.csv` and DELETE `ac_crosswalk` - the bind that folds the column is the same bind that resolves results -> entity, so it also clears the candidacies FK blockage; (3) **add an `aliases` column** to `electoral.csv` + `geo.csv` for transliteration/old-name variants (also powers yen-ask grounding); (4) the AC/PC->district 1:many is LGD-canonical (LGD's own Constituency Coverage Report asserts an AC can span districts), re-sourced + renamed `entities/electoral_lgd_xwalk.csv` -> `entities/electoral_district_membership.csv` with `is_primary` (resolving the `{primary|partial}` vs `{wholly_inside|partial}` enum drift); (5) **one-place state-code map** `entities/state_codes.csv` parsed from the ephemeral LGD State HTML export `allStateofIndia*.htm` (LGD code + name + census; ECI `st_code` + ISO from a small re-verified committed mapping, since the LGD export omits them - existing `lgd_states.json` treated as corrupted); (6) `delim_year` is the sole time-travel axis (interval `valid_from`/`valid_to` REJECTED); predecessor/successor lineage columns DEFERRED (Fork C: add one nullable column only if a real successor query appears - the trade is OWID-grade lineage queryability vs one more column on a small dim, not worth it at v1); (7) `party.csv` -> `parties.csv` (plural; user override of the singular convention); (8) **clean-start / rip-and-replace mandatory** - delete every conflicting artifact and re-emit from the freshly-downloaded LGD HTML exports in `datasets/ephemeral/` parsed to CSV (existing `datasets/taxonomy/*.json` treated as distrusted in-repo derivations and overridden, kept as a cross-check oracle) + TCPD for results. Three bonus catches folded: enum drift (resolved via `is_primary`), live doc trap (`docs/concepts/electoral-hierarchy.md` AC-nests-in-one-district claim corrected to LGD 1:many membership), state-code one-place (`state_codes.csv`). No code written here (plan-only); execution is B2b.5.0 onward. Round-7b (2026-06-05, debate Gregor/Hans/Max/Fowler): refined the ephemeral-as-source-of-truth into a committed-parsed-CSV pipeline (raw HTML gitignored/throwaway; parser = `tools/lgd/` via lxml), a MANDATORY diff-receipt that must EVIDENCE the override before any "corrupted" label (Hans: the LGD portal data was never corrupt - only the in-repo Wikipedia-join + arithmetic-id DERIVATIONS are distrusted), a committed state-codes seed (ECI/ISO are not in any LGD export), vintage = download date 2026-06-05, sub-district parse-and-park, and staged deletion-last; full verdict + open forks (S1 envelope / S2 snapshot-location / S3 state_codes-provenance) in B2b.5 sub-plan section 0c. Round-8 (2026-06-05, user challenge + web-research + Hans): PROVED there is no authoritative, stable ECI state/UT code registry (ECI publishes none; its result exports key on state NAME; the `S22`/`U08` codes came from an in-repo Wikipedia join, are absent from the downloaded result files, and renumber on reorganization - only LGD code + ISO 3166-2 are authority-backed). DROPPED `eci_st_code` from the state spine; state identity = LGD code + LGD name + ISO 3166-2 (transcribed) + kind + slug; census / former-names / any-ECI-code demoted to a long-format validity-scoped `entities/state_aliases.csv` (the user's 'when it was, when it is not'); the reset framing 'unattributable ECI+LGD mix -> re-baseline to the dated LGD snapshot, compare-forward via the diff receipt, keep-useful-else-discard' replaces round-7b's 'distrusted derivations'. Round-8b (2026-06-05, user): R1 RESOLVED to a FULL DROP (no dormant alias) + a mandatory ECI + census decommission sweep (B2b.5 PR-stage 0e) that finds and cleans every other in-repo `eci_st_code` / census-as-join-key consumer; R2 RESOLVED to the OWID long-format `state_aliases.csv` standard (the pipe `aliases` column survives only as a derived synonym projection); the per-constituency `eci_no` ballot serial is RETAINED (it is in the result files and is the candidacies bind - only the STATE code is dropped); and B2b.5 section 0d now enumerates the LGD source files the operator provides, so this plan is self-sufficient for an execution agent to deliver the elections data. Round-8c (2026-06-05, user-directed + on-disk validation of the ephemeral exports): aliases are inline COLUMNS (census as two dated columns + one pipe `aliases` column per grain), so `entities/state_aliases.csv` is SUPERSEDED and a generic `entity_aliases.csv` deferred; the on-disk LGD Constituency Coverage Report for ALL states (`Constituency_Report_*.csv`) is the structural super-file (AC + PC + AC<->PC + AC<->district with Fully/Partly = is_primary), carrying LGD codes not ECI codes - so `eci_no` binds by NAME-MATCH against the result files and the PRI-with-ECI export is an optional accelerator; and the ephemeral corpus is SOURCE-only with a Tiers-S/R/E routing manifest where Tier S + Tier R are REQUIRED and Tier E enrichment (office-holders, affidavits, rolls, gender, pincodes) is OPTIONAL / load-on-demand / not required (round-8d: one-time-bulk-then-delta; plan execution-ready). Full verdict + scope-change ledger + LGD file list + sweep step in B2b.5 sub-plan section 0d.
