...
| YA1-YA3 | yen-ask re-point | IN-FLIGHT #813 (semantic-catalogue startup queries flip to CSV + `fetchElectionEvents()` per Andre deviation; sentinel coercer for X1a-NULL'd source fields; YA2 model-runtime blockers parked) |
...
| YA yen-ask re-point | B1, B2b, X1a | F2* | intent-eval>=90% | #813 | IN-FLIGHT (#813) - flips yenask `semantic-catalogue.ts` startup queries from parquet (`elections.dim_acs` + `elections.elections_candidacies`) to CSV. State enumeration switches to inline `read_csv('datasets/data/entities/electoral.csv', columns={...}) WHERE entity_kind='ac'`. Election-period enumeration switches to the existing `fetchElectionEvents()` helper reading `datasets/taxonomy/election_events.json` (Andre deviation from literal "UNION ALL of 211 candidacies.csv URLs" - orchestrator-approved per CLAUDE.md section 0a authority assignment; cleaner D-04 spirit since `election_events.json` IS the catalogue, no fact-scan). `CATALOGUE_QUERY_ALLOWLIST` shrinks (drops `dim_acs` + `elections_candidacies`). Also adds sentinel coercion at the `coerceSourceRow` boundary in `execute-plan.ts` for the 4 X1a-NULL'd source fields (`license ?? "unknown-public"`, `confidence_tier ?? "bronze"`, `is_issuing_authority ?? false`, `verification_method ?? "editorial"`) so the Zod parser's strict enums accept the post-X1a 5-field source.csv shape (Holy Law #9 + O3 doctrine binding). Andre's 5 architectural locks INTACT: `device=wasm` pin, per-turn stateless extraction, `source_strip` Zod min(1), provenance JOIN in TS, ADR-0038 two-LLM rejection still in force. Slice E impact = ZERO (CONCEPT_CATALOGUE in `catalogue-embed.ts` is hand-authored 4-entry array; embedding INPUT independent of any parquet/CSV row count). YA2 (model-runtime blockers) explicitly parked. |
