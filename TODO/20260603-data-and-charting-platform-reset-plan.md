...
| section 9 | Repo hygiene (ADR retire-keep-receipts, datasets/schemas/config/notes) | **ADR rule FINALIZED 2026-06-03 (Hans)**; ADR tier RETIRED 2026-06-05 (D-DOC3); `config/` row DONE 2026-06-08 (feat/g9-config-audit-and-prune): KEPT topojson + topojson-config schema, DELETED eci-pins + elections + processing + their 3 schemas + categories.py orphan reader + 3 ProcessingConfig pydantic classes, FOLDED-INLINE processing.results.* into cli.py constants; net = 1 file in `config/`; `datasets/reference/` PARTIAL 2026-06-08 across 2 PRs: (a) `feat/g8-reshape-pincode-and-reference-mechanical` (5 files moved + pincode parquet->CSV per section 21.2, 165627 rows preserved; deprecation row in manifest); (b) `feat/g8-finish-reference-tier-retire` (sub-item 1 pincode writer body fully rewritten parquet -> direct CSV emit + 9 tests re-pointed; sub-item 2 the 6 reference/lgd/* relocated - 5 CSVs to `data/entities/lgd/` + parse-receipt to `_ops/lgd-parse-receipt.json` + 6 manifest deprecation rows + 5 backend readers + tool repointed). SUB-ITEM 3 STOP-AND-SURFACE: 31 `reference/in/states/S##/constituencies.json` are the boundary-pipeline SoT for 5 active consumers (snapshot.py, verify_ac_parity.py, pipeline.json, 2 tests, 2 docs) NOT just hand-authored curator inputs as the brief said; audit shows only S08 has perfect (eci_no, name) match against electoral.csv while 30 of 31 have name-set mismatches + `reservation` is None for all 4189 rows in electoral.csv - data NOT preserved in canonical store; follow-on PR needed for either Option C path-move-and-repoint or Option B1 multi-delim schema extension. `notes/` row DONE 2026-06-08 (feat/g4-retire-notes-into-docs-and-todo: 17 notes dispositioned, dir removed). Rest (schemas hygiene) NOT STARTED. |
