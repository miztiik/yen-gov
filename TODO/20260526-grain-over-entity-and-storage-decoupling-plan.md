...
| 19 | State distribution efficiency (overall + billing + collection + td-loss) | ✅ PR-B3-batch3 #_pending_ — strip `state-` prefix: `state-distribution-efficiency-pct{,billing,collection,td-loss}` → `distribution-efficiency-pct{,billing,collection,td-loss}` (4 ids). Direct mirror of #392/#394 pattern. Entity_kinds=["state"] retained. | done (rename) |
...
| 23 | Peak electricity demand ↔ supplied | ✅ PR-B3-batch3 #_pending_ — strip `state-` prefix: `state-peak-electricity-{demand,supplied}-mw` → `peak-electricity-{demand,supplied}-mw` (2 ids). Direct mirror of #392/#394 pattern. Entity_kinds=["state"] retained. | done (rename) |
