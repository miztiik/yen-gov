...
- **STATUS**: Z3a SHIPPED PR #361 `ee5a831b` (concepts schema v1.0 + concepts.json seed + `find_overlap` helper). Z3b-doctrine SHIPPED PR #362 (CLAUDE.md §10 + guardrails.md anti-patterns for guardrails #13 + #18; Rosling-rule #19 already present from PR-Z1). Z3b-cli SHIPPED PR #363 (`check-overlap` CLI command consuming `find_overlap`; `tier_b_indicator_freshness_declared` DARK check + tests). Z3b-tail-actionA SHIPPED PR #364 (`.github/workflows/indicator-add-gate.yml` enforcing guardrail #17: PR adding >1 indicator row must carry "Hans+Max ratified" OR per-id `facet-collapse-not-applicable: <id> - <reason>` line in body). Z3b-tail REMAINING: 3 other Tier-B checks (tier_b_one_indicator_per_concept, tier_b_no_hand_typed_source_id, tier_b_indicator_has_justification) + indicator-catalogue v2.1 (concept_id FK + meta.justification + update_period_days REQUIRED) + indicators.json backfill + per-handover-doc concept-overlap-audit section.
