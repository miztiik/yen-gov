...
**Status**: **CLOSED** - all 4 PR-P* PRs + BOTH deferrals shipped 2026-06-10: [#867](https://github.com/miztiik/yen-gov/pull/867) (PR-P1) + [#868](https://github.com/miztiik/yen-gov/pull/868) (PR-P2) + [#869](https://github.com/miztiik/yen-gov/pull/869) (PR-P3) + [#871](https://github.com/miztiik/yen-gov/pull/871) (PR-P4) + [#883](https://github.com/miztiik/yen-gov/pull/883) (D1, Option A ratified) + [#881](https://github.com/miztiik/yen-gov/pull/881) (D2, REQUIRED `url_slug` + permanent redirect ledger + 5-way disjointness). **Grammar A is the live URL grammar.** Grammar B `/s/<state>/...` URLs now 404 with the NotFound recovery surface. District URLs are positional `/<state>/<district>` (no `/d/` literal marker). Indicator `url_slug` is contract-required on `datasets/taxonomy/indicators.json` v3.0 with a forever-redirect ledger via `url_slug_history[]`; the 4-way namespace-disjointness contract is extended to 5-way. The 4-phase strangler-fig + BOTH deferrals are complete; FU-1 (optional Hans + Max corpus rename of district-colliding ACs) remains deferred under "Follow-up deferrals".
...
| --- | --- | --- | --- |
| D2 | **Indicator `url_slug` REQUIRED + permanent redirect ledger** (Hans + Max + Gregor 2026-06-10 unanimous): bump `datasets/schemas/indicator-catalogue.schema.json` to v3.0 making `url_slug` REQUIRED on every row, add OPTIONAL `url_slug_history[]` as the OWID-precedent permanent forever-redirect ledger; backfill all rows in `datasets/taxonomy/indicators.json`; widen the runtime Zod schema + index in `frontend/src/lib/indicator-catalogue.ts` with current/current + current/history + history/history collision throws + an O(1) `bySlug` lookup; extend `frontend/src/contracts/url-namespace-disjointness.test.ts` from 4-way to 5-way disjointness (state, topic, AC, indicator url_slug, RESERVED); add Tier-B `tier_b_indicator_url_slug_unique` chained live in `backend/yen_gov/validate.py` with 7 new tests. Editorial url_slug sweep + route-layer 301 wiring at `/<state>/<slug>` remain follow-ups (the indicator-page route does not yet exist; the redirect ledger ships ahead of the consumer). | Hans + Max + Gregor | [x] MERGED + PUSHED ([#881](https://github.com/miztiik/yen-gov/pull/881) — merge SHA `24888504`) |
