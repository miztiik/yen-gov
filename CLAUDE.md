...
>
> Where this file still says Parquet, DDF, `sources.parquet`, the meadow tier, JSON-Schema-on-Parquet validators, or "frontend reads Parquet only", read it as MIGRATING per the chunk that deletes it (writers B3, fetch B4, reader-flip X1a / X1a-followup, Parquet-delete X1b - X1b retired the 12 zero-reader parquets it could on 2026-06-06: `dim_parties` / `dim_pcs` / `dim_persons` + `taxonomy/ac_crosswalk` / `taxonomy/persons` / `taxonomy/sources` + 6 small taxonomy orphans `election_events` / `facet-axes` / `indicator_topic_tags` / `methodology_breaks` / `state_tiers` / `topics`; residual parquets `election_results` + `dim_party_alliances` + `dim_acs` + `elections_candidacies` + `entities` + `indicators` + `boundary_layers` retire via YA-apply + B3 + a later partial-X1b pass). NO agent may reintroduce a Parquet writer, a network fetcher, the DDF grammar, or a JSON projection of canonical data. Agent memory (`/memories/`) is derived, not authoritative (section 5) and self-corrects.
...
| `datasets/`     | created    | Canonical store + schemas + reference data + upstream snapshots. Long-format CSV under `datasets/data/` per [docs/concepts/data-spine.md](docs/concepts/data-spine.md). B2b MERGED (per-family CSVs emitted across `data/datapoints/geo/` + `data/entities/` + `data/elections/{assembly,parliament}/.../candidacies.csv` + summary); X1b PARTIAL 2026-06-06 (12 zero-reader parquets retired: `elections/dim_parties` + `elections/dim_pcs` + `elections/dim_persons` + `taxonomy/ac_crosswalk` + `taxonomy/persons` + `taxonomy/sources` + 6 small taxonomy orphans); the residual parquets (MIGRATING per a later partial-X1b pass + YA-apply for `dim_acs` + `elections_candidacies`) are `election_results` (sharded by state), `dim_party_alliances`, `dim_acs`, `elections_candidacies`, `entities`, `indicators`, `boundary_layers`. Sole writer: `backend/`. See [docs/architecture/data/canonical-store.md](docs/architecture/data/canonical-store.md). |
...
- Emit JSON projections of canonical data for the citizen frontend. Frontend reads long-format CSV via DuckDB-WASM `read_csv(columns=...)` for the X1a-flipped surfaces (dim_parties via `data/entities/parties.csv`; sources via `data/entities/source.csv`; election candidacies/summary via per-(state,year) CSV; ac_crosswalk via `data/entities/ac_crosswalk.csv` per X1a-followup #811) AND the X1b-retired tables (dim_persons + dim_pcs + taxonomy/persons + 6 small taxonomy orphans). Residual parquet reads pending YA-apply / B3 / a later partial-X1b pass: `election_results` (3 aggregators), `dim_party_alliances` (no CSV emit), `dim_acs` + `elections_candidacies` (yenask semantic-catalogue, until YA-apply lands), `boundary_layers`, `entities`, `indicators`.
