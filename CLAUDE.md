...
- Stamp `$schema_version` on a citizen-facing JSON data emit file. The field is retiring per [TODO/20260612-schema-version-field-refactor-plan.md](TODO/20260612-schema-version-field-refactor-plan.md) (scoped pivot ratified 2026-06-12 by Gregor + Hans + Max + Fowler debate). Schema-shape identity lives in the `.schema.json` file's own `x-version` (already true); citizen-facing data files do not need to duplicate the stamp. Carve-out: `datasets/manifest.json` MAY (and currently MUST) stamp `$schema_version` because the deployed static bundle reads it at bootstrap via `isCompatibleSchemaVersion()` in [frontend/src/lib/duckdb.ts](frontend/src/lib/duckdb.ts) + [frontend/src/lib/canonical/manifest.ts](frontend/src/lib/canonical/manifest.ts). Manifest is bootstrap, never citizen-rendered, gated by the live deployed bundle. Until the pivot ships, every new artifact still stamps `$schema_version` per the existing template (writer-strict per [ADR-0047](docs/architecture/data/schema-evolution.md#adr-0047-schema-version-compatibility-contract)); the pivot retires the field per artifact family in reader-before-producer rollout. See [schema-evolution.md section Pending OWID-conformance pivot](docs/architecture/data/schema-evolution.md#pending-owid-conformance-pivot-stop-stamping-schema_version-onto-data-emit-files).
