...
- Create new files under `datasets/indicators/in/<topic>/<id>.json`. That path is retiring per [ADR-0041](docs/architecture/data/canonical-store.md#adr-0041-meadow-tier). New backend-internal parsed rows go to `datasets/<family>/_meadow/<source>/<vintage>/<file>.json`; citizen-facing canonical data goes to `datasets/<family>/<family>_<role>.parquet`. Enforced by Tier-B; see [docs/architecture/backend/validator.md](docs/architecture/backend/validator.md). MIGRATING (plan D1) - the entire `datasets/indicators/in/**` JSON write path retires per [TODO/20260603-data-and-charting-platform-reset-plan.md](TODO/20260603-data-and-charting-platform-reset-plan.md) §8 D1; G5-PR-A 2026-06-08 ripped 11 silent-orphan shards (8 fiscal + 3 economy; allowlist-only, never wired). 31 topics.json-wired shards remain on disk pending user signoff for G5-PR-B (would blackout fiscal/economy/environment/demography/prices citizen surface until W1/W2/W3 canonical-CSV acquisition lands).
