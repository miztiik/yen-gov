...
// StackedTrendModel + a separately-resolved StackedTrendV2Source[] from
// the view-model (which now JOINs taxonomy.sources directly) and returns
// a fully-typed StackedTrendV2Model the v2 renderer can consume.
//
...
// or may not equal the citation ledger's `url`, and `fetched_at` is
// forbidden in citizen chrome. The right place to source the v2 ledger
// row is the view-model query that already JOINs `taxonomy.sources` by
// `source_id` - adapter consumers pass that row[] directly to this
// function.
//
 ...
 * @param sourcesV2  5-col StackedTrendV2Source rows resolved by the
 *                   view-model from `taxonomy.sources`. Post the
 *                   sources-simplification PR-1 (2026-06-11) the shape
 *                   is `{source_id, producer, title, vintage, url}` -
 *                   no enums.
 *
