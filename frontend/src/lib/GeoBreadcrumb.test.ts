...
describe("computeCrumbs - StateOverview route", () => {
  it("renders India (link) > State (current) on /<slug>", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu",
...
describe("computeCrumbs - StateTopic route", () => {
  it("renders India > State (link) > Topic (current) on /<slug>/t/<topic>", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu/t/fiscal",
...
describe("computeCrumbs - District route", () => {
  it("renders India > State (link) > District (current) on /<slug>/d/<district>", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu/d/coimbatore",
  ...
  it("title-cases a multi-word district slug (north-24-parganas)", () => {
    const crumbs = computeCrumbs({
      path: "/west-bengal/d/north-24-parganas",
...
describe("computeCrumbs - Constituency route", () => {
  it("recovers AC name from `<n>-<slug>` shape on /<slug>/ac/<ac>", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu/ac/167-mylapore",
      params: {
        state: "tamil-nadu",
  ...
  it("handles the canonical nested route /<slug>/elections/<event>/ac/<n-slug>", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu/elections/AcGenMay2026/ac/167-mylapore",
      params: {
        state: "tamil-nadu",
        event: "AcGenMay2026",
        ac_slug: "167-mylapore",
  ...
  it("title-cases a multi-word AC name (uda-jayer-pira-style)", () => {
    const crumbs = computeCrumbs({
      path: "/west-bengal/ac/42-cooch-behar-uttar",
      params: {
        state: "west-bengal",
...
describe("computeCrumbs - ascend href shape (canonical URL grammar)", () => {
  ...
  it("State crumb's ascend href is /<slug> (canonical, never uppercase ECI)", () => {
    const crumbs = computeCrumbs({
      path: "/tamil-nadu/d/coimbatore",
      params: { state: "tamil-nadu", district_slug: "coimbatore" },
      stateCode: "S22",
    ...
    expect(state.href).toMatch(/^\/[a-z0-9-]+$/);
    // The negative assertions mirror the ones in url.test.ts so a future
    // change to the state-builder shape (whether url.state or link.state)
    // that regresses the grammar fails both contracts symmetrically.
