...
test.describe("extended routes", () => {
  ...
  // ------------------------------------------------------------------
  // Sources-simplification PR-1 (2026-06-11) - publisher pills footer
  // ------------------------------------------------------------------
  //
  // The StateOverview totals-card footer mounts the new SourceList
  // component from `$lib/sources`. The render contract:
  //
  //   - Plain text on a single line: `Source: <publisher> [(vintage)]
  //     [middot <publisher>] ...`.
  //   - No triangle disclosure, no legacy `data-component` attribute,
  //     attribute, no confidence-tier / verification-method chips.
  //   - The row appears at the bottom of `[data-testid="state-summary-sources"]`.

  test("state hub /tamil-nadu renders the publisher-pill footer", async ({ page }) => {
    await page.goto("/tamil-nadu");

    const footer = page.locator('[data-testid="state-summary-sources"]').first();
