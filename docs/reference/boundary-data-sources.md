...
The pipeline that consumes these sources lives in [`tools/boundaries/`](../../tools/boundaries/README.md); the file-by-file selection is encoded in [`tools/boundaries/pipeline.json`](../../tools/boundaries/pipeline.json); the on-disk ledger of what landed is [`datasets/data/entities/boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv) — the SoT for "what's currently shipping" and the right place to verify the count + producer for any layer.

## Terminology

| Term | Meaning |
| --- | --- |
| **Assembly Constituency (AC)** | The electoral district that elects **one MLA** to a state legislative assembly. Same shape ECI publishes results against. The map layer is `level=ac` in [`boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv). |
| **Parliamentary Constituency (PC)** | Same idea, one level up — the electoral district that elects **one MP** to the national lower house. Map layer is `level=pc`. |
...
Sourced from [`datasets/data/entities/boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv) (join key on `source_id` against [`datasets/data/entities/source.csv`](../../datasets/data/entities/source.csv)). As of 2026-06-11 the CSV contains 4045 rows (one per file/shard).
...
| **Census 2011 District / Sub-district / Village** | indirect -- `data/entities/geo.csv` district rows carry `census_2011_code` | no polygons | ramSeraph census-2011: `Districts_2011` + `SubDistricts_2011` (CC0-1.0 polygons), `Census_Villages` (CC0 **points**, not polygons). SHRUG variants are **CC-BY-NC-SA -- non-commercial, NOT safe for our static-site redistribution**. Adopt when first Census-2011 indicator needs polygon joins. |
...
| --- | :---: | --- |
| [`datasets/data/entities/lgd/districts-latest.csv`](../../datasets/data/entities/lgd/districts-latest.csv) | **784 districts** | Current modern district list per LGD master, refreshed via `tools/lgd/snapshot.py`. Carries `Census 2001 Code` + `Census 2011 Code` columns — i.e. the LGD CSV ALREADY cross-enriches each modern district with its Census-2011 ancestor. |
| [`datasets/boundaries/in/districts/all.geojson`](../../datasets/boundaries/in/districts/all.geojson) | 785 polygons | One row per shard in [`boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv); polygons keyed by `dist_lgd`. The +1 over the LGD CSV is a known cosmetic delta (a bifurcation-bookkeeping row). |
| [`datasets/data/entities/geo.csv`](../../datasets/data/entities/geo.csv) (where `entity_type='district' AND entity_valid_to IS NULL`) | **145 districts** | The subset hand-curated into the geo entity store so far — Assam (35), Haryana (33), Kerala (14), TN (38), West Bengal (23), Puducherry (2). Every district in this list has a confirmed display name + parent + `entity_valid_from`. |
...
**Gap: 784 − 145 = 639 modern districts have polygons + LGD codes on disk but no curated entity row.** Closing this gap requires a follow-up backfill PR (combine LGD master + Census-2011 cross-references already on the LGD CSV + geo.csv conventions).
...
| **district entity rows** (`data/entities/geo.csv`) | **784/784 live** | - | landed in PR #267 |
...
| AC | 31/31 elective states live (10 ramSeraph LGD + 20 HTL + 1 J&K shijithpk) | survey-grade consolidation onto ramSeraph `LGD_Assembly_Constituencies` for the remaining HTL states | 10 states swapped 2026-05-25 (parity-gated per-state recon). S03 Assam + U08 J&K remain on HTL/shijithpk — LGD upstream does not cleanly reflect the post-2023 Assam or post-2022 J&K re-delim layouts. |
| PC | 545 features live (shijithpk, 2024 delim) | survey-grade `LGD_Parliament_Constituencies` is the upgrade target | Upgrade candidate; evaluate when needed. |
...
1. **LGD master CSV** (`datasets/data/entities/lgd/districts-latest.csv`) supplies the `District Code` (LGD), `District Name (In English)`, parent state code, AND already carries `Census 2001 Code` + `Census 2011 Code` columns. So the LGD CSV is itself a pre-joined LGD ⇔ Census table.
...
The current inventory above gives the high-level shape. This table is the per-state AC catalogue for the 31 elective states/UTs (10 ramSeraph LGD + 20 HTL + 1 J&K shijithpk) that actually ship an Assembly Constituency layer today (see [`boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv) for the full per-file ledger).
...
1. **Keep what already works.** State outlines (datameet) and the 19 HTL AC layers we still ship today are correct for the gazetted geometry they cover. We do not wholesale-replace them just because a newer aggregator exists — every swap is a regression risk and an attribution churn for zero functional gain. Parity-gated promotions ARE allowed: the 10-state ramSeraph LGD AC swap (2026-05-25) used per-state parity-gated recon, **not** "ramSeraph exists, ship it everywhere".
...
| --- | --- | --- | --- |
| `states` (`LGD_States`, `bhuvan_states`, `SOI_States`) | LGD/BharatMaps (survey-grade), Bhuvan, Survey of India | datameet `Admin2` today | **Swap candidate.** ramSeraph `LGD_States` is survey-grade BharatMaps lineage vs DataMeet's curated GIS lineage — the upgrade is a citizen-visible quality lift at coastlines + disputed borders and aligns the state layer with the LGD code system used everywhere else. `data/entities/geo.csv` already carries all three code systems (`legacy_id`=ECI `S22`, `lgd_code`=`33`, `iso_3166_2`=`IN-TN`) per the T.0e port, so the swap is property repoint + `lgdCodeToEci` view-model helper + re-validate the 3-entry `boundary_join_name` override map. |
| `districts` (`LGD_Districts`, `bhuvan_districts`, `SOI_Districts`) | same three lineages | **none — no district polygon layer in `pipeline.json`** | **Adopt to fill the gap.** `LGD_Districts` is the natural pick (carries LGD codes — joins directly to our `district.lgd_code` field per [ADR-0015](../concepts/electoral-hierarchy.md#adr-0015-constituency-hierarchy-fields)). Staged entry in `pipeline.json#staged_inputs`; activate when the first district choropleth ships. |
| `constituencies` (`LGD_Assembly_Constituencies`, `LGD_Parliament_Constituencies`, Susewind 2014 AC/PC) | BharatMaps `mapservice.gov.in/.../AC_PC/MapServer/2` (AC) + `/MapServer/1` (PC) for LGD; Susewind 2014 academic dataset | 29 states from HTL per-state files; J&K from shijithpk; PC national from shijithpk (2024 delimitation) | **Adopt — AC + PC consolidation targets.** `LGD_Assembly_Constituencies.geojsonl.7z` (CC0-1.0, BharatMaps lineage) is the canonical AC source going forward (per-state parity-gated promotion; current status in `boundary_layer.csv`). `LGD_Parliament_Constituencies` is the **survey-grade swap target for `pc`** — replaces today's GIS-traced shijithpk file. Recon gate: verify the LGD file reflects the 2024 General Election delim (post-J&K UT + post-Telangana, 545 PCs); if pre-2024, defer. **Susewind 2014 files are CC-BY-SA-NC 4.0 (non-commercial) — NOT safe for our static-site redistribution; tracked for reference only.** |
| `subdistricts` | LGD/Bharatmaps | 36 states/UTs live | Adopted countrywide. Rollout ships one shard per state/UT under `subdistricts/state=in_<sNN>/all.geojson`. |
| `villages` | LGD/Bharatmaps | 27/36 states/UTs live | Adopted. Rollout ships 645 per-(state, district) shards under `villages/state=in_<sNN>/district=<lgd>/all.geojson`. The 9 missing states/UTs are tracked in the village gap table above. Run `python tools/boundaries/simplify.py --dry-run --skip-parquet` for the gzip budget check. |
...
The mitigation in [`pipeline.json`](../../tools/boundaries/pipeline.json) is the `delimitation_warning` field on the `S03` entry, plus the cross-check requirement in [`tools/boundaries/README.md`](../../tools/boundaries/README.md#assam-delimitation-caveat) — every `AC_NO` 1..126 in the simplified GeoJSON must match the corresponding constituency name in [`datasets/data/entities/boundaries_sot/S03/constituencies.json`](../../datasets/data/entities/boundaries_sot/S03/constituencies.json) before the boundaries PR can merge.

When Assam falls inside an election cycle yen-gov is publishing, the LGD AC release is the first candidate to evaluate for replacement (snapshot ramSeraph `LGD_Assembly_Constituencies`, per-state parity-check against `constituencies.json`, promote on match). The Assam-specific gate: confirm `AC_NO`/`lgd_ac_code` count is **126** AND every name resolves against the S03 SoT before merging.
...
### Directory CSV (citizen lookup -- primary)
...
- [`datasets/data/entities/boundary_layer.csv`](../../datasets/data/entities/boundary_layer.csv) -- current on-disk inventory of all shipped layers
- [`lgd-opendata.md`](lgd-opendata.md) -- LGD identifier registry catalogue
- [`docs/architecture/frontend/topojson-loader.md`](../architecture/frontend/topojson-loader.md) -- TopoJSON adoption contract (ADR-0047)
