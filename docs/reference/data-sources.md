...
**Layout we depend on**: each Statement / Appendix Table ships as **its own XLSX** on the listing page (87 files in the Jan 23, 2026 edition). URL slugs include a `DDMMYYYY` date stamp and an opaque hex hash, so URLs change wholesale every edition — there is no stable redirect. The per-state ECI-keyed parser (`rbi_xlsx`) was **retired 2026-06-17**; the 2 fiscal indicators it produced are frozen canonical CSV (W1 migration), and the forward path for any new RBI table is the [`rbi_handbook` adapter](../architecture/backend/sources-rbi-handbook.md). The national-aggregate Appendix parser survives in [`rbi_appendix_national`](../architecture/backend/sources-rbi-appendix-national.md).
