...
- **CPI sub-baskets that NSO does not publish per-state** — most notably *rural* housing CPI (NSO publishes urban-only because rural housing is imputed differently in the methodology). The [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) artifact's `housing_urban` facet carries the urban-only series with the rural absence flagged inline.
...
| "Which state has the highest food inflation this year?" | [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) (`food` facet) | The Food and Beverages sub-basket carries ~46% weight in CPI-Combined; food shocks (tomato, onion, pulses) drive headline CPI volatility. |
| "How are LPG / electricity tariff hikes hitting households?" | [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) (`fuel` facet) | The Fuel and Light sub-basket bundles LPG cylinder, kerosene, electricity tariff, firewood — captures the per-state pass-through from the Centre's LPG subsidy regime and SERC tariff orders. |
| "How fast are urban housing rents rising?" | [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) (`housing_urban` facet) | NSO's Housing CPI is urban-only by methodology; it captures owner-equivalent rent and actual contract rent in urban centres. |
| "What is general headline CPI by state?" | [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) (`general` facet, default) | The state-level YoY corresponding to national CPI-Combined. |
...
- **State-wise rural housing inflation does not exist in the corpus or the upstream data.** The `housing_urban` facet on [`prices/cpi_inflation_pct`](../../../datasets/indicators/in/prices/cpi_inflation_pct.json) is urban-only because NSO's CPI-Rural methodology imputes housing differently (it does not publish a rural housing sub-index at all). This is not a yen-gov omission; it is an upstream methodology choice that any rural-housing question must respect.
