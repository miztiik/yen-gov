...
> Topic spine for the [`fiscal/`](../indicators/fiscal/) indicator family.
...
- **GSDP / NSDP denominators** live under [`economy/`](../indicators/economy/) — fiscal indicators that need a denominator (debt-to-GSDP, deficit-to-GSDP) reach into the economy topic for it.
- **Inflation-indexation of nominal flows** uses [`prices/`](../indicators/prices/) — when interpreting a multi-year nominal series like state pension expenditure, deflate against `prices/national_cpi_combined_index_annual` (post-2012) or splice with `prices/national_cpi_iw_index_annual` for the longer back-history.
- **Quasi-fiscal liabilities** carried by state-owned electricity discoms (the ACS-ARR gap, AT&C losses, off-balance-sheet debt) live in [`energy/`](../indicators/energy/) — they are economically fiscal but the data lineage is power-sector regulatory, not RBI HBS.
...
| --- | --- | --- |
| "How much is the Union Government borrowing this year?" | [`fiscal/union_gross_fiscal_deficit`](../indicators/fiscal/union_gross_fiscal_deficit.md) | This is the FRBM headline number, glide-path-anchored, Budget-Day commentary's centre-piece. Pair with [`fiscal/union_primary_deficit`](../indicators/fiscal/union_primary_deficit.md) to strip out interest. |
| "How much are all states combined borrowing this year?" | [`fiscal/states_combined_gross_fiscal_deficit`](../indicators/fiscal/states_combined_gross_fiscal_deficit.md) | The Centre + states-combined "general government" deficit is the IMF / rating-agency view; this is its states component. |
| "How indebted is a particular state?" | [`fiscal/outstanding_debt_pct_gsdp`](../indicators/fiscal/outstanding_debt_pct_gsdp.md) | The 15th FC's 20%-by-FY25 anchor. A ratio, so movement reflects both numerator (stock) and denominator (nominal GSDP). |
| "How much fiscal autonomy does this state have?" | [`fiscal/state_own_tax_revenue_inr_crore`](../indicators/fiscal/state_own_tax_revenue_inr_crore.md) divided by [`fiscal/state_revenue_expenditure_inr_crore`](../indicators/fiscal/state_revenue_expenditure_inr_crore.md) | "Own-tax / total-spend" is the cleanest read on whether the state can pay for itself or depends on Centre transfers. |
| "How dependent on the Centre is this state?" | [`fiscal/state_share_central_taxes_inr_crore`](../indicators/fiscal/state_share_central_taxes_inr_crore.md) + [`fiscal/state_grants_in_aid_inr_crore`](../indicators/fiscal/state_grants_in_aid_inr_crore.md) | Devolution + grants together capture the constitutional and discretionary Centre flows. |
| "Is the Centre cutting transfers to states?" | [`fiscal/centre_transfers_to_states_net`](../indicators/fiscal/centre_transfers_to_states_net.md) | Net of loan repayments. Pair with [`fiscal/centre_transfers_to_states_gross`](../indicators/fiscal/centre_transfers_to_states_gross.md) to see whether the gap reflects gross compression or repayment churn. |
| "What is the cess-vs-divisible-pool composition?" | Read [`fiscal/centre_transfers_to_states_tax_devolution`](../indicators/fiscal/centre_transfers_to_states_tax_devolution.md) (the constitutional share) against the Centre's Receipts Budget table on cess + surcharge collections. The yen-gov corpus does not yet carry an explicit cess artifact. | The cess-share-of-gross-tax wedge is the structural states-vs-Centre fiscal grievance of the 2020s. |
| "Is OPS / pension cost a fiscal-stability risk for state X?" | [`fiscal/state_pension_expenditure_inr_crore`](../indicators/fiscal/state_pension_expenditure_inr_crore.md) divided by [`fiscal/state_own_tax_revenue_inr_crore`](../indicators/fiscal/state_own_tax_revenue_inr_crore.md) | Pension is committed revenue expenditure; its share of own-tax is the cleanest read on residual discretionary room. |
| "How tight is the Centre's primary stance after stripping out interest?" | [`fiscal/union_primary_deficit`](../indicators/fiscal/union_primary_deficit.md) | The cleanest read on the *current* government's fiscal posture, free of legacy debt-servicing inheritance. |
...
3. **Grants-in-aid** ([`centre_transfers_to_states_grants`](../indicators/fiscal/centre_transfers_to_states_grants.md)). Three families: (a) Finance Commission grants (post-devolution revenue deficit, local-body, SDRF, sector-specific); (b) Centrally Sponsored Scheme grants (PMAY, MGNREGA, PMGSY, Samagra Shiksha — Centre share); and (c) Central Sector Scheme transfers. Grants are largely *tied* — strings attached.
4. **Loans.** Gross loans from Centre to states ([`centre_transfers_to_states_gross`](../indicators/fiscal/centre_transfers_to_states_gross.md) minus the devolution + grants components) net of repayments give the [`centre_transfers_to_states_net`](../indicators/fiscal/centre_transfers_to_states_net.md) wedge. Most legacy Centre-state loans were converted to grants on 14th FC's recommendation, so the gross-vs-net gap has narrowed sharply since FY15.
...
- **15th FC's 0.5% conditional borrowing window for power-sector reform.** States like Andhra Pradesh, Punjab and Rajasthan have used it heavily; this leaks fiscal-stress signal into the [`energy/`](../indicators/energy/) topic via discom finances.
...
- **Nominal vs real (price-deflation).** Every artifact in the topic is in nominal ₹ Crore. A 10% YoY growth in pension expenditure during a year of 7% CPI inflation is roughly 3% real growth, not 10%. Charts that show fiscal levels over 10+ years should either deflate using [`prices/national_cpi_combined_index_annual`](../indicators/prices/national_cpi_combined_index_annual.md) (or the spliced WPI/CPI-IW for deeper history) or normalise by GSDP / total expenditure to control for nominal growth.
- **Comparability ladder (4-level).** Most fiscal indicators are at level 3 (`comparable_within_state_over_time`) when read as raw ₹ Crore — Maharashtra and Sikkim are not directly rankable on a debt-stock basis. They become level 1 (`comparable_across_states_and_time`) once normalised by GSDP, population, or own-revenue. The [`fiscal/outstanding_debt_pct_gsdp`](../indicators/fiscal/outstanding_debt_pct_gsdp.md) artifact is one of the few in the topic that ships at level 1 because the denominator is baked in.

## Related topic spines

- **[Economy](../indicators/economy/)** — every fiscal ratio (debt/GSDP, deficit/GSDP, own-tax/GSDP, capex/GSDP) reaches into the economy topic for its denominator. The `economy/state_gdp_current_inr_lakh_crore` and `economy/state_per_capita_nsdp_*` family are the canonical denominators.
...
- **[Demography](../indicators/demography/)** — per-capita fiscal reads (per-capita capex, per-capita revenue receipts) need population from `demography/state_population_lakhs`.
...
- [`fiscal/centre_transfers_gross`](../indicators/fiscal/centre_transfers_gross.md) — Per-state gross transfers from Centre (devolution + grants + gross loans), FY17–FY23.
- [`fiscal/centre_transfers_to_states_grants`](../indicators/fiscal/centre_transfers_to_states_grants.md) — All-India Centre-to-states grants (CSS + FC grants + State Plan legacy + statutory), FY08 onwards.
- [`fiscal/centre_transfers_to_states_gross`](../indicators/fiscal/centre_transfers_to_states_gross.md) — All-India gross transfers (Item IV = devolution + grants + gross loans), FY08 onwards.
- [`fiscal/centre_transfers_to_states_net`](../indicators/fiscal/centre_transfers_to_states_net.md) — All-India net Centre-to-states transfers (gross minus loan repayments and interest), FY08 onwards.
- [`fiscal/centre_transfers_to_states_tax_devolution`](../indicators/fiscal/centre_transfers_to_states_tax_devolution.md) — All-India divisible-pool devolution (the FC-formula stream), FY08 onwards.
- [`fiscal/net_transfers_from_centre`](../indicators/fiscal/net_transfers_from_centre.md) — Per-state net Centre-to-State transfers, FY24–FY26.
- [`fiscal/outstanding_debt_pct_gsdp`](../indicators/fiscal/outstanding_debt_pct_gsdp.md) — Per-state outstanding liabilities as % of GSDP, end-March, FY08 onwards.
- [`fiscal/state_external_debt_inr_crore`](../indicators/fiscal/state_external_debt_inr_crore.md) — Per-state external debt outstanding (one-shot Rajya Sabha snapshot, ~Aug 2023).
- [`fiscal/state_grants_in_aid_inr_crore`](../indicators/fiscal/state_grants_in_aid_inr_crore.md) — Per-state grants-in-aid received from Centre, FY17–FY23.
- [`fiscal/state_non_tax_revenue_inr_crore`](../indicators/fiscal/state_non_tax_revenue_inr_crore.md) — Per-state non-tax revenue (interest, dividends, royalties, user charges), FY17–FY23.
- [`fiscal/state_own_tax_revenue_inr_crore`](../indicators/fiscal/state_own_tax_revenue_inr_crore.md) — Per-state own-tax revenue (SGST, excise, stamp duty, MV tax), FY17–FY23.
- [`fiscal/state_pension_expenditure_inr_crore`](../indicators/fiscal/state_pension_expenditure_inr_crore.md) — Per-state pension expenditure (revenue account), FY05–FY25.
- [`fiscal/state_revenue_expenditure_inr_crore`](../indicators/fiscal/state_revenue_expenditure_inr_crore.md) — Per-state total revenue expenditure (salaries, pensions, interest, subsidies, scheme outlays), FY17–FY23.
- [`fiscal/state_share_central_taxes_inr_crore`](../indicators/fiscal/state_share_central_taxes_inr_crore.md) — Per-state share in divisible-pool central taxes, FY17–FY23.
- [`fiscal/states_combined_gross_fiscal_deficit`](../indicators/fiscal/states_combined_gross_fiscal_deficit.md) — All-states-combined gross fiscal deficit, FY08 onwards.
- [`fiscal/states_combined_primary_deficit`](../indicators/fiscal/states_combined_primary_deficit.md) — All-states-combined primary deficit (GFD minus interest), FY08 onwards.
- [`fiscal/states_combined_primary_revenue_deficit`](../indicators/fiscal/states_combined_primary_revenue_deficit.md) — Strictest fiscal-discipline indicator, all-states, FY08 onwards.
- [`fiscal/states_combined_revenue_deficit`](../indicators/fiscal/states_combined_revenue_deficit.md) — All-states-combined revenue deficit, FY08 onwards.
- [`fiscal/union_gross_fiscal_deficit`](../indicators/fiscal/union_gross_fiscal_deficit.md) — Centre's gross fiscal deficit, FY87 onwards. The headline FRBM number.
- [`fiscal/union_primary_deficit`](../indicators/fiscal/union_primary_deficit.md) — Centre's primary deficit, FY87 onwards.
- [`fiscal/union_primary_revenue_deficit`](../indicators/fiscal/union_primary_revenue_deficit.md) — Centre's primary revenue deficit, FY87 onwards.
- [`fiscal/union_revenue_deficit`](../indicators/fiscal/union_revenue_deficit.md) — Centre's revenue deficit, FY87 onwards.
