# `energy/installed_capacity_nuclear_mw`

<!-- AUTO-GENERATED by `python -m yen_gov indicator-pages`. Do not hand-edit. -->

**Title**: Installed nuclear capacity  
**One-line**: Nuclear generation capacity in megawatts, per state.  
**Source artifact last modified**: 2026-05-16 (auto-generated)  
**Source artifact**: [`datasets/indicators/in/energy/installed_capacity_nuclear_mw.json`](../../../../datasets/indicators/in/energy/installed_capacity_nuclear_mw.json)

## Definition

Nuclear generation capacity in megawatts, per state. Eight operating sites: Tarapur (Maharashtra), Kaiga (Karnataka), Madras / Kalpakkam (Tamil Nadu), Kudankulam (Tamil Nadu), RAPS (Rajasthan), NAPS (Uttar Pradesh), KAPS (Gujarat), Kakrapar (Gujarat). All capacity is centrally allocated by NPCIL across the regional grid; CEA reports the SHARE that flows back to each state, which is why the values look small relative to nameplate at each plant.

## Signature

| Field | Value |
| --- | --- |
| Entity kind | `state` |
| Time grain | `month` |
| Value kind | `raw` |
| Unit | `MW` |
| Direction | `neutral` |
| Comparability | `comparable_with_normalisation` |
| Attribution geography | `where_produced` |
| Implementing authority | `joint` |
| Scale hint | `linear` |

## Coverage

- **Temporal**: 2026-03 (1 period)
- **Spatial**: 35 states/UTs (all CEA-reported per-state entities) (35 entities)
- **Rows on disk**: 35

## Methodology vintage

CEA Monthly Executive Summary, IC sheet, snapshot 2026-03; cached file mtime 2026-05-13T21:54:16Z

## Notes

About 1.6% of all-India installed capacity. The state-level split here is the central-sector ALLOCATION, not the physical site of the plant — most states have a small nuclear allocation even without a reactor in their boundary. Source: CEA monthly Executive Summary, 'IC' sheet, per-state Sub-Total row (sum of State + Private + Central ownership tiers, including allocated shares from joint and central-sector utilities). Snapshot is point-in-time as of the last day of the report month — NOT a year-average. Capacity is **nameplate** MW, not generation; a state with high coal capacity isn't necessarily a high coal-generation state if those plants run at low PLF. Two CEA-reported entities are intentionally dropped: 'NLC' (a central PSU on the Tamil Nadu list) and 'DVC' (a central corporation on the West Bengal list) — their capacity is not state-attributable. 'Central - Unallocated' shares are also dropped for the same reason. CEA bundles 'Jammu & Kashmir and Ladakh' into a single row; the combined capacity is attributed to U08 (J&K UT) — the alternative would require a fabrication split. The Andaman & Nicobar (U01) and Lakshadweep (U04) entries in the Islands region are tiny but real.

## Sources

- <https://cea.nic.in/installed-capacity-report/?lang=en> — cea.nic.in (fetched 2026-05-13T21:54:16Z)

## License

**Government of India open publication** ([link](https://data.gov.in/government-open-data-license-india)) · redistributable

## Citation

> cea.nic.in, *Installed nuclear capacity*. Re-published by yen-gov as `energy/installed_capacity_nuclear_mw`, schema v3.0. Retrieved 2026-05-13.

## Schema

`indicator.schema.json` v3.0 · artifact: [`datasets/indicators/in/energy/installed_capacity_nuclear_mw.json`](../../../../datasets/indicators/in/energy/installed_capacity_nuclear_mw.json)
