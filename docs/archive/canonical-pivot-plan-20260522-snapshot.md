...
- [`TODO/20260521-phase-2-preflight-audit-gregor.md`](20260521-phase-2-preflight-audit-gregor.md) — **Gregor (Architect) audit of the seven contract violations between today's disk shape and §0e.7 P.\* targets; recommends three small Tier-A PRs (forbid-new-folded guard, T.1+G.1 cleanup, io.py legacy namespace) before NFHS-5 kicks off.**
...
This section captures verbatim the prose lifted out of [TODO/20260517-canonical-long-format-pivot.md](../../TODO/20260517-canonical-long-format-pivot.md) when the plan-doc was slimmed to a pending-only view (per the doc-class routing rule in ADR-0034 + the plan-doc's own `no rationale, no rejected alternatives, no executed-work narrative` discipline). Three blocks landed here: (1) the verbose Status: header that was a single-paragraph ship-log of T.x + P.1.A/B sub-commits; (2) the `Active PR ledger (six-PR strangler-fig sequence)` table where all six PRs are now DONE; (3) the `Active retirement ledger` table where 5 of 6 rows are DONE; (4) the `Strategy F -- Meadow-tier rename` decision rationale (now resident in ADR-0041 + concepts/meadow-tier.md). Lifted on 2026-05-25 during plan-cleanup PR.
...
| Flat datasets/boundaries/in/geojson/*.geojson (73 files) | Hive-partitioned datasets/boundaries/in/{country,states,districts,ac,pc,subdistricts,villages,postal}/state=<S>/... per [T.0d spec §1](../../TODO/20260522-t0d-boundaries-consolidation-spec.md) | T.0d | ✅ DONE 2026-05-22 (9e2ee3db) |
