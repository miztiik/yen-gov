# Canonical long-format pivot — handover plan (PRE-SLIM SNAPSHOT, 2026-05-22)

> **ARCHIVED 2026-05-22** — This is a verbatim snapshot of [`docs/archive/plans/20260517-canonical-long-format-pivot.md`](plans/20260517-canonical-long-format-pivot.md) taken immediately before the **plan-doc slim** that landed in commit `refactor/plan-doc-decompose-and-energy-prep`. The slim retired the three stacked header layers, removed §1 (One Rule duplicate of CLAUDE.md §0a), §2 (D1–D36 — all verbatim in [ADR-0030](../architecture/decisions/0030-canonical-store-duckdb-wasm.md)), §3 (R1–R31 rejected alternatives — also in ADR-0030), §4 (CLAUDE.md amendments — already shipped), §5 (target architecture spec — duplicate of [canonical-store.md](../architecture/data/canonical-store.md)), §6 (Phase 0 audit narrative — executed, see below), §7 (Phase 1 deletion-sweep narrative — executed, see below), §0e.1 / §0e.2 / §0e.3 / §0e.6 / §0e.10 (executed-work narrative), and §14 (Round-log debate history).
>
> §0e.4 (topic taxonomy) was lifted to [docs/concepts/topic-taxonomy.md](../concepts/topic-taxonomy.md). §0e.5 (persons fork = Option B) was lifted to [ADR-0035](../architecture/decisions/0035-persons-fork-option-b.md). The slim followed [ADR-0034](../architecture/decisions/0034-documentation-routing-contract.md) — the doc-class routing contract that defines plan-docs as carrying status + active PRs + TBD with no rationale, no rejected alternatives, no executed-work narrative.
>
> Everything below this banner is the pre-slim plan-doc verbatim. The active slim plan-doc carries forward only §0a / §0b / §0c / §0d / §0e.7 (live PR ledger, active rows only) / §0e.8 (live retirement ledger) / §0e.9 (cross-refs) / §8 (Phase 2 P.1 Energy pointer) / §9 (Phase 3+ sketches) / §10–§13 (open questions, future phases, handoff). Execution history is preserved here by intent — `git log` and merge-commit titles are the primary history surface per ADR-0034; this snapshot is a convenience archive of the pre-slim narrative.

---

# Canonical long-format pivot — handover plan

**Last Updated**: 2026-05-22 — **Phase-2 pre-flight CLOSED**. All seven pre-flight steps done: PR1 (Tier-B forbid-new-folded-shards) ✅ `8de71a4a` PR #87; T.1 (`_test/`→`_ops/` + folded-indicator-writer extraction = PR3 absorbed) ✅ `76bc5fde`; PR2 (T.1 status reconcile + features-KEEP + G.1 handover) ✅ `8fb3e935` PR #88; **G.1 office-bearers consolidation ✅ DONE** via 3-PR strangler-fig — G.1.a entity-lift `ee441193` PR #89, G.1.b reader-switch `cc9ad5b7` PR #90, G.1.c **Option B consolidation** `36e64c87` PR #91 (31 per-state `cm_terms.json` → 1 `datasets/taxonomy/office_holdings.json`; `office-holdings.schema.json` v1.0; `office_holdings_seed.py` replaces `cm_terms_seed.py`; frontend `governments.ts` rewritten with in-loader field-name adapter; byte-identical Parquet verified 3rd time; 779 backend + 15983 vitest green). **NEXT: Phase 2 P.1 NFHS-5** — first family pivot. Planning handover at [`TODO/20260522-phase-2-p1-nfhs-5-planning.md`](20260522-phase-2-p1-nfhs-5-planning.md). Pre-flight audit closeout + consolidated deferrals list at [`docs/archive/plans/20260521-phase-2-preflight-audit-gregor.md`](20260521-phase-2-preflight-audit-gregor.md) §"Deferrals & open decisions".
**Previous Header (2026-05-20)**: §0e.10 amendment LOCKED — Phase-0 closeout four-way concurrence (Hans+Max+Jony+Fowler): office_id shape, partition-by-state for `election_results.parquet`, 3-stage sub-sequencing T.0a/T.0b/T.0c, ledger AMEND-then-FREEZE. **T.0a SHIPPED** with scope cut: writer partition only — seeds + LGD + districts deferred to T.0a-ii follow-up. Phase 1 deletion sweep ✅ closed (1.8e + 1.8f shipped); Phase 2 Energy unblocked once full T.0 series lands.
...
**Single source of truth**: `docs/archive/plans/20260517-canonical-long-format-pivot.md` (this file). All implementation work follows this document. Do not invent parallel plans. If something is missing, add it here, do not start a sibling doc.

**Input artifacts** (referenced FROM this plan; not themselves the plan):

| Path | Role | Status |
| --- | --- | --- |
| `docs/archive/plans/20260517-canonical-long-format-pivot.md` | **THE plan** (you are here) | authoritative |
...
| - | --- | --- | --- | --- |
| **T.1** | **Tidy first — dir hygiene.** Delete `_test/`. Create `_ops/`. Move operator state → `_ops/`. Audit `features/` (delete or document). Update `manifest.json` `path` fields. **DONE 2026-05-22** — see commit on `feat/hard-pivot-t1-and-legacy-namespace`: `datasets/_test/range-mime-probe.parquet` → `datasets/_ops/range-mime-probe.parquet`, `datasets/_test/temporal-range-fixtures/cases.json` → `backend/tests/fixtures/temporal-range-cases.json` (cross-language shared fixture, single source), `datasets/_test/` deleted, validator + admin/inventory exclusions updated, `datasets/_ops/README.md` created. `features/in/energy/power-plants.geojson` audited — DOCUMENTED (3 active doc references in `docs/research/energy-power-plants.md`, `docs/reference/data-coverage-report.md`, `TODO/SOCIO-ECONOMIC-EXPANSION.md`), kept in place. | structural | no | — |
...
| **G.1** | **Office-bearers consolidation.** ✅ **DONE 2026-05-22** via 3-PR strangler-fig (G.1.a entity-lift → G.1.b reader-switch → G.1.c consolidate + retire). G.1.a `ee441193` (PR #89): 31 `entity_type='office_bearer'` rows in `taxonomy/entities.json`; `entity.schema.json` v1.2. G.1.b `cc9ad5b7` (PR #90): `cm_terms_seed.py` resolves office identity from `entities.parquet`; byte-identical Parquet outputs. G.1.c `36e64c87` (PR #91): **Option B chosen** after Hans+Fowler+Max 3-persona review — 31 per-state `cm_terms.json` consolidated into ONE long-form `datasets/taxonomy/office_holdings.json` (359 holdings + 31 office_citations); `office-holdings.schema.json` v1.0; `office_holdings_seed.py` (~430 LOC) replaces `cm_terms_seed.py`; frontend `governments.ts` rewritten with in-loader adapter; `git rm` of 31 JSON + cm_terms_seed.py + 5 other legacy files. Byte-identical `dim_offices.parquet` + `governments_office_holdings.parquet` + `sources.parquet` verified 3rd time. Full design + 4 rejected alternatives + acceptance criteria archive in [`TODO/20260522-g1-cm-terms-retirement-handover.md`](20260522-g1-cm-terms-retirement-handover.md). | structural + behavioural (fused per sub-PR) | yes (frontend `governments.ts` rewritten) | T.3 + S.1 (relaxed: G.1 ran without T.3/S.1 because office identity is its own taxonomy island per §0e.6) |
...
| `datasets/governments/in/states/` | `datasets/taxonomy/office_holdings.json` (consolidated long-form; compiles to `governments_office_holdings.parquet` + `dim_offices.parquet`) + `datasets/taxonomy/entities.parquet` (`entity_type='office_bearer'`) | G.1 ✅ DONE 2026-05-22 via G.1.a/b/c strangler-fig (PRs #89/#90/#91; final consolidation in `36e64c87`). Per [`TODO/20260522-g1-cm-terms-retirement-handover.md`](20260522-g1-cm-terms-retirement-handover.md). |
| `datasets/features/` | KEEP per PR2 audit (`3cb11036`) — citizen geometry layer (2 GeoJSON files, no Parquet analytical path); retires when boundaries become a P.\* family. See [`datasets/features/README.md`](../datasets/features/README.md). | T.1 (audited 2026-05-22, decision: KEEP) |
...
- **Debate transcript** (3-agent synthesis, OWID screenshot taxonomy verbatim, all rejected designs archived): [docs/archive/plans/20260519-indicator-topic-taxonomy-and-dir-structure-plan.md](20260519-indicator-topic-taxonomy-and-dir-structure-plan.md).
...
| --- | --- | --- | --- | --- | --- | --- |
| **T.0a** *(SHIPPED 2026-05-20, scope-cut)* | **Additive — writer partition only.** Writer change (partition `elections.election_results` by state per §0e.10.2-B): `FAMILY_FACT_PARTITION_BY = {"elections": ["state"]}` registry + `_STATE_PARTITION_SQL` derivation + partition-aware `_load_existing` + `_emit_observations` + `_regenerate_manifest` + new `_describe_partitioned_table` helper. Locked grammar `state=in_<two-char-lower>` (§0e.10 lock B). One-shot migration tool `tools/repartition_elections.py` (idempotent). Real-corpus migration: 201,292 rows → 31 partitions. ALL legacy files untouched. **Scope cut**: seeds + LGD + districts split off into **T.0a-ii** to keep the writer-partition PR surgically reviewable per Hans+Max+Fowler standing convention (small bisectable PRs). | structural + behavioural (fused) | backend writer + 2 test files (1 new, 1 updated) + 1 one-shot migration tool + 31 new partition Parquets + monolith deletion + manifest regen + docs (`canonical-store.md` §10/§12/§14, deletion manifest §6a, this row). | NO change — legacy `/data/reference/...` URLs unaffected; frontend still reads via existing loaders (reader port lands in T.0b). | 8 new partition tests in `test_canonical_writer_partition.py` (Holy Law #7, real DuckDB COPY on `tmp_path`, no mocks) + 2 existing tests updated. Full backend `pytest -q` green (779+ tests). | Fully reversible — pure git revert; one-shot tool re-runnable to round-trip back to monolith if needed. |
| **T.0a-ii** *(planned — deferred from T.0a)* | **Additive — seeds + LGD + districts.** New `taxonomy/{state_tiers,topics,election_events}.{json,parquet}` (via §8.3 Python-compiles-to-Parquet seeds). New `governments/{governments_office_holdings,dim_offices}.parquet` (via new `cm_terms_seed.py`). Move LGD CSVs to `taxonomy/lgd/<role>-<date>.csv` + `<role>-latest.csv`. Lift district rows into `taxonomy/entities.parquet` (entity_type='district'). 6 new schemas. Wire seeds into `python -m yen_gov emit-taxonomy`. ALL legacy files **untouched**. | structural + behavioural (fused) | backend new seed tools + 6 new schemas + taxonomy + governments + manifest regen. ~30–50 files. | NO change — legacy `/data/reference/...` URLs still serve legacy bytes. Site continues to render identically. | Compile-tool round-trip tests per tool against `tmp_path`. FK closure assertion `cm_terms_seed` → `dim_offices`. Tier-A schema sanity on new schemas. | Fully reversible. |
| **T.0b** | **Frontend port + §13 browser smoke.** Switch every consumer of `/data/reference/in/*` to the new canonical path (or DuckDB-WASM for the Parquet ones). Switch DuckDB-WASM loaders to glob `elections/state=*/election_results.parquet` (or manifest-directed per-shard fetch). Inline `unmapped_regions.json` into `frontend/src/lib/unmapped-regions.ts`. | structural (no schema change; pure import swap) | `frontend/src/lib/` only. ~10–15 files. | YES — every reader switched. Mandatory §13 browser smoke: `/`, `/in/s22`, `/in/s22/elections/ac/167`, `/data-completeness`, one compare route. | Vitest + Playwright tier. Snapshot the network panel before/after — no new 404s. | Reversible via revert; both paths exist in `datasets/` until T.0c. |
...
Why three (now four with T.0a-ii) not one (Jony+Fowler verdict): A single fat PR loses bisectability for ~4,000 file deletions where each deletion has a different blast radius; T.0b's `§13` browser smoke MUST run with both old and new paths present in `datasets/` so the network panel can prove the swap before the bytes vanish. T.0a-ii split off mid-sequence on 2026-05-20 when T.0a was already a 31-Parquet emit + writer surgery + manifest regen — folding seeds + LGD + districts into the same PR would have pushed review burden past the bisectable-commit ceiling without any sequencing benefit. Single-PR variants rejected during debate.
...
| `reference/in/states/<S>/districts.json` | DELETE | lift to `entity_type='district'` in `taxonomy/entities.parquet` (`lgd_code` is PK; legacy slug → `legacy_id` column) | T.0c — **Phase A done 2026-05-22** (branch `feat/districts-fold-in-phase-a`, PR #81 `a3d45611`; 145 districts folded INTO `taxonomy/entities.json` as `entity_type='district'` rows; `entities_seed.py` dedup contract = entities.json wins on `entity_id`; `entities.parquet` byte-stable SHA-256 `771ECEC3…62243ED`; 2 Mahe/Yanam UT-sub rows skipped per missing `lgd_code` per Phase 2 preflight). **Phase B done 2026-05-22** (branch `feat/districts-fold-in-phase-b`, PR #82 `2c9d9712`; `entities_seed.py` districts.json loader stripped — `_District`, `_DistrictsFile`, `_load_districts_files`, `_district_*` transforms gone; `compile_to_parquet` signature shrunk from 3-arg → 2-arg; CLI `emit-taxonomy` no longer walks `datasets/reference/in/states/*/districts.json`; one-shot `tools/fold_districts_into_entities.py` migration script + `test_entities_fold_in_parity.py` parity test deleted as no-longer-applicable; `entities.parquet` SHA-256 still `771ECEC3…62243ED` — byte-stable invariant proves loader removal had zero data effect). **Phase C handover done 2026-05-22** (PR #83 `70bd303e`; docs-only — 7-file deletion deferred to D.3 per pre-deletion grep audit). **Phase D.1 done 2026-05-22** (branch `feat/wikipedia-districts-adapter-retire`, PR #84 `93a9c175`, ADR-0033; wikipedia districts adapter retired entirely — `sources/wikipedia/districts.py` deleted, `DistrictEntry`/`DistrictsCollection` deleted, `districts_url()` deleted, `pipeline/reference.py` now sources district lookup from `entities.json` via new `_district_lookup_from_entities()` helper; constituencies parser's two-pass `_strip_parens` + `_norm` resolver unchanged; Hans+Max+Gregor unanimous D.1.c per §0a authority assignment). **Phase D.2 done 2026-05-22** (branch `feat/lgd-backfill-tool-retire`; `tools/lgd/backfill_lgd_codes.py` + `backend/tests/test_lgd_backfill.py` deleted; doc bullet on `boundaries.md` removed; `core/models.py` retirement comment trimmed). **Phase D.3 done 2026-05-22** (branch `feat/districts-final-delete`; `git rm` the 7 files — 6 per-state `districts.json` + `district.schema.json` — plus ~14 doc scrubs, migration-ledger appended with 7 D.3 rows, datasets/CHANGELOG.md 2026-05-22 entry, ADR-0033 Future-work flipped to "Subsequent phases (now landed)"; `entities.parquet` SHA-256 still `771ECEC3…62243ED` byte-stable through whole arc; T.0c-iii strangler-fig CLOSED). See `TODO/20260522-districts-wikipedia-adapter-retirement-handover.md`. |
...
THE PLAN (single source of truth): docs/archive/plans/20260517-canonical-long-format-pivot.md
...
START SEQUENCE:
1. Read THE PLAN (docs/archive/plans/20260517-canonical-long-format-pivot.md) end-to-end.
...
- [`docs/archive/plans/20260521-phase-2-preflight-audit-gregor.md`](20260521-phase-2-preflight-audit-gregor.md) — **Gregor (Architect) audit of the seven contract violations between today's disk shape and §0e.7 P.\* targets; recommends three small Tier-A PRs (forbid-new-folded guard, T.1+G.1 cleanup, io.py legacy namespace) before NFHS-5 kicks off.**


---

# 2026-05-25 supplement -- slim-mode plan-doc lifts

This section captures verbatim the prose lifted out of [docs/archive/plans/20260517-canonical-long-format-pivot.md](plans/20260517-canonical-long-format-pivot.md) when the plan-doc was slimmed to a pending-only view (per the doc-class routing rule in ADR-0034 + the plan-doc's own `no rationale, no rejected alternatives, no executed-work narrative` discipline). Three blocks landed here: (1) the verbose Status: header that was a single-paragraph ship-log of T.x + P.1.A/B sub-commits; (2) the `Active PR ledger (six-PR strangler-fig sequence)` table where all six PRs are now DONE; (3) the `Active retirement ledger` table where 5 of 6 rows are DONE; (4) the `Strategy F -- Meadow-tier rename` decision rationale (now resident in ADR-0041 + concepts/meadow-tier.md). Lifted on 2026-05-25 during plan-cleanup PR.

## L1. Pre-slim Status: header (lifted verbatim)

Phase 0 ✅ + Phase 1 elections deletion sweep ✅ + T.1 (_test/->_ops/ hygiene) ✅ + G.1 (office-bearers consolidation) ✅ + **T.0d (boundaries consolidation -- Hive partitioning + parquet ledger) ✅ MERGED 2026-05-22 (9e2ee3db)** + **T.0e (STATE_NAME_TO_ECI retirement -> states view-model via taxonomy.entities) ✅ MERGED 2026-05-22 (PR #96, de463eca)** + **T.2 (topic-catalogue schema bump v1.2->v1.3 + 9 placeholder topics + doc-ref scrub) ✅ MERGED 2026-05-24 (PR #182, 39f42b9c)** + **T.3 (indicator catalogue v1.1) ✅ MERGED 2026-05-23** (PR #107,  a8d8e83). **Phase 2 P.1 = Energy** is the active phase; sub-commits C0-C4 + **C4.5 CEA per-state per-fuel snapshot lift (lift-only -- reader-switch deferred per PR #177 strangler-fig lesson)** + **C4.6 RBI Handbook Table 140 FY05-FY14 long-arc splice (SHIP-LIFT-ONLY -- reader-switch + legacy-shard retire deferred to follow-up)** + **C4.8 sub-fuel preservation (Option B additive -- methodology_breaks row + Tier-B fence; shard retire descoped to follow-up)** + **P.1.B DISCOM finance + demand/supply extension (SHIP-LIFT-ONLY -- 6 new canonical indicators on energy_distribution_performance.parquet + energy_demand_supply.parquet; +5 source ledger rows; +2 facet axes efficiency_dimension + po_segment; frontend allowlist + topics.json retire-edits + legacy-shard git rm deferred to subsequent PRs mirroring the P.1.A C5+C6 pattern)** MERGED to main (PR #101 umbrella 4f79e319 + PR #106 C4 replay e04f85a6 + C4.5 PR + C4.6 PR + C4.8 PR + P.1.B PR). PR-D #277 f723ef6 (FacetPicker primitive) + PR-E #279 82c3bc70 (RPO honesty caveats) + PR-F-prereq #280 d745d7b3 + PR-G #282 81bbe968 (5 more /t/energy 404 fixes) + PR-H #283 ebda30e (Hans-curated known_caveats for 4 honesty-debt indicators) layered atop. S.1 persons fork ✅ DONE 2026-05-23 (PR #176 8d11c376).

## L2. Pre-slim `Active PR ledger (six-PR strangler-fig sequence)` (lifted verbatim; all 6 DONE)

Each PR independently mergeable, each reversible. Two-hat discipline: purely structural (paths / renames / no row content change) OR purely behavioural (schema rows change). Fused atomic per the §15 paired-test discipline when ` == x-version` strict check applies.

| # | PR | Status | Hat | Depends on |
| - | --- | :-: | --- | --- |
| **T.1** | Tidy first -- dir hygiene. Delete _test/. Create _ops/. Move operator state -> _ops/. Audit eatures/ (delete or document). Update manifest.json path fields. | ✅ DONE 2026-05-22 (eat/hard-pivot-t1-and-legacy-namespace) | structural | -- |
| **T.2** | Lift topic catalogue into taxonomy. Move eference/in/topic-catalogue.json -> 	axonomy/topics.json. Add ackend/yen_gov/canonical/topics_seed.py. Compile 	axonomy/topics.parquet. **Add new top-level topics** per [topic-taxonomy concept](../../docs/concepts/topic-taxonomy.md) (governance, schemes, local_govt_finance, work, judiciary, crime, health, education, menities, 	echnology). Retire eference/in/. | ✅ DONE 2026-05-24 (PR #182, 39f42b9c -- schema v1.2->v1.3 minor bump: 	opics.items.artifacts.minItems 1->0; pydantic _Topic.artifacts default []; 9 placeholder topics added with empty artifacts arrays; per-topic landing pages OPEN on each P.* ingestion. Lift of eference/in/topic-catalogue.json -> 	axonomy/topics.json happened in T.0b -- rontend/src/lib/catalogue.ts:97 already points at 	axonomy/topics.json; T.2 scrubs 7 stale doc references and 1 bonus reference in peer-sets.md. health topic already existed in 	opics.json and is NOT re-added.) | structural | T.1 |
| **T.3** | Indicator catalogue widens for topic tags + drops topic prefix. Bump indicator.schema.json minor: add 	opic_tags[] (FK -> 	axonomy/topics.parquet), add id_aliases[] (one-release back-compat), enforce new id shape per canonical-store.md §2a. Migrate 110 legacy ids; populate id_aliases with old <topic>/<id> form; frontend dereferences via alias. Add 	axonomy/indicator_topic_tags.parquet M:N join + 	opic_tags[] denormalised projection on 	axonomy/indicators.parquet. **Fused atomic commit.** | ✅ MERGED 2026-05-23 (PR #107 v1.1 replay onto main  a8d8e83; 5 cherry-picked commits 57c78892 / cac95f32 / 5e450051 / dbb304a5 / d11901ec). Schema v1.0->v1.1 (	opic_tags[] + id_aliases[] + deprecated_in); Tier-B 	ier_b_indicator_alias_window enforces 60-day grace; first frontend canonical-Parquet reader landed at [rontend/src/lib/indicator-catalogue.ts](../../frontend/src/lib/indicator-catalogue.ts). 110 legacy id rewrites + frontend dereferencer remain DESCOPED -- each family's P.* PR rewrites its own ids and fills id_aliases[] at lift time, per the Q2 = Option A scope. | structural + behavioural (fused) | T.2 + P.1.A merge |
| **S.1** | Persons Option B -- one shot. Rename dim_candidates -> dim_persons (schema bump major; id_aliases keeps old shape for one release). Add elections/elections_candidacies.parquet fact. Add 	axonomy/person_aliases.json + compiled 	axonomy/persons.parquet. Seed person clusters for TN-AE using TCPD Candidate_ID groupings as **editorial input only**; source_id cites **ECI candidate lists directly** (TCPD repackages ECI data -- no separate TCPD citation per user 2026-05-22). Delete datasets/people/AcGenApr2021/. Fused atomic commit. Runs in parallel with T.1+T.2+T.3 (no contention). Design + rejected alternatives: [ADR-0035](../decisions/0035-persons-fork-option-b.md). | ✅ DONE 2026-05-23 (PR #176 8d11c376 -- 34906 persons / 34906 candidacies / 34906 taxonomy.persons; Layer-1 collision repair shipped; 	axonomy/person_aliases.json ships with empty clusters per S.1 asserts no cross-candidacy merges yet -- Layer-3 TCPD seed is a separate later effort per ADR-0035 layering) | structural + behavioural (fused) | independent (parallel with T.x) |
| **T.0d** | Boundaries consolidation. Lift 73 .sources.json sidecars onto 	axonomy/sources.parquet per §12 v2.0 (FK pattern matches Energy; **4 sources seeded** -- DataMeet/HTL/shijithpk/ramSeraph; postal subtree forward-looking only). Fold 39 .metadata.json + 2 .unkeyed.json + 1 S22-villages-index.json into NEW datasets/boundaries/boundary_layers.parquet control table (15 columns; 12 required + 3 optional). Restructure datasets/boundaries/in/ to Hive partitioning matching elections/state=in_<S>/... across 8 admin-spine levels (country/, states/, districts/, c/, pc/, subdistricts/, illages/, postal/). Fused atomic per §15: 1 new schema + 3 deleted schemas + new Pydantic + 	ools/boundaries/snapshot.py rewrite + rontend/src/lib/maplibre/sources.ts path repointing + ADR-0031 amendment + Tier-B forbidden-paths gate + 115 sidecar deletions + 73 git mv of geometry files. Methodology breaks remain on 	axonomy/entities.parquet (already canonical, no T.0d change). **Spec**: [T.0d execution spec](plans/20260522-t0d-boundaries-consolidation-spec.md) (Gregor + Hans + Max, 2026-05-22; OPENs all resolved by user 2026-05-22). | ✅ DONE 2026-05-22 (9e2ee3db) | structural + behavioural (fused) | independent (parallel with T.x); can run in parallel with Energy P.1.A |
| **T.0e** | STATE_NAME_TO_ECI map retirement. Replace the inline frontend constant (in rontend/src/lib/maplibre/sources.ts:217) with a DuckDB-WASM loadStates() view-model querying 	axonomy.entities WHERE entity_type IN ('state','ut'). The constant currently masks the three coexisting code systems (ECI: S22; LGD/MoHA: 33; ISO 3166-2: IN-TN) by exposing only the ECI projection. 	axonomy/entities.parquet already carries all three (legacy_id, lgd_code, iso_3166_2). 9 frontend files touched (Home, CompareIndicator, IndicatorChoropleth, IndicatorRanked, IndicatorSmallMultiples, IndiaMap, drilldown.ts + 2 routes). Many call sites are synchronous (Object.entries(STATE_NAME_TO_ECI)) and need converting to async-aware. §13 browser smoke MANDATORY on every indicator route + Home + Compare. Separate PR (carved out of T.0d on user direction 2026-05-22). View-model at rontend/src/lib/view-models/states.ts (Hive-paths consumer; exposes ECI/LGD/ISO_3166_2 alongside boundary_join_name override table). | ✅ DONE 2026-05-22 (PR #96, de463eca) | structural + behavioural (fused) | depends on T.0d (boundary-paths must stop using the constant first) |
| **G.1** | Office-bearers consolidation. 3-PR strangler-fig (G.1.a entity-lift -> G.1.b reader-switch -> G.1.c consolidate + retire). | ✅ DONE 2026-05-22 (PRs #89 / #90 / #91; see [G.1 handover](../../TODO/20260522-g1-cm-terms-retirement-handover.md)) | structural + behavioural (fused per sub-PR) | (ran without T.3/S.1 -- office identity is its own taxonomy island per §0e.6 of the snapshot) |
| **P.*** | **Per-family pivot -- Phase 2 active phase.** Each family from the topic taxonomy becomes its own sub-PR following the §2a naming rule + FK contract + empty-parent pruning. **P.1 = Energy** (active; plan at [docs/archive/plans/20260522-phase-2-p1-energy-pivot.md](plans/20260522-phase-2-p1-energy-pivot.md)). | ⏳ ACTIVE on 2026-05-25 (P.1 = Energy partly done; P.2 = Livestock NDLM in progress on parallel branch) | structural + behavioural (fused per family) | T.3 (P.* can start before T.3 if family-specific schema bump is independent -- Energy is in this position) |

## L3. Pre-slim `Active retirement ledger` rows (5 of 6 DONE)

| Old path | Replacement | Retiring PR | Status |
| --- | --- | --- | :-: |
| datasets/reference/in/topic-catalogue.json | datasets/taxonomy/topics.parquet (compiled from 	axonomy/topics.json) | T.2 | ✅ DONE 2026-05-24 (path lift completed in T.0b; T.2 ships schema v1.3 + 9 placeholder topics + doc-ref scrub) |
| datasets/reference/in/states/<S>/districts.json | already DONE per [ADR-0033](../decisions/0033-retire-wikipedia-districts-adapter.md) -- districts folded into 	axonomy/entities.parquet as entity_type='district' | T.0c (PR #68) | ✅ DONE 2026-05-21 |
| datasets/people/AcGenApr2021/ | datasets/elections/dim_persons.parquet + 	axonomy/persons.parquet | S.1 | ✅ DONE 2026-05-23 (PR #176 8d11c376) |
| datasets/boundaries/in/geojson/*.{sources,metadata,unkeyed}.json (115 sidecars) + S22-villages-index.json | datasets/boundaries/boundary_layers.parquet + FK to 	axonomy/sources.parquet | T.0d | ✅ DONE 2026-05-22 (9e2ee3db) |
| Flat datasets/boundaries/in/geojson/*.geojson (73 files) | Hive-partitioned datasets/boundaries/in/{country,states,districts,ac,pc,subdistricts,villages,postal}/state=<S>/... per [T.0d spec §1](plans/20260522-t0d-boundaries-consolidation-spec.md) | T.0d | ✅ DONE 2026-05-22 (9e2ee3db) |

The remaining ACTIVE row stays on the slim plan-doc and is tracked under §0e.8a (energy indicators/in/ residue).

## L4. `Strategy F -- Meadow-tier rename` decision rationale (lifted verbatim; ratified 2026-05-25 by Hans + Max + Gregor)

This is decision-rationale prose now resident in [ADR-0041](../decisions/0041-meadow-tier.md) + [docs/concepts/meadow-tier.md](../concepts/meadow-tier.md). Lifted here verbatim because the plan-doc's class rule (no rationale, no rejected alternatives) does not permit it.
...
Five OWID-aligned tiers (per CLAUDE.md §0a `The One Rule`): `upstream -> snapshots (ephemeral, .runtime/raw/) -> meadow (committed, datasets/<family>/_meadow/) -> canonical (committed, datasets/<family>/<family>_<role>.parquet) -> grapher (frontend view-models)`.
...
**Forcing function**: each per-adapter git mv simultaneously (a) repoints the backend canonical adapter, (b) breaks any legacy frontend etch('/data/indicators/in/...') URL -> forces Phase B allowlist to be the only path, (c) deletes the old path. Phase-C + Phase-D + C5 unified per slice.

**Completion criterion**: datasets/indicators/in/ does not exist on main. Single git ls-tree query.

**Doc impact** (landed in PR 7c-0):
- NEW: [docs/concepts/meadow-tier.md](../concepts/meadow-tier.md) -- define meadow vocabulary, meadow->canonical contract, OWID precedent, `no frontend fetch from _meadow/` rule.
- NEW: [ADR-0041 `Meadow tier -- parsed publisher rows as canonical input`](../decisions/0041-meadow-tier.md) -- captures F-vs-(a/b/c/d) tradeoff with rejected alternatives.
...
- AMEND: [docs/architecture/data/canonical-store.md](../data/canonical-store.md) §2 -- add _meadow/ to per-family directory invariant + backend-only consumer constraint.
- DEFERRED to PR 7c-residue: rename datasets/_ops/legacy-folded-indicator-shards.txt -> datasets/_ops/meadow-shard-contract.txt; rewrite header from `countdown to retirement` -> `perimeter for canonical-input contract.`
...
4. Vintage in meadow path MUST match intage field of citation row the FK resolves to.
