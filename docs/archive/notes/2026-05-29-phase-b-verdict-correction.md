...
> **SUPERSEDED 2026-05-30** by Furfur Wahlkreise SVG pivot (initial hypothesis); the SVG pivot was itself REFUTED by structure-probe PR #472 ([notes/2026-05-30-s03-furfur-svg-structure-probe-verdict.md](2026-05-30-s03-furfur-svg-structure-probe-verdict.md)): the Furfur SVG has only 20 `<path>` elements / 25 subpath-starts (district-shape groups, NOT 126 per-AC polygons) + 132 numeric label-only `<text>` nodes. Final disposition: plan-doc Row 5.1 COLLAPSED via PR #478 with user choosing Path C (keep T4 district fallback as the honest shipped experience); gap documented in [docs/concepts/admin-level-sourcing.md](../docs/concepts/admin-level-sourcing.md) `Known coverage gaps`. The 4-tier ladder framework remains durable in [docs/how-to/digitize-ac-from-pdf.md](../docs/how-to/digitize-ac-from-pdf.md) for FUTURE delim-PDF states where neither a Furfur-style cartographer nor LGD coverage exists. The S01 AP section above is unaffected.
