...
| R1.5 | AssemblyElections mart coverage: DELETE display-string-parsing bridge in `event_summary.py`; iterate catalogue's outer-dict key (`eci_code`); use existing `eci_to_lgd_slug()` helper; regen mart so J&K U08 lights up                              | [x] MERGED              | #1063 (+ followup #1065)    | S      |
| R1.6 | Bihar 2005 catalogue identity rip: catalogue schema 1.3 -> 1.4 (extend event_id grammar); split `assembly-2005` -> `assembly-2005-feb` + `assembly-2005-nov`; fan out on-disk `election=2005/` dir; regen mart; freeze catalogue PK invariant test  | [D] DEFERRED — Path A'   | spec at [TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md](TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md) | L      |
...
| R5   | NEW `StateEventCrossEventSankey.svelte` (diverging-bar always-on + Sankey collapsed) + opt-in caption                                                                                                                                                | [x] MERGED              | #1079 (re-open after #1073 close) | M      |
| R6   | Alliance honesty: `formation` column + render-when-data-exists + caption above panel + first-event-no-prior gating                                                                                                                                  | [x] MERGED              | #1069                       | M      |
| R7   | NEW build-step: per-event share-card PNG at `bun run build` (`frontend/scripts/build-share-cards.ts` + `@resvg/resvg-js` devDep + `<svelte:head>` og:* + twitter:card meta on `StateElection.svelte`); J-elevated-14                                  | [x] MERGED              | #1077 (313/313 cards generated end-to-end; +24 vitest assertions)  | M      |
| SC   | Scrub `Lok Sabha` / `Vidhan Sabha` from citizen chrome + freeze with `english-only-citizen-chrome.test.ts` contract test                                                                                                                          | [x] MERGED              | #1074                       | XS     |

**Status legend**: `[x] MERGED` = on origin/main; `[~] IN-FLIGHT` = work-in-progress branch exists with commits + PR open; `[D] DECIDED` = persona panel converged, executor brief drafted, awaiting dispatch; `[D] DEFERRED` = persona panel converged + deferred to a follow-up cycle with executable spec linked; `[ ] PENDING` = not yet started; `[!] BLOCKED-NEEDS-SIGNOFF` = STOP-AND-SURFACE per CLAUDE.md §10.
...
| 2026-06-15 | execution-cycle | Continuation execution cycle (2nd orchestrator) per Section 8 EXECUTION BLOCK + user 'deliver next phase' direction. Status sync: R4 (#1071) MERGED — IA spine + SiblingEventsRail year-chip rail (no arrows; Jony J-elevated-4) + HeroCards TopicIcon glyphs + turnout pp-delta + ConstituencyList district-fold/sticky-search + delete InlineCounterfactualSwing mount + 5 static-source contract assertions freezing the IA. R5 (#1073) IN-FLIGHT stacked on R4 — StateEventCrossEventSankey wrapping the existing SwingSankey primitive + always-on diverging bar + collapsed-by-default Sankey behind 'Show vote-flow' pill + no-prior copy gating + Max's approximate-flow caption verbatim + 12 vitest model assertions. R7 (#1077) IN-FLIGHT — build-step share-card PNGs at 1200x630 (BJP saffron + 'BJP won 132 of 288' + ECI attribution + yen-gov.org wordmark); 313/313 cards generated end-to-end; @resvg/resvg-js devDep + svelte:head og:* + twitter:card on StateElection + StateElectionsLanding; +24 vitest assertions (11 build-svg + 13 plan + 2 yen-gov-producer-normalisation; the normalisation prevents the 'Source: yen-gov' Generalisation per Holy Law #9). SC (#1074) IN-FLIGHT new row — scrub Lok Sabha / Vidhan Sabha from citizen chrome (6 strings across 2 svelte files + 3 code-comment files for spine consistency) + new english-only-citizen-chrome.test.ts contract test walking every svelte file's template region. R1.6 DEFERRED — receipt at TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md captures Section 2.6.1 panel verdicts + scope-honesty rationale for deferral (catalogue split + schema bump alone would move corruption from one slug to two; Path A' requires writer-kwarg + Bihar re-emit + mart-resolver fix in the SAME PR per Gregor; ~250-400 LOC across backend + datasets + tests, parallel-unsafe with R5/R7). This ledger row + R1.6 receipt + Status Reckoner sync (R5/R7/SC entries + status-legend extension for [D] DEFERRED) compose the docs-PR for this cycle. R6 absorbed: Hero null-delta omit shipped in R4; Sankey no-prior copy shipped in R5; the noPriorSameBody wiring rule from plan Section 7 R6 row is fully realised across the two PRs. |
| 2026-06-15 | cycle-close | Cycle close-out: all 5 in-flight PRs from the continuation cycle MERGED to origin/main. R4 (#1071), R5 (#1079 - re-opened after #1073 was auto-closed by R4's squash-merge), SC (#1074), R7 (#1077), R1.6-receipt (#1078) all on main as of 2026-06-15 23:30 IST. R5 + R7 + SC marked MERGED in this final sync. The state-event-redesign plan-doc is now fully closed except for R1.6 Path A' which has its full executable spec captured at [TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md](TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md) for the future executor. Plan-doc may be archived to docs/archive/plans/ in a follow-up distillation PR per docs/how-to/distill-a-plan.md (durable findings already live in: docs/architecture/frontend/charts/election-views.md sankey doctrine; docs/architecture/frontend/url-grammar.md english-only-chrome doctrine; the in-repo contract tests at frontend/src/contracts/english-only-citizen-chrome.test.ts + frontend/src/lib/elections/StateElection.section-order.test.ts + frontend/src/lib/elections/AllianceTotals.no-pending-pill.test.ts which freeze the doctrinal commitments machine-enforced on every vitest run). |

## Plan complete

Closed 2026-06-15. All in-scope rows MERGED or DEFERRED-with-receipt:

| Row | Disposition | PR | Distilled to |
|-----|-------------|----|--------------|
| R1   | MERGED | #1061 | docs/architecture/data/canonical-store.md (writer-discipline) + datasets/migration-ledger.csv (266→288 receipt) |
| R1.5 | MERGED | #1063 (+ followup #1065) | (structural bridge over existing catalogue; no separate doc distillation — helper `eci_to_lgd_slug()` in `backend/yen_gov/canonical/adapters/eci/state_slug.py` is the spec) |
| R1.6 | DEFERRED — full executable spec preserved | (no PR) | [TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md](../../../TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md) for future executor |
| R2   | MERGED | #1066 (+ gap-close #1072) | docs/architecture/frontend/url-grammar.md (/<state>/elections/ landing route) |
| R3   | MERGED | #1067 + #1068 | (structural-only refactor; no doc distillation needed) |
| R4   | MERGED | #1071 | docs/architecture/frontend/charts/election-views.md (IA section-order doctrine) + frontend/src/lib/elections/StateElection.section-order.test.ts (contract-frozen) |
| R5   | MERGED | #1079 (+ e2e followup #1081) | docs/architecture/frontend/charts/election-views.md (Sankey approximate-flow caption doctrine) |
| R6   | MERGED | #1069 | docs/concepts/owid-alignment.md (alliance-honesty silence-on-uncurated) + frontend/src/lib/elections/AllianceTotals.no-pending-pill.test.ts (contract-frozen) |
| R7   | MERGED | #1077 | (build-step share-card spec lives in `frontend/scripts/build-share-cards.ts` + vitest assertions; no separate doc — the script + tests are the spec) |
| SC   | MERGED | #1074 | docs/architecture/frontend/url-grammar.md (english-only-chrome doctrine) + frontend/src/contracts/english-only-citizen-chrome.test.ts (contract-frozen) |

R1.6 Path A' is the single open exception. Its full ratified spec (catalogue 2-row split + schema 1.3→1.4 + path-builder `event_id` kwarg + AE-panel Bihar-only re-emit + mart `_find_assembly_event` resolver fix + `EVENTS_BY_MONTH` audit helper + PK-invariant contract test) lives at [TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md](../../../TODO/20260615-R1.6-bihar-2005-path-a-prime-receipt.md) and is parallel-unsafe with any other backend-writer work — pick it up only when (a) the TCPD source CSV is on disk and (b) no other backend-writer PR is in flight.

Durable doctrinal commitments machine-enforced on every vitest run:
- [frontend/src/contracts/english-only-citizen-chrome.test.ts](../../../frontend/src/contracts/english-only-citizen-chrome.test.ts)
...
- [frontend/src/lib/elections/AllianceTotals.no-pending-pill.test.ts](../../../frontend/src/lib/elections/AllianceTotals.no-pending-pill.test.ts)

Plan-doc archived to `docs/archive/plans/20260615-state-election-event-page-redesign-plan.md` per [docs/how-to/distill-a-plan.md](../../how-to/distill-a-plan.md) Step 5. New work starts a new plan-doc.
