...
**Status**: ✅ DONE — rip-and-replace mandate complete (PRs #336-#412); 5/5 §0quat Tier-B gates LIVE; forward-build items (D4-tail/D5/D7/D8/C/E) reclassified in §0bis and tracked via separate plan-docs.
**Doc-class**: plan-doc per [ADR-0034](../../architecture/decisions/0034-documentation-routing-contract.md). Carries PR sequence + status only; rationale lives in cited ADRs and concept docs.
...
| **A5a** | Strip `datetime.now()` non-livestock | DONE (7 of 8 orchestrators; iced_common/client.py deferred to A5a-tail2) | #391, #393 | — |
...
4. **The agent may amend other plan-docs** ([docs/archive/plans/20260517-canonical-long-format-pivot.md](20260517-canonical-long-format-pivot.md), [TODO/20260525-livestock-ndlm-ingest-plan.md](../../../TODO/20260525-livestock-ndlm-ingest-plan.md), [20260525-pashu-aadhaar-ingest-plan.md](20260525-pashu-aadhaar-ingest-plan.md), any `AGENTS.md`, any `docs/`) when this plan's PRs invalidate their content. Already enumerated in PR-Z1 + PR-Z2.
...
| --- | --- | --- |
| [docs/archive/plans/20260517-canonical-long-format-pivot.md](20260517-canonical-long-format-pivot.md) | ACTIVE umbrella. Row 5 ✅ done (PR-I/J/O/P). Row 6 P.1.C energy adapters in flight as PR-Q/R/S/T/U arc (PR #307/#309/#314/#316/#318 merged; 4 of 9 indicators left: plant-load-factor, power-purchase-share, final-energy-consumption, renewable-potential). Row 7 P.1.D + Row 8 Citizen-1 panel still open. | **Runs in parallel.** Do not abandon. PR-Z2 of this plan amends its §1 with a pointer. |
| [docs/archive/plans/20260522-t0d-boundaries-consolidation-spec.md](20260522-t0d-boundaries-consolidation-spec.md) | ✅ MERGED `9e2ee3db` (2026-05-22). | **Zero overlap.** Boundaries are a sibling family, not indicator-catalogue. |
...
| [20260525-pashu-aadhaar-ingest-plan.md](20260525-pashu-aadhaar-ingest-plan.md) | PR #303 + PR-P #304 shipped (2 indicators + Hans caveats). | **Identity collapsed by PR-B5.** Caveats survive verbatim. |
...
| 14 | National installed capacity (total + 5 fuels) | ✅ PR-B9-energy-national #_pending_ — 6 ids prefix-stripped (`national-installed-capacity-mw{,-coal,-gas,-hydro,-nuclear,-renewable}` → `installed-capacity-mw{,-coal,...}`). Adapter + 5 parent_indicator_id FKs + Tier-B fixture + 2 comment refs updated; indicators.parquet + energy_installed_capacity.parquet + manifest regen. `entity_kinds=["country"]` unchanged. | `installed-capacity-mw` | `country` | `fuel ∈ {all,coal,gas,hydro,nuclear,renewable}` | B4 (energy) | done |
| 15 | State installed capacity (geographical, total + 5 fuels) | ✅ PR-B9-geocap #389 — strip `state-` prefix: `state-installed-capacity-geographical-mw{,coal,gas,hydro,nuclear,renewable}` → `installed-capacity-geographical-mw{...}` (6 ids). Cross-grain merge into #14 NOT applied: state-grain attribution facet is a structurally distinct concept from country-grain absolute capacity per PRs #378-#383 cluster split (`coal-mw-geographical` etc. vs `coal-mw-absolute`). Entity_kinds=["state"] retained. | `installed-capacity-geographical-mw{*}` | `state` | `fuel ∈ {coal,gas,hydro,nuclear,renewable}` | B4 | done |
| 16 | State installed capacity (allocated, total + 5 fuels) | ✅ PR-B9-allocated #390 — strip `state-` prefix: `state-installed-capacity-allocated-mw{,coal,gas,hydro,nuclear,renewable}` → `installed-capacity-allocated-mw{...}` (6 ids). Cross-grain merge into #14 NOT applied: state-grain allocated-attribution facet is structurally distinct from country-grain absolute capacity per PRs #378-#383 cluster split. Entity_kinds=["state"] retained. | `installed-capacity-allocated-mw{*}` | `state` | `fuel ∈ {coal,gas,hydro,nuclear,renewable}` | B4 | done |
| 17 | State installed capacity snapshot (total + 5 fuels) | ✅ PR-B3-batch2 #394 — strip `state-` prefix: `state-installed-capacity-snapshot-mw{,coal,gas,hydro,nuclear,renewable}` → `installed-capacity-snapshot-mw{...}` (6 ids). Direct mirror of #392 pattern. Adapter `installed_capacity.py` + 5 parent_indicator_id FKs + parity tests + allowlist canonical_indicator_id/parent/child fields + indicator-from-canonical.test.ts updated; indicators.parquet + energy_installed_capacity.parquet + sources.parquet + manifest regen. Entity_kinds=["state"] retained. DROP-entirely (supersede by geographical+allocated) remains queued as a separate retire PR. | `installed-capacity-snapshot-mw{*}` | `state` | `fuel ∈ {coal,gas,hydro,nuclear,renewable}` | B4 | done (rename) |
| 18 | State generation by fuel (total + 5 fuels) | ✅ PR-B3-generation #392 — strip `state-` prefix: `state-electricity-generation-gwh{,coal,gas,hydro,nuclear,renewable}` → `electricity-generation-gwh{...}` (6 ids). Direct mirror of #389/#390 pattern. Entity_kinds=["state"] retained. Adapter `generation.py` + 5 parent_indicator_id FKs + parity tests + allowlist canonical_indicator_id/parent/child fields + indicator-from-canonical.test.ts updated; indicators.parquet + energy_generation.parquet + manifest regen. | `electricity-generation-gwh{*}` | `state` | `fuel ∈ {coal,gas,hydro,nuclear,renewable}` | B4 | done |
| 19 | State distribution efficiency (overall + billing + collection + td-loss) | ✅ PR-B3-batch3 #395 — strip `state-` prefix: `state-distribution-efficiency-pct{,billing,collection,td-loss}` → `distribution-efficiency-pct{,billing,collection,td-loss}` (4 ids). Direct mirror of #392/#394 pattern. Entity_kinds=["state"] retained. | done (rename) |
| 20 | State RPO compliance (3 facets + redundant `-total` duplicate) | ✅ PR-B3-batch4 #397 — strip `state-` prefix: `state-rpo-compliance-pct{,-solar,-non-solar,-total}` → `rpo-compliance-pct{,-solar,-non-solar,-total}` (4 ids). Direct mirror of #392/#394/#395 pattern. Entity_kinds=["state"] retained. Adapter `distribution.py` + 3 parent_indicator_id FKs + parity tests + allowlist canonical_indicator_id/parent/child fields + indicator-from-canonical.test.ts updated; indicators.parquet + energy_distribution_performance.parquet + manifest regen. `-total` drop deferred to a separate retire PR. | done (rename) |
...
| 23 | Peak electricity demand ↔ supplied | ✅ PR-B3-batch3 #395 — strip `state-` prefix: `state-peak-electricity-{demand,supplied}-mw` → `peak-electricity-{demand,supplied}-mw` (2 ids). Direct mirror of #392/#394 pattern. Entity_kinds=["state"] retained. | done (rename) |
...
| 26 | Pashu Aadhaar district (parent + 10 species) | collapsed with #27 | — | `pashu-aadhaar-count` | both | same | B5 | ✅ DONE PR #405 — collapsed with #27 |
| 27 | Pashu Aadhaar state (parent + 10 species, mirror of #26) | 11 ids | — | `pashu-aadhaar-count` | both | same | B5 | ✅ DONE PR #405 — 22→11, entity_kinds=["district","state"] |
| 28 | Livestock owner-reg district (parent + 6 size facets) | 7 ids | [L2594-L2765](../datasets/taxonomy/indicators.json#L2594) | merges into #29 | both | `holding_size ∈ {landless_marginal,small,semi_medium,medium,large,not_specified}` | B5 | ✅ DONE PR #403 — collapsed with #29 |
| 29 | Livestock owner-reg state (mirror of #28) | 7 ids | [L2794-L2965](../datasets/taxonomy/indicators.json#L2794) | `livestock-owner-reg-count` | both | same | B5 | ✅ DONE PR #403 — 14→7, entity_kinds=["district","state"] |
| 30 | Livestock NAIP-IV district (4 metrics, no parent) | 4 distinct ids | [L2994-L3072](../datasets/taxonomy/indicators.json#L2994) | each prefix-strip; KEEP 4 distinct (different units: inseminations, pregnancies, calves, farmers) | both | — | B5 | ✅ DONE PR #401 — collapsed with #31 |
| 31 | Livestock NAIP-IV state (mirror of #30) | 4 distinct ids | [L3098-L3176](../datasets/taxonomy/indicators.json#L3098) | merges into #30 | both | — | B5 | ✅ DONE PR #401 — 8→4, entity_kinds=["district","state"] |
...
| Group | Why split |
| --- | --- |
| `fiscal/states_combined_*_deficit` vs `fiscal/union_*_deficit` | Different fiscal entity (28 states combined vs Union government), not different scopes of one fact. Anti-pattern resolution PIN #2 in [indicator-naming.md §9](../../concepts/indicator-naming.md). |
...
- **MODIFY**: [datasets/taxonomy/topics.json](../datasets/taxonomy/topics.json) — audit every topic; collapse duplicate-measure-per-facet refs. Energy already done (PR #296). Apply to fiscal, economy, health, environment, demography, prices, transport. [docs/concepts/schema-is-the-design-system.md](../../concepts/schema-is-the-design-system.md) — document the rule.
...
- **MODIFY**: [datasets/boundaries/](../datasets/boundaries/) PMTiles emission — add `subdistricts.pmtiles` + per-district `villages/<lgd_district>.pmtiles` (scope by district to keep per-fetch payload <1 MB; ref [docs/architecture/frontend/map.md](../../architecture/frontend/map.md) §"performance budget").
...
- **MODIFY** [docs/agents/guardrails.md](../../agents/guardrails.md): mirror the CLAUDE.md §10 additions.
...
- **MODIFY** [docs/archive/plans/20260517-canonical-long-format-pivot.md](20260517-canonical-long-format-pivot.md) §1: ADD a row pointing at this plan-doc with note "Identity collapse + storage/UI decoupling runs IN PARALLEL; does NOT supersede this umbrella's Row 6 (P.1.C energy adapters PR-Q/R/S/T/U arc) or Row 7 (P.1.D sweep) or Row 8 (Citizen-1 panel)." Mark Row 5 ✅ DONE.
...
- **MODIFY** [20260525-pashu-aadhaar-ingest-plan.md](20260525-pashu-aadhaar-ingest-plan.md): note PR-B5 replaces the per-species sibling-id model with one `pashu-aadhaar-count` id + species facet.
- **MODIFY** [docs/research/indicator-id-grain-axis.md](../../research/indicator-id-grain-axis.md): status flip OPEN → RESOLVED-Path-B-rip-and-replace, cite ADR-0044.
- **MODIFY** [docs/concepts/owid-alignment.md](../../concepts/owid-alignment.md): table row "indicator id (producer-side)" — flip the divergence to "yen-gov ALIGNS with OWID on grain-over-entity after ADR-0044; prior divergence retired."
- **MODIFY** [docs/architecture/data/canonical-store.md](../../architecture/data/canonical-store.md) §6: rewrite the indicator-catalogue field table to drop `renderer_rules`; add `entity_kinds` + `default_entity_kind`. §7: drop the `<entity>-` segment from the indicator-naming format spec.
...
- **MODIFY** [docs/agents/guardrails.md](../../agents/guardrails.md) — mirror.
...
- [docs/architecture/data/canonical-store.md](../../architecture/data/canonical-store.md) — current spec
- [docs/concepts/indicator-naming.md](../../concepts/indicator-naming.md) §2.2 §2.4 §8 — to be rewritten by PR-A1
- [docs/research/indicator-id-grain-axis.md](../../research/indicator-id-grain-axis.md) — Path A vs Path B inputs (will be marked RESOLVED in PR-A1)
- [docs/architecture/decisions/0043-auto-rollup-at-canonical-write-time.md](../../architecture/decisions/0043-auto-rollup-at-canonical-write-time.md) — rollup rules that survive the collapse
- [docs/archive/plans/20260517-canonical-long-format-pivot.md](20260517-canonical-long-format-pivot.md) — umbrella plan; this plan-doc registers a new arc under §1
...
- [20260525-pashu-aadhaar-ingest-plan.md](20260525-pashu-aadhaar-ingest-plan.md) — PR-B5 + C2 close the agriculture mess this plan describes
