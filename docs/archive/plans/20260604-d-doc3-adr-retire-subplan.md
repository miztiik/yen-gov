...
| D-DOC3.4 fold indicator-naming + catalogue ADRs into `docs/concepts/indicator-naming.md` + `docs/architecture/data/indicator-catalogue.md` (LIVE in this row: 0020, 0025, 0026, 0027, 0044, 0045; SUPERSEDED-archive in this row: 0024) | D-DOC3.2 | grep-receipts-eq | #734 | MERGED |
| D-DOC3.5 fold elections + electoral-hierarchy ADRs into `docs/concepts/electoral-hierarchy.md` + `docs/architecture/backend/sources-eci.md` + new `docs/architecture/data/elections-persons.md` + new `docs/architecture/frontend/charts/election-views.md` + `docs/architecture/frontend/url-grammar.md` (LIVE: 0015, 0016-eci-stats, 0017-eci-current-year, 0023, 0035, 0048, 0049, 0051, 0052) | D-DOC3.2 | grep-receipts-eq | #759 | MERGED |
| D-DOC3.6 fold url-grammar + place-IA + nav ADRs into `docs/concepts/url-grammar.md` (new if absent) + `docs/concepts/place-first-ia.md` (new if absent) + `docs/architecture/frontend/url-grammar.md` (LIVE: 0022, 0028, 0034, 0037, 0050; SUPERSEDED-archive: 0016-frontend-hash) | D-DOC3.2 | grep-receipts-eq | #732 | MERGED |
...
Per-row scope adjustments (made when this row was claimed, 2026-06-04): ADR-0044 was DEFERRED FROM D-DOC3.3 (its primary home is `indicator-naming.md` which is D-DOC3.4's write target, not D-DOC3.3's) and is folded HERE. The row label and Execution Ledger entry above are updated to LIVE: 0020, 0025, 0026, 0027, 0044, 0045 (six LIVE folds; one more than the original five in the spawn body). ADR-0045's secondary cross-link target `docs/architecture/frontend/grapher.md` (a NEW doc, per the redirect index) is LEFT FUTURE - the primary fold for 0045 lands at `docs/architecture/data/indicator-catalogue.md#adr-0045-grapher-catalogue-split` (the index's primary target column), and the grapher.md doc-creation is a frontend-chunk concern (a future PR may author it and add a cross-link FROM grapher.md TO indicator-catalogue.md#adr-0045 at that time). ADR-0020's row note "(or canonical-store.md if catalogue doesn't exist yet)" is moot - this PR CREATES `docs/architecture/data/indicator-catalogue.md` so 0020 lands at its primary home. Gate `grep-receipts-eq` running total (current main pre-PR, post-D-DOC3.3+D-DOC3.6+D-DOC3.7+D-DOC3.8+D-DOC3.9 merges, measured 2026-06-05: 50 strict / 58 broader; baseline 33 / 38): this PR ADDS new `## Rejected alternatives` h2 sections to indicator-catalogue.md + indicator-naming.md (each with N h3 subsections per fold; the h3 subsections carry an `ADR-NNNN` prefix so they do NOT match the strict OR broader regex which both require `(Rejected alternatives|Alternatives considered)` to be the FIRST token after `##` / `##` / `###` + whitespace - only the h2 section header matches). indicator-catalogue.md gains 1 strict + 1 broader (the h2 receipt); indicator-naming.md gains 1 strict + 1 broader (the h2 receipt). The ADR-0024 archive move is path-only (same file, same count, just shifted from `architecture/decisions/` to `archive/decisions/` - both included in the union path set). Net: strict 50 -> 52; broader 58 -> 60. Both >= baselines (33 / 38); gate passes. Mirrors D-DOC3.8's owner-correction precedent (PR #728) by flipping ADR-0044's owner column in `docs/reference/decision-index.md` from "D-DOC3.3" to "D-DOC3.4" (the same shape D-DOC3.8 applied to ADR-0046).
...
Per-row scope adjustments (made when this row was claimed, 2026-06-05): the row label is rewritten to enumerate the FIVE actual destination docs per the authoritative `docs/reference/decision-index.md` map (the original row label's `docs/architecture/data/elections/*` glob is moot - there is no `docs/architecture/data/elections/` directory; ADR-0035's primary home per the redirect index is `docs/architecture/data/elections-persons.md` (singular file, NEW), not `elections/persons.md`). All NINE ADRs are LIVE (zero archive moves this row), so no inbound-link rewrites are needed in this PR - the LIVE `architecture/decisions/0015...` etc. paths in the 23 inbound LIVE refs stay as-is until D-DOC3.10's directory-DELETE sweep, mirroring D-DOC3.4's pattern (only its archived ADR-0024 inbound refs got rewritten; the 6 LIVE ADRs' refs were left alone). ADR-0052's secondary cross-link target `docs/architecture/frontend/url-grammar.md` is NOT NEW (D-DOC3.6 created it in PR #732); ADR-0052 folds ADDITIVELY as h3 subsections under D-DOC3.6's existing `## Design rationale` + `## Rejected alternatives` h2s (mirrors D-DOC3.7's ADR-0036 additive fold into canonical-store.md and D-DOC3.6's ADR-0050 additive fold into canonical-store.md). ADR-0016-eci + ADR-0017-eci fold ADDITIVELY into the existing `## Design rationale` + `## Alternatives considered` h2s on `docs/architecture/backend/sources-eci.md` (pre-existing h2s from the original distill; same shape as D-DOC3.8's additive fold into sources-wikipedia.md). Gate `grep-receipts-eq` running total (current main pre-PR, post-D-DOC3.3+D-DOC3.4+D-DOC3.6+D-DOC3.7+D-DOC3.8+D-DOC3.9 merges, measured 2026-06-05: **52 strict / 60 broader**; baseline 33 / 38): this PR ADDS new `## Rejected alternatives` h2 sections to THREE target docs (electoral-hierarchy.md + elections-persons.md + election-views.md; +3 strict + 3 broader). The two ADDITIVE folds (sources-eci.md + url-grammar.md) carry their h3 subsections under existing h2s and add zero new strict or broader matches (h3 headers are `### ADR-NNNN: <title>` and `### ADR-NNNN rejected alternatives` - both have the literal `ADR-NNNN ` prefix between the hashes and the keyword, so neither matches the strict `^## (Rejected\|Alternatives)` regex NOR the broader `^#{2,3}\s*(Rejected\|Alternatives)` regex). Net across this row: strict 52 -> 55; broader 60 -> 63. Both well above baselines (33 / 38); gate passes.
...
| ADR | Target doc | Notes |
| --- | --- | --- |
| 0003 no-fetch-cache | `docs/architecture/backend/core.md` (per redirect-index primary target) | no-fetch-cache rule; MIGRATING marker since `core/http.py` is deleted in B4 |
| 0018 wikipedia-district-name-resolution | `docs/architecture/backend/sources-wikipedia.md` | wiki name-resolution rule (note: 0033 retires the adapter; both live ADRs co-fold) |
| 0033 retire-wikipedia-districts-adapter | `docs/architecture/backend/sources-wikipedia.md` (same doc) | retirement rationale |
| 0046 pre-flight-ingest-gate-contract | `docs/architecture/backend/preflight.md` | deferred from D-DOC3.3 (its primary destination is `backend/preflight.md` per the redirect-index, not `canonical-store.md`); the six-check gate contract |
...
Per-row scope adjustments (made when this row was claimed, 2026-06-04): ADR-0046 was DEFERRED FROM D-DOC3.3 (its primary home is `backend/preflight.md` which is D-DOC3.8's write target, not D-DOC3.3's) and is folded HERE. The row label and Execution Ledger entry above are updated to LIVE: 0003, 0018, 0033, 0046, 0047-schema-version (five LIVE folds; one more than the original four in the spawn body). The redirect-index `docs/reference/decision-index.md` already pinned 0046's target anchor at `docs/architecture/backend/preflight.md#adr-0046-pre-flight-ingest-gate-contract`; the `Owned by` column is tightened in this PR from "D-DOC3.3 (or D-DOC3.8 if the fold-row authors against backend/)" to just "D-DOC3.8" now that this row is the realised owner.

Gate `grep-receipts-eq` running total (post-D-DOC3.7 main is 41 strict / 49 broader per filesystem audit 2026-06-04; baseline 33 / 38): this PR ADDS new `## Rejected alternatives` h2 sections to THREE target docs (core.md + preflight.md + schema-evolution.md; +3 strict + 3 broader). The fourth target sources-wikipedia.md ALREADY had `## Design rationale` AND `## Alternatives considered` h2 sections (both match the broader regex; the latter matches the strict regex too) so the ADR-0018 + ADR-0033 fold lands as ADDITIVE h3 subsections under those existing h2s (the h3 headers are `### ADR-0018 rejected alternatives` etc., which do NOT match either regex's anchored `^#{2,3}\s*(Rejected|Alternatives)...` because the literal `ADR-NNNN ` prefix sits between the hashes and the keyword; +0 strict, +0 broader on that file - mirrors the D-DOC3.7 additive fold of ADR-0036 into canonical-store.md). Net across this row: strict 41 -> 44; broader 49 -> 52. Both well above baselines (33 / 38); gate passes.
...
| D-DOC3.10 closure (CLAUDE.md rewrite + AGENTS.md sweep + decisions-dir DELETE + sub-plan archive + parent ledger flip) | #761 | this PR; 137 inbound-link rewrites across 44 LIVE docs (CLAUDE.md + docs/* + named AGENTS.md files); 38 ADR files + README.md deleted from `docs/architecture/decisions/`; sub-plan `git mv`'d to this archive location; parent D-DOC3 row flipped to MERGED |
