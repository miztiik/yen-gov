...
**Status**: COMPLETE (archived 2026-06-06 in F2b.8 closure PR)
...
- **F2a CategoryBar consolidation** (parent F2a / `OrderedCategoryBar` + `HorizontalGroupedBar` + `composition-bar/` -> `CategoryBar{ranked,stacked,diverging}`): MERGED ([archive](../../archive/plans/20260605-f2a-categorybar-consolidation-subplan.md), PRs #781 + #782 + #784 + #785 + #786 + #787 closure). F2b builds on F2a's discriminated-union pattern for the GeoChoropleth `mode: "fill" | "symbol"` extension.
...
| F2b.6 CirclePack renderer (modes pack + bubble) + sandbox demo + section-13 smoke | F2b.2 | F2b.3, F2b.4, F2b.5 | vitest(pack layout summary, mode discriminator) + section-13 in-browser smoke (sandbox) | #796 | MERGED |
| F2b.7 GeoChoropleth{symbol} mode extension + sandbox demo + section-13 smoke | F2b.3 | F2b.4, F2b.5, F2b.6 (file-disjoint - extends F2b.3 only) | vitest(centroid-by-feature, sqrt-area scale, glyph fallback) + section-13 in-browser smoke (sandbox) | #797 | MERGED |
| F2b.8 closure (distil into design-system.md F2b row; flip parent F2b ledger; archive this sub-plan) | F2b.2, F2b.3, F2b.4, F2b.5, F2b.6, F2b.7 | - | docs-review | #798 | MERGED |
...
- Distil the F2b renderer set + the C2/C3/C5 primitive shape into [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) section "F2b - new renderers + map primitives". Add See-also entries for this archived sub-plan + the F4 island-render-smoke contract that GeoChoropleth consumes.
- Flip the parent F2b ledger row to MERGED in this same PR; stamp the closure PR# + the per-sub-row PR#s.
- Archive this sub-plan to `docs/archive/plans/20260606-f2b-new-renderers-subplan.md` with a "Plan complete" block per [docs/how-to/distill-a-plan.md](../../how-to/distill-a-plan.md).
...
- [docs/archive/plans/20260605-f2a-categorybar-consolidation-subplan.md](../../archive/plans/20260605-f2a-categorybar-consolidation-subplan.md) - the F2a sub-plan F2b builds on (discriminated-union pattern + sandbox seam).
- [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) - the canonical chart-architecture home; F2b distillation lands here at F2b.8 closure.
- [docs/reference/chart-index.md](../../reference/chart-index.md) - the 12-member ChartType contract + drift gate. U4 pre-loaded all F2b Machine ids; F2b makes the rows LIVE.
...
All eight sub-rows MERGED:

| Sub-row | PR# | Description |
| --- | --- | --- |
| F2b.1 spawn sub-plan | #789 | Authored this sub-plan + parent ledger DEFERRED-TO-SUBPLAN stamp. |
| F2b.2 C2 + C3 + C5 + color-scale | #790 | Primitives + shared `binnedSequential` / `sqrtAreaScale` helpers. |
| F2b.3 GeoChoropleth{fill} | #793 | d3-geo SVG static welfare-map renderer; consumes F4 topojson island-render smoke. |
| F2b.4 Matrix | #794 | Entity x time heatmap; shares ColorScale with GeoChoropleth per parent §14.5 #5. |
| F2b.5 Treemap | #795 | d3-hierarchy tiled part-to-whole; sqrt-area HONESTY; adds d3-hierarchy first-class dep. |
| F2b.6 CirclePack | #796 | d3-hierarchy pack() with `mode={"pack" \| "bubble"}` discriminator; same HONESTY rule. |
| F2b.7 GeoChoropleth{symbol} | #797 | Mode extension to F2b.3 via geoCentroid + sqrtAreaScale; F2a discriminated-union pattern repeated. |
| F2b.8 closure (THIS PR) | #798 | Distil into `docs/architecture/frontend/design-system.md` F2b row + subsection; flip parent F2b ledger to MERGED; archive this sub-plan to `docs/archive/plans/`. |

Per [docs/how-to/distill-a-plan.md](../../how-to/distill-a-plan.md) the durable findings (renderer seam shapes, ColorScale-sharing doctrine, discriminated-union pattern repeats, sqrt-area HONESTY rule, sandbox-as-§13-smoke-surface, additive-rule preservation) are lifted into [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) section "F2b - new renderers + map primitives". This sub-plan stays as a thin audit ledger with back-pointers per the same how-to.

Parent ledger row F2b in [TODO/20260603-data-and-charting-platform-reset-plan.md](../../../TODO/20260603-data-and-charting-platform-reset-plan.md) flips from `DEFERRED-TO-SUBPLAN #789` to `MERGED #789 + #790 + #793 + #794 + #795 + #796 + #797 + #798` in this same PR.

Downstream rows now unblock:
- **E3 state silhouette on StateAcMap + TileCartogram** (parent ledger, blocks on F2b): READY for the next session.
- All four new renderers (`GeoChoropleth{fill,symbol}`, `Matrix`, `Treemap`, `CirclePack`) are sandbox-only for v1; the production-route migration from maplibre (`IndicatorChoropleth.svelte` / `MapChoropleth.svelte`) is a separate post-F2b strangler-fig chunk.

Operational lessons captured for the next agent (lifted to `/memories/session/` not `docs/`):
- Stacked PRs onto a sub-plan tip work cleanly when each rebase-onto-main skips the already-merged predecessor (`hint: skipped previously applied commit`).
- Sub-plan ledger flips bundled into the FOLLOWING PR's commit beat per-merge stamp PRs - one less PR per sub-row, no race condition with parent PR-author-cannot-stamp-own-PR limitation.
- `gh pr merge --admin --delete-branch` after vitest + build SUCCESS (Playwright still in progress) is the established F-track admin-merge precedent (matches U5b / U5d / F2a.5 from session memory).
- Pre-existing characters (box-drawing `\u2500`, em-dash `\u2014`, `\u00d7` etc.) in `DevChartsSandbox.svelte` are NOT retroactive-fix candidates; new ASCII text is the standard going forward per CLAUDE.md §5.
- d3-geo `geoCentroid` returns the antipodal centroid for clockwise polygons (test fixture gotcha; rings must be counter-clockwise exterior).
