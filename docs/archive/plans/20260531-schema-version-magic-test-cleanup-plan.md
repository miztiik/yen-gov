...
- [docs/agents/bootstrap.md](../../agents/bootstrap.md) - load before any execution turn.
- [docs/agents/guardrails.md](../../agents/guardrails.md) - authority, git hygiene, and stop conditions.
- [docs/architecture/testing.md](../../architecture/testing.md) - test-tier policy and fixture conventions.
- [docs/architecture/backend/validator.md](../../architecture/backend/validator.md) - validator Tier A / Tier B split.
- [docs/architecture/data/canonical-store.md](../../architecture/data/canonical-store.md) - canonical store and runtime-reader contract.
- [docs/concepts/data-provenance.md](../../concepts/data-provenance.md) - citation-ledger provenance; no fetch telemetry in `taxonomy.sources`.
- [docs/architecture/frontend/charts/source-list-v2.md](../../architecture/frontend/charts/source-list-v2.md) - frontend render surface for citation-ledger sources.
- [docs/architecture/decisions/0032-sources-citation-ledger.md](../../architecture/decisions/0032-sources-citation-ledger.md) and [docs/architecture/decisions/0042-sources-schema-v3-vintage-as-period-anchor.md](../../architecture/decisions/0042-sources-schema-v3-vintage-as-period-anchor.md) - source rows are citation identity, not fetch events.
- [docs/architecture/decisions/0045-grapher-catalogue-split.md](../../architecture/decisions/0045-grapher-catalogue-split.md) - render hints live in grapher, not canonical indicator schema.
- [TODO/20260530-schema-version-compatibility-plan.md](../../../TODO/20260530-schema-version-compatibility-plan.md) - related compatibility policy plan. This plan is narrower: stale current-version literals and tests.
...
| Active tools emit stale indicator schema | [tools/livestock_meadow_naip_iv.py](../../../tools/livestock_meadow_naip_iv.py), [tools/livestock_meadow_owner_reg.py](../../../tools/livestock_meadow_owner_reg.py), [tools/livestock_meadow_pashu_aadhaar.py](../../../tools/livestock_meadow_pashu_aadhaar.py), RBI HBS-IE state SDP producer | Fix active emitters. Tools must not import backend runtime; source schema version through schema JSON or a backend adapter with tests. |
...
```text
runSubagent(<agent>, "Review Row <row id> of docs/archive/plans/20260531-schema-version-magic-test-cleanup-plan.md. Do not edit. Return must-fix risks, exact files to touch/avoid, and gates to run.")
```
...
[frontend/src/lib/canonical/indicator-from-canonical.ts](../../../frontend/src/lib/canonical/indicator-from-canonical.ts) currently degrades `taxonomy.sources.parquet` citation-ledger rows into legacy `IndicatorSource` rows with `fetched_at: ""`. That is not an honest route to current `indicator.schema.json`: canonical source rows intentionally do not carry fetch telemetry per [docs/concepts/data-provenance.md](../../concepts/data-provenance.md), [ADR-0032](../../architecture/decisions/0032-sources-citation-ledger.md), and [ADR-0042](../../architecture/decisions/0042-sources-schema-v3-vintage-as-period-anchor.md).
...
- **Files likely touched**:
  ...
  - [docs/architecture/frontend/charts/source-list-v2.md](../../architecture/frontend/charts/source-list-v2.md), if canonical indicators become a SourceListV2 adopter.
- **Required checks**:
...
2026-05-31 livestock slice merged as PR #513 on `feat/tools-schema-version-livestock`:

- Fixes [tools/livestock_meadow_naip_iv.py](../../../tools/livestock_meadow_naip_iv.py), [tools/livestock_meadow_owner_reg.py](../../../tools/livestock_meadow_owner_reg.py), and [tools/livestock_meadow_pashu_aadhaar.py](../../../tools/livestock_meadow_pashu_aadhaar.py) to source `indicator.schema.json` `$id` and `x-version` from the schema file, not hand-typed `4.4` literals.
- Updates livestock tests so newly built docs assert current schema metadata through the backend registry, while existing on-disk meadow files may declare a compatibility-accepted older minor version.
- Left the RBI HBS-IE state SDP producer and backend producer constants for later Row D slices after shape classification; the later ICED and RBI bullets below close the actionable indicator-emitter work.

2026-05-31 ICED backend slice merged as PR #515 on `feat/iced-schema-version-emitter`:

- Fixes [backend/yen_gov/sources/iced_power/fetch_pipeline.py](../../../backend/yen_gov/sources/iced_power/fetch_pipeline.py) to source the ICED meadow payload `$schema` and `$schema_version` through the backend schema registry.
- Adds a focused fake-client test proving `_build_meadow_payload()` and `fetch_plant_pipeline_info()` stamp current indicator schema metadata and that the representative payload validates against the current indicator schema.
- Leaves the synthetic `6.0` fixture in [backend/tests/test_capacity_pipeline_envelope.py](../../../backend/tests/test_capacity_pipeline_envelope.py) intact because it is historical fixture input, not a current producer stamp.

2026-05-31 RBI state SDP backend slice merged as PR #518 on `feat/rbi-state-sdp-adapter`:

- Retires the stale `tools/` state-GDP script instead of importing backend runtime from `tools/` or bumping its old literals.
- Adds [backend/yen_gov/sources/rbi_hbs_ie_state_sdp/ingest.py](../../../backend/yen_gov/sources/rbi_hbs_ie_state_sdp/ingest.py), which sources `indicator.schema.json` metadata through the backend schema registry and writes through `core.io.write_artifact`.
- Adds [backend/tests/test_sources_rbi_hbs_ie_state_sdp.py](../../../backend/tests/test_sources_rbi_hbs_ie_state_sdp.py) with in-memory workbook fixtures proving current schema stamps, latest-base collapse, all-India per-capita retention, and absence of v6-removed render fields.
- Documents the adapter in [docs/architecture/backend/sources-rbi-hbs-ie-state-sdp.md](../../architecture/backend/sources-rbi-hbs-ie-state-sdp.md).
- Hosted public citizen Playwright passed before merge. The non-admin pytest CI failure was a known baseline unrelated to the RBI adapter and was documented on PR #518.
...
- **Files likely touched**:
  ...
  - [backend/yen_gov/sources/rbi_hbs_ie_state_sdp/ingest.py](../../../backend/yen_gov/sources/rbi_hbs_ie_state_sdp/ingest.py)
...
- **Implementation rule**:
  ...
  - Tools use schema JSON reading, because [docs/agents/guardrails.md](../../agents/guardrails.md) forbids `tools/` importing backend runtime modules.
...
2026-05-31 Row E implementation merged as PR #516 on `feat/preflight-schema-version-test`:

- Fixes [backend/yen_gov/preflight/\_\_init\_\_.py](../../../backend/yen_gov/preflight/__init__.py) to source the preflight report schema version from the backend schema registry.
- Replaces current point literals in grapher catalogue, source row, and preflight CLI/report tests with relationships against `schema_version(...)`.
- Keeps bad-version and historical fixture literals intact.
- Targeted gate after rebasing on PR #515: `pytest -q backend/tests/test_grapher_catalogue_schema.py backend/tests/test_source_row_v2.py backend/tests/test_preflight_cli.py backend/tests/test_preflight_report_schema.py backend/tests/test_schema_registry.py` -> `38 passed`.
...
2026-05-31 Row F implementation merged as PR #517 on `feat/indicator-schema-test-cleanup`:

- Removes the stale current row-count sentinel from [backend/tests/test_indicator_catalogue_schema_v21.py](../../../backend/tests/test_indicator_catalogue_schema_v21.py); the test now proves every catalogue row declares a positive `update_period_days`.
- Keeps historical changelog checks where the literal is deliberately historical input.
- Adds a v6 regression test proving the canonical indicator schema does not declare `default_mode`, `facet_labels`, or `renderer_rules` on the indicator block.
- Targeted gate after rebasing on PR #516: `pytest -q backend/tests/test_indicator_schema_v5.py backend/tests/test_indicator_catalogue_schema_v2.py backend/tests/test_indicator_catalogue_schema_v21.py backend/tests/test_indicator_catalogue_schema_v22.py` -> `42 passed`.
...
2026-05-31 trigger fired during PR #513 CI: non-admin pytest failed in the five national `remove_stale_shards()` tests because `Path.rglob()` kept walking parent directories after the helper deleted/pruned them. Fowler reviewed the trigger and recommended a separate Row G PR with an eager shard-list snapshot before mutation.
...
- This plan is archived at [docs/archive/plans/20260531-schema-version-magic-test-cleanup-plan.md](20260531-schema-version-magic-test-cleanup-plan.md) with the plan-complete map below.
...
## 9. Final residual-literal classification

Closure grep was run after #518 merged. Remaining explicit schema-version literals are classified as follows:

| Class | Examples | Closure verdict |
| --- | --- | --- |
| Synthetic fixtures | [backend/tests/test_core_events.py](../../../backend/tests/test_core_events.py), coverage/preflight helper payloads | Keep. These tests construct small envelopes disconnected from current schema JSON. |
| Bad-version fixtures | `9.9` rejection cases in backend and frontend tests | Keep. These prove validators reject unsupported versions. |
| Historical/backcompat and migration inputs | indicator migration tests, `tools/bump_indicator_schema_1_3_to_1_4.py`, `tools/migrate_indicators_v15_to_v20.py`, sidecar bump script | Keep. The literal is the behavior input or output of a versioned migration. |
| Row-schema constants | ECI event/backfill/pipeline/adapters with `schema_version="1.0"` row envelopes | Keep. These are row/envelope schema constants, not current indicator-schema pins. Future row-schema registry work should treat them under its own plan. |
| Boundary schema stamps | boundary snapshot tests and constituency bootstrap tools using `4.1` | Keep. Boundary snapshot schema literals are outside this indicator/current-schema cleanup and belong to boundary schema evolution work. |
| Frontend contract sentinels | [frontend/src/contracts/sources-v2-shape.test.ts](../../../frontend/src/contracts/sources-v2-shape.test.ts) | Keep. The source schema `3.0` assertion is an ADR-0042 contract sentinel paired with relationship tests and field-discipline checks. |
| Frontend/runtime fixtures | manifest, governments, state tiers, election events, grapher/catalogue fixtures | Keep unless a future PR finds one is a current producer policy pin. The execution rows already removed or centralized the active current indicator-emitter pins. |

No remaining row in this plan is open/actionable.

## 10. Plan complete

| Row | PR | Durable output |
| --- | --- | --- |
| A | #501 | Authored this PR-sized execution plan and locked the classification-first approach. |
| B | #503 | Distilled schema-version-in-tests doctrine into [docs/architecture/testing.md](../../architecture/testing.md), [docs/architecture/backend/validator.md](../../architecture/backend/validator.md), and the root operating guidance. |
| C0 | #506 | Split frontend indicator cleanup into source-surface, render-hint, schema-id, and version-policy rows so no fake provenance or dishonest schema stamp shipped. |
| C1 | #507 | Routed canonical-backed indicator provenance through SourceListV2 citation-ledger rows instead of fake legacy fetch telemetry. |
| C2 | #509 | Removed v6-deleted render-hint carryover from canonical indicator artifacts while preserving rendering behavior through allowed frontend/grapher surfaces. |
| C2.5 | #511 | Widened indicator schema id grammar to accept canonical kebab ids and documented the compatibility/evolution release. |
| C3 | #512 | Centralized frontend emitted-indicator schema-version policy and removed the stale `4.4` artifact stamp. |
| D | #513, #515, #518 | Fixed active indicator emitters: livestock tools now read schema JSON, ICED backend uses schema registry, and RBI state SDP moved from stale tools script to a backend adapter. |
...
- [docs/agents/guardrails.md](../../agents/guardrails.md)
- [docs/architecture/testing.md](../../architecture/testing.md)
- [docs/architecture/backend/validator.md](../../architecture/backend/validator.md)
- [docs/architecture/data/canonical-store.md](../../architecture/data/canonical-store.md)
- [docs/architecture/decisions/0045-grapher-catalogue-split.md](../../architecture/decisions/0045-grapher-catalogue-split.md)
- [TODO/20260530-schema-version-compatibility-plan.md](../../../TODO/20260530-schema-version-compatibility-plan.md)
