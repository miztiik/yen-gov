...
**Parent**: [TODO/20260603-data-and-charting-platform-reset-plan.md](../../../TODO/20260603-data-and-charting-platform-reset-plan.md) chunk U2
...
| U2d closure (distil into `docs/architecture/frontend/design-system.md` U2 row; flip parent U2 ledger; archive this sub-plan) | U2a, U2b, U2c | - | docs-review | _pending_ | MERGED |
...
6. Post-merge cleanup per [docs/how-to/ship-a-pr.md](../../how-to/ship-a-pr.md).
...
| --- | --- | --- |
| U2a url.district + `/s/:state/d/:district` route + minimal `District.svelte` + `url.test.ts` assertions | #739 | [frontend/src/lib/url.ts](../../../frontend/src/lib/url.ts) `url.district` builder + [frontend/src/main.ts](../../../frontend/src/main.ts) `/s/:state/d/:district` + `/sd/:subdistrict` route registrations + [frontend/src/routes/District.svelte](../../../frontend/src/routes/District.svelte) + [frontend/src/lib/url.test.ts](../../../frontend/src/lib/url.test.ts) grammar assertions; distilled into [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) section "U2 - GeoBreadcrumb + glass app bar + district URL node" (the U2a paragraph) and [docs/architecture/frontend/url-grammar.md](../../architecture/frontend/url-grammar.md) (district node added under the place-first grammar) |
| U2b `GeoBreadcrumb.svelte` + integration into Home / StateOverview / StateTopic / Constituency / District | #742 | [frontend/src/lib/GeoBreadcrumb.svelte](../../../frontend/src/lib/GeoBreadcrumb.svelte) (sticky glass primary-nav spine; `computeCrumbs()` pure helper) + [frontend/src/lib/GeoBreadcrumb.test.ts](../../../frontend/src/lib/GeoBreadcrumb.test.ts) (vitest unit tests on the crumb chain) + 5-route integration in [Home.svelte](../../../frontend/src/routes/Home.svelte) / [StateOverview.svelte](../../../frontend/src/routes/StateOverview.svelte) / [StateTopic.svelte](../../../frontend/src/routes/StateTopic.svelte) / [Constituency.svelte](../../../frontend/src/routes/Constituency.svelte) / [District.svelte](../../../frontend/src/routes/District.svelte); distilled into [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) section "U2 - GeoBreadcrumb + glass app bar + district URL node" (the U2b paragraph). Trailing sibling-jump popover deferred per the stop-and-surface trigger; ascend-only chain is the load-bearing spine. |
| U2c LeftRail re-cluster (same-side fix + glass app bar + spring drawer + token migration + ballot-motif retone + `build <sha>` footer) | #745 | [frontend/src/app-tokens.css](../../../frontend/src/app-tokens.css) `--brand-saffron` / `--brand-green` / `--brand-chakra` / `--app-bar-bg` + [frontend/tailwind.config.js](../../../frontend/tailwind.config.js) Tailwind mirrors + [frontend/src/contracts/app-tokens.test.ts](../../../frontend/src/contracts/app-tokens.test.ts) drift-contract token-count bump (34 -> 38) + [frontend/src/lib/LeftRail.svelte](../../../frontend/src/lib/LeftRail.svelte) same-side cluster + glass bar + spring drawer + token migration + `build <sha>` footer + [frontend/vite.config.ts](../../../frontend/vite.config.ts) `VITE_BUILD_SHA` define; distilled into [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) token map (4 new tokens) + section "U2 - GeoBreadcrumb + glass app bar + district URL node" (the U2c paragraph) |
| U2d closure (archive + design-system.md U2 row + GeoBreadcrumb section + token-map additions + parent ledger flip) | _pending_ | this file under `docs/archive/plans/` + parent section 22.5 U2 row flipped to MERGED + [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) per-component table U2 row flipped to MERGED + 4 token-map rows added + "U2 - GeoBreadcrumb + glass app bar + district URL node" section appended + drift-contract count updated 34 -> 38 |

Lessons that did not belong in `docs/` were captured in `/memories/lessons.md` under the U2a / U2b / U2c dated entries (parallel-agent dirty-branch isolation via explicit-path staging, `computeCrumbs` pure-helper testability pattern, in-CSS data-URL retone comment-receipt convention, stamp-PR shipping cadence on `TODO/*.md`-only diffs).

The ADDITIVE rule still holds at U2 closure: no Tailwind default was redefined, no existing component was migrated outside LeftRail, no existing URL changed. The four new tokens (`--brand-saffron` / `--brand-green` / `--brand-chakra` / `--app-bar-bg`) are reachable by name from any later component; U4 / U5 consume them as they migrate their own surfaces.

## See also

- [TODO/20260603-data-and-charting-platform-reset-plan.md](../../../TODO/20260603-data-and-charting-platform-reset-plan.md) sections 21.8 + 23.5 - the design-spec source.
- [docs/archive/plans/20260604-u1-tokens-fonts-subplan.md](20260604-u1-tokens-fonts-subplan.md) - the U1 precedent (4 sub-rows, same shape).
- [docs/architecture/frontend/design-system.md](../../architecture/frontend/design-system.md) - the distillation home for U2's per-component migration row.
- [docs/architecture/frontend/url-grammar.md](../../architecture/frontend/url-grammar.md) - the URL grammar contract (ADR-0028 / ADR-0037 / ADR-0048 / ADR-0050 receipts; U2a extends with the district node).
