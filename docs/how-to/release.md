...
4. Branch, commit, push, open PR. Path A from here on (`deploy-site.yml` -> merge -> auto-deploy from the same workflow on green main; `backend.yml` also runs because `datasets/**` changed, but does not gate the publish).

## Deploy timing

Pages deploy follows the workflow, not a separate schedule:

- Automatic: every push to `main` runs `deploy-site.yml`. On green, the `deploy-pages` job publishes the bundle this run built. A burst of N commits collapses naturally via `concurrency.cancel-in-progress: true` keyed on the ref - the in-progress runs cancel and only the latest completes.
- Manual: dispatch `deploy-site.yml` from the Actions tab to re-publish the current `main` without a new commit (e.g. to test a Pages settings change).
...
- Independent: `backend.yml` (pipeline + admin checks) runs in parallel on commits that touch `backend/**`, `admin/**`, or `datasets/**`. Its result never blocks the publish - it is dev-only tooling.
...
2. The revert is a push to `main`, so `deploy-site.yml` runs against the revert commit. On green, `deploy-pages` publishes the reverted bundle automatically.
3. If you need it live before the workflow finishes (rare), dispatch `deploy-site.yml` manually after the revert merges - it just runs the same workflow again.
