...
ADR-0016 made ECI Statistical Reports the canonical past-election enrichment source and split the work into two phases: Phase A (recon — done; see `tools/eci_recon/recon.py` and `notes/eci-recon-2026-05-09.md`) and Phase B (download, parse, ingest). Phase A surfaced two facts that change Phase B's shape:
...
1. **Catalog**, per state, by calling `GET /eci-backend/public/api/election-result?category_id=<id>` once. The `category_id` per (state, year) is harvested from the React bundle's `/statistical-reports` table and pinned in `backend/yen_gov/sources/eci/categories.py` as a `dict[(state, year), int]` — extending it requires a code change with the URL the value came from in a comment, not a config file. Phase A confirmed `S22 × 2026 → 26`.
...
- `tools/eci_recon/recon.py` and `notes/eci-recon-2026-05-09.md` — Phase A output that this ADR builds on
