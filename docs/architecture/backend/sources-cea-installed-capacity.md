...
- **Snapshot only — bounded by the upstream publisher.** This adapter ships one period per ingest run because that is the granularity CEA publishes for **per-state** capacity: a monthly Executive Summary workbook with current-month figures only. There is no per-state historical time-series workbook on `cea.nic.in`. Constructing one would require scraping the monthly archive (~360+ workbooks for 30 years), and the WordPress URL pattern (`wp-content/uploads/installed/<YYYY>/<MM>/Website-1.xlsx`) returns 200 only for the current month — older months are gated behind a JS-driven AJAX date picker we have not reverse-engineered. The all-India long-history analogue lives in the RBI Handbook of Statistics (~75 years of national capacity) — that companion is tracked separately in the long-coverage ledger; it does not live in this adapter because its grain is national, not per-state.
