...
> **DOCTRINE PARTIALLY STALE (post-B4 + post-G9).** B4-pt2.2 (#826) deleted the `eci-statreport` + `eci-statreport-emit` CLIs and B4-pt2.3 (#827) deleted `urls.py` + `statistical_report.py` + `static_catalog.py` (the network-fetch surface). G9 (2026-06-08) then retired `categories.py` + `config/eci-pins.json` + `datasets/schemas/eci_pins.schema.json` as orphan code with zero remaining importers. The only operator-facing CLI that survives is `python -m yen_gov eci-statreport-emit-local <xlsx-path>` (XLSX-on-disk emit; reads no config). The narrative below is preserved as historical design doctrine for the ECI statistical-report ingest and the surviving parsers (`partywise.py`, `constituencywise.py`, `statistical_report_detailed.py`); references to the deleted modules + config + CLIs read as background, not as operator instructions. A full rewrite belongs in the post-B4 doc sweep, not in G9.
