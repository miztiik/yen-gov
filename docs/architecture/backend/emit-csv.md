...
4. **Committed to git.** Same reasoning as the pre-PR-R.3 SQLite emitter ([ADR-0014](../../reference/decision-index.md), now superseded): symmetric with JSON, byte-deterministic, small enough to track (~50–150 KB per state). The deploy step already does `cp -r datasets _site/data`.
...
| `ac_eci_no` | per-AC JSON `.eci_no` | canonical name per [ADR-0019](../../reference/decision-index.md) |
...
- [ADR-0014](../../reference/decision-index.md) — derived-artifact philosophy (now superseded by canonical Parquet + DuckDB-WASM; the CSV emitter still follows the same patterns).
- [ADR-0019](../../reference/decision-index.md) — column naming.
