...
Use [`tools/datagovin_download_resource.py`](../../../tools/datagovin_download_resource.py) for the repeatable path. It fetches metadata first, resumes from `<output>.partial`, writes the header once, validates page headers, and replaces the final CSV only after the assembled row count equals the API `total`.

Example shape:

```powershell
$env:DATAGOVIN_API_KEY = "<key from user or API tab>"
python tools/datagovin_download_resource.py <resource_uuid> `
	--output .runtime/raw/datagovin/<leaf>.csv `
	--limit 10000 `
	--delay-seconds 60
Remove-Item Env:\DATAGOVIN_API_KEY
```

Do not pass the key as a command-line argument; command lines are easy to leak through process listings and logs.
...
- [`tools/datagovin_download_resource.py`](../../../tools/datagovin_download_resource.py) — paged CSV downloader.
