...
- `core/models.py` defines one pydantic v2 `BaseModel` per `*.schema.json` file. Naming follows readability over schema-filename mechanical mapping (`result.constituency.schema.json` → `ConstituencyResult`, `district.schema.json` → `DistrictsCollection`), since the file *contains* a collection. The historical `state.schema.json` → `StatesCollection` mirror was retired in Phase C of the strangler-fig closeout (the state + UT roster now lives on `datasets/taxonomy/entities.json` and backend consumers read the JSON directly via `_load_states_from_entities`).
