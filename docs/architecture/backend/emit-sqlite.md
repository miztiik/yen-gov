...
`backend/yen_gov/emit/` writes the per-state `results.sqlite` artifact at `datasets/elections/<event>/<state>/results.sqlite`. SQLite is a **derived secondary artifact** built from the same validated `ConstituencyResult` records the canonical pipeline produces in-memory (primary API `emit_state_sqlite_from_data(parties_doc, constituencies, output_path)`, see [ADR-0029](../decisions/0029-canonical-store-pipeline-shapes.md)), intended for cross-cutting queries on the [`/explore`](../frontend/data-loading.md#the-explore-page-uses-duckdb-wasm) page. It is **not a contract surface** — no JSON Schema, no `x-version`/`x-changelog`. The layout is documented in [`docs/reference/sqlite-schema.md`](../../reference/sqlite-schema.md) and versioned via `PRAGMA user_version`. Column names follow [ADR-0019](../decisions/0019-dataset-topology-and-column-discipline.md) (canonical: `ac_eci_no`, `state_eci_code`, `district_lgd_code`, `year`).

> The per-state `result.summary.json` sibling that historically sat in the same directory retired in PR-O.4 (TODO row `1.8b-ii`, 2026-05-19), and the per-AC `results/<ac>.json` shards retired in PR-P (TODO row `1.8c`, 2026-05-19). The canonical Parquet at `datasets/elections/election_results.parquet` + the four dim tables are now the single source of truth for election data; this SQLite emitter is unaffected — it now reads directly from the in-memory `ConstituencyResult` objects the canonical pipeline produces and still ships under row 1.8e / PR-R as the Psephlab Compare backend. The disk-walking wrapper `emit_state_sqlite(state_dir=...)` is preserved only for ad-hoc replays against legacy checkouts that still carry the per-AC JSON shards.

## Decisions in one screen

1. **One `.sqlite` per event/state.** Path: `datasets/elections/<event>/<state>/results.sqlite`. Sole on-disk artifact in `datasets/elections/<event>/<state>/` after PR-P (TODO row `1.8c`, 2026-05-19) alongside the sibling `results.csv`. The per-AC `results/<ac>.json` shards no longer exist; the SQLite emitter is built directly from the canonical pipeline's in-memory `ConstituencyResult` list.
