...
3. **Committed to git.** Reasons:
   ...
   - The deploy step ([prod data placement](../frontend/data-loading.md#production-placement)) already does `cp -r datasets _site/data`. Adding a regeneration step in `deploy.yml` would couple deploy to a Python install.
   - A 234-row state DB is ~200 kB; even the full 36-state corpus is a few MB.
   - Reviewers see the JSON diff next to the opaque `.sqlite` diff in PRs from `pipeline.yml`. Combined, the PR is auditable.
