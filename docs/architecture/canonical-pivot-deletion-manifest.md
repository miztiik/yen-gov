...
| 1.8d (PR-Q) | `datasets/events/in/eci/` (27 files, 27 dirs) — DONE 2026-05-19. `datasets/taxonomy/delimitation_lineage.json`, `datasets/taxonomy/facet-axes.json`, `datasets/taxonomy/operator_state.parquet` move — DEFERRED to PR-Q.2 (parquet siblings don't yet exist; defer to atomic emit+retire). | `EVENTS` Python registry + citizen catalogue `election-events.json` (cross-validated by `test_election_events_catalogue_matches_backend_registry`); `dim_acs.(event_id,state)` columns. Also retired: one-shot tool `tools/register_2016_2023_event_metadata.py` + corpus-walker test `test_emitted_states_are_declared_in_event_metadata`. | git-grep + frontend/backend test pass | completed 2026-05-19 (events/in/eci only); siblings deferred to PR-Q.2 |
