...
| **T.0a** (Phase-0 closeout additive) | _Adds_: `datasets/taxonomy/{state_tiers,topics,election_events}.{json,parquet}`, `datasets/governments/{governments_office_holdings,dim_offices}.parquet`, `datasets/taxonomy/lgd/{states,districts}-YYYY-MM-DD.csv` + `*-latest.csv`, `datasets/elections/state=in_<XX>/election_results.parquet` partition layout. **Deletes nothing.** Legacy `datasets/reference/in/*` + `datasets/people/AcGenApr2021/` + per-state `results.csv` all remain on disk. | new tables emitted by `backend/yen_gov/canonical/{state_tiers,topics,election_events,cm_terms}_seed.py` (Python-compiles-to-Parquet pattern per canonical-store.md §8.3); writer adds `FAMILY_FACT_PARTITION_BY = {"elections": ["state"]}` analog to `FAMILY_FACT_TABLE_STEM`; manifest emits N `tables[].files[]` per partitioned family. | writer-partition tests on `tmp_path` (Holy Law #7); seed-tool round-trip tests per tool; parity oracle straddles partition switch (per-state slice from monolith vs partitioned must return identical row counts + identical FPTP winners); backend `pytest -q` green; legacy reader URLs still work because legacy bytes still present. | **planned** (TODO §0e.10.3) |
