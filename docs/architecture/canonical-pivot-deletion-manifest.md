...
**THE PLAN**: [`docs/archive/plans/20260517-canonical-long-format-pivot.md`](../archive/plans/20260517-canonical-long-format-pivot.md). On conflict, THE PLAN wins; update this manifest.
...
`constituency.schema.json`, `election.schema.json`, `election-events.schema.json`, `party.schema.json`, `result.summary.schema.json`, `result.constituency.schema.json`, `state.schema.json`, `subdistrict.schema.json`, `postal.schema.json`, `state-tiers.schema.json`, `csv.sources.schema.json`, `upstream-sources.schema.json`, `topic-catalogue.schema.json`, `feature_collection.metadata.schema.json`, `boundary-layers.schema.json`, `boundary.no-geometry-registry.schema.json`, `unmapped-regions.schema.json`, `elections-inventory.schema.json`, `elections-config.schema.json`, `office-holdings.schema.json`. (`state_government.schema.json` previously appeared here; retired in G.1.c on 2026-05-22 alongside the 31 per-state `cm_terms.json` files. `district.schema.json` previously appeared here; retired in T.0c-iii Phase D.3 along with the 6 per-state `districts.json` files — see ADR-0033. `boundary.sources.schema.json` + `boundary.unkeyed.schema.json` + `boundary.villages_index.schema.json` previously appeared here; retired in T.0d on 2026-05-22 — content folded into `boundary-layers.schema.json` backing the new `datasets/boundaries/boundary_layers.parquet` ledger; see ADR-0031 Amendment 2026-05-22.)
...
| **T.0a-ii** (Phase-0 closeout additive — seeds + LGD + districts, follow-up to T.0a) | _Adds_: `datasets/taxonomy/{state_tiers,topics,election_events}.{json,parquet}`, `datasets/governments/{governments_office_holdings,dim_offices}.parquet`, `datasets/taxonomy/lgd/{states,districts}-YYYY-MM-DD.csv` + `*-latest.csv`, district rows lifted into `datasets/taxonomy/entities.parquet` (`entity_type='district'`, PK=`lgd_code`). **Deletes nothing legacy.** Legacy `datasets/reference/in/*` remains on disk pending T.0c. CM tenure source consolidated from 31 per-state `cm_terms.json` into one long-form `datasets/taxonomy/office_holdings.json` in G.1.c (2026-05-22). | New seed modules under `backend/yen_gov/canonical/` (`office_holdings_seed.py`, `topics_seed.py`, `election_events_seed.py`, `state_tiers_seed.py`) modelled on `facet_axes_seed.py` (Python-compiles-to-Parquet pattern per canonical-store.md §8.3); wired into `python -m yen_gov emit-taxonomy`. 6 new schemas: `dim-offices`, `governments-office-holdings`, `taxonomy-topics`, `taxonomy-indicator-topic-tags`, `taxonomy-election-events`, `taxonomy-state-tiers`. | seed-tool round-trip tests per tool on `tmp_path` (FK closure assertions for `office_holdings_seed` → `dim_offices`); Tier-A schema sanity on the 6 new schemas; backend `pytest -q` green; legacy reader URLs still resolve. | **planned** (deferred from T.0a; TODO §0e.10.3) |
...
- [THE PLAN](../archive/plans/20260517-canonical-long-format-pivot.md) — single source of truth.
