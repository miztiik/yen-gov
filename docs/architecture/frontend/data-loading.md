...
**Explicit read options (DuckDB-WASM dialect + null-padding).** `columns={...}` pins the column TYPES but does NOT stop the deployed DuckDB-WASM build's dialect sniffer from running - and that sniffer mis-detects CSVs whose text fields carry commas plus RFC-4180 doubled-quote escapes (`taxonomy.sources`, the per-indicator `datapoints`, and the election `candidacies.csv` candidate-name / education fields), failing the whole query with "could not detect the CSV parsing dialect". Every `read_csv(...)` therefore splices explicit options after the columns clause: `header=true, auto_detect=false, delim=',', quote='"', escape='"'`. The election reads ([`view-models/election-results.ts`](../../../frontend/src/lib/view-models/election-results.ts)`::withReadOpts`) additionally pass `null_padding=true`: `columns.json` declares 24 columns for the candidacies file class (4 trailing affidavit fields) but 923 of 924 on-disk files carry only the base 20 columns, so the affidavit columns are NULL-padded for the events without affidavit data and read natively for the one (parliament 2014) that carries them. See [`canonical/election-results-csv.ts`](../../../frontend/src/lib/canonical/election-results-csv.ts) for the original `auto_detect=false` precedent (PR #1098).
