...
- **(QR4) Add a `<select>` event picker on `/s/<state>/t/elections` next to the header card, defaulting to latest.** Rejected — would duplicate the picker contract already present on `/s/<state>` (the per-state event picker per ADR-0023 Addendum 2026-05-13). The collapsed `<details>` "Other elections on file" list on the topic page is the lightweight discovery surface, not a control. **Premise update (2026-06-02, EGC-A2/A3 — [ADR-0048 Addendum](../decisions/0048-elections-drill-ia-and-tile-cartogram.md#addendum-2026-06-02-topic-doors-mount-the-experience-supersedes-s1-topic-as-card)):** the "the topic page is a *topic*, not the elections landing" reasoning that originally justified this rejection no longer holds — the topic door now **mounts the full experience** (map + equal-seats cartogram + time-slider + filter rail) at the default event, and the experience carries its own event-selection control. The `<select>`-next-to-a-card design is still rejected, but now because the experience's own slider/event control supersedes it, not because the topic door is "just a topic". Event switching happens inside the mounted experience (which navigates the `:event` permalink / updates local state via `onSelectEvent`), not via a bolted-on `<select>`.
...
- **Render election results inline on the topic page (party-bar / seats-donut / turnout %).** **DONE 2026-06-02 (EGC-A2 + EGC-A3, [ADR-0048 Addendum](../decisions/0048-elections-drill-ia-and-tile-cartogram.md#addendum-2026-06-02-topic-doors-mount-the-experience-supersedes-s1-topic-as-card)).** The topic doors no longer show event identity only. `/s/<state>/t/elections` mounts the shared `StateElectionExperience` (results map + equal-seats cartogram + time-slider + filter rail) at the state's default assembly event, and the national `/t/elections` mounts `NationalElectionsAtlas` at the default Lok Sabha event. The Tier-A drift guard the deferral asked for is `frontend/src/contracts/election-topic-experience.test.ts` (asserts the topic door and the event route mount the same component and resolve the same default via the shared resolver, so the two surfaces cannot drift). This shipped the **full** experience rather than the originally-scoped "summarised preview", so the party-bar/seats-donut/turnout preview is no longer a separate item.
- **Promote turnout %, women candidates %, party-share etc. to first-class `indicator` artifacts under the elections topic.** Today these live as derivable facts in the per-event Parquets but not as catalogue entries the StateTopic renderer can dispatch on. Promoting them would let the elections topic render `[election header card] + [turnout choropleth] + [women-elected ranked list] + ...` via the existing polymorphic dispatch, with zero new component code. Deferred because the canonical-store pivot for elections (docs/archive/plans/20260517-canonical-long-format-pivot.md) needs to land first so the indicators have a proper Parquet home.
- **Topic-grid `/t/elections` page (cross-state).** **DONE 2026-06-02 (EGC-A3, [ADR-0048 Addendum](../decisions/0048-elections-drill-ia-and-tile-cartogram.md#addendum-2026-06-02-topic-doors-mount-the-experience-supersedes-s1-topic-as-card)).** The global `/t/elections` page now mounts `NationalElectionsAtlas` at the default Lok Sabha event (national PC choropleth + equal-seats cartogram + filter rail), with an "Other Lok Sabha elections on file" list below. `TopicLanding.svelte` resolves the default via `defaultNationalLokSabhaEvent`; `/t/elections/:event` remains the per-cohort permalink.
