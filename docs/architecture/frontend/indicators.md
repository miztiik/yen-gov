...
yen-gov began as an election-data viewer. The **indicator system** is the contract that lets it grow into a place-first comparison site without per-indicator UI code. Topic membership lives in the catalogue; renderer choice lives in metadata; the page code stays generic.
...
| `cadence` | (v4.1+, optional) Publisher release cadence — `annual_cy`/`annual_fy`/`quarterly_*`/`monthly`/`weekly`/`daily`/`decennial`/`ad_hoc`. **Distinct from `time_grain`** (which is the resolution of one row's `time` token). Drives the temporal-range caption: `decennial`/`ad_hoc` suppress any gap/completeness pill because the cadence is undefined. Per [ADR-0027](../../reference/decision-index.md). |
...
The helper is the single seam: both `buildIndicatorArtifact()` (single descriptors) and `loadFacetMultiplexedFromCanonical()` (RPO-style fused artifacts) read `admin_level` from it instead of hard-coding `"state"`. A load-bearing contract test asserts the round-trip for every entry in `CANONICAL_BACKED_INDICATORS`. This retires the regression class introduced by [ADR-0043](../../reference/decision-index.md) — once a single canonical adapter started emitting BOTH state-grain (SUM rollup) AND district-grain (source-of-truth) rows under different `entity_kind`s, every state-only literal in the reader path became a bug waiting for the first district descriptor.
...
**Future work (Jony / Hans).**

- **Election timeline strip on `/s/<state>/t/elections`.** A chronological visualisation showing all events on file for the state (chips along a date axis, current event highlighted) would be a stronger citizen surface than the current "header card + collapsed details" pattern. Promote when at least one state has four or more events on file.
...
- **Government-card cross-link on the StateElection page.** Per [ADR-0023 §Doctrine](../../reference/decision-index.md), election is the cause and government is the consequence. The new `/s/<state>/elections/<event>` page currently links forward to `/lab` (results) and `/compare` (cross-state); it does NOT link sideways to "the government this election produced" on `/s/<state>`. Deferred because the government-card on `/s/<state>` indexes by current term, not by elected-via-event, and the mapping is non-trivial for the cohort-event cases (Parliament 2024 elected MPs, not state governments). Needs an editorial decision before wiring.
