...
## Alliance totals panel: force ranking + pre-poll honesty

The Alliance totals panel on the per-event surfaces ([StateElection.svelte](../../../../frontend/src/routes/StateElection.svelte) + [NationalElection.svelte](../../../../frontend/src/routes/NationalElection.svelte)) tallies seats by pre-poll alliance. Component: [frontend/src/lib/elections/AllianceTotals.svelte](../../../../frontend/src/lib/elections/AllianceTotals.svelte); the join + ranking is a pure model at [frontend/src/lib/elections/alliance-totals-model.ts](../../../../frontend/src/lib/elections/alliance-totals-model.ts) (node-env unit-testable). Shipped 2026-06-24 (PR #1219) on a Max + Jony + Citizen verdict.

**Problem.** The panel ranked ONLY declared alliances and stamped the top one with a green "Winner" badge. When the single largest party is non-aligned it was buried inside the residual "Others" bucket, so a smaller declared alliance was crowned "Winner" - e.g. Tamil Nadu 2026, where TVK (108 seats) sat inside "Others (113)" while a 68-seat alliance got the badge. The superlative also contradicted the panel's own pre-poll caption (an alliance seat-share lead is not the formed government; government formation is frequently a post-poll fact in India).

**Decision - rank FORCES, not declared alliances.** A *force* is a declared pre-poll alliance (seats summed over members) OR a single non-aligned party large enough to stand on its own. The promotion rule is RELATIONAL and parameter-free (no hardcoded threshold, Holy Law #6): a non-aligned party is promoted to its own force when its seats are `>=` the seats of the SMALLEST declared alliance; everything smaller falls to the residual "Others" bucket. Forces rank by seats desc (name asc tie-break, deterministic). The headline reads the top two forces; rank 3+ are tail chips with "Others" last. A promoted lone-party force carries the data-state sub-label "no pre-poll alliance recorded" - NOT "contested alone", which is a world claim; the absence is a CURATION state (the alliance ledger at `datasets/data/entities/party_alliances.csv` is known-partial, see [office-holders.md](../../../concepts/office-holders.md): an alliance is a free-text curated label, so a bloc-of-one is as real a competitive unit as a coalition). The rule is uniform across every event; no per-state special-casing.

**Honesty framing.**

- No "Winner / Runner-up" superlative. Eyebrows are seat-rank-neutral ("Most seats" / "Second").
- The emerald "majority" accent is earned ONLY when a force holds `floor(total/2)+1` seats (the same Indian-legislature majority rule the seat arc uses - see [psephlab.md](../psephlab.md), e.g. 118 of 234 in Tamil Nadu, not 117). A plurality leader gets a neutral accent and a "No bloc holds a majority (N of M seats needed)" line.
- `share_pct` is SEAT-share over ALL seats won (incl. "Others"), never vote-share (the alliance model does not carry votes).
- The pre-poll honesty caption is mandatory and unchanged; when the `(event_id, state)` lookup returns zero alliance rows the whole panel is SUPPRESSED (no "pending" pill), per the R6 silence-on-uncurated rule pinned by [AllianceTotals.no-pending-pill.test.ts](../../../../frontend/src/lib/elections/AllianceTotals.no-pending-pill.test.ts).

**Breakdown + per-alliance mute.** The expanded breakdown is one stacked seat-bar per group (each segment a member party in its brand colour via `getPartyColor`) above a swatch list; the long "Others" tail collapses to the top 6. Each group carries a mute eye that folds every member key into the page's existing `hidden_parties` set in one pass, so muting an alliance recedes all its parties across the seat arc + AC/PC maps at once. Mute is VISUAL-ONLY - seat counts never recompute - with partial-state handling and a "Show all (N muted)" reset. The route wiring (`hidden_keys` + `onToggleForce` + brand fields on both event-page routes) reuses the existing per-party mute set.

**Rejected.**

- *Ranking the aggregate "Others" bucket as if it were a contender* - rejected (Max): "Others" is heterogeneous (one big party + independents + fragments); ranking the bag implies coordination that isn't there and keeps the largest party anonymous. The honest fix promotes the NAMED party, never cards the bucket.
- *A fixed numeric seat threshold for promotion* - rejected (Holy Law #6): a relational "at least as big as the smallest declared alliance" rule is scale-free across a 32-seat (Sikkim) and a 403-seat (Uttar Pradesh) house with no magic number to justify per state/year.
- *Keeping the "Winner" badge* - rejected (Max + Citizen): a pre-poll seat-sharing tally is not an election outcome, and a green badge on a sub-majority plurality reads as a mandate the seats do not confer.
