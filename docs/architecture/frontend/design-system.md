...
The `mode` literal is the discriminator; TypeScript narrows each branch so each body sees only the props it needs. Renderer-internal helpers (axis, scale, sort, tiny-segment lift, hatch placeholder) stay shared. Per parent plan section 23.5 doctrine: there are now exactly **8 base renderers + 1 optional + 3 primitives + 2 modes** in the chart engine; adding a fourth bar mode is a design-spec change, not an additive PR.
...
- **Storage-format-agnostic**: all four renderers consume `(entity, time, value)` rows only (parent §22.4 invariant #1). The loader seam below owns parquet-vs-csv abstraction; renderers stay byte-identical when X1a/X1b cutover lands.
...
- [docs/archive/plans/20260605-u5-skeleton-jump-doc-chartshell-states-subplan.md](../../archive/plans/20260605-u5-skeleton-jump-doc-chartshell-states-subplan.md) - the U5 sub-plan that shipped Skeleton + ChartShell state slots + IndicatorDoc route + IndicatorJump strip (PRs #752, #755, #757).
- [docs/archive/plans/20260605-f2a-categorybar-consolidation-subplan.md](../../archive/plans/20260605-f2a-categorybar-consolidation-subplan.md) - the F2a sub-plan that shipped CategoryBar consolidation (PRs #781, #782, #784, #785, #786).
- [docs/archive/plans/20260605-f2a-5-diverging-mode-subsubplan.md](../../archive/plans/20260605-f2a-5-diverging-mode-subsubplan.md) - the F2a.5 sub-sub-plan that shipped the diverging-mode body lift + StateOverview production migration + composition-bar/ README rewrite (PRs #784, #785, #786).
- [docs/archive/plans/20260606-f2b-new-renderers-subplan.md](../../archive/plans/20260606-f2b-new-renderers-subplan.md) - the F2b sub-plan that shipped GeoChoropleth + Matrix + Treemap + CirclePack + C2/C3/C5 primitives + shared color-scale (PRs #789, #790, #793, #794, #795, #796, #797 + closure).
...
- [TODO/20260603-data-and-charting-platform-reset-plan.md](../../../TODO/20260603-data-and-charting-platform-reset-plan.md) sections 20.12, 21.7, 21.8, 21.9, 23.5 - the design-spec source.
