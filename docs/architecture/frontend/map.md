...
### Correction note to prior turn

A claim made in a previous research turn that "Bharat Pashudhan has no map" was wrong - the user's 2026-06-11 screenshot of the `keyStatistics` route proves a state-level India choropleth. The earlier sweep stopped at the landing page (which has no map; only dashboard tiles) and was further blocked by the SPA's devtools-detection modal before the `keyStatistics` route hydrated. This re-investigation went via `python urllib` directly against the static bundle (no devtools, no modal), confirmed the amCharts ammap renderer, and pulled their topology + properties for the comparison table above.

A separate claim that "ToS / scraping prohibits research" was also wrong. NDLM is a Department of Animal Husbandry & Dairying (DAHD, Government of India) public-data surface; citizen-facing reuse of public information served to citizens carries no scraping prohibition. The devtools modal is a UX-friction control, not a permission boundary.
