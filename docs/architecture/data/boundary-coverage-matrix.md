...
**Tracked by:** LGD execution handover log (updated whenever a layer/state ships)
...
`-` = layer absent. Numbers are feature counts (small layers) or file counts (large layers; villages/panchayats/wards/blocks/subdistricts). LGD-canonical state names sourced from the LGD portal; current yen-gov folder labels (`andhra-pradesh` etc.) retire in LGD execution rows L3-L4.
...
Per LGD execution handover source-of-truth doctrine:
