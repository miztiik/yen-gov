...
- **LGD = registry**: the Local Government Directory CSVs (`datasets/taxonomy/lgd/{states,districts,subdistricts,villages}-latest.csv` plus dated immutable snapshots `<role>-YYYY-MM-DD.csv`; moved from `datasets/reference/in/lgd/` in T.0c-ii closeout, 2026-05-21). These are *codes + hierarchy + names*, no geometry. Maintained by `tools/lgd/snapshot.py` from `ramSeraph/opendata` release `lgd-latest-extra1`.
...
- **Citizen-DX**: `git ls-tree origin/main -- datasets/boundaries/in/subdistricts/` returns self-describing `state=andhra-pradesh/`, `state=tamil-nadu/` — readable without a lookup table.

Scope today: this rule binds the slug-keyed per-state subtrees of the admin spine (`boundaries/in/<level>/state=<slug>/...`) and any future per-state electoral partitions. NOTE (2026-06-16, Row 3 of the map-geometry rip): the electoral AC subtree no longer has per-state `state=<slug>` dirs — the 31 per-state `delim=2008/ac/state=<slug>/all.geojson` shards were consolidated into ONE national `electoral/delim=2024/ac/all.topojson` (each feature stamped `state_ut_code`), so the slug-keyed-folder doctrine now applies to the admin spine + the national PC GeoJSON's in-row keys, not a per-state electoral folder tree. The admin spine (`boundaries/in/<level>/state=<slug>/...`) is slug-keyed on disk; the admin-spine rows in `boundary_layer.csv` are resynced via the LGD-canonical-keys wave (see `datasets/_ops/eci-census-decommission-sweep-2026-06-05.md` section C). No re-litigation: `boundary-layers.schema.json` already widened the `partition_path` regex to accept slugs without a schema bump. Regression-guarded by `backend/tests/test_derive_hive_signature.py` + `backend/tests/test_no_eci_state_in_electoral_partition.py` (PR #856).
...
1. **District entity backfill** — 145/784 districts curated in `entities.json`; 639 missing. Closed via LGD master + Census-2011 cross-enrichment (boundary coverage expansion, 2026-05-24).
...
## 2026-05-31 2026-05-31-geoboundaries-udit001-source-audit

> Historical receipt lifted from `notes/2026-05-31-geoboundaries-udit001-source-audit.md`
> on 2026-06-08 (G4 closure of `notes/`). This audit is a load-bearing
> standing refutation against re-adopting `wmgeolab/geoBoundaries` or
> `udit-001/india-maps` as a canonical or render-only boundary source;
> future agents should consult it before re-litigating either source.
> Original verdict was authored by Max (data-shape authority per CLAUDE.md
> section 0a).
...
All 10 in-scope boundary layers in the TopoJSON migration convert IN-PLACE from existing LGD-keyed GeoJSON. No external source is adopted. The encoding swap (GeoJSON to TopoJSON) is a derivative transform; the canonical `source_id` of each layer is unchanged. `boundary_layers.parquet` rows are byte-identical across the conversion. Holy Law section 9 satisfied: encoding is not provenance (see [docs/concepts/data-provenance.md](../../concepts/data-provenance.md)).
