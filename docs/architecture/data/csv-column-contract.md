...
#### `datasets/data/entities/boundary_encoding.csv` (TopoJSON/GeoJSON receipt)

Producer receipt for render encodings under `datasets/boundaries/in/`.
Generated by `python -m tools.topojson.emit_receipt --root .`; validated
by backend Tier-B. One row per TopoJSON shard. This file records encoding
facts, not source provenance; the source-bearing geometry inventory remains
`datasets/data/entities/boundary_layer.csv`.

| column | dtype | nullable | note |
| --- | --- | --- | --- |
| `topojson_path` | string | no (pk) | repo-relative POSIX path to the TopoJSON shard under `datasets/boundaries/in/` |
| `geojson_path` | string | no | repo-relative POSIX path to the GeoJSON sibling |
| `layer_id` | string | yes (fk -> entities/boundary_layer.csv.layer_id) | best-effort link to the boundary inventory ledger when the current path vocabulary resolves |
| `level` | string | no | boundary family level (`state`, `district`, `panchayat`, etc.) |
| `topojson_object` | string | no | object key under the TopoJSON `objects` dictionary |
| `geojson_feature_count` | integer | no | sibling GeoJSON `features.length` at receipt generation time |
| `topojson_feature_count` | integer | no | TopoJSON geometry count for `topojson_object` at receipt generation time |
| `geojson_sha256` | string | no | SHA-256 of the GeoJSON file bytes |
| `topojson_sha256` | string | no | SHA-256 of the TopoJSON file bytes |
| `mapshaper_version` | string | no | pinned version from `tools/topojson/.mapshaper-version` |
| `topojson_config_hash` | string | no | SHA-256 of `config/topojson.json` |
| `generated_by` | string | no | producer tool id, currently `tools.topojson.emit_receipt` |

Tier-B fails if the receipt is missing while TopoJSON files exist, a row
points at missing files, hashes or feature counts differ from disk,
`geojson_feature_count != topojson_feature_count`, a TopoJSON shard has
zero or multiple receipt rows, or a receipt row points at an orphan path.
