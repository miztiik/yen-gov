...
2. Run `python -m yen_gov emit-taxonomy --root .` to regenerate the facet-axes taxonomy artifact. (`emit-taxonomy` is the sole trigger; the legacy belt-and-suspenders refresh that ran inside the Parquet writer was removed with that writer in the rip.)
...
The legacy Parquet writer's belt-and-suspenders `compile_to_parquet` refresh was removed when that writer was deleted in the rip; the operator CLI `python -m yen_gov emit-taxonomy` is now the sole trigger.
