...
> **DOCTRINE NOTE (2026-06-04, plan section 22.7).** The state-identity rule survives the CSV cutover. The partition-token policy MIGRATES with the rip-and-replace: long-format CSV under `datasets/data/` uses LGD-slug folder naming (per [ADR-0050](../../reference/decision-index.md#) folded at D-DOC3.6) rather than ECI / ISO partition tokens. `registerSlice`'s manifest-native contract is unchanged in shape; the manifest itself moves to enumerate CSV files in place of Parquet files per plan chunks B2b / X1.
