...
> **DOCTRINE NOTE (2026-06-04, plan section 22.7).** ADR-0041's meadow tier retires under the broader data-platform reset per plan chunk B4 (local-CSV reingest replaces meadow-tier reads). The OWID 5-tier vocabulary survives; the on-disk `_meadow/` directories do not survive once B4 lands. Until B4, the contract above remains in force.
...
> **DOCTRINE NOTE (2026-06-04, plan section 22.7).** The auto-rollup policy survives the CSV cutover verbatim; the write seam moves from Parquet writer to long-format CSV writer per plan chunks B1/B2b. The SUM-and-COUNT-only fence + `source_id` reuse + compute-on-read parent shape stay binding.
...
The 31 rows below are settled per THE PLAN section 3. Each was either considered and rejected in rounds R1-R11, or is a band-aid that the canonical pivot makes unnecessary. The "Why" column is verbatim from THE PLAN.
