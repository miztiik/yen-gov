...
The slugs below were locked 2026-05-19 (3-agent debate: Hans + Max + Gregor) and amended 2026-05-19 via user override (`accountability` → `governance`). The first seven (`fiscal`, `energy`, `elections`, `economy`, `demography`, `human_development`, `environment`) are the live-on-disk set as of 2026-05-22; the remaining ten are planned.
...
| `human_development` | Human development indices | HDI; MPI; HCI (composite indices only -- `health` / `education` / `amenities` split out when those topics land) |
...
| --- | --- | --- | --- |
| `governance` | Measuring the government | CAG state-audit findings + performance audits; PRS bill tracker; RTI compliance (CIC); Lokpal / CVC; PHC vacancy %; teacher absenteeism; NJDG pendency; FIR-to-chargesheet ratio |
| `schemes` | Where the money goes | MGNREGA; PMAY-G/U; PM-KISAN; ICDS; PM-POSHAN; NFSA; CSS + CS scheme delivery |
| `local_govt_finance` | Panchayats & local bodies | e-GramSwaraj; 15th Finance Commission grant flows; State Finance Commission transfers; ULB own revenue; ZP/BP receipts-payments; CAG Local Bodies Audit |
| `work` | Work & jobs | PLFS (quarterly + annual); NSS-EUS (with methodology break vs PLFS); wages; female LFPR; self-employment; work-related migration |
| `judiciary` | Courts | NJDG pendency + disposal; eCourts metrics |
| `crime` | Crime | NCRB Crime in India (IPC + SLL); Prison Statistics India; FIR-to-chargesheet ratio |
| `health` | Health | NFHS-5 (and successors); HMIS monthly; SRS annual; CRS births/deaths; public health expenditure |
| `education` | Education | UDISE+ school metrics; AISHE higher-ed; ASER learning outcomes; literacy |
| `amenities` | Household amenities | NFHS HH module (water, sanitation, electricity, cooking fuel); JJM; SBM; PMAY-U/G housing (HH module) |
...
- **Not a directory.** `datasets/indicators/<topic>/` no longer exists (retired; all indicators live under `datasets/<family>/` keyed by the publisher's natural family, not by topic).
