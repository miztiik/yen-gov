...
3. **UPSERT into the EXISTING canonical parquet.** A new endpoint that supplements an existing concept (e.g. another fuel type for installed-capacity) extends the SAME `energy_installed_capacity.parquet` via the writer PK `(entity_id, year, period_label, indicator_id)`. No new parquet stems per endpoint. No new `indicator_id` per vintage / publisher / base-year — per [ADR-0044](../architecture/decisions/0044-grain-over-entity.md) and the [CLAUDE.md](../../CLAUDE.md) anti-pattern list.
...
- HTTP only. Pull bytes from upstream and write to `datasets/<family>/_meadow/<source>/<vintage>/<endpoint>.json` per [ADR-0041](../architecture/decisions/0041-meadow-tier.md).
...
- UPSERTs rows into the existing family parquet via PK `(entity_id, year, period_label, indicator_id)`.
- NEVER creates new parquet stems per endpoint. The writer fails fast if asked to emit to a stem that does not already exist in the canonical-allowlist.
- Vintage shifts use the `methodology_breaks.parquet` ledger per [ADR-0042](../architecture/decisions/0042-sources-schema-v3-vintage-as-period-anchor.md) — NEVER a new `indicator_id`.
...
- **Enrich+Emit commit** touches `ingest.py` + the existing canonical parquet (row count increases monotonically) + a contract test that proves UPSERT (not overwrite).
...
When an upstream endpoint publishes a fact already in canonical (a new fuel for installed-capacity; another publisher of GSDP; a finer period grain of an existing series), the enrich layer UPSERTs into the EXISTING family parquet via a new `(entity_id, year, period_label, indicator_id)` tuple. It does NOT create `energy_installed_capacity_<new_fuel>.parquet`.
...
- [docs/architecture/data/canonical-store.md](../architecture/data/canonical-store.md) — writer PK + family parquet layout
- [ADR-0041](../architecture/decisions/0041-meadow-tier.md) — meadow tier
- [ADR-0042](../architecture/decisions/0042-sources-schema-v3-vintage-as-period-anchor.md) — vintage as publisher edition vs operator snapshot
- [ADR-0044](../architecture/decisions/0044-grain-over-entity.md) — grain lives on the row, not the id
