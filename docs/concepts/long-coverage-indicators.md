...
| `fiscal/centre_transfers_to_states_grants` | fiscal | FY08–FY26 (19y, all-India) | RBI App T2 Item II | `rbi_appendix_national` | sources-rbi-appendix-national.md |
| `fiscal/centre_transfers_to_states_gross` | fiscal | FY08–FY26 (19y, all-India) | RBI App T2 Item IV | `rbi_appendix_national` | sources-rbi-appendix-national.md |
...
| `economy/national_gva_by_industry_quarterly_constant_2011_12_inr_crore` | economy | 2011-Q1–2025-Q2 (all-India quarterly by industry) | MoSPI NAS consolidated annual + quarterly CSV via data.gov.in | `ingest_merged_aq` | Quarter stored as quarter-start `YYYY-MM`. |
...
| 2 | `fiscal/states_combined_gross_fiscal_deficit` | fiscal | FY08–FY26 (20y, all-India incl. RE/BE) | RBI AppT1 Major Deficit Indicators | ✅ shipped | Two-row-per-year layout (₹ Crore + % GDP interleaved); rows=year/cols=indicator (transpose of App T2). New sibling adapter `rbi_appendix_deficits`. See [sources-rbi-appendix-deficits.md](../architecture/backend/sources-rbi-appendix-deficits.md). |
| 3 | `fiscal/states_combined_revenue_deficit` | fiscal | FY08–FY26 (20y, all-India) | RBI AppT1 | ✅ shipped | Same workbook as #2; co-shipped. |
| 4 | `fiscal/states_combined_primary_deficit` | fiscal | FY08–FY26 (20y, all-India) | RBI AppT1 | ✅ shipped | Same workbook as #2; co-shipped. Bonus #4b: `fiscal/states_combined_primary_revenue_deficit` co-shipped from column 5 of the same workbook — the strictest fiscal-discipline indicator (interest-stripped revenue balance). |
| 4c | `fiscal/union_*_deficit` (4 indicators: gross_fiscal, revenue, primary, primary_revenue) | fiscal | FY87–FY26 (40y, Union Government) | RBI HBS-IE Table 89 — Key Deficit Indicators of the Central Government | ✅ shipped | Symmetric Centre-actor pairing for the states-combined quartet at #2-4 — closes the asymmetry surfaced by ADR-0025. New sibling adapter `rbi_hbs_ie_centre_deficits` reuses the AppT1 parser (identical layout minus the %-GDP rows). 40 years of history vs AppT1's 20. See [sources-rbi-hbs-ie-centre-deficits.md](../architecture/backend/sources-rbi-hbs-ie-centre-deficits.md) and [fiscal-actor-naming.md](fiscal-actor-naming.md). |
| 5 | `economy/national_gdp_constant_2011_12` | economy | FY12–FY26 at constant prices | MoSPI NAS consolidated annual + quarterly CSV via data.gov.in | ✅ shipped | Covered as the `Gross Domestic Product` facet in `economy/national_macro_aggregates_constant_2011_12_inr_crore`; no separate single-indicator artifact yet. |
...
- **2026-05-15** MoSPI NAS/Merged-AQ ships as constant-price 2011-12 artifacts only. Current-price, growth-rate, sparse 2022-23 rebase, institutional-sector, and subindustry rows are deferred so the first renderer path has one real-economy basis and one level-series source of truth. See [sources-mospi-national-accounts.md](../architecture/backend/sources-mospi-national-accounts.md).
