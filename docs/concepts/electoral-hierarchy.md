...
Locked by PR-0 of the election experience overhaul (2026-06-09). Authority: Jony + Citizen on URL grammar + UX (per [CLAUDE.md section 0a](../../CLAUDE.md)). Receipt + named divergences from the canonical URL grammar live in [url-grammar.md - Named divergences](../architecture/frontend/url-grammar.md#named-divergences-from-canonical-url-grammar).
...
- "How did the 7 ACs that make up the Sriperumbudur Parliament seat split between alliances?" — needs `pc_id` on each AC.
...
This section preserves the rejected-alternatives receipts for the ADRs whose rationale is folded above. Each subsection is anchored as `#adr-NNNN-rejected-alternatives` for the redirect index.
...
**AC-grain backfill (Hans verdict 2026-06-09, ratified):** the Q6 reactive trigger remains "an AC-grain INGEST surfaces unbound rows attributable to publisher name drift." Until that trigger fires (e.g. a TCPD `All_States_AE.csv` reingest for a new election cycle surfaces real unbound rows), the AC `aliases` column stays empty across all ~4189 AC rows. Pre-mining AC aliases against the curator-authored `constituencies.json` set would mint reorganization-drift names (post-2014 AP+TG bifurcation, post-2023 Assam re-delim) as aliases and silently mis-bind pre-reorganization results onto post-reorganization polygons -- defeating the methodology-break discipline (G8 sub-item 3 audit, CLAUDE.md 2026-06-08: 30 of 31 states have name-set mismatches between `constituencies.json` and `electoral.csv`, only S08 Himachal Pradesh has perfect `(eci_no, name)` parity). OWID precedent: `owid/etl` `etl/data_helpers/geo.py` + `owid/catalog/regions.py` carry country aliases ONLY for variants OWID's actual source onboardings have encountered (e.g. `Korea, Rep.` from World Bank, `USSR` from IHME); alias growth is per-source-onboarded, never pre-mined -- direct precedent for the Q6 reactive rule. The `_build_pc_lookup` alias-walking pattern (PR #844) applies unchanged when an AC ingest trigger lands; the equivalent `_build_ac_lookup` is one-PR work at that time. Closure receipt: PR #853.
