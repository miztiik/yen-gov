...
5. **`$schema_version` stamped on every data emit file** (open divergence, PR-0 scope-narrowed pivot — 2026-06-12).
   OWID stamps schema identity only in the `.schema.json` file itself; data files don't carry a `$schema_version` sibling. yen-gov today stamps `$schema_version` at the top of every JSON artifact in `datasets/` (boundary SoT, manifest, taxonomy, indicators-completeness, etc.) populated from the schema's own `x-version`. The conflation is honest at the writer (the field is correctly semver, not a date), but it duplicates schema identity onto the data tier where OWID keeps schema identity, data freshness (`origin.date_accessed`), publisher edition (`origin.version_producer`), and refresh cadence (`dataset.update_period_days`) as four separate semantic concerns. PR-0 (2026-06-12) closed the 4-persona debate (Gregor + Hans + Max + Fowler) and ratified a **scoped retirement** that drops the stamp from citizen-facing data files only and **keeps `datasets/manifest.json`** as a documented control-plane carve-out (CLAUDE.md section 10; deployed static bundle gates on it via `isCompatibleSchemaVersion()`). The four OWID concerns (1)/(2)/(3)/(4) are already covered by yen-gov-native surfaces — `.schema.json` `x-version` / `_meadow/<source>/<vintage>/` snapshot directories / `source.csv.vintage` per [ADR-0042](../concepts/data-provenance.md#adr-0042-sources-schema-v3-vintage-as-period-anchor) / `indicators.json.update_period_days` per ADR-0046 — so the pivot adds NO new fields. When the scoped pivot lands, this row moves from "open divergence" to "**PARTIAL ALIGNMENT** with `manifest.json` as documented control-plane carve-out" but it does NOT leave the table because manifest keeps the stamp. Tracked in [schema-evolution.md §Pending OWID-conformance pivot](../architecture/data/schema-evolution.md#pending-owid-conformance-pivot-stop-stamping-schema_version-onto-data-emit-files) + [TODO/20260612-schema-version-field-refactor-plan.md](../../TODO/20260612-schema-version-field-refactor-plan.md). User picks Path A (close as permanent named divergence; ship 1-PR drift-hazard fix only) vs Path B (proceed with scoped 5-7 PR sequence) per plan-doc section 5.
