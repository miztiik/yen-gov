...
| **Provenance** | Every chart cites its source; no anonymous data | Every observation row carries `source_id` FK to one global citation ledger at `datasets/data/entities/source.csv` per CLAUDE.md Holy Law #9 + section 12. The ledger is a deliberate 5-column subset of OWID `origin.*` -- see [Named divergence #7](#named-divergences-from-owid-with-reasons) for the full divergence inventory. |
...
6. **`processing_level` scope: per-row vs per-variable** (2026-06-14, [docs/concepts/data-quality.md](data-quality.md#per-row-processing-level-vocabulary)).
   OWID's [Metadata reference for ETL](https://docs.owid.io/projects/etl/architecture/metadata/reference/) defines `processing_level` (enum: `minor` | `major`) as a **per-variable** field — one tag for the whole indicator / variable / table. yen-gov adopts the OWID vocabulary verbatim but moves the scope to **per-row**: each row of the four election file_classes (assembly + parliament x candidacies + summary) carries its own `processing_level` + `processing_note`. **Why per-row**: (i) the yen-gov election corpus carries individual records (one candidate per row, one constituency per summary row), each with its own processing history — a per-variable tag would force the whole candidacies.csv to inherit the most-discretionary row's level (any UNK fall-through -> the whole file becomes `major`), which destroys the audit signal; (ii) the discretionary calls are **localised** — a single row's party_id may need curator review while every other row in the same CSV is mechanical, so the per-row tag lets the citizen see exactly which rows need review; (iii) the vocabulary is unchanged — a future per-variable summary on a topic page can derive `MAX(level)` over rows + concatenate distinct notes, so the per-row tag does not preclude per-variable rollups. The companion `processing_note` mirrors OWID's `description_processing` (free text, ASCII-only, cited verbatim in the source-pill expander when the citizen-facing UI lands in the follow-up PR). Case example: pre-2024 Lok Sabha PC totals (1999, 2004, 2009, 2014, 2019) derived from AC-segment aggregation tag `major` with note pointing at `TCPD All_States_GA.csv`; 2024 onward, ingested from direct-PC TCPD returns, stays `minor`. See [docs/concepts/data-quality.md](data-quality.md#when-pc-totals-come-from-ac-segment-aggregation).

7. **Sources schema: a 5-column subset of OWID `origin.*`** (user-ratified 2026-06-11 after 4-persona convergence; Hans + Gregor + Jony + Citizen).
   OWID's `origin.*` block carries roughly a dozen fields (`producer`, `title`, `description`, `citation_full`, `attribution`, `attribution_short`, `url_main`, `url_download`, `date_accessed`, `date_published`, `version_producer`, `license`). yen-gov ships exactly five: `source_id` (deterministic 12-char hash PK), `producer`, `title`, `vintage`, `url`. The seven sub-divergences are all deliberate:

   - **No `license`, `confidence_tier`, `verification_method`, `is_issuing_authority`, `citation_full`, `notes` columns.** These were declared in v2.0 doctrine in May 2026 and never populated by any writer in the six weeks before the user mandate to retire them. The citizen-facing footer was rendering "License unknown / Confidence unknown / Verification unknown" on every row -- worse than not having the surface. License terms are a republisher concern (one global footer suffices); per-row trust tiers are researcher-grade, not citizen-grade. Exceptional context for one row lives in the indicator's hand-authored Markdown narrative on the IndicatorDoc page, not as a structured column.
   - **No `date_accessed` or fetch-telemetry columns on the citation row.** Removing `first_fetched_at` / `last_seen_at` / `content_hash` from the citizen-facing row was the cause-side fix for the `fetched_at smear` failure mode (a re-poll with byte-identical bytes was rewriting hundreds of artifacts). Adapters that still need byte-change detection write `.runtime/<adapter>/<source_id>.json` sidecars (CLAUDE.md section 2 ephemeral; never contract).
   - **One `url` field, not OWID's `url_main` + `url_download` pair.** yen-gov surfaces one URL to the citizen (the publisher landing page); there is no direct-file-vs-landing distinction worth disambiguating. The OWID `_main` suffix is dead grammar without `_download`.
   - **Deterministic `source_id` PK from sha256 of `(producer, title, vintage)`.** OWID has no PK because OWID is editorially curated. yen-gov's writer-driven ingest needs a stable FK target that collapses live-fetched and hand-imported paths to one citation row when the triple matches.
   - **`vintage` extended to operator-snapshot semantics.** OWID `origin.vintage` is publisher-edition only ("NFHS-5", "Handbook 2024-25"). yen-gov sharpens to "strongest period anchor available" so continuously-updated publishers (NITI ICED, MoSPI bulletins) still carry a non-empty vintage anchored to the operator's snapshot window. The publisher-edition path is unchanged; only the previously-permitted empty-string fallback was retired.
   - **One global `source.csv`, not per-variable `origins[]` arrays.** OWID lists origins inline on each variable's metadata; yen-gov factors them into one global table and FK-references from every observation row. The trade-off is a single source of truth at scale (3000+ rows in the canonical store) instead of repeated inline arrays.
   - **`producer` column name (not `owner`).** A 2026-06-11 mechanical rename across CSV header + 4 writers + frontend SQL alias; identity hash is unchanged because the input string was unchanged. The rename brings the column name in line with citation standards (OWID `origin.producer`, DataCite, Dublin Core, BibTeX).

   **Why this divergence earns its keep**: yen-gov's audience is the median Indian citizen reading a chart on a mid-tier Android phone, not a global researcher exporting metadata. The citizen needs publisher + report + period, in that order. The 7-field overhead OWID carries for the researcher use case has, in 6 weeks of real use, surfaced no citizen benefit and one concrete harm ("unknown" labels on every footer). The full design rationale + the rejected alternatives (backfilling sentinels; an operator-only annotation layer; collapsing the ledger itself to one row per producer) lives in [data-provenance.md](data-provenance.md) and [canonical-store.md section 5](../architecture/data/canonical-store.md#5-sources-schema-d5).

8. **In-row dimension columns, NOT flattened per-member variables at the serving boundary** (2026-06-16, [csv-column-contract.md section 3.3](../architecture/data/csv-column-contract.md), [geo-facet plan](../../TODO/20260616-geo-facet-dimension-column-plan.md)).
   OWID's ETL stores a multi-dimensional indicator long-format with dimension columns, but at the grapher *serving* boundary it FLATTENS the cross-product into atomic per-member variables (each carrying a `dimensions` filter). yen-gov does NOT flatten: the long CSV with the dimension column IS the served artifact (Holy Law #1 - no backend/ETL-grapher split), read in-browser by DuckDB-WASM with the `FacetPicker` selecting distinct facet values. Flattening would re-introduce the exact per-member fragmentation this contract removes (one CSV + `variable_id` per facet member - the very over-fragmentation the geo-facet PR fixed). Two sub-divergences on HOW the axis is declared: (i) the facet axis + closed enum live on a per-axis homogeneous sibling file-class `datapoints/geo_by_<axis>/*.csv` in `columns.json` (Gregor contract ruling: the reader stays `columns.json`-only), NOT on OWID's per-variable `dimensions` metadata blob, which would fork the header contract across two files and add a second drift site; (ii) admission is gated by the four-gate facet test (plan F2) so a methodology/vintage axis (e.g. fiscal estimate-stage BE/RE/Accounts) is NEVER laundered into a dimension column - it stays a separately-named indicator or a labelled gap per plan F1.
