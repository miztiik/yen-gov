...
- **Source provenance.** All Pashu Aadhaar rows FK to `src-7e5d4aac4995` (ndlm_pashu_aadhaar), seeded by PR #276 (`tools/livestock_sources_seed.py`). The writer's FK gate verifies closure against `datasets/data/entities/source.csv` before bytes touch disk.
