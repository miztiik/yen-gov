...
`results.csv` next to it in **long format** — one row per candidate (real or
NOTA), with all per-AC and per-candidate fields denormalised onto each row.
...
Determinism: same JSON inputs MUST produce a byte-identical .csv output, so PR
diffs only appear when JSON changed. We achieve this by:
...
def emit_state_csv(*, state_dir: Path, output_path: Path | None = None) -> Path:
    """Build `results.csv` (long format) from the JSON under `state_dir`.

    `state_dir` must contain `results/<n>.json` files. The CSV is written
    atomically: a temp file in the same directory, then `os.replace`. Returns
    the final path.
    """
    result_files = sorted(
    ...
    for path in result_files:
        rows.extend(_rows_for_constituency(_load_json(path)))
