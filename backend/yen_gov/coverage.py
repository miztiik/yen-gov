        ...
        for n, cat in enumerate(sorted(by_cat)):
            ...
            for ind in sorted(by_cat[cat], key=lambda x: x.id):
                if wiring_known:
                    ...
                    wired_cell = ""
                # Link the id cell to the per-indicator page generated by
                # ``coverage_indicator_pages``. Indicator ids follow the
                # ``<topic>/<basename>`` convention that mirrors the artifact
                # path 1:1, so the link target is mechanical.
                id_cell = f"[`{ind.id}`](indicators/{ind.id}.md)"
                out.append(
