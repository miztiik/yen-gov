"""ICED captive-power - ingest orchestrator.

One pipeline for each captive-power measure:

    stage encrypted JSON  ->  decrypt + keep stateWise + sum measure (parser)
                          ->  resolve entity (shared RBI-Handbook resolver)
                          ->  emit datapoints CSV  +  upsert variables /
                              concepts / source catalogue rows (canonical
                              csv_writer)

Both shipped measures read the SAME staged feed and share the SAME source
triple, so the citation ledger ends up with ONE source row for the pair (the
catalogue upsert is keyed by ``source_id``).
...
same surface the RBI Handbook + ICED renewable-potential ingests write - so
one command produces a corpus the canonical validator accepts (datapoint
filename == ``indicator_id`` in ``variables.csv``; every ``source_id`` FK
closes).
"""
...
from .parser import CaptiveDropReport, CaptivePowerSpec, parse_captive_feed
from .registry import SHIPPED_SPECS

__all__ = ["IngestResult", "IngestedIndicator", "ingest"]
...
class IngestedIndicator:
    """Per-measure outcome reported to the CLI."""
...
class IngestResult:
    ...
    indicators: tuple[IngestedIndicator, ...]

    @property
    def total_rows(self) -> int:
        return sum(i.row_count for i in self.indicators)
...
def ingest(
    *,
    repo_root: Path,
    staging_dir: Path,
    specs: Iterable[CaptivePowerSpec] | None = None,
) -> IngestResult:
    """Ingest the staged ICED captive-power feed into the canonical store.
    ...
    Args:
        ...
        staging_dir: directory the operator dropped the ICED JSON response
            into. Each spec's ``staging_filename`` is resolved inside it (both
            measures point at the same file). Never a committed contract
            surface (operator input).
        specs: the measures to ingest; defaults to :data:`SHIPPED_SPECS`.

    Returns:
        :class:`IngestResult` summarising every indicator written.

    Raises:
        FileNotFoundError: ``geo.csv`` or the staged feed is missing.
        CaptivePowerShapeError: the feed no longer matches its spec.
    """
    ...
    indicators: list[IngestedIndicator] = []
    ...
    for spec in specs:
        ...
        long_rows, drop_report = parse_captive_feed(
    ...
    return IngestResult(indicators=tuple(indicators))
...
def _upsert_rows(
    ...
    *,
    ...
    verbatim; a re-ingest of the same edition is a no-op. Two specs that emit
    the SAME source row (capacity + generation share one citation triple)
    collapse to a single row here because the merge is keyed by ``source_id``.
