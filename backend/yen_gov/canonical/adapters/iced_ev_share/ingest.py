"""ICED ICE-vs-EV (VAHAN) registrations - ingest orchestrator.

One pipeline for the state-grain EV-share feed:

    stage encrypted JSON  ->  decrypt + accumulate + ratio (parser)
                          ->  resolve entity (shared RBI-Handbook resolver)
                          ->  emit datapoints CSV  +  upsert variables /
                              concepts / source catalogue rows (canonical
                              csv_writer)

The operator stages the raw ICED response (one JSON) under a local staging
directory; this module reads it - NO network (parent plan section 21.4). Each
run is idempotent: the canonical writer skip-writes byte-identical output, so
...
same surface the renewable-potential / RBI Handbook ingests write - so one
...
from .parser import EvShareSpec, parse_ev_share_feed
...
def ingest(
    *,
    repo_root: Path,
    staging_dir: Path,
    specs: Iterable[EvShareSpec] | None = None,
) -> IngestResult:
    """Ingest the staged ICED ICE-vs-EV (VAHAN) feed(s).
    ...
    Args:
        ...
        staging_dir: directory the operator dropped the ICED JSON response(s)
            into. Each spec's ``staging_filename`` is resolved inside it. Never
            a committed contract surface (operator input).
    ...
    Raises:
        ...
        EvShareShapeError: a feed no longer matches its spec.
    """
    ...
    for spec in specs:
        ...
        long_rows, dropped = parse_ev_share_feed(
...
def _upsert_rows(
    ...
    *,
    ...
    A self-contained copy of the renewable-potential / rbi_handbook catalogue-
    merge helper: keeping it local makes this adapter purely additive (no edit
    to a sibling adapter, no cross-adapter private import). It is a generic PK
    merge - distinct from ``csv_writer.upsert_source_scoped`` (which is
    source-scoped and replaces an incoming source's rows wholesale).
    """
