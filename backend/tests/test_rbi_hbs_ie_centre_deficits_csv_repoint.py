...
class ``datasets/data/datapoints/geo/*.csv``, and stamps every row with
the deterministic ``source_id`` derived from one shared citation triple
(producer / title / vintage = single HBS-IE Table 89 publication).
...
from yen_gov.canonical.adapters.rbi_hbs_ie_centre_deficits.ingest import (
    _CSV_FILE_CLASS,
    _CSV_OUT_REL_DIR,
...
def _parsed_indicator() -> tuple[DeficitSpec, ParsedIndicator]:
    spec = DeficitSpec(
    ...
    parsed = ParsedIndicator(
        ...
        rows=[
            ParsedRow(entity_id="IN", time="1986-04", value=1000.0),
            ParsedRow(entity_id="IN", time="1987-04", value=2000.5),
            # Null rows are dropped (real workbooks have N.A. cells).
            ParsedRow(entity_id="IN", time="1988-04", value=None),
        ],
        period_count=3,
    )
    return spec, parsed
...
def test_build_csv_variables_maps_one_variable_per_spec_with_canonical_columns():
    ...
    assert set(by_variable.keys()) == {"union-gross-fiscal-deficit-inr-crore"}
    rows = by_variable["union-gross-fiscal-deficit-inr-crore"]
...
def test_fy_start_year_lifts_integer_year():
    assert _fy_start_year("1986-04") == 1986
    assert _fy_start_year("2024-04") == 2024


def test_slug_segment_is_kebab_and_ban_safe():
    # Plan section 21.6 / 21.12 ban `__`; ADR-0044 bans grain prefixes.
    assert _slug_segment("Union Gross Fiscal Deficit") == "union-gross-fiscal-deficit"
...
def test_indicator_variable_id_map_covers_all_shipped_specs():
    ...
    from yen_gov.canonical.adapters.rbi_hbs_ie_centre_deficits.ingest import SHIPPED_SPECS
...
def test_emit_csv_variables_writes_one_file_per_variable(tmp_path: Path):
    ...
    target = out_dir / "union-gross-fiscal-deficit-inr-crore.csv"
    ...
    assert [r["time"] for r in parsed_rows] == ["1986", "1987"]
