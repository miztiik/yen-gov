...
def test_state_gas_allocated_uses_allocated_concept():
    row = next(r for r in _rows() if r["indicator_id"] == "state-installed-capacity-allocated-mw-gas")
    assert row["concept_id"] == "gas-mw-allocated"


def test_state_gas_snapshot_uses_snapshot_concept():
    row = next(r for r in _rows() if r["indicator_id"] == "state-installed-capacity-snapshot-mw-gas")
