"""B1.5.3 writer-unit gate: rbi_appendix_deficits row-builder + write_csv.

Locks the contract for sub-row B1.5.3 of
``docs/archive/plans/20260604-b1.5-rbi-repoint-subplan.md``: each of the four
SHIPPED_SPECS RBI State Finances Appendix Table 1 states-combined
deficit indicators maps 1:1 to a kebab-case ``variable_id`` (no
faceting on this family), is emitted via the canonical
...
(producer / title / vintage = single State Finances AppT1 publication).
...
def _parsed_indicator() -> tuple[DeficitSpec, ParsedIndicator]:
    ...
    parsed = ParsedIndicator(
        ...
        rows=[
            ParsedRow(entity_id="IN", time="2007-04", value=75455.0),
            ParsedRow(entity_id="IN", time="2008-04", value=120945.5),
...
def test_build_csv_variables_maps_one_variable_per_spec_with_canonical_columns():
    ...
    rows = by_variable["states-combined-gross-fiscal-deficit-inr-crore"]
...
def test_slug_segment_is_kebab_and_ban_safe():
    ...
    assert (
        _slug_segment("States Combined Gross Fiscal Deficit")
        == "states-combined-gross-fiscal-deficit"
    )
...
def test_emit_csv_variables_writes_one_file_per_variable(tmp_path: Path):
    ...
    target = out_dir / "states-combined-gross-fiscal-deficit-inr-crore.csv"
