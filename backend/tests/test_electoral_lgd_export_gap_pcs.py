...
def test_build_parliament_2024_binds_four_gap_pcs(tmp_path: Path) -> None:
    ...
    # Belt-and-braces: total unbound is exactly 0 after the follow-on UT-PC
    # backfill PR (Delhi x7 + Chandigarh + A&N + Dadar+DNH closed via the
    # same eci<N> fallback pattern; see test_electoral_ut_pc_gap_fill.py).
    # The progression across PRs is 50 (G16 baseline) -> 14 (PR #844 alias
    # backfill) -> 10 (PR #849 this 4-metro PR) -> 0 (UT-PC follow-on).
    assert len(unbound) == 0, (
        f"expected unbound count = 0 (all UT-classification gaps closed by "
        f"the UT-PC backfill PR); got {len(unbound)}: {sorted(unbound)}"
    )


# --- Test 5: live summary.csv has 542 rows; coverage receipt says unbound=0


def test_live_summary_has_542_rows_and_receipt_says_unbound_0() -> None:
    """The committed LS2024 summary.csv has 532 + 10 = 542 rows after the
    UT-PC follow-on PR; the regenerated coverage receipt at
    datasets/_ops/parliament-2024-eci-coverage-2026-06-09.md says unbound=0
    (was 10 after this PR alone, then UT-PC backfill closed the remaining 10)."""
    summary_path = (
        _repo_root()
        / "datasets"
        / "elections"
    ...
    assert len(summary_rows) == 542, (
        f"summary.csv expected 542 rows (528 baseline + 4 metros + 10 UT "
        f"PCs after follow-on backfill), got {len(summary_rows)}"
    )
    ...
    # Post-UT-PC-backfill the final tally is 36 / 8359 / 542 / 0 / 8909.
