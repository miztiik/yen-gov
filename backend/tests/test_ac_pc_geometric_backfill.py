...
def test_gate_passes_and_crosswalk_invariants_hold(tmp_path: Path) -> None:
    ...
    for row in rows:
        ...
        # class admits seven methods (geometric_overlap, single_pc_state, the
        # Survey-of-India composition backfill values soi_composition /
        # soi_centroid, composition_alias for official-composition rows
        # bridged via a verified name alias, eci_delimitation_order for
        # rows read directly from an ECI delimitation-order PC-wise AC
        # composition table, and district_composition for seats resolved to
        # the PC their already-linked same-district sibling seats compose);
        # the generator under test emits only the first two.
