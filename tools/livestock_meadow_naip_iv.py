...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "_meadow/ndlm/2024-25/naip_iv_district.json (the seeded "
            "source vintage); raw_vintage is recorded as each row's "
            "`time` field. The inventory deriver requires a "
            "homogeneous `time` vocabulary per indicator so the "
            ...
            "citation (`src-93a2a72db482`) has vintage='2024-25' only."
        ),
    )
    parser.add_argument(
        "--skip-header",
